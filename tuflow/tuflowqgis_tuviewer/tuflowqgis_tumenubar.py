...
class TuMenuBar:
    ...
    def loadViewMenu(self, plotNo, **kwargs):
        ...
        if self.menu is None:
            ...
            self.clearPlotWindow_action.triggered.connect(
                lambda: self.tuView.tuPlot.clearPlot2(
                    self.tuView.tabWidget.currentIndex()
                )
            )
            self.clearAllPlotWindows_action.triggered.connect(
                self.tuView.tuPlot.clearAllPlots
            )
        else:
            self.viewMenu.addAction(toolbar[0])
            self.viewMenu.addAction(toolbar[1])
    ...
    def loadSettingsMenu(self, plotNo, **kwargs):
        ...
        if self.menu is None:
            ...
            self.saveStyleForVectorResult_action.triggered.connect(
                self.tuMenuFunctions.saveDefaultStyleVector
            )
            self.loadStyleForActiveResult_action.triggered.connect(
                self.tuMenuFunctions.loadDefaultStyleScalar
            )
            self.loadStyleForVectorResult_action.triggered.connect(
                self.tuMenuFunctions.loadDefaultStyleVector
            )
            self.resetDefaultStyles_action.triggered.connect(
                self.tuMenuFunctions.resetDefaultStyles
            )
            self.options_action.triggered.connect(self.tuMenuFunctions.options)
            self.addPlotColourRamp_action.triggered.connect(
                self.tuMenuFunctions.addColourRampFromXML
            )
            self.resetPlotColours_action.triggered.connect(
                self.tuMenuFunctions.resetMatplotColours
            )
            self.resetAxisNames_action.triggered.connect(
                self.tuMenuFunctions.resetPlotAxisNames
            )
            self.dockTuflowViewer_action.triggered.connect(
                self.tuMenuFunctions.redockTuflowViewer
            )
        else:
    ...
    def loadExportMenu(self, plotNo, **kwargs):
        ...
        if self.menu is None:
            ...
            self.autoPlotExport_action.triggered.connect(
                self.tuMenuFunctions.batchPlotExportInitialise
            )
            self.exportDataToClipboard_action.triggered.connect(
                self.tuMenuFunctions.exportDataToClipboard
            )
            self.exportImageToClipboard_action.triggered.connect(
                self.tuMenuFunctions.exportImageToClipboard
            )
            self.exportTempLine_action.triggered.connect(
                self.tuMenuFunctions.exportTempLines
            )
            self.exportTempPoint_action.triggered.connect(
                self.tuMenuFunctions.exportTempPoints
            )
            self.exportAnimation_action.triggered.connect(
                self.tuMenuFunctions.exportAnimation
            )
    ...
    def loadResultsMenu(self):
        ...
        if self.menu is None:
            ...
            self.showSelectedElements_action.triggered.connect(
                self.tuMenuFunctions.showSelectedElements
            )
            self.showMedianEvent_action.triggered.connect(
                self.tuMenuFunctions.showMedianEvent
            )
            self.showMeanEvent_action.triggered.connect(
                self.tuMenuFunctions.showMeanEvent
            )
        else:
    ...
    def loadHelpMenu(self):
        ...
        aboutIcon = QIcon(
            os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "icons", "tuview.png"
            )
        )
    ...
    def qgisDisconnect(self):
        ...
        # settings menu
        try:
            self.tuView.tuPlot.tuPlotToolbar.lstActionsTimeSeries[
                7
            ].triggered.disconnect(self.tuMenuFunctions.updateLegend)
        except:
            pass
        try:
            self.tuView.tuPlot.tuPlotToolbar.lstActionsLongPlot[7].triggered.disconnect(
                self.tuMenuFunctions.updateLegend
            )
        except:
            pass
        try:
            self.tuView.tuPlot.tuPlotToolbar.lstActionsCrossSection[
                7
            ].triggered.disconnect(self.tuMenuFunctions.updateLegend)
        except:
            pass
        try:
            self.tuView.tuPlot.tuPlotToolbar.lstActionsVerticalProfile[
                7
            ].triggered.disconnect(self.tuMenuFunctions.updateLegend)
        except:
            pass
