...
class TuView(QDockWidget, Ui_Tuplot):
    def __init__(self, iface=None, **kwargs):
        ...
        self.tuMenuBar = TuMenuBar(
            self,
            removeTuview=removeTuview,
            reloadTuview=reloadTuview,
            layout=self.mainMenu,
        )
        # self.tuMenuBar.setNativeMenuBar(False)
        ...
        # secondary menu bar
        self.tuMenuBarSecond = TuMenuBar(
            self,
            removeTuview=removeTuview,
            reloadTuview=reloadTuview,
            menu_bar=self.tuMenuBar,
        ...
        if QSettings().value(
            ...
            == "previous_state"
            and QSettings().value("TUFLOW/tuview_previouslayout", "plot") == "narrow"
        ):
    ...
    def currentLayerChanged(self):
        ...
        if self.iface is not None:
            ...
            if (
                ...
                and not self.tuResults.tuResults1D.activeType
            ):
                self.tuResults.updateActiveResultTypes(
    ...
    def setTsTypesEnabled(self, context=None):
        ...
        if isinstance(self.currentLayer, QgsVectorLayer):
            ...
            try:
                if (
                    ...
                    not in self.tuResults.tuResults1D.activeType
                ):
                    self.tuResults.tuResults1D.activeType.append(
                        self.currentLayer.geometryType()
                    )
            except:
                pass
        ...
        if (
            ...
            or isTSLayer(self.currentLayer, self.tuResults.results)
        ):
            # self.OpenResultTypes.model().setEnabled(4, 5, 6, 7, 8)
        ...
        for rlayer in removedLayers:
            ...
            if layer in self.tuResults.tuResults2D.copied_results:
                if self.tuOptions.del_copied_res:
                    self.tuResults.tuResults2D._copied_res_for_rem.append(
                        self.tuResults.tuResults2D.copied_results[layer]
                    )
            ...
            if layer is not None and isinstance(layer, QgsMeshLayer):
                ...
                layer is not None
                and layer.name() in self.tuResults.tuResultsParticles.resultsParticles
            ):
                self.tuResults.tuResultsParticles.removeResults(
                    [layer.name()], remove_vlayer=False
                )
            elif (
                layer is not None
                and layer.name() in self.tuResults.tuResultsNcGrid.results
            ):
                self.tuResults.tuResultsNcGrid.removeResults(
        ...
        if self.btnTimePlay.isChecked():
            if self.tuResults.activeResults:
                ...
                self.timer.setInterval(
                    int(self.tuOptions.playDelay * 1000)
                )  # sec to ms
    ...
    def qgisConnect(self):
        ...
        if not self.connected:
            ...
            self.cbShowCurrentTime.stateChanged.connect(
                lambda: self.tuPlot.clearPlot2(
                    TuPlot.TimeSeries, TuPlot.DataCurrentTime
                )
            )
            self.tuPlot.tuPlotToolbar.fluxSecAxisButton.released.connect(
        ...
        if self.connected or completely_remove:
            ...
            for layer in findAllMeshLyrs():
                try:
                    ...
                    pass
                try:
                    signal = self.tuView.tuResults.tuResults2D.layer_style_changed_signals.get(
                        layer.id()
                    )
                    if signal is not None:
                        layer.rendererChanged.disconnect(signal)
                        del self.tuView.tuResults.tuResults2D.layer_style_changed_signals[
                            layer.id()
                        ]
                except:
                    pass
                if Qgis.QGIS_VERSION_INT >= 32800:
                    try:
                        signal = self.tuView.tuResults.tuResults2D.layer_reloaded_signals.get(
                            layer.id()
                        )
                        if signal is not None:
                            layer.reloaded.disconnect(signal)
                            del self.tuView.tuResults.tuResults2D.layer_reloaded_signals[
                                layer.id()
                            ]
                    except:
                        pass
            ...
            # plotting
            try:
                self.tuPlot.verticalMesh_action.triggered.disconnect(
                    self.tuPlot.vmeshToggled
                )
            except:
                pass
    ...
    def resultTypeDoubleClicked(self, event):
        ...
        if event is not None:
            ...
            if (
                self.tuContextMenu.resultTypeContextItem.parentItem.ds_name
                == "Map Outputs"
            ):
                if self.iface is not None:
                    self.iface.showLayerProperties(meshLayer)
    ...
    def timeSliderChanged(self):
        ...
        if self.cbShowCurrentTime.isChecked():
            self.tuPlot.clearPlot2(
                TuPlot.TimeSeries, TuPlot.DataCurrentTime, clear_selection=False
            )
    ...
    def qgsTimeChanged_old(self):
        ...
        if self.iface is not None:
            ...
            if tf is None:
                tf = convertTimeToFormattedTime(
                    t, unit=self.tuOptions.timeUnits
                )  # time formatted
        ...
        if type(t) is datetime:
            if self.tuOptions.xAxisDates:
                ...
                for i, (cboTime, timeKey) in enumerate(
                    self.tuResults.cboTime2timekey.items()
                ):
                    ...
                    if date > t:
                        ...
                        if diff < diff2:
                            tf = [x for x in self.tuResults.cboTime2timekey][
                                max(0, i - 1)
                            ]
                            break
                        else:
    ...
    def iconSizeChanged(self, size):
        self.toolbarIconSizeChanged(
            size, self.tuPlot.tuPlotToolbar.mpltoolbarTimeSeries, self.mplToolbarFrame
        )
        self.toolbarIconSizeChanged(
            size, self.tuPlot.tuPlotToolbar.mpltoolbarLongPlot, self.mplToolbarFrame
        )
        self.toolbarIconSizeChanged(
            size, self.tuPlot.tuPlotToolbar.mpltoolbarLongPlot, self.mplToolbarFrame
        )
        self.toolbarIconSizeChanged(
            size,
            self.tuPlot.tuPlotToolbar.mpltoolbarVerticalProfile,
            self.mplToolbarFrame,
        )
        self.toolbarIconSizeChanged(
            size,
            self.tuPlot.tuPlotToolbar.mapOutputPlotToolbar,
            self.MapOutputPlotFrame,
        )
        self.toolbarIconSizeChanged(
            size, self.tuPlot.tuPlotToolbar.mesh3dPlotToolbar, self.Mesh3DToolbarFrame
        )
        for i in range(4):
