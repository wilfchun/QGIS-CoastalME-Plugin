...
class TuflowPluginItem(plugin_base.PluginItemBase):
    ...
    def able_to_run(self, gpus_avail, n_gpus_total):
        ...
        #for config_index, (gpus, cpus, cpus_counted) in avail_configs:
            ...
            #    if bit_functions.bit_test(gpus, i_gpu) and i_gpu not in gpus_avail:
            #        return False
        #    if cpus_counted <= cpus_avail:
        #        return True
        #return False
    ...
    def create_additional_tabs(self, tab_widget):
        self.volume_plot_window = tuflow_plots.VolumePlot()
...
class DlgTuflowAddSimulations(QDialog):
    def __init__(self, plugin, sim_filename):
        ...
        if "TUFLOW_file_options" in settings_obj.childGroups():
            settings_obj.beginGroup("TUFLOW_file_options")
    ...
    try:
        ...
        if dlg.exec():
            ...
            for s_e in combinations:
                tuflow_plugin_item = TuflowPluginItem(plugin, simulation_filename, s_e, len(scenarios))
                tuflow_plugin_items.append(tuflow_plugin_item)
    except ValueError:
        # No scenarios or events
        tuflow_plugin_item = TuflowPluginItem(plugin, simulation_filename, [], 0)
        tuflow_plugin_items.append(tuflow_plugin_item)

    return tuflow_plugin_items
