...
class CmeflowPluginItem(plugin_base.PluginItemBase):
...
class DlgCmeflowAddSimulations(QDialog):
...
def dlg_new_plugin_items(plugin, simulation_filename):
    ...
    try:
        dlg = DlgCmeflowAddSimulations(plugin, simulation_filename)
        if dlg.exec():
            ...
            for s_e in combinations:
                coastalme_plugin_item = CmeflowPluginItem(plugin, simulation_filename, s_e, len(scenarios))
                coastalme_plugin_items.append(coastalme_plugin_item)
    except ValueError:
        # No scenarios or events
        coastalme_plugin_item = CmeflowPluginItem(plugin, simulation_filename, [], 0)
        coastalme_plugin_items.append(coastalme_plugin_item)
