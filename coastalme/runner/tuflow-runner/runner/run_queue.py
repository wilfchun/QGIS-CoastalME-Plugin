    ...
    def resources_in_use(self):
        ...
        for row, sim in enumerate(self.run_queue):
            if sim.status == queue_item.RunStatus.Running:
                gpus = sim.resources()[0]
                gpus_in_use = gpus_in_use | gpus

        return gpus_in_use

    def run_ready_models(self, ngpus):
        for row, sim in enumerate(self.run_queue):
            gpus_in_use = self.resources_in_use()
            ...
            if sim.status == queue_item.RunStatus.Waiting and \
                    sim.able_to_run(gpus_avail, ngpus):
                sim.run_simulation(gpus_avail, ngpus)
