...
class RunItem(QObject):
    ...
    def able_to_run(self, gpus_avail, n_total_gpus):
        return self.plugin_item.able_to_run(gpus_avail, n_total_gpus)

    def resources(self):
        return self.plugin_item.resources()

    def run_simulation(self, gpus_avail, n_gpus_total):
        arguments = self.plugin_item.get_commandline_arguments(gpus_avail, n_gpus_total)
    ...
    def handle_stdout(self):
        ...
        try:
            ...
            if data is not None:
                print(f'Error handling stdout: {stdout}\n{str(e)}')
            else:
