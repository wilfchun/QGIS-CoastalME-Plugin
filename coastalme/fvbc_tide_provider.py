...
class FVBCTideProvider:
    """Class for providing FV BC tide data to the COASTALME Viewer class."""
...
class FVBCTideNCProvider:
    ...
    def _get_units(self) -> None:
        ...
        if rt:
            self.reference_time = datetime.strptime(rt[0], '%Y-%m-%d %H:%M:%S')  # keep timezone naive for now (COASTALME Viewer assumes UTC)
