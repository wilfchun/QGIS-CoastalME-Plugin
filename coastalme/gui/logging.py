...
class _Logging:
    ...
    def info(self, msg: str, silent: bool = False) -> None:
        ...
        if self.iface is not None and not silent:
            self.iface.messageBar().pushMessage("COASTALME Plugin", msg, level=Qgis.MessageLevel.Info)
        else:
            QgsApplication.messageLog().logMessage(msg, 'COASTALME Plugin', level=Qgis.MessageLevel.Info, notifyUser=False)
    ...
    def warning(self, msg: str, silent: bool = False) -> None:
        ...
        if self.iface is not None:
            self.iface.messageBar().pushMessage("COASTALME Plugin", msg, level=Qgis.MessageLevel.Warning)
        else:
            QgsApplication.messageLog().logMessage(msg, 'COASTALME Plugin', level=Qgis.MessageLevel.Warning, notifyUser=False)
    ...
    def error(self, msg: str, more_info: str = None, silent: bool = False) -> None:
        ...
        if self.iface is not None:
            if more_info:
                ...
                self.dlg.setWindowTitle('COASTALME')
                self.dlg.setText(msg)
                self.dlg.setInformativeText(more_info)
                self.dlg.setIcon(QT_MESSAGE_BOX_WARNING)
                widget = self.iface.messageBar().createMessage("COASTALME Plugin", msg)
                ...
                self.iface.messageBar().pushMessage("COASTALME Plugin", msg, level=Qgis.MessageLevel.Critical)
        else:
            QgsApplication.messageLog().logMessage(msg, 'COASTALME Plugin', level=Qgis.MessageLevel.Critical, notifyUser=False)
