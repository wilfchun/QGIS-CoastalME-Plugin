...
def apply_cme_style(e: bool = False) -> None:
    lyr = plugins['coastalme'].iface.activeLayer()
    styling = Styling(lyr)
    styling.apply_styling(lyr)


def apply_cme_style_message_ids(e: bool = False) -> None:
    lyr = plugins['coastalme'].iface.activeLayer()
    styling = Styling(lyr, 'msgs')
    styling.apply_styling(lyr)


def apply_cme_style_gpkg_ts(lyr: QgsVectorLayer, type_: str, field_name: str):
...
def apply_cme_style_temporal(type_name: str):
    coastalme = plugins['coastalme']
    lyr = coastalme.iface.activeLayer()
    ...
    if coastalme.resultsPlottingDockOpened and res_name in coastalme.resultsPlottingDock.tuResults.tuResults1D.results1d:
        # just apply styling - temporal settings should already be active
        apply_cme_style_gpkg_ts(lyr, type_name, type_name)
    else:
        # setup layer temporal properties and style
        set_vector_temporal_properties(lyr, True)
        turn_on_temporal_controller_animated_nav(coastalme.iface)
        refresh_temporal_controller_range(coastalme.iface)
        ...
        if dt > 0.:
            ...
            set_temporal_controller_time_interval(coastalme.iface, dt, res.units)
        res.close()
        apply_cme_style_gpkg_ts(lyr, type_name, type_name)


def apply_cme_style_static(field_name: str):
...
class Styling:
    ...
    def styling_folder(sub_folder: str = ''):
        if sub_folder:
            return plugins['coastalme'].plugin_dir / 'QGIS_Styles' / sub_folder
        return plugins['coastalme'].plugin_dir / 'QGIS_Styles'
