...
from ..gui.processing.processsing_param_conv_tuf_model_format_dir_settings import \
    ProcessingParameterConvCmefModelDirSettings
...
def count_lines(file, write_empty=False):
    from ..convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.empty_files import CmeflowEmptyFiles
        ...
        with open(file, 'r', errors='ignore') as f:
            for line in f:
                ...
                if command in CONTROL_FILES:
                    ...
                    value = (CmeflowPath(file).parent / value).resolve()
    ...
    if write_empty:
        line_count += CmeflowEmptyFiles.empty_count()
...
class ConvertCmeflowModelGisFormat(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        param = QgsProcessingParameterFile('roocmeolder', 'Root Folder', optional=True,
        ...
        param = QgsProcessingParameterString('scenarios',
                                             'Scenarios (list as you would with COASTALME e.g. -s BASE -s 5m -e Q100)',
                                             optional=True)
        param.setFlags(param.flags() | QgsProcessingParameterDefinition.FlagAdvanced)
        self.addParameter(param)
        ...
        param.setFlags(param.flags() | QgsProcessingParameterDefinition.FlagAdvanced)
        self.addParameter(param)
        param = QgsProcessingParameterBoolean('coastalme_dir_struct', 'Force COASTALME Directory Structure')
        param.setFlags(param.flags() | QgsProcessingParameterDefinition.FlagAdvanced)
        self.addParameter(param)

        struct_json = Path(
            __file__).parent.parent / 'convert_coastalme_model_gis_format' / 'conv_cme_gis_format' / 'data' / 'dir_relationships.json'
        with struct_json.open() as fo:
            default_struct = json.load(fo)
        param = ProcessingParameterConvCmefModelDirSettings('coastalme_dir_struct_settings',
                                                           'COASTALME Directory Structure Settings',
                                                           optional=True,
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        # script path
        script = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'convert_coastalme_model_gis_format',
                              'conv_cme_gis_format', 'convert_coastalme_gis_format.py')
        ...
        if parameters['coastalme_dir_struct']:
            args.append('-coastalme-dir-struct')
            args.extend(['-coastalme-dir-struct-settings', json.dumps(parameters['coastalme_dir_struct_settings'])])
    ...
    def name(self):
        return 'Convert COASTALME Model GIS Format'
    ...
    def shortHelpString(self):
        return self.tr(
            "<a href=\"https://wiki.coastalme.com/index.php?title=Convert_COASTALME_Model_GIS_Format\">"
            "Documentation on COASTALME Wiki</a><p>"
            "<p>"
            "This tool runs a Python script that will convert a given COASTALME model's vector and raster GIS files "
            ...
            "Tool Inputs:<p>"
            "<ol>"
            "  <li>TCF: The location of the input COASTALME model's TCF file to be converted</li>"
            ...
            "COASTALME commands that read multiple geometry types on a single line will all be converted into a single "
            "database.</li>"
            ...
            "  <li>Root Folder - only required if the tool can't find the the root folder (traditionally called 'COASTALME') "
            ...
            "files and are not used to filter event sources provided in the COASTALME Event File (.tef). To use "
            "this functionality:</li>"
            "  <ol>"
            "    <li>Ensure 'Restrict Conversion by Scenario' is checked on</li>"
            "    <li>List scenario and event names in the text edit. This should be done in the same manner as COASTALME "
            ...
            " will be guessed unless a relative path from the root directory (e.g. COASTALME folder) is provided. The "
            ...
            "commonly this is a folder called 'COASTALME' - so the relative path may be 'model/gis/empty). If this is not "
            ...
            "  <li>Force COASTALME Directory Structure - If this flag is present, the output files will try and be placed "
            "in a standard COASTALME directory structure (ignoring the input directory structure completely)</li>"
            "  <li>Explode MultiPart Features - If checked, multipart geometry will be exploded into separate features. "
            "</ol>"
        )

    def shortDescription(self):
        return self.tr("Package and convert a COASTALME model's GIS files into a different format.")

    def createInstance(self):
        return ConvertCmeflowModelGisFormat()
