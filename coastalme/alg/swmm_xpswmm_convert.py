...
import coastalme.coastalme_swmm.xpswmm_xpx_convert as xpswmm_xpx_convert
...
class SwmmXpswmmConvert(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        if tcf:
            struct_json = (Path(
                __file__).parent.parent / 'convert_coastalme_model_gis_format' / 'conv_tf_gis_format' / 'data' /
            ...
            params_gis_format = {
                ...
                'output_crs': crs,
                'write_empties': True,
                'coastalme_dir_struct': True,
                'usescenarios': False,
                'rootfolder': None,
                'gpkg_name': '',
                'empty_directory': '',
                'coastalme_dir_struct_settings': default_struct,
                'explode_multipart': True,
            }

            # Convert the COASTALME model
            processing.run('COASTALME:Convert COASTALME Model GIS Format',
                           params_gis_format,
    ...
    def shortDescription(self):
        return self.tr("Convert 1D (xpx) and 2D (tcf) portions of XPSWMM model to COASTALME-SWMM.")
