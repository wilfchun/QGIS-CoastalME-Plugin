...
class PackageModel(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, configuration: typing.Dict[str, typing.Any] = ...) -> None:
        # COASTALME exe
        cme_settings = TF_Settings()
        cme_settings.Load()
        default = cme_settings.combined.cme_exe
        self.addParameter(
    ...
    def processAlgorithm(
        ...
        # save COASTALME exe
        if Path(parameters['cme_exe']).exists():
            cme_settings = TF_Settings()
            cme_settings.Load()
            cme_settings.project_settings.cme_exe = parameters['cme_exe']
            cme_settings.Save_Project()
            cme_settings.global_settings.cme_exe = parameters['cme_exe']
            cme_settings.Save_Global()

        tcf = Path(parameters['tcf'])
        outdir = tcf.parent / 'pm_{0}'.format(tcf.stem)

        # start building args list
        args = [parameters['cme_exe']]
        ...
        # run COASTALME
        feedback_.pushInfo('Running COASTALME with the following arguments...')
        args_ = args[1:]
        while args_:
            a = args_.pop(0)
    ...
    def shortDescription(self) -> typing.AnyStr:
        return self.tr("Package a COASTALME model")
