...
from coastalme.toc.toc import findAllRasterLyrs, findAllMeshLyrs, findAllVectorLyrs, coastalmeqgis_find_layer

# import processing
from .coastalmeqgis_library import *
...
from .coastalmeqgis_library import interpolate, convertStrftimToCmeviewftim, convertCmeviewftimToStrftim, browse
...
# ----------------------------------------------------------
#    coastalmeqgis increment selected layer
# ----------------------------------------------------------
...
class OverWrite_or_IncrementName_Dialog(QDialog):
	...
	def __init__(self, parent, db, layername):
		try:
			from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.gis import GPKG
		except ImportError:
...
class SuperSededRun:
	...
	def run(self):
		try:
			from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.gis import ogr_copy
		except ImportError:
...
from .forms.ui_coastalmeqgis_increment import *

class coastalmeqgis_increment_dialog(QDialog, Ui_coastalmeqgis_increment):
	def __init__(self, iface):
		...
		self.btnBrowseDatabase.clicked.connect(lambda: browse(self, 'output database', "COASTALME/increment_database",
												              "Spatial Database", "GPKG (*.gpkg *.GPKG)",
												              self.outfolder))
		self.pbDiscardDb_browse.clicked.connect(lambda: browse(self, 'output database', "COASTALME/increment_discard_db",
			                                                   "Spatial Database", "GPKG (*.gpkg *.GPKG)",
			                                                   self.leDiscardedDb))
		...
		if self.sourcelayer.currentIndex() == -1:
			...
			last_database = QSettings().value('COASTALME/increment_discard_db')
			if last_database:
			...
			if self.has_file_management_plugin:
				self.leRenameLayer.setText(coastalmeqgis_increment_fname(self.sourcelayer.currentText()))

		increment_db_method = QSettings().value('COASTALME/increment_db_method')
		...
		self.cbRenameLayer.setChecked(QSettings().value('COASTALME/increment_rename_layer_cb') == 'True')
	...
	def set_outputs(self, layer):
		...
		if self.isgpkg:
			...
			new_lyrname = coastalmeqgis_increment_fname(lyrname)
			...
			# 	else:
			# 		new_db = coastalmeqgis_increment_fname(Path(db).stem)
			# else:
			# 	new_db = new_lyrname
		...
		self.outfilename.setText(coastalmeqgis_increment_fname(lyrname))
		self.leRenameLayer.setText(coastalmeqgis_increment_fname(lyrname))
		self.leDiscardedLayer.setText(lyrname)

	def source_layer_changed(self):
		layer = coastalmeqgis_find_layer(self.sourcelayer.currentText())
	...
	def cb_rename_layer_toggled(self, checked):
		QSettings().setValue('COASTALME/increment_rename_layer_cb', str(checked))
	...
	def rbSSLayerToggled(self):
		QSettings().setValue('COASTALME/increment_db_method', self.buttonGroup_2.checkedButton().text())

	def discharded_db_changed(self):
		QSettings().setValue('COASTALME/increment_discard_db', self.leDiscardedDb.text())
	...
	def populateTableNames(self, layer=None):
		try:
			from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.gis import GPKG  # req. python 3.9+
		except ImportError:
			from .compatibility_routines import GPKG
		...
		for i, table in enumerate(tablenames):
			...
			if table == lyrname:
				item = QTableWidgetItem()
				item.setText(coastalmeqgis_increment_fname(table))
	...
	def browse_outfile(self):
		...
		if len(newname) > 0:
			...
			outfname = coastalmeqgis_increment_fname(fname)
	...
	def run(self):
		...
		if self.isgpkg and self.rbSaveLayerOut.isChecked():
			...
			if self.has_file_management_plugin and self.cbRenameLayer.isChecked():
				res = ss_run.rename_layer(plugins.get('file_management'),
				                          coastalmeqgis_find_layer(self.sourcelayer.currentText()),
		...
		if layername == self.iface.activeLayer().name():
			...
			layer = coastalmeqgis_find_layer(layername)
		dp = layer.dataProvider()
		ds = dp.dataSourceUri()
		...
		if self.isgpkg:
			...
			message = coastalmeqgis_duplicate_file(self.iface, layer, savename, False)
		if message != None:
...
# ----------------------------------------------------------
#    coastalmeqgis import empty coastalme files
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_import_empties import *


class coastalmeqgis_import_empty_cme_dialog(QDialog, Ui_coastalmeqgis_import_empty):
	def __init__(self, iface, project):
		QDialog.__init__(self)
		self.iface = iface
		self.setupUi(self)
		self.cmesettings = TF_Settings()
		showToolTip = QgsProject.instance().readBoolEntry("COASTALME", "import_empty_tooltip", True)[0]
		...
		# find out which coastalme engine to use
		self.engine = 'classic'  # set a default - other option is 'flexible mesh'
		self.cmesettings = TF_Settings()
		error, message = self.cmesettings.Load()
		if self.cmesettings.project_settings.engine:
			self.engine = self.cmesettings.project_settings.engine
			
		engine = self.cmesettings.combined.engine
		self.parent_folder_name = 'COASTALMEFV' if engine == 'flexible mesh' else 'COASTALME'

		self.browsedir.clicked.connect(lambda: browse(self, 'existing folder', 'COASTALME/empty_directory',
		...
		self.pbOk.clicked.connect(self.run)
		self.pbCancel.clicked.connect(self.reject)
		self.btnDatabaseBrowse.clicked.connect(lambda: browse(self, 'output database', "COASTALME/import_empty_database",
													          "Spatial Database", "gpkg (*.gpkg *.GPKG)",
		...
		if self.cmesettings.combined.empty_dir:
			self.emptydir.setText(self.cmesettings.combined.empty_dir.replace('/', os.sep).replace('\\', os.sep))
		elif self.cmesettings.combined.base_dir:
			subfolders = [self.parent_folder_name.lower(), 'model', 'gis', 'empty']
			emptydir = self.cmesettings.combined.base_dir
			...
			#self.emptydir.setText(os.path.join(self.cmesettings.combined.base_dir, self.parent_folder_name, "model", "gis", "empty"))
		else:
		...
		if kart_version > -1:
			...
			self.cb_import_into_kart.setChecked(QSettings().value('COASTALME/import_empty/kart_cb', 'False') == 'True')
			kart_repo = QSettings().value('COASTALME/import_empty/kart_repo', '')
			if not kart_repo:
				kart_repo = kart_repo_from_empty_folder(self.emptydir.text())
			self.le_kart_repo.setText(kart_repo)
			self.cb_import_into_kart.toggled.connect(lambda state: QSettings().setValue('COASTALME/import_empty/kart_cb', str(state)))
			self.le_kart_repo.textEdited.connect(lambda text: QSettings().setValue('COASTALME/import_empty/kart_repo', text))
		else:
		...
		b = True if QSettings().value('COASTALME/import_empty/convert_to_dbase', False) in ['true', 'True', True] else False
		self.cbConvertToDb.setChecked(b)

		db_setting = QSettings().value('COASTALME/import_empty/db_output_option', 'separate')
		...
		db_path = QSettings().value('COASTALME/import_empty/db_path', '<Existing or New Database>')
		self.leDatabaseBrowse.setText(db_path)

	def save_settings_convert_to_database(self):
		QSettings().setValue('COASTALME/import_empty/convert_to_dbase', self.cbConvertToDb.isChecked())

	def save_settings_to_separate(self):
		if self.rbDatabaseSeparate.isChecked():
			QSettings().setValue('COASTALME/import_empty/db_output_option', 'separate')

	def save_settings_group_geometries(self):
		if self.rbDatabaseGrouped.isChecked():
			QSettings().setValue('COASTALME/import_empty/db_output_option', 'geometries')

	def save_settings_group_together(self):
		if self.rbDatabaseOne.isChecked():
			QSettings().setValue('COASTALME/import_empty/db_output_option', 'grouped')

	def save_settings_database(self):
		QSettings().setValue('COASTALME/import_empty/db_path', self.leDatabaseBrowse.text())
		
	def saveDir(self, type_):
		if type_ == 'project':
			self.cmesettings.project_settings.empty_dir = self.emptydir.text()
			self.cmesettings.Combine()
			self.cmesettings.Save_Project()
		elif type_ == 'global':
			self.cmesettings.global_settings.empty_dir = self.emptydir.text()
			self.cmesettings.Save_Global()
	...
	def updateToolTip(self):
		...
		for item in items:
			...
			if tooltip['wiki link'] is not None:
				...
				self.teToolTip.setFontPointSize(10)
				self.teToolTip.setFontWeight(QT_FONT_BOLD)
				self.teToolTip.append('COASTALME Wiki')
				self.teToolTip.append('\n')
			...
			if tooltip['manual link'] is not None:
				...
				self.teToolTip.setFontPointSize(10)
				self.teToolTip.setFontWeight(QT_FONT_BOLD)
				self.teToolTip.append('COASTALME Manual')
				self.teToolTip.append('\n')
				page = ''
	...
	def run(self):
		...
		if self.cb_import_into_kart.isChecked():
			...
			message = coastalmeqgis_import_empty_cme(self.iface, basedir, runID, empty_types, points, lines, regions, self, databaseOption, databaseLoc, convert)
		#message = coastalmeqgis_create_cme_dir(self.iface, crs, basedir)
		if message == 'pass':
			pass
...
# ----------------------------------------------------------
#    coastalmeqgis Run COASTALME (Simple)
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_run_cme_simple import *


class coastalmeqgis_run_cme_simple_dialog(QDialog, Ui_coastalmeqgis_run_cme_simple):
	def __init__(self, iface, project):
		QDialog.__init__(self)
		self.iface = iface
		self.setupUi(self)
		...
		if self.cmesettings.combined.cme_exe:
			cmeexe = self.cmesettings.combined.cme_exe
			self.exefolder, dum  = os.path.split(cmeexe)
			project_loaded = True
		else: #load last used exe
			cmeexe = self.cmesettings.get_last_exe()
		if self.cmesettings.combined.base_dir:
			self.cmefolder = self.cmesettings.combined.base_dir
			self.runfolder = os.path.join(self.cmefolder,'COASTALME','runs')
			project_loaded = True
		else: #load last used directory
			self.runfolder = self.cmesettings.get_last_run_folder()
		...
		self.COASTALME_exe.setText(cmeexe)
	...
	def browse_tcf(self):
		# Get the file name
		inFileName = QFileDialog.getOpenFileName(self, 'Select COASTALME Control File', self.runfolder,
		...
		#self.cmesettings.save_last_exe(self,last_exe)("COASTALME_Run_COASTALME/tcf", inFileName)
		
		self.tcf.setText(inFileName)
		head, tail = os.path.split(inFileName)
		if head != os.sep and head.lower() != 'c:\\' and head != '':
			self.cmesettings.save_last_run_folder(head)
			#self.cmesettings.setValue("COASTALME_Run_COASTALME/tcfDir", head)

	def browse_exe(self):
		# Get the file name
		inFileName = QFileDialog.getOpenFileName(self, 'Select COASTALME exe', self.exefolder, "COASTALME Executable (*.exe)")
		inFileName = inFileName[0]
		if len(inFileName) == 0: # If the length is 0 the user pressed cancel 
			return
		# Store the exe location and path we just looked in
		self.cmesettings.save_last_exe(inFileName)
		self.exe = inFileName
		#self.cmesettings.setValue("COASTALME_Run_COASTALME/exe", inFileName)
		self.COASTALME_exe.setText(inFileName)
		#head, tail = os.path.split(inFileName)
		#if head <> os.sep and head.lower() <> 'c:\\' and head <> '':
		#	self.cmesettings.setValue("COASTALME_Run_COASTALME/exeDir", head)

	def on_error(self, err):
		self._running = False
		QMessageBox.critical(self, 'Run COASTALME', err)

	def on_finish(self, text):
		self._running = False
		dlg = QDialog(parent=self)
		dlg.setWindowTitle('COASTALME')
		layout = QHBoxLayout()
	...
	def update(self, finished):
		if not finished:
			...
			if self.prog_inc > 5:
				self.prog_inc = 1
			text = 'COASTALME Running' + ' .' * self.prog_inc
	...
	def run(self):
		...
		cmeexe = unicode(self.COASTALME_exe.displayText()).strip()
		capture_output = self.cbCaptureOutput.isChecked()
		if capture_output:
			self.progressText.setText('COASTALME Running')
			self.progressText.show()
			self.pbKillRun.show()
		self.prog_inc = 0
		self._kill_run = False
		self.run_coastalme = RunCmeflow(cmeexe, tcf, capture_output, self.update)
		self.run_coastalme.finished.connect(self.on_finish)
		self.run_coastalme.error.connect(self.on_error)
		self.run_coastalme.run()
		if capture_output:
			self.pbOk.setEnabled(False)
			self._running = True

# ----------------------------------------------------------
#    coastalmeqgis points to lines
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_line_from_points import *

class coastalmeqgis_line_from_points(QDialog, Ui_coastalmeqgis_line_from_point):
	def __init__(self, iface):
		QDialog.__init__(self)
		self.iface = iface
		self.setupUi(self)
	...
	def source_changed(self):
		layername = unicode(self.sourcelayer.currentText())
		self.cLayer = coastalmeqgis_find_layer(layername)
	...
	def run(self):
		import math
		layername = unicode(self.sourcelayer.currentText())
		self.layer = coastalmeqgis_find_layer(layername)
...
# ----------------------------------------------------------
#    coastalmeqgis configure coastalme project
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_configure_coastalme_project import *


class coastalmeqgis_configure_cme_dialog(QDialog, Ui_coastalmeqgis_configure_cme):
	def __init__(self, iface, project, parent=None):
		QDialog.__init__(self, parent)
		self.iface = iface
		self.setupUi(self)
		self.canvas = self.iface.mapCanvas()
		self.setWindowTitle('Configure / Create COASTALME Project')
		cLayer = self.canvas.currentLayer()
		self.cmesettings = TF_Settings()
		self.crs = None
		fname = ''
		...
		#set fields
		if self.cmesettings.project_settings.base_dir:
			self.outdir.setText(self.translate(self.cmesettings.project_settings.base_dir))
		elif self.cmesettings.global_settings.base_dir:
			self.outdir.setText(self.translate(self.cmesettings.global_settings.base_dir))
		else:
			self.outdir.setText("Not Yet Set")
			self.outdir.setText("Not Yet Set")
		
		if self.cmesettings.project_settings.cme_exe:
			self.COASTALME_exe.setText(self.translate(self.cmesettings.project_settings.cme_exe))
		elif self.cmesettings.global_settings.cme_exe:
			self.COASTALME_exe.setText(self.translate(self.cmesettings.global_settings.cme_exe))
		else:
			self.COASTALME_exe.setText("Not Yet Set")

		if self.cmesettings.project_settings.CRS_ID:
			self.form_crsID.setText(self.cmesettings.project_settings.CRS_ID)
			self.crs = QgsCoordinateReferenceSystem()
			success = self.crs.createFromString(self.cmesettings.project_settings.CRS_ID)
			if success:
				self.crsDesc.setText(self.crs.description())
		elif self.cmesettings.global_settings.CRS_ID:
			self.form_crsID.setText(self.cmesettings.global_settings.CRS_ID)
			self.crs = QgsCoordinateReferenceSystem()
			success = self.crs.createFromString(self.cmesettings.global_settings.CRS_ID)
			if success:
				self.crsDesc.setText(self.crs.description())
		else:
			if cLayer:
		...
		# engine
		if self.cmesettings.project_settings.engine:
			if self.cmesettings.project_settings.engine == 'classic':
				self.rbCmeflowCla.setChecked(True)
			elif self.cmesettings.project_settings.engine == 'flexible mesh':
				self.rbCmeflowFM.setChecked(True)
			else:
				self.rbCmeflowCla.setChecked(True)
		elif self.cmesettings.global_settings.engine:
			if self.cmesettings.global_settings.engine == 'classic':
				self.rbCmeflowCla.setChecked(True)
			elif self.cmesettings.global_settings.engine == 'flexible mesh':
				self.rbCmeflowFM.setChecked(True)
			else:
				self.rbCmeflowCla.setChecked(True)
				
		# tutorial
		if self.cmesettings.combined.tutorial:
			if type(self.cmesettings.combined.tutorial) is str:
				self.cmesettings.combined.tutorial = True if self.cmesettings.combined.tutorial == 'True' else False
			self.cbCmetorial.setChecked(self.cmesettings.combined.tutorial)
	...
	def browse_exe(self):
		...
		#get last used dir
		last_exe = self.cmesettings.get_last_exe()			
		if last_exe:
		...
		if sys.placmeorm == 'win32':
			ftypes = "COASTALME Executable (*.exe)"
		else:
			ftypes = "COASTALME Executable (*)"
		inFileName = QFileDialog.getOpenFileName(self, 'Select COASTALME exe', last_dir, ftypes)
		...
		if len(inFileName) == 0: # If the length is 0 the user pressed cancel 
			return
		# Store the exe location and path we just looked in
		self.COASTALME_exe.setText(self.translate(inFileName))
		self.cmesettings.save_last_exe(inFileName)
	...
	def layer_changed(self):
		...
		if layername != "Use saved projection":
			...
			if layer != None:
				...
				self.crsDesc.setText(self.crs.description())
	def run(self):
		cme_prj = self.form_crsID.displayText().strip()
		engine = 'flexible mesh' if self.rbCmeflowFM.isChecked() else 'classic'
		parent_folder_name = 'COASTALMEFV' if engine == 'flexible mesh' else 'COASTALME'
		tutorial = self.cbCmetorial.isChecked()
		...
		cmeexe = self.COASTALME_exe.displayText().strip()
		
		baseexe = os.path.basename(cmeexe)
		if 'coastalmefv' in baseexe.lower():
			if engine == 'classic':
				fv = QMessageBox.question(self, "COASTALME Project Settings",
				                          "Executable Appears to be COASTALME Flexible Mesh . . . "
				                          "Would You Like to Create a COASTALME Flexible Mesh Project "
				                          "Instead of COASTALME Classic / HPC?",
				                          QT_MESSAGE_BOX_YES | QT_MESSAGE_BOX_NO | QT_MESSAGE_BOX_CANCEL)
		...
		self.cmesettings.project_settings.CRS_ID = cme_prj
		self.cmesettings.project_settings.cme_exe = cmeexe
		self.cmesettings.project_settings.base_dir = basedir
		self.cmesettings.project_settings.engine = engine
		self.cmesettings.project_settings.tutorial = tutorial
		cme_folder = 'COASTALME' if engine == 'classic' else 'COASTALMEFV'
		if re.findall(r'COASTALME\\?$', basedir, flags=re.IGNORECASE):
			empty_dir = os.path.join(basedir, 'model', 'gis', 'empty')
		else:
			empty_dir = os.path.join(basedir, cme_folder, 'model', 'gis', 'empty')

		self.cmesettings.project_settings.empty_dir = empty_dir
		error, message = self.cmesettings.Save_Project()
		if error:
		...
		if self.cbGlobal.isChecked():
			self.cmesettings.global_settings.CRS_ID = cme_prj
			self.cmesettings.global_settings.cme_exe = cmeexe
			self.cmesettings.global_settings.base_dir = basedir
			self.cmesettings.global_settings.engine = engine
			self.cmesettings.global_settings.tutorial = tutorial
			self.cmesettings.global_settings.empty_dir = empty_dir
			error, message = self.cmesettings.Save_Global()
			if error:
		...
		crs.createFromString(cme_prj)
		if self.cbCreate.isChecked():
			gisFormat = 'GPKG' if self.rbGPKG.isChecked() else 'SHP'
			message = coastalmeqgis_create_cme_dir(self, crs, basedir, engine, tutorial, gisFormat)
			if message == 'user cancelled':
				return
			if message != None:
				QMessageBox.critical(self.iface.mainWindow(), "Creating COASTALME Directory ", message)
				return
		...
		if self.cbRun.isChecked():
			...
			message = run_coastalme(self.iface, cmeexe, runfile)
			if message != None:
...
# ----------------------------------------------------------
#    coastalmeqgis flow trace
# ----------------------------------------------------------

from .forms.ui_coastalmeqgis_flowtrace import *

class coastalmeqgis_flowtrace_dialog(QDialog, Ui_coastalmeqgis_flowtrace):
	def __init__(self, iface):
		QDialog.__init__(self)
		self.iface = iface
		self.setupUi(self)
...
#
	def run_clicked(self):
		...
		for f in self.cLayer.getFeatures():
			...
			cme_selected.append(False) #not selected by default
		...
		for feature in features:
			...
			cme_selected[feature.id()-1] = True
...
# MJS added 11/02
# ----------------------------------------------------------
#    coastalmeqgis import check files
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_import_check import *


class coastalmeqgis_import_check_dialog(QDialog, Ui_coastalmeqgis_import_check):
	def __init__(self, iface, project):
		QDialog.__init__(self)
		self.iface = iface
		self.setupUi(self)
		...
		# load stored settings
		self.last_chk_folder = self.cmesettings.get_last_chk_folder()
		error, message = self.cmesettings.Load() #exe, coastalme dircetory and projection
		if error:
			QMessageBox.information( self.iface.mainWindow(),"Error", "Error Loading Settings: "+message)
		...
		if self.last_chk_folder == "Undefined":
			if self.cmesettings.combined.base_dir:
				...
				checkdir = self.cmesettings.combined.base_dir
				for i, subfolder in enumerate(subfolders):
	...
	def browse_empty_dir(self):
		...
		if newname != None:
			try:
				...
				self.cmesettings.save_last_chk_folder(newname)
			except:
	...
	def run(self):
		...
		# run create dir script
		#message = coastalmeqgis_import_check_cme(self.iface, basedir, runID, empty_types, points, lines, regions)
		message = coastalmeqgis_import_check_cme(self.iface, basedir, runID, showchecks)
		#message = coastalmeqgis_create_cme_dir(self.iface, crs, basedir)
		if message != None:
			QMessageBox.critical(self.iface.mainWindow(), "Importing COASTALME Empty File(s)", message)


# ----------------------------------------------------------
#    coastalmeqgis insert coastalme attributes
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_insert_coastalme_attributes import *
from .coastalmeqgis_settings import TF_Settings

class coastalmeqgis_insert_coastalme_attributes_dialog(QDialog, Ui_coastalmeqgis_insert_coastalme_attributes):
	def __init__(self, iface, project):
		QDialog.__init__(self)
		self.iface = iface
		self.setupUi(self)
		...
		self.pbOk.clicked.connect(self.run)
		self.pbCancel.clicked.connect(self.reject)
		self.browseDatabase.clicked.connect(lambda: browse(self, 'output database', "COASTALME/import_empty_database",
													      "Spatial Database", "gpkg (*.gpkg *.GPKG)",
													      self.leDatabase))
		self.pbSaveToProject.clicked.connect(lambda: self.saveDir('project'))
		self.pbSaveToGlobal.clicked.connect(lambda: self.saveDir('global'))
	...
	def run(self):
		...
		inputLayer = coastalmeqgis_find_layer(inputFile)
		...
		# run insert coastalme attributes script
		message = coastalmeqgis_insert_cme_attributes(self.iface, inputLayer, basedir, runID, template, lenFields, self,
		                                          output_dbase)
...
# ----------------------------------------------------------
#    coastalmeqgis cmeplot axis editor
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_cmeplotAxisEditor import *


class coastalmeqgis_cmeplotAxisEditor(QDialog, Ui_cmeplotAxisEditor):
...
# ----------------------------------------------------------
#    coastalmeqgis cmeplot axis labels
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_cmeplotAxisLabels import *


class coastalmeqgis_cmeplotAxisLabels(QDialog, Ui_cmeplotAxisLabel):
...
# ----------------------------------------------------------
#    coastalmeqgis scenario selection
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_scenarioSelection import *


class coastalmeqgis_scenarioSelection_dialog(QDialog, Ui_scenarioSelection):
...
# ----------------------------------------------------------
#    coastalmeqgis event selection
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_eventSelection import *


class coastalmeqgis_eventSelection_dialog(QDialog, Ui_eventSelection):
...
# ----------------------------------------------------------
#    coastalmeqgis mesh selection
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_meshSelection import *


class coastalmeqgis_meshSelection_dialog(QDialog, Ui_meshSelection):
	...
	def run(self):
		...
		if selection:
			...
			QMessageBox.information(self.iface.mainWindow(), 'Cmeview', 'Please select a result layer to save style.')


# ----------------------------------------------------------
#    coastalmeqgis Output Zone selection
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_outputZoneSelection import *


class coastalmeqgis_outputZoneSelection_dialog(QDialog, Ui_outputZoneSelection):
...
class coastalmeqgis_outputSelection_dialog(QDialog, Ui_outputZoneSelection):
...
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_CmeOptionsDialog import *


class CmeOptionsDialog(QDialog, Ui_CmeViewOptions):
	def __init__(self, CmeOptions):
		qv = Qgis.QGIS_VERSION_INT

		QDialog.__init__(self)
		self.setupUi(self)
		self.tuOptions = CmeOptions
		...
		# date format
		self.leDateFormat.setText(convertStrftimToCmeviewftim(self.tuOptions.dateFormat))
		...
		# curtain vector
		self.pbVectorTVDefaults.clicked.connect(self.curtain_vector_set_coastalme_viewer_defaults)
	...
	def cb_copy_mesh_toggled(self, e):
		...
		QSettings().setValue('COASTALME/cmeview_copy_mesh', self.tuOptions.copy_mesh)

	def cb_show_mesh_copy_dlg_toggled(self, e):
		self.tuOptions.show_copy_mesh_dlg = e
		QSettings().setValue('COASTALME/cmeview_show_copy_mesh_dlg', self.tuOptions.show_copy_mesh_dlg)

	def cb_del_copied_mesh_toggled(self, e):
		self.tuOptions.del_copied_res = e
		QSettings().setValue('COASTALME/cmeview_del_copied_res', self.tuOptions.del_copied_res)

	def del_all_cache_res(self):
		from .coastalmeqgis_cmeviewer.tmp_result import TmpResult
	...
	def curtain_vector_set_coastalme_viewer_defaults(self):
	...
	def curtain_vector_scale_changed(self, e):
		...
		QSettings().setValue("COASTALME/cmeview_curtain_vector_scale", self.sbScale.value())
	...
	def curtain_vector_scale_units_changed(self, e):
		...
		QSettings().setValue("COASTALME/cmeview_curtain_vector_scale_units", self.cboScaleUnits.currentText())
	...
	def curtain_vector_units_changed(self, e):
		...
		QSettings().setValue("COASTALME/cmeview_curtain_vector_units", self.cboUnits.currentText())

	def curtain_vector_width_changed(self, e):
		self.tuOptions.curtain_vector_width = self.sbWidth.value()
		QSettings().setValue("COASTALME/cmeview_curtain_vector_width", self.sbWidth.value())

	def curtain_vector_head_width_changed(self, e):
		self.tuOptions.curtain_vector_head_width = self.sbHeadWidth.value()
		QSettings().setValue("COASTALME/cmeview_curtain_vector_head_width", self.sbHeadWidth.value())

	def curtain_vector_head_length_changed(self, e):
		self.tuOptions.curtain_vector_head_length = self.sbHeadLength.value()
		QSettings().setValue("COASTALME/cmeview_curtain_vector_head_length", self.sbHeadLength.value())
	...
	def curtain_vector_horizontal_factor_changed(self, e):
		...
		QSettings().setValue("COASTALME/cmeview_curtain_vector_horizontal_factor", self.sbHorizontalFactor.value())

	def vertical_velocity_factor_changed(self, e):
		self.tuOptions.curtain_vector_vertical_factor = self.sbVertVelFactor.value()
		QSettings().setValue("COASTALME/cmeview_curtain_vector_vertical_factor", self.sbVertVelFactor.value())

	def cbPlotInactiveAreasToggled(self, e):
		self.tuOptions.plotInactiveAreas = bool(self.cbPlotInactiveAreas.isChecked())
		QSettings().setValue("COASTALME/cmeview_plot_inactive_areas", self.tuOptions.plotInactiveAreas)

	def tcfLoadMethodChanged(self, tcf_load_method):
		self.tuOptions.tcfLoadMethod = tcf_load_method
		QSettings().setValue("COASTALME/cmeview_tcf_load_method", tcf_load_method)

	def saveDefaultLayout(self, layoutType):
		QSettings().setValue("COASTALME/cmeview_defaultlayout", layoutType)

	def saveDefaultFontSize(self, value):
		size = self.sbDefaultFontSize.value()
		QSettings().setValue("COASTALME/cmeview_defaulcmeontsize", size)

	def updatePreview(self):
		self.tuOptions.dateFormat, self.tuOptions._dateFormat = convertCmeviewftimToStrftim(self.leDateFormat.text())
		self.datePreview.setText(self.tuOptions._dateFormat.format(self.date))
	...
	def run(self):
		...
		settings.setValue('COASTALME/cmeview_zeroTime', self.tuOptions.zeroTime)
		
		# format time
		self.tuOptions.dateFormat, self.tuOptions._dateFormat = convertCmeviewftimToStrftim(self.leDateFormat.text())
		settings.setValue('COASTALME/cmeview_dateFormat', self.tuOptions.dateFormat)
		settings.setValue('COASTALME/cmeview__dateFormat', self.tuOptions._dateFormat)
		...
		settings.setValue("COASTALME/cmeview_plotbackgroundcolour", self.tuOptions.plotBackgroundColour)
		...
		settings.setValue("COASTALME/cmeview_iconsize", self.tuOptions.iconSize)
...
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_selectedElements import *


class CmeSelectedElementsDialog(QDialog, Ui_selectedElements):
...
class CmeBatchPlotExportDialog(QDialog, Ui_BatchPlotExport):
	def __init__(self, CmeView, **kwargs):
		QDialog.__init__(self)
		self.setupUi(self)
		self.tuView = CmeView
		self.iface = CmeView.iface
		self.project = CmeView.project
		self.canvas = CmeView.canvas
		...
		self.btnBrowse.clicked.connect(lambda: browse(self, 'existing folder', 'COASTALME/batch_export', 'Ouput Folder',
		                                              "", self.outputFolder))
		self.btnAddRes.clicked.connect(lambda: browse(self, 'existing files', 'COASTALME/batch_export_res', 'COASTALME Results',
	...
	def populateNameAttributes(self):
		...
		layer = coastalmeqgis_find_layer(self.cbGISLayer.currentText())
		if layer is not None:
	...
	def populateResultTypes(self):
		from .coastalmeqgis_cmeviewer.coastalmeqgis_turesults import CmeResults

		onlyTemporal = False
		gisLayer = coastalmeqgis_find_layer(self.cbGISLayer.currentText())
		...
		for res in self.lwResultMesh.selectedItems():
			layer = coastalmeqgis_find_layer(res.text())
			if layer is not None:
				if firstFound:
				...
				if layer.name() in self.tuView.tuResults.results:
					...
					for type, t in r.items():
						...
						if not onlyTemporal or 'isTemporal' not in t or t['isTemporal']:
							type = CmeResults.stripMaximumName(type)
	...
	def browse(self):
		settings = QSettings()
		outFolder = settings.value('COASTALME/batch_export')
		startDir = None
		...
		if outFolder:
			...
			settings.setValue('COASTALME/batch_export', outFolder)
	...
	def run(self):
		...
		settings = QSettings()
		settings.setValue('COASTALME/batch_export', self.outputFolder.text())
		...
		for r in resultMesh:
			...
			if neededLoading:
				layer = coastalmeqgis_find_layer(r2)
				if layer is not None:
...
class CmeUserPlotDataPlotView(QDialog, Ui_UserPlotData):
	def __init__(self, iface, CmeUserPlotData):
		QDialog.__init__(self)
		self.setupUi(self)
		self.iface = iface
		self.tuUserPlotData = CmeUserPlotData
...
from .coastalmeqgis_cmeviewer.coastalmeqgis_cmeuserplotdata import CmeUserPlotDataSet


class CmeUserPlotDataTableView(QDialog, Ui_UserTableData):
	def __init__(self, iface, CmeUserPlotData):
		QDialog.__init__(self)
		self.setupUi(self)
		self.iface = iface
	...
	def saveData(self, widget=None, dummy=False):
		...
		if dummy:
			data = CmeUserPlotDataSet('dummy', [x, y], 'time series', False, 100, dates)
			return data
		else:
	...
	def showPlot(self):
		...
		self.tableDialog = CmeUserPlotDataPlotView(self.iface, data)
...
class CmeUserPlotDataImportDialog(QDialog, Ui_UserPlotDataImportDialog):
	def __init__(self, iface):
		QDialog.__init__(self)
		self.setupUi(self)
		self.iface = iface
		...
		self.btnBrowse.clicked.connect(lambda: browse(self, 'existing file', 'COASTALME/import_user_data',
...
class CmeUserPlotDataManagerDialog(QDialog, Ui_UserPlotDataManagerDialog):
	
	def __init__(self, iface, CmeUserPlotDataManager, **kwargs):
		QDialog.__init__(self)
		self.setupUi(self)
		self.tuUserPlotDataManager = CmeUserPlotDataManager
		self.iface = iface
	...
	def addData(self, **kwargs):
		self.addDataDialog = CmeUserPlotDataImportDialog(self.iface)
		if 'add_data' in kwargs:
			self.pythonPopulateGui(**kwargs)
		else:
	...
	def showDataTable(self):
		...
		for item in selectedItems:
			...
			self.tableDialog = CmeUserPlotDataTableView(self.iface, data)
			self.tableDialog.exec()
...
# ----------------------------------------------------------
#    Filter and Sort COASTALME Layers in Map Window
# ----------------------------------------------------------
from .forms.ui_filter_sort_COASTALME_layers import *
...
class FilterSortLayersDialog(QDialog, Ui_FilterAndSortLayers):
	...
	def initialiseTable(self):
		"""Set up TableWidget with open coastalme layer types"""
		...
		# collect open layer coastalme types
		inputLayers = getOpenCOASTALMELayers('input_types')
		self.tableWidget.setRowCount(len(inputLayers) + 1)
		for i, inputLayer in enumerate(inputLayers):
			# coastalme type label
			item = QTableWidgetItem(0)
	...
	def filter(self):
		...
		# input layers
		coastalmeLayers = getOpenCOASTALMELayers('input_all')
		for coastalmeLayer in coastalmeLayers:
			if self.type2buttonGroup['all_layers'].checkedId() == 0:
				filterProp[coastalmeLayer] = 'on'
			elif self.type2buttonGroup['all_layers'].checkedId() == 2:
				filterProp[coastalmeLayer] = 'off'
			else:
				comp = coastalmeLayer.split('_')
				...
				filterProp[coastalmeLayer] = filterKey[self.type2buttonGroup[ltype].checkedId()]
		
		# check layers
		if self.bgCheck.checkedId() != 1:
			checkLayers = getOpenCOASTALMELayers('check_all')
...
#    COASTALME Utilities
# ----------------------------------------------------------
from .forms.COASTALME_utilities import *


class CmeflowUtilitiesDialog(QDialog, Ui_utilitiesDialog):
	def __init__(self, iface):
		QDialog.__init__(self)
		self.setupUi(self)
		self.iface = iface
		...
		self.cbo_output_format.setCurrentText(QSettings().value('COASTALME/asc_to_asc_output_format', 'TIF'))
		...
		self.cboToGisMeshDataset.currentIndexChanged.connect(self.coastalme_to_gis_result_type_changed)

		self.leAsc2Asc.textChanged.connect(lambda x: QSettings().setValue("COASTALME_Utilities/ASC_to_ASC_exe", x))
		self.leCOASTALME2GIS.textChanged.connect(lambda x: QSettings().setValue("COASTALME_Utilities/COASTALME_to_GIS_exe", x))
		self.leRes2Res.textChanged.connect(lambda x: QSettings().setValue("COASTALME_Utilities/Res_to_Res_exe", x))
		self.le12da2GIS.textChanged.connect(lambda x: QSettings().setValue("COASTALME_Utilities/12da_to_from_GIS_exe", x))
		self.leConvert2TS1.textChanged.connect(lambda x: QSettings().setValue("COASTALME_Utilities/Convert_to_TS1_exe", x))
		self.leTin2Tin.textChanged.connect(lambda x: QSettings().setValue("Tin_to_Tin executable location", x))
		self.leXSGenerator.textChanged.connect(lambda x: QSettings().setValue("COASTALME_Utilities/xsGenerator_exe", x))

		self.connectBrowseButtons()

	def toggle_output_format(self):
		QSettings().setValue('COASTALME/asc_to_asc_output_format', self.cbo_output_format.currentText())
	...
	def get_asc_to_asc_version(self, button_pushed=True):
		...
		if not path or not os.path.exists(path):
			if button_pushed:
				QMessageBox.warning(self, "COASTALME Utilities", "Warning: ASC to ASC executable is not valid")
			return
	...
	def findFile(self):
		if not self.leAdvWorkingDir.text():
			QMessageBox.warning(self, "COASTALME Utilities",
		...
		try:
			...
			QMessageBox.warning(self, "COASTALME Utilities", "Warning: file is on a different drive than working directory")
			relPaths = files
	...
	def downloadExecutables(self):
		...
		if sys.placmeorm == 'win32':
			...
			self.downloadUtilities = DownloadCmeflowUtilities()
			...
			QMessageBox.critical(self, "COASTALME Utilities", "Download feature only available on Windows")
	...
	def downloadFinished(self, e):
		utilities = {'asc_to_asc': self.leAsc2Asc, 'coastalme_to_gis': self.leCOASTALME2GIS,
		...
		if self.downloadUtilities.error:
			QMessageBox.critical(self, "COASTALME Utilities", self.downloadUtilities.errmsg)
	...
	def populateGrids(self):
		...
		for raster in rasters:
			layer = coastalmeqgis_find_layer(raster)
			layer_helper = LayerHelper(layer)
			if layer_helper.is_coastalme_type(self.asc_to_asc_version):
				grids.append(raster)
	...
	def populateGis(self):
		...
		gis = []
		for layer in layers:
			layer = coastalmeqgis_find_layer(layer)
			layer_helper = LayerHelper(layer)
			if layer_helper.is_coastalme_type(self.asc_to_asc_version):
	...
	def addGrid(self):
		if self.cboGrid.currentText():
			if self.cboGrid.currentText().replace('/', os.sep).count(os.sep) > 0:
				...
				layer = coastalmeqgis_find_layer(self.cboGrid.currentText().strip('"').strip("'"))
				if layer is not None:
					layer_helper = LayerHelper(layer)
					dataSource = layer_helper.coastalme_path
	...
	def commonUtilityChanged(self, i):
		if i == 0:  # asc_to_asc
			...
			self.coastalme2Gis.setVisible(False)
			self.res2Res.setVisible(False)
		elif i == 1:
			self.asc2Asc.setVisible(False)
			self.coastalme2Gis.setVisible(True)
			self.res2Res.setVisible(False)
		elif i == 2:
			self.asc2Asc.setVisible(False)
			self.coastalme2Gis.setVisible(False)
	...
	def applyIcons(self):
		...
		browseButtons = [self.btnBrowseComOutputDir, self.btnBrowseDiffGrid1, self.btnBrowseDiffGrid2,
		                 ...
		                 self.btnBrowseAsc2Asc, self.btnBrowseCOASTALME2GIS, self.btnBrowseRes2Res, self.btnBrowse12da2GIS,
	...
	def populateXmdfInfo(self):
		if not self.leMeshToGis.text():
			QMessageBox.critical(self, "COASTALME Utilities", "Please specify an XMDF first.")
			return

		if os.path.splitext(self.leMeshToGis.text())[1].lower() != '.xmdf':
			QMessageBox.critical(self, "COASTALME Utilities", "Please file must be an XMDF to load information.")
			return

		if not self.leRes2Res.text():
			QMessageBox.critical(self, "COASTALME Utilities", "Must specify res_to_res.exe location before populating XMDF information.")
			return
		...
		if error:
			QMessageBox.critical(self, "COASTALME Utilities", "Error occurred reading XMDF header information.\nPlease see output log information for more information.")
		...
		if not self.xmdf_header.loaded:
			QMessageBox.critical(self, "COASTALME Utilities", "Error occurred reading XMDF header information. Please contact support@coastalme.com if problem persists.")
			return
		...
		self.coastalme_to_gis_result_type_changed(None)

	def coastalme_to_gis_result_type_changed(self, e):
	...
	def check(self):
		if self.rbCommonFunctions.isChecked():
			# if self.leOutputName.text():
				...
				# 	QMessageBox.critical(self, "COASTALME Utilities", "Must specify output location if specifying output name")
				# 	return
				# if not os.path.exists(self.leComOutputDir.text().strip('"').strip("'")):
				# 	QMessageBox.critical(self, "COASTALME Utilities", "Output location does not exist")
				# 	return
			if self.cboCommonUtility.currentIndex() == 0:  # asc_to_asc
				if not self.leAsc2Asc.text():
					QMessageBox.critical(self, "COASTALME Utilities", "Must specify asc_to_asc.exe location")
					return
				else:
					if not os.path.exists(self.leAsc2Asc.text().strip('"').strip("'")):
						QMessageBox.critical(self, "COASTALME Utilities", "asc_to_asc.exe location does not exist")
						return
				if self.rbAscDiff.isChecked():
					if not self.cboDiffGrid1.currentText() or not self.cboDiffGrid2.currentText():
						QMessageBox.critical(self, "COASTALME Utilities", "Must specify grid 1 and grid 2")
						return
					if self.cboDiffGrid1.currentText().replace('/', os.sep).count(os.sep) == 0:
						layer = coastalmeqgis_find_layer(self.cboDiffGrid1.currentText().strip('"').strip("'"))
						if layer is None:
							QMessageBox.critical(self, "COASTALME Utilities", "Could not find grid 1 in workspace")
							return
					if self.cboDiffGrid2.currentText().replace('/', os.sep).count(os.sep) == 0:
						layer = coastalmeqgis_find_layer(self.cboDiffGrid2.currentText().strip('"').strip("'"))
						if layer is None:
							QMessageBox.critical(self, "COASTALME Utilities", "Could not find grid 2 in workspace")
							return
					if self.cboDiffGrid1.currentText().strip('"').strip("'") == \
							self.cboDiffGrid2.currentText().strip('"').strip("'"):
						reply = QMessageBox.warning(self, "COASTALME Utilities",
					...
					if not self.lwGrids.count():
						QMessageBox.critical(self, "COASTALME Utilities", "Must specify at least one grid")
						return
				elif self.rbBrklineFunc.isChecked():
					if not self.cboBrkline_vector.currentText() and not self.cboBrkline_raster.currentText():
						QMessageBox.critical(self, "COASTALME Utilities", "Must specify a vector and raster for breakline function")
						return
					if not coastalmeqgis_find_layer(self.cboBrkline_vector.currentText()) and not os.path.exists(self.cboBrkline_vector.currentText()):
						QMessageBox.critical(self, "COASTALME Utilities", "Breakline function - Could not find vector layer")
						return
					if not coastalmeqgis_find_layer(self.cboBrkline_raster.currentText()) and not os.path.exists(self.cboBrkline_raster.currentText()):
						QMessageBox.critical(self, "COASTALME Utilities", "Breakline function - Could not find raster layer")
						return
				else:
					if self.lwGrids.count() < 2:
						QMessageBox.critical(self, "COASTALME Utilities", "Must specify 2 or more grids")
						return
					grids = []
					for i in range(self.lwGrids.count()):
						item = self.lwGrids.item(i)
						...
						if grid in grids:
							...
							grids.append(grid)
			elif self.cboCommonUtility.currentIndex() == 1:  # coastalme_to_gis
				if not self.leCOASTALME2GIS.text():
					QMessageBox.critical(self, "COASTALME Utilities", "Must specify coastalme_to_gis.exe location")
					return
				else:
					if not os.path.exists(self.leCOASTALME2GIS.text().strip('"').strip("'")):
						QMessageBox.critical(self, "COASTALME Utilities", "coastalme_to_gis.exe location does not exist")
						return
				if not self.leMeshToGis.text():
					QMessageBox.critical(self, "COASTALME Utilities", "Must specify and input mesh (XMDF or DAT)")
					return
				elif not os.path.exists(self.leMeshToGis.text().strip('"').strip("'")):
					QMessageBox.critical(self, "COASTALME Utilities", "Input mesh layer does not exist")
					return
				elif os.path.splitext(self.leMeshToGis.text())[1].upper() == '.XMDF':
					if not self.cboToGisMeshDataset.currentText():
						QMessageBox.critical(self, "COASTALME Utilities", "Must specify an input data type for XMDF")
						return
				if not self.cboTimestep.currentText():
					QMessageBox.critical(self, "COASTALME Utilities", "Must specify a timestep")
					return
				if self.cboTimestep.currentText().lower() != 'max' and self.cboTimestep.currentText().lower() != 'maximum':
					try:
						...
						QMessageBox.critical(self, "COASTALME Utilities", "Timestep must be a number or max")
						return
			elif self.cboCommonUtility.currentIndex() == 2:  # res_to_res
				if not self.leRes2Res.text():
					QMessageBox.critical(self, "COASTALME Utilities", "Must specify res_to_res.exe location")
					return
				else:
					if not os.path.exists(self.leRes2Res.text().strip('"').strip("'")):
						QMessageBox.critical(self, "COASTALME Utilities", "res_to_res.exe location does not exist")
						return
				if self.rbMeshInfo.isChecked() or self.rbMeshConvert.isChecked():
					if not self.leMeshToRes.text():
						QMessageBox.critical(self, "COASTALME Utilities", "Must an input mesh file")
						return
					elif not os.path.exists(self.leMeshToRes.text().strip('"').strip("'")):
						QMessageBox.critical(self, "COASTALME Utilities", "Input mesh file does not exist")
						return
					ext = os.path.splitext(self.leMeshToRes.text())[1].upper()
				else:
					if self.lwMeshes.count() == 0:
						QMessageBox.critical(self, "COASTALME Utilities", "Must specify at least one mesh file")
						return
					meshes = []
					for i in range(self.lwMeshes.count()):
						if not os.path.exists(self.lwMeshes.item(i).text().strip('"').strip("'")):
							QMessageBox.critical(self, "COASTALME Utilities",
				...
				if ext == '.XMDF':
					if not self.rbMeshInfo.isChecked():
						if not self.cboToResMeshDataset.currentText():
							QMessageBox.critical(self, "COASTALME Utilities", "Must specify datatype for XMDF inputs")
							return
				
		else:
			if not self.leAdvWorkingDir.text():
				QMessageBox.critical(self, "COASTALME Utilities", "Must specify a working directory")
				return
			if not os.path.exists(self.leAdvWorkingDir.text().strip('"').strip("'")):
				QMessageBox.critical(self, "COASTALME Utilities", "Working directory does not exist")
				return
			if not self.teCommands.toPlainText():
				QMessageBox.critical(self, "COASTALME Utilities", "Must specify some flags")
				return
			utilities = ['Asc2Asc', 'COASTALME2GIS', 'Res2Res', '12da2GIS', 'Convert2TS1','Tin2Tin', 'XSGenerator']
			i = self.cboAdvancedUtility.currentIndex()
			leutil = eval("self.le{0}".format(utilities[i]))
			if not leutil.text():
				QMessageBox.critical(self, "COASTALME Utilities", "{0} path is not specified in the Executables tab".format(self.cboAdvancedUtility.currentText()))
				return
	...
	def run(self):
		...
		if self.rbCommonFunctions.isChecked():
			...
			if self.cboCommonUtility.currentIndex() == 0:
				...
				if self.rbAscDiff.isChecked():
					...
					grids = []
					for inp in inps:
						if coastalmeqgis_find_layer(inp):
							layer_helper = LayerHelper(coastalmeqgis_find_layer(inp))
							ds = layer_helper.coastalme_path
						else:
							ds = inp
						grids.append(ds)
				elif self.rbBrklineFunc.isChecked():
					if not workdir:
						if coastalmeqgis_find_layer(self.cboBrkline_vector.currentText()):
							layer_helper = LayerHelper(coastalmeqgis_find_layer(self.cboBrkline_vector.currentText()))
					...
					# get raster
					if coastalmeqgis_find_layer(self.cboBrkline_raster.currentText()):
						layer_helper = LayerHelper(coastalmeqgis_find_layer(self.cboBrkline_raster.currentText()))
						ds = layer_helper.coastalme_path
					else:
			...
			# coastalme_to_gis
				...
				util = 'coastalme_to_gis'
				if not workdir:
					workdir = os.path.dirname(self.leCOASTALME2GIS.text().strip('"').strip("'"))
				...
				if self.xmdf_header is None or not self.xmdf_header.loaded() or dataset is None or time is None:
					dataset = self.cboToGisMeshDataset.currentText()
					time = self.cboTimestep.currentText()
				error, message = coastalmeToGis(self.leCOASTALME2GIS.text().strip('"').strip("'"), function, workdir, self.leMeshToGis.text(),
		...
		# user input arguments (advanced mode)
			...
			cbo2utility = {0: self.leAsc2Asc.text().strip('"').strip("'"), 1: self.leCOASTALME2GIS.text().strip('"').strip("'"), 2: self.leRes2Res.text().strip('"').strip("'"),
			...
			util_name = {0: 'asc_to_asc', 1: 'coastalme_to_gis', 2: 'res_to_res', 3: '12da_to_gis', 4: 'convert_to_ts1', 5: 'tin_to_tin', 6: 'xsGenerator'}
			util = util_name[self.cboAdvancedUtility.currentIndex()]
			error, message = coastalmeUtility(cbo2utility[self.cboAdvancedUtility.currentIndex()],
		...
		if error:
			if message.count('\n') > 50:
				QMessageBox.critical(self, "COASTALME Utilities", "Error Occured")
				self.errorDialog = UtilityErrorDialog(message)
				self.errorDialog.exec()
			else:
				QMessageBox.critical(self, "COASTALME Utilities", "Error Occured: {0}".format(message))
			self.pbOK.setEnabled(True)
			self.pbCancel.setEnabled(True)
		else:
			if self.rbCommonFunctions.isChecked() and \
				...
				#QMessageBox.information(self, "COASTALME Utilities", "Utility Finished")
				# self.accept()
				from coastalme.gui.logging import Logging
				if util:
					util = '[{}]'.format(util)
	...
	def saveProjectSettings(self):
		...
		project.writeEntry("COASTALME", "utilities_current_tab", self.tabWidget.currentIndex())
		project.writeEntry("COASTALME", "utilities_common_functions_cb", self.rbCommonFunctions.isChecked())
		project.writeEntry("COASTALME", "utilities_advanced_cb", self.rbAdvanced.isChecked())
		if self.rbCommonFunctions.isChecked():
			project.writeEntry("COASTALME", "utilities_common_functions", self.cboCommonUtility.currentIndex())
			project.writeEntry("COASTALME", "utilities_output_directory", self.leComOutputDir.text())
			if self.leOutputName.text():
				project.writeEntry("COASTALME", "utilities_output_name", self.leOutputName.text())
			if self.cboCommonUtility.currentIndex() == 0:
				project.writeEntry("COASTALME", "utilities_asc_diff", self.rbAscDiff.isChecked())
				project.writeEntry("COASTALME", "utilities_asc_max", self.rbAscMax.isChecked())
				project.writeEntry("COASTALME", "utilities_asc_stat", self.rbAscStat.isChecked())
				project.writeEntry("COASTALME", "utilities_asc_conv", self.rbAscConv.isChecked())
				if self.rbAscDiff.isChecked():
					project.writeEntry("COASTALME", "utilities_asc_diff_grid1", self.cboDiffGrid1.currentText())
					project.writeEntry("COASTALME", "utilities_asc_diff_grid2", self.cboDiffGrid2.currentText())
				else:
					grids = []
					for i in range(self.lwGrids.count()):
						grids.append(self.lwGrids.item(i).text())
					project.writeEntry("COASTALME", "utilities_asc_diff_grids", grids)
			elif self.cboCommonUtility.currentIndex() == 1:
				project.writeEntry("COASTALME", "utilities_coastalme_to_gis_mesh", self.leMeshToGis.text())
				project.writeEntry("COASTALME", "utilities_coastalme_to_gis_datatype", self.cboToGisMeshDataset.currentText())
				project.writeEntry("COASTALME", "utilities_coastalme_to_gis_togrid", self.rbMeshToGrid.isChecked())
				project.writeEntry("COASTALME", "utilities_coastalme_to_gis_topoints", self.rbMeshToPoints.isChecked())
				project.writeEntry("COASTALME", "utilities_coastalme_to_gis_tovectors", self.rbMeshToVectors.isChecked())
				project.writeEntry("COASTALME", "utilities_coastalme_to_gis_timestep", self.cboTimestep.currentText())
			elif self.cboCommonUtility.currentIndex() == 2:
				project.writeEntry("COASTALME", "utilities_res_to_res_datatype", self.cboToResMeshDataset.currentText())
				project.writeEntry("COASTALME", "utilities_res_to_res_mesh", self.leMeshToRes.text())
				project.writeEntry("COASTALME", "utilities_res_to_res_info", self.rbMeshInfo.isChecked())
				project.writeEntry("COASTALME", "utilities_res_to_res_max", self.rbMeshMaximum.isChecked())
				project.writeEntry("COASTALME", "utilities_res_to_res_conv", self.rbMeshConvert.isChecked())
				project.writeEntry("COASTALME", "utilities_res_to_res_conc", self.rbMeshConcatenate.isChecked())
				meshes = []
				for i in range(self.lwMeshes.count()):
					meshes.append(self.lwMeshes.item(i).text())
				project.writeEntry("COASTALME", "utilities_res_to_res_meshes", meshes)
		else:
			project.writeEntry("COASTALME", "utilities_advanced", self.cboAdvancedUtility.currentIndex())
			project.writeEntry("COASTALME", "utilities_working_directory", self.leAdvWorkingDir.text())
			project.writeEntry("COASTALME", "utilities_flags", self.teCommands.toPlainText())
			
	def loadProjectSettings(self):
		project = QgsProject.instance()
		self.tabWidget.setCurrentIndex(project.readNumEntry("COASTALME", "utilities_current_tab")[0])
		self.rbCommonFunctions.setChecked(project.readBoolEntry("COASTALME", "utilities_common_functions_cb")[0])
		self.rbAdvanced.setChecked(project.readBoolEntry("COASTALME", "utilities_advanced_cb")[0])
		self.cboCommonUtility.setCurrentIndex(project.readNumEntry("COASTALME", "utilities_common_functions")[0])
		self.commonUtilityChanged(self.cboCommonUtility.currentIndex())
		self.leComOutputDir.setText(project.readEntry("COASTALME", "utilities_output_directory")[0])
		self.leOutputName.setText(project.readEntry("COASTALME", "utilities_output_name")[0])
		self.rbAscDiff.setChecked(project.readBoolEntry("COASTALME", "utilities_asc_diff")[0])
		self.rbAscMax.setChecked(project.readBoolEntry("COASTALME", "utilities_asc_max")[0])
		self.rbAscStat.setChecked(project.readBoolEntry("COASTALME", "utilities_asc_stat")[0])
		self.rbAscConv.setChecked(project.readBoolEntry("COASTALME", "utilities_asc_conv")[0])
		self.cboDiffGrid1.setCurrentText(project.readEntry("COASTALME", "utilities_asc_diff_grid1")[0])
		self.cboDiffGrid2.setCurrentText(project.readEntry("COASTALME", "utilities_asc_diff_grid2")[0])
		self.lwGrids.addItems(project.readListEntry("COASTALME", "utilities_asc_diff_grids")[0])
		self.leMeshToGis.setText(project.readEntry("COASTALME", "utilities_coastalme_to_gis_mesh")[0])
		self.cboToGisMeshDataset.setCurrentText(project.readEntry("COASTALME", "utilities_coastalme_to_gis_datatype")[0])
		self.rbMeshToGrid.setChecked(project.readBoolEntry("COASTALME", "utilities_coastalme_to_gis_togrid")[0])
		self.rbMeshToPoints.setChecked(project.readBoolEntry("COASTALME", "utilities_coastalme_to_gis_topoints")[0])
		self.rbMeshToVectors.setChecked(project.readBoolEntry("COASTALME", "utilities_coastalme_to_gis_tovectors")[0])
		self.cboTimestep.setCurrentText(project.readEntry("COASTALME", "utilities_coastalme_to_gis_timestep", 'Max')[0])
		self.cboToResMeshDataset.setCurrentText(project.readEntry("COASTALME", "utilities_res_to_res_datatype")[0])
		self.leMeshToRes.setText(project.readEntry("COASTALME", "utilities_res_to_res_mesh")[0])
		self.rbMeshInfo.setChecked(project.readBoolEntry("COASTALME", "utilities_res_to_res_info")[0])
		self.rbMeshMaximum.setChecked(project.readBoolEntry("COASTALME", "utilities_res_to_res_max")[0])
		self.rbMeshConvert.setChecked(project.readBoolEntry("COASTALME", "utilities_res_to_res_conv")[0])
		self.rbMeshConcatenate.setChecked(project.readBoolEntry("COASTALME", "utilities_res_to_res_conc")[0])
		self.lwMeshes.addItems(project.readListEntry("COASTALME", "utilities_res_to_res_meshes")[0])
		self.cboAdvancedUtility.setCurrentIndex(project.readNumEntry("COASTALME", "utilities_advanced")[0])
		self.leAdvWorkingDir.setText(project.readEntry("COASTALME", "utilities_working_directory")[0])
		self.teCommands.setPlainText(project.readEntry("COASTALME", "utilities_flags")[0])
	
	def setDefaults(self, executables_only=False):
		if not executables_only:
			if self.leComOutputDir.text():
				QSettings().setValue('COASTALME_Utilities/output_directory', self.leComOutputDir.text())
			if self.cboDiffGrid1.currentText():
				if self.cboDiffGrid1.currentText().count(os.sep) > 0:
					QSettings().setValue('COASTALME_Utilities/ASC_to_ASC_difference_grid1', self.cboDiffGrid1.currentText())
			if self.cboDiffGrid2.currentText():
				if self.cboDiffGrid2.currentText().count(os.sep) > 0:
					QSettings().setValue('COASTALME_Utilities/ASC_to_ASC_difference_grid2', self.cboDiffGrid2.currentText())
			if self.leAdvWorkingDir.text():
				QSettings().setValue("COASTALME_Utilities/advanced_working_directory", self.leAdvWorkingDir.text())
			if self.leMeshToGis.text():
				QSettings().setValue("COASTALME_Utilities/COASTALME_to_GIS_mesh", self.leMeshToGis.text())
			if self.leMeshToRes.text():
				QSettings().setValue("'COASTALME_Utilities/Res_to_Res_mesh'", self.leMeshToRes.text())
		if self.leAsc2Asc.text():
			QSettings().setValue("COASTALME_Utilities/ASC_to_ASC_exe", self.leAsc2Asc.text())
		if self.leCOASTALME2GIS.text():
			QSettings().setValue("COASTALME_Utilities/COASTALME_to_GIS_exe", self.leCOASTALME2GIS.text())
		if self.leRes2Res.text():
			QSettings().setValue("COASTALME_Utilities/Res_to_Res_exe", self.leRes2Res.text())
		if self.le12da2GIS.text():
			QSettings().setValue("COASTALME_Utilities/12da_to_from_GIS_exe", self.le12da2GIS.text())
		if self.leConvert2TS1.text():
			QSettings().setValue("COASTALME_Utilities/Convert_to_TS1_exe", self.leConvert2TS1.text())
		if self.leTin2Tin.text():
			QSettings().setValue("Tin_to_Tin executable location", self.leTin2Tin.text())
		if self.leXSGenerator.text():
			QSettings().setValue("COASTALME_Utilities/xsGenerator_exe", self.leXSGenerator.text())
	
	def applyPrevExeLocations(self):
		self.leAsc2Asc.setText(QSettings().value("COASTALME_Utilities/ASC_to_ASC_exe"))
		self.leCOASTALME2GIS.setText(QSettings().value("COASTALME_Utilities/COASTALME_to_GIS_exe"))
		self.leRes2Res.setText(QSettings().value("COASTALME_Utilities/Res_to_Res_exe"))
		self.le12da2GIS.setText(QSettings().value("COASTALME_Utilities/12da_to_from_GIS_exe"))
		self.leConvert2TS1.setText(QSettings().value("COASTALME_Utilities/Convert_to_TS1_exe"))
		self.leTin2Tin.setText(QSettings().value("Tin_to_Tin executable location"))
		self.leXSGenerator.setText(QSettings().value("COASTALME_Utilities/xsGenerator_exe"))
	...
	def connectBrowseButtons(self):
		...
		signal = self.btnBrowseComOutputDir.clicked.connect(lambda: browse(self, 'existing folder',
		                                                                   'COASTALME_Utilities/output_directory',
		...
		signal = self.btnBrowseDiffGrid1.clicked.connect(lambda: browse(self, 'existing file',
		                                                                'COASTALME_Utilities/ASC_to_ASC_difference_grid1',
		...
		signal = self.btnBrowseDiffGrid2.clicked.connect(lambda: browse(self, 'existing file',
		                                                            'COASTALME_Utilities/ASC_to_ASC_difference_grid2',
		...
		signal = self.btnBrowseGrid.clicked.connect(lambda: browse(self, 'existing files',
		                                                       'COASTALME_Utilities/ASC_to_ASC_grid',
		...
		signal = self.btnBrowseAdvWorkingDir.clicked.connect(lambda: browse(self, 'existing folder',
		                                                                "COASTALME_Utilities/advanced_working_directory",
		                                                                "Working Directory", None,
		                                                                self.leAdvWorkingDir))
		self.browse_signals[self.btnBrowseAdvWorkingDir] = signal

		signal = self.btnBrowseAsc2Asc.clicked.connect(lambda: browse(self, 'existing file', "COASTALME_Utilities/ASC_to_ASC_exe",
		                                                          "ASC_to_ASC executable location", "EXE (*.exe *.EXE)",
		                                                          self.leAsc2Asc))
		self.browse_signals[self.btnBrowseAsc2Asc] = signal

		signal = self.btnBrowseCOASTALME2GIS.clicked.connect(lambda: browse(self, 'existing file',
		                                                             "COASTALME_Utilities/COASTALME_to_GIS_exe",
		                                                             "COASTALME_to_GIS executable location",
		                                                             "EXE (*.exe *.EXE)", self.leCOASTALME2GIS))
		self.browse_signals[self.btnBrowseCOASTALME2GIS] = signal

		signal = self.btnBrowseRes2Res.clicked.connect(lambda: browse(self, 'existing file',
		                                                          "COASTALME_Utilities/Res_to_Res_exe",
		...
		signal = self.btnBrowse12da2GIS.clicked.connect(lambda: browse(self, 'existing file',
		                                                           "COASTALME_Utilities/12da_to_from_GIS_exe",
		...
		signal = self.btnBrowseConvert2TS1.clicked.connect(lambda: browse(self, 'existing file',
		                                                              "COASTALME_Utilities/Convert_to_TS1_exe",
		...
		signal = self.btnBrowseTin2Tin.clicked.connect(lambda: browse(self, 'existing file',
		                                                          "COASTALME_Utilities/Tin_to_Tin_exe",
		...
		signal = self.btnBrowseXSGenerator.clicked.connect(lambda: browse(self, 'existing file',
		                                                              "COASTALME_Utilities/xsGenerator_exe",
		...
		signal = self.btnBrowseMeshToGis.clicked.connect(lambda: browse(self, 'existing file',
		                                                            'COASTALME_Utilities/COASTALME_to_GIS_mesh',
		...
		signal = self.btnBrowseMeshToRes.clicked.connect(lambda: browse(self, 'existing file',
		                                                            'COASTALME_Utilities/Res_to_Res_mesh',
		                                                            'XMDF or DAT location',
		                                                            "All mesh formats (*.xmdf *.XMDF *.dat *.DAT);;"
		                                                            "XMDF format(*.xmdf *.XMDF);;"
		...
		signal = self.btnBrowseMeshMulti.clicked.connect(lambda: browse(self, 'existing files',
		                                                            'COASTALME_Utilities/COASTALME_to_GIS_meshes',
		                                                            'XMDF or DAT location',
		                                                            "All mesh formats (*.xmdf *.XMDF *.dat *.DAT);;"
		                                                            "XMDF format(*.xmdf *.XMDF);;"
		...
		signal = self.btnBrowseBrkline_vector.clicked.connect(lambda: browse(self, 'existing file', "COASTALME_Utilities/brkline_input",
		                                                            'Vector input',
		                                                            gis_filters, self.cboBrkline_vector))
		self.browse_signals[self.btnBrowseBrkline_vector] = signal

		signal = self.btnBrowseBrkline_raster.clicked.connect(lambda: browse(self, 'existing file', "COASTALME_Utilities/brkline_input",
		                                                            'Raster input',
...
# ----------------------------------------------------------
#    Cmeflow utility error
# ----------------------------------------------------------
from .forms.Cmeflow_utility_error import *
...
# ----------------------------------------------------------
#    Cmeflow utility download progress bar
# ----------------------------------------------------------
...
class DownloadCmeflowUtilities(QObject):
	finished = pyqtSignal(dict)
	updated = pyqtSignal(str)
	
	utilities = ['asc_to_asc', 'coastalme_to_gis', 'res_to_res', '12da_to_from_gis', 'convert_to_ts1', 'tin_to_tin',
	             'xsGenerator']
	paths = {}
...
# ----------------------------------------------------------
#    coastalmeqgis broken links
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_brokenLinks import *


class coastalmeqgis_brokenLinks_dialog(QDialog, Ui_scenarioSelection):
...
# ----------------------------------------------------------
#    Import Flood Modeller results into coastalme viewer
# ----------------------------------------------------------
...
class FloodModellerResultImportDialog(QDialog, Ui_FMResDialog):
	def __init__(self):
		...
		# browse to files
		self.btnBrowseGXY.clicked.connect(lambda: browse(self, 'existing file', 'COASTALME/import_FM_Dialog',
		                                                 'Flood Modeller GXY File', 'GXY (*.gxy *.GXY)', self.leGXY))
		self.btnBrowseSectionData.clicked.connect(lambda: browse(self, 'existing file', 'COASTALME/import_FM_Dialog',
		                                                         'Flood Modeller Cross-Section DAT File', 'DAT (*.dat *.DAT)', self.leSectionData))
		self.btnBrowseCSV.clicked.connect(lambda: browse(self, 'existing files', 'COASTALME/import_FM_Dialog',
