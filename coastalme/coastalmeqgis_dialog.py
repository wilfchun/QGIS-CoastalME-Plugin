...
from .coastalmeqgis_library import interpolate, convertStrftimToTuviewftim, convertTuviewftimToStrftim, browse
...
class OverWrite_or_IncrementName_Dialog(QDialog):
	...
	def __init__(self, parent, db, layername):
		try:
			from .convert_coastalme_model_gis_format.conv_tf_gis_format.helpers.gis import GPKG
		except ImportError:
...
class SuperSededRun:
	...
	def run(self):
		try:
			from .convert_coastalme_model_gis_format.conv_tf_gis_format.helpers.gis import ogr_copy
		except ImportError:
...
class coastalmeqgis_increment_dialog(QDialog, Ui_coastalmeqgis_increment):
	...
	def populateTableNames(self, layer=None):
		try:
			from .convert_coastalme_model_gis_format.conv_tf_gis_format.helpers.gis import GPKG  # req. python 3.9+
		except ImportError:
			from .compatibility_routines import GPKG
...
class coastalmeqgis_import_empty_tf_dialog(QDialog, Ui_coastalmeqgis_import_empty):
	...
	def run(self):
		...
		if self.cb_import_into_kart.isChecked():
			...
			message = coastalmeqgis_import_empty_tf(self.iface, basedir, runID, empty_types, points, lines, regions, self, databaseOption, databaseLoc, convert)
		#message = coastalmeqgis_create_tf_dir(self.iface, crs, basedir)
		if message == 'pass':
			pass
...
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_run_tf_simple import *


class coastalmeqgis_run_tf_simple_dialog(QDialog, Ui_coastalmeqgis_run_tf_simple):
	def __init__(self, iface, project):
		QDialog.__init__(self)
		self.iface = iface
		self.setupUi(self)
		...
		if self.tfsettings.combined.base_dir:
			...
			self.runfolder = os.path.join(self.tffolder,'COASTALME','runs')
			project_loaded = True
		...
		self.COASTALME_exe.setText(tfexe)
	...
	def browse_tcf(self):
		...
		#self.tfsettings.save_last_exe(self,last_exe)("COASTALME_Run_COASTALME/tcf", inFileName)
		...
		if head != os.sep and head.lower() != 'c:\\' and head != '':
			...
			#self.tfsettings.setValue("COASTALME_Run_COASTALME/tcfDir", head)
	...
	def browse_exe(self):
		...
		self.exe = inFileName
		#self.tfsettings.setValue("COASTALME_Run_COASTALME/exe", inFileName)
		self.COASTALME_exe.setText(inFileName)
		#head, tail = os.path.split(inFileName)
		#if head <> os.sep and head.lower() <> 'c:\\' and head <> '':
		#	self.tfsettings.setValue("COASTALME_Run_COASTALME/exeDir", head)
	...
	def run(self):
		...
		tfexe = unicode(self.COASTALME_exe.displayText()).strip()
		...
		self.prog_inc = 0
		self._kill_run = False
		self.run_coastalme = RunTuflow(tfexe, tcf, capture_output, self.update)
...
class coastalmeqgis_configure_tf_dialog(QDialog, Ui_coastalmeqgis_configure_tf):
	def __init__(self, iface, project, parent=None):
		...
		if self.tfsettings.project_settings.tf_exe:
			self.COASTALME_exe.setText(self.translate(self.tfsettings.project_settings.tf_exe))
		elif self.tfsettings.global_settings.tf_exe:
			self.COASTALME_exe.setText(self.translate(self.tfsettings.global_settings.tf_exe))
		else:
	...
	def run(self):
		...
		tfexe = self.COASTALME_exe.displayText().strip()
		...
		tf_folder = 'COASTALME' if engine == 'classic' else 'COASTALMEFV'
		...
		if self.cbCreate.isChecked():
			...
			message = coastalmeqgis_create_tf_dir(self, crs, basedir, engine, tutorial, gisFormat)
		...
		if self.cbRun.isChecked():
			...
			message = run_coastalme(self.iface, tfexe, runfile)
			if message != None:
...
class coastalmeqgis_import_check_dialog(QDialog, Ui_coastalmeqgis_import_check):
	def __init__(self, iface, project):
		...
		error, message = self.tfsettings.Load() #exe, coastalme dircetory and projection
		if error:
			QMessageBox.information( self.iface.mainWindow(),"Error", "Error Loading Settings: "+message)
	...
	def run(self):
		...
		# run create dir script
		#message = coastalmeqgis_import_check_tf(self.iface, basedir, runID, empty_types, points, lines, regions)
		message = coastalmeqgis_import_check_tf(self.iface, basedir, runID, showchecks)
		#message = coastalmeqgis_create_tf_dir(self.iface, crs, basedir)
		if message != None:
...
# ----------------------------------------------------------
#    coastalmeqgis extract ARR2016
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_arr2016 import *
from coastalme.ARR2016.arr_cc_dlg import ARRCCDialog
from coastalme.ARR2016.ARR_to_COASTALME import ARR_to_COASTALME


class coastalmeqgis_extract_arr2016_dialog(QDialog, Ui_coastalmeqgis_arr2016):
	...
	def __init__(self, iface):
		...
		coastalmeLMindex = int(QSettings().value("ARR2016_COASTALME_loss_method", 0))
		self.cboTuflowLM.setCurrentIndex(coastalmeLMindex)
	...
	def check(self):
		...
		if not self.AEP_list:
			QMessageBox.critical(self, "ARR2016 to COASTALME", "Must select at least one AEP")
			return
		self.durations()
		if self.dur_list == 'none' and self.nonstnd_list == 'none':
			QMessageBox.critical(self, "ARR2016 to COASTALME", "Must select at least one duration")
			return
		self.climateChange()
		if self.lePTP.text():
			if not os.path.exists(self.lePTP.text()):
				QMessageBox.critical(self, "ARR2016 to COASTALME", "Point Temporal Pattern CSV does not exist")
				return
		if self.leATP.text():
			if not os.path.exists(self.leATP.text()):
				QMessageBox.critical(self, "ARR2016 to COASTALME", "Areal Temporal Pattern CSV does not exist")
				return
		if self.mar_staticValue.text():
			try:
				float(self.mar_staticValue.text())
				if float(self.mar_staticValue.text()) < 0:
					QMessageBox.critical(self, "ARR2016 to COASTALME",
		...
		if self.outfolder.text() == '<outfolder>':
			QMessageBox.critical(self, "ARR2016 to COASTALME", "Must specify an output folder")
			return
		...
		if self.gbOfflineMode.isChecked():
			if not self.leARRFile.text():
				QMessageBox.critical(self, "ARR2016 to COASTALME", "Must specify an ARR datahub file in offline mode")
				return
			...
			if not self.leBOMFile.text():
				QMessageBox.critical(self, "ARR2016 to COASTALME", "Must specify an BOM IFD file in offline mode")
				return
	...
	def run(self):
		...
		# get coastalme loss method
		coastalmeLossMethod = 'infiltration' if self.cboTuflowLM.currentIndex() == 0 else 'excess'
		...
		script = os.path.join(currentFolder, 'ARR2016', 'ARR_to_COASTALME.py')
		for i in range(len(name_list)):
			sys_args = ['python3', script, '-out', outFolder, '-name', name_list[i], 
			            ...
			            '-coastalme_loss_method', coastalmeLossMethod, '-point_tp', point_tp_csv, '-areal_tp', areal_tp_csv,
	...
	def saveDefaults(self):
		...
		settings.setValue("ARR2016_COASTALME_loss_method", self.cboTuflowLM.currentIndex())
...
class coastalmeqgis_insert_coastalme_attributes_dialog(QDialog, Ui_coastalmeqgis_insert_coastalme_attributes):
	...
	def run(self):
		...
		message = coastalmeqgis_insert_tf_attributes(self.iface, inputLayer, basedir, runID, template, lenFields, self,
		                                          output_dbase)
...
# ----------------------------------------------------------
#    coastalmeqgis tuplot axis editor
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_tuplotAxisEditor import *


class coastalmeqgis_tuplotAxisEditor(QDialog, Ui_tuplotAxisEditor):
...
# ----------------------------------------------------------
#    coastalmeqgis tuplot axis labels
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_tuplotAxisLabels import *


class coastalmeqgis_tuplotAxisLabels(QDialog, Ui_tuplotAxisLabel):
...
# ----------------------------------------------------------
from .forms.ui_coastalmeqgis_TuOptionsDialog import *
...
class TuOptionsDialog(QDialog, Ui_TuViewOptions):
	...
	def cb_copy_mesh_toggled(self, e):
		...
		QSettings().setValue('COASTALME/tuview_copy_mesh', self.tuOptions.copy_mesh)

	def cb_show_mesh_copy_dlg_toggled(self, e):
		self.tuOptions.show_copy_mesh_dlg = e
		QSettings().setValue('COASTALME/tuview_show_copy_mesh_dlg', self.tuOptions.show_copy_mesh_dlg)

	def cb_del_copied_mesh_toggled(self, e):
		self.tuOptions.del_copied_res = e
		QSettings().setValue('COASTALME/tuview_del_copied_res', self.tuOptions.del_copied_res)

	def del_all_cache_res(self):
		from .coastalmeqgis_tuviewer.tmp_result import TmpResult
	...
	def curtain_vector_scale_changed(self, e):
		...
		QSettings().setValue("COASTALME/tuview_curtain_vector_scale", self.sbScale.value())
	...
	def curtain_vector_scale_units_changed(self, e):
		...
		QSettings().setValue("COASTALME/tuview_curtain_vector_scale_units", self.cboScaleUnits.currentText())
	...
	def curtain_vector_units_changed(self, e):
		...
		QSettings().setValue("COASTALME/tuview_curtain_vector_units", self.cboUnits.currentText())

	def curtain_vector_width_changed(self, e):
		self.tuOptions.curtain_vector_width = self.sbWidth.value()
		QSettings().setValue("COASTALME/tuview_curtain_vector_width", self.sbWidth.value())

	def curtain_vector_head_width_changed(self, e):
		self.tuOptions.curtain_vector_head_width = self.sbHeadWidth.value()
		QSettings().setValue("COASTALME/tuview_curtain_vector_head_width", self.sbHeadWidth.value())

	def curtain_vector_head_length_changed(self, e):
		self.tuOptions.curtain_vector_head_length = self.sbHeadLength.value()
		QSettings().setValue("COASTALME/tuview_curtain_vector_head_length", self.sbHeadLength.value())
	...
	def curtain_vector_horizontal_factor_changed(self, e):
		...
		QSettings().setValue("COASTALME/tuview_curtain_vector_horizontal_factor", self.sbHorizontalFactor.value())

	def vertical_velocity_factor_changed(self, e):
		self.tuOptions.curtain_vector_vertical_factor = self.sbVertVelFactor.value()
		QSettings().setValue("COASTALME/tuview_curtain_vector_vertical_factor", self.sbVertVelFactor.value())

	def cbPlotInactiveAreasToggled(self, e):
		self.tuOptions.plotInactiveAreas = bool(self.cbPlotInactiveAreas.isChecked())
		QSettings().setValue("COASTALME/tuview_plot_inactive_areas", self.tuOptions.plotInactiveAreas)

	def tcfLoadMethodChanged(self, tcf_load_method):
		self.tuOptions.tcfLoadMethod = tcf_load_method
		QSettings().setValue("COASTALME/tuview_tcf_load_method", tcf_load_method)

	def saveDefaultLayout(self, layoutType):
		QSettings().setValue("COASTALME/tuview_defaultlayout", layoutType)

	def saveDefaultFontSize(self, value):
		size = self.sbDefaultFontSize.value()
		QSettings().setValue("COASTALME/tuview_defaultfontsize", size)
	...
	def run(self):
		...
		settings.setValue('COASTALME/tuview_zeroTime', self.tuOptions.zeroTime)
		
		# format time
		self.tuOptions.dateFormat, self.tuOptions._dateFormat = convertTuviewftimToStrftim(self.leDateFormat.text())
		settings.setValue('COASTALME/tuview_dateFormat', self.tuOptions.dateFormat)
		settings.setValue('COASTALME/tuview__dateFormat', self.tuOptions._dateFormat)
		...
		settings.setValue("COASTALME/tuview_plotbackgroundcolour", self.tuOptions.plotBackgroundColour)
		...
		settings.setValue("COASTALME/tuview_iconsize", self.tuOptions.iconSize)
...
from .coastalmeqgis_tuviewer.coastalmeqgis_tuuserplotdata import TuUserPlotDataSet
