...
from ..coastalmeqgis_library import (getPathFromRel, coastalmeqgis_apply_check_tf_clayer, datetime2timespec,
...
import re
from ..COASTALME_FM_data_provider import TuFloodModellerDataProvider
...
class TuResults1D():
		...
		for filePath in inFilePaths:
			...
			# post 2013 results
				...
				if error:
					...
					QMessageBox.critical(self.tuView, "COASTALME Viewer", message)
					return False
			elif ext.upper() == '.GPKG':
				res = ResData_GPKG()
				error, message = res.Load(filePath)
	...
	def importResultsFM(self, gxy, dat, inFilePaths):
		...
		for i, filePath in enumerate(inFilePaths):
			...
			if i == 0:
				...
				if res.isgxyValid:
					...
					if fmLinksLayer.isValid():
						...
						coastalmeqgis_apply_check_tf_clayer(self.iface, layer=fmLinksLayer)
					if fmNodesLayer.isValid():
						self.tuView.project.addMapLayer(fmNodesLayer)
						coastalmeqgis_apply_check_tf_clayer(self.iface, layer=fmNodesLayer)
				if err:
				...
				if dat is not None:
					...
					if self.tuView.crossSectionsFM.fm_nXs:
						...
						if fmXSLayer.isValid():
							...
							coastalmeqgis_apply_check_tf_clayer(self.iface, layer=fmXSLayer)
					if err:
			...
			error, message = res.Load(filePath)
			if error:
				QMessageBox.critical(self.tuView, "COASTALME Viewer", 'ERROR: {0} errors occured loading in results: {1}:\n{2}'.format(len(message), filePath, '\n'.join(message)))
				continue
	...
	def importResultsFVBCTide(self, nc_fpath, ns_fpath, use_local_time):
		...
		if error:
			QMessageBox.critical(self.tuView, "COASTALME Viewer", f'ERROR: loading FV BC tide data:\n{message}')
			return False
	...
	def openGis(self, tpc):
		...
		if gisPoints is not None:
			...
			if layer.featureCount():  # only load if there are any features
				...
				coastalmeqgis_apply_check_tf_clayer(self.iface, layer=layer)
		# load lines gis file
					...
					if res.GIS.R and res.gis_region_layer_name:
						...
						if lyr.isValid() and (lyr.featureCount() or driver == 'memory'):
							...
							if isinstance(res, ResData_GPKG):
								...
								coastalmeqgis_apply_check_tf_clayer(self.iface, layer=lyr)
					# lines
