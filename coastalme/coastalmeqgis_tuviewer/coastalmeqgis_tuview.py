...
from ..forms.coastalme_plotting_dock import Ui_Tuplot
...
from .coastalmeqgis_tumenubar import TuMenuBar
from .coastalmeqgis_tuoptions import TuOptions
from .coastalmeqgis_tumenucontext import TuContextMenu
from .coastalmeqgis_tuproject import TuProject
from ..coastalmeqgis_library import (convertTimeToFormattedTime,
...
class TuView(QDockWidget, Ui_Tuplot):
	...
	def __init__(self, iface=None, **kwargs):
		...
		# options
		#self.tuOptions = coastalmeqgis_tuoptions.TuOptions()
	...
	def loadXsSelections(self, lyr=None):
		"""
		Load COASTALME 1D cross sections from selected features
		"""
	...
	def onDockLocationChange(self, area=None):
		...
		QSettings().setValue("COASTALME/tuview_docklocation", area)

		if self.iface is not None:
			if self.iface.mainWindow().tabifiedDockWidgets(self):
				QSettings().setValue("COASTALME/tuview_isdocktabified", True)
				tabifiedWith = [x.windowTitle() for x in self.iface.mainWindow().tabifiedDockWidgets(self)]
				QSettings().setValue("COASTALME/tuview_tabifiedwith", tabifiedWith)
			else:
				QSettings().setValue("COASTALME/tuview_isdocktabified", False)
				QSettings().setValue("COASTALME/tuview_tabifiedwith", [])
	...
	def onTopLevelChange(self, topLevel=None):
		...
		QSettings().setValue("COASTALME/tuview_isdockfloating", topLevel)
	...
	def plotWindowVisibilityToggled(self, initialisation=False):
		...
		if self.PlotLayout.isVisible():
			QSettings().setValue("COASTALME/tuview_previouslayout", "narrow")
			self.PlotLayout.setVisible(False)
			self.SecondMenu.setVisible(True)
			self.OpenResultsWidget.setVisible(False)
			self.moveOpenResults("narrow")
			...
			QSettings().setValue("COASTALME/tuview_previouslayout", "plot")
	...
	def resultTypeDoubleClicked(self, event):
		...
		# 	if self.iface is not None:
		# 		self.meshDialog = coastalmeqgis_meshSelection_dialog(self.iface, self.tuResults.tuResults2D.activeMeshLayers,
