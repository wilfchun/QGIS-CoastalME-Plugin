...
from ..coastalmeqgis_tuviewer.coastalmeqgis_tuanimation import (ImagePropertiesDialog, PlotProperties,
		...
		for i in range(count):
			...
			if scalar not in rendering:
				...
				if layer not in self.tuView.tuResults.results and \
					...
					if not imported:
						continue
					layer = coastalmeqgis_find_layer(self.tuView.OpenResults.item(self.tuView.OpenResults.count() - 1).text())
				else:
