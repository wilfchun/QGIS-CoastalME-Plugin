...
import ctypes
import re
from .coastalmeqgis_library import (getOSIndependentFilePath, NC_Error,
...
class Timeseries():
	...
	def loadFromNetCDFCDLL(self, fullpath, resName, resType, ncid, ncdll, ncDims, ncVars):
		...
		if resType in [x.name for x in ncVars]:
			...
			except UnicodeDecodeError:  # 1D loss headers are written at end of COASTALME and if unclean exit don't bother loading
				return False, ""
		else:
			return False, ""
		self.Header = ['Timestep', 'Time'] + self.ID[:]
			...
			with open(fullpath, 'r', encoding='utf-8', errors='ignore') as csvfile:
				...
				for (counter, row) in enumerate(reader):
					...
					try:
						...
						# caused when coastalme outputs ****** for steep slopes
