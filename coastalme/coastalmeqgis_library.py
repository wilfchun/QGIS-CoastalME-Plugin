"""
 --------------------------------------------------------
        coastalmeqgis_library - coastalmeqgis operation functions
        begin                : 2013-08-27
...
from .coastalmeqgis_styles import TF_Styles
...
from coastalme.compatibility_routines import (
    QT_DOUBLE,
    QT_FLOAT,
    QT_LONG_LONG,
    QT_LONG,
    ...
    QT_EVENT_LOOP_EXCLUDE_USER_INPUT_EVENTS,
    QT_INT,
    QT_MESSAGE_BOX_NO,
    QT_RED,
    QT_NETWORK_REQUEST_HTTP_STATUS_CODE_ATTRIBUTE,
    QT_CURSOR_WAIT,
    ...
    QT_DOUBLE,
    QT_RICH_TEXT,
    QT_MESSAGE_BOX_ACTION_ROLE,
    QT_ITEM_DATA_EDIT_ROLE,
    QT_MESSAGE_BOX_CLOSE,
    QT_FILE_DIALOG_ACCEPT_OPEN,
    QT_FILE_DIALOG_ACCEPT_SAVE,
    QT_FILE_DIALOG_ANY_FILE,
    QT_FILE_DIALOG_DONT_CONFIRM_OVERWRITE,
    QT_FILE_DIALOG_DETAIL,
    QT_FILE_DIALOG_DIRECTORY,
    QT_FILE_DIALOG_EXISTING_FILE,
    QT_FILE_DIALOG_EXISTONG_FILES,
    QT_FILE_DIALOG_SHOW_DIRS_ONLY,
    QT_KEEP_ASPECT_RATIO,
    QT_LAYOUT_SET_FIXED_SIZE,
    QT_MESSAGE_BOX_YES_TO_ALL,
    QT_FONT_MONOSPACE,
    QT_MESSAGE_BOX_OK,
)


# CoastalmeTools, well hello there...
from CoastalmeTools import Cme
# --------------------------------------------------------
#    coastalmeqgis Utility Functions
# --------------------------------------------------------
...
class NcVar:
    ...
    def print_(self):
        return "id: {0}, name: {1}, type: {2}, nDim: {3}, dims: ({4})".format(
            self.id, self.name, self.type, self.nDims, ", ".join(self.dimNames)
        )
...
def get_latest_dev_plugin_version():
    try:
        ...
        downloader = Downloader(
            "https://downloads.coastalme.com/Private_Download/QGIS_COASTALME_Plugin/VERSION"
        )
        downloader.download()
...
def about(window):
    ...
    class AboutDialog(QDialog, Ui_AboutDialog):
        def __init__(self, iface, plugin_version, python_dependencies):
            ...
            python_lib_msg = "\n".join(
                [
                    "{0:{2}s}: {1}".format(k, v, max_len)
                    for k, v in python_dependencies.items()
                ]
            )
            self.textEdit.setText(
                ...
                "{4}\t\t: {5}\n\n"
                "Python Libraries\n{8}".format(
                    "COASTALME Plugin Version",
                    plugin_version,
                    "QGIS Version",
                    Qgis.QGIS_VERSION,
                    "Python Version",
                    sys.version.split("(")[0].strip(),
                    "Latest Plugin Dev Version",
                    get_latest_dev_plugin_version(),
                    python_lib_msg,
                )
            )
...
def coastalmeqgis_duplicate_file(qgis, layer, savename, keepform):
    ...
    outfile = QgsVectorFileWriter(
        vectorFileName=savename,
        fileEncoding="System",
        fields=layer.dataProvider().fields(),
        geometryType=layer.wkbType(),
        srs=layer.dataProvider().sourceCrs(),
    ...
    correct_prj = "{0}.prj".format(
        os.path.splitext(layer.dataProvider().dataSourceUri())[0]
    )
...
def duplicate_database(
    iface, layer, db, layername, incrementDatabase, incrementDatabaseLayers
):
    ...
    if incrementDatabase:
        ...
        db_old, layername_old = re.split(
            re.escape(r"|layername="), db_old, flags=re.IGNORECASE
        )
        ...
        for table in dbLayer.dataProvider().subLayers():
            ...
            if tablename in incrementDatabaseLayers:
                ...
                layer_temp = QgsVectorLayer(
                    "{0}|layername={1}".format(db_old, tablename), "temp", "ogr"
                )
                ...
                if not layer_temp.isSpatial():
                    if layer_temp.crs().isValid():
                        ...
                        layer_temp = QgsVectorLayer(
                            "point?crs={0}".format(crs), tablename, "memory"
                        )
                try:
                    if Qgis.QGIS_VERSION_INT >= 31030:
                        error = QgsVectorFileWriter.writeAsVectorFormatV2(
                            layer_temp, db, QgsCoordinateTransformContext(), options
                        )
                        if error[0] != QgsVectorFileWriter.NoError:
                            return "Error writing layer to database: {0} | {1}\n{2}".format(
                                db, tablename, error[1]
                            )
                    else:
                        err, msg = QgsVectorFileWriter.writeAsVectorFormat(
                            layer_temp, db, "SYSTEM", layer.crs(), options.driverName
                        )
                        if err:
                            return "Error writing layer to database: {0} | {1}\n{2}".format(
                                db, tablename, msg
        ...
        try:
            if Qgis.QGIS_VERSION_INT >= 31030:
                error = QgsVectorFileWriter.writeAsVectorFormatV2(
                    layer.clone(), db, QgsCoordinateTransformContext(), options
                )
                if error[0] != QgsVectorFileWriter.NoError:
                    return "Error creating new layer: {0} | {1}\n{2}".format(
                        db, layername, error[1]
                    )
            else:
                err, msg = QgsVectorFileWriter.writeAsVectorFormat(
                    layer.clone(), db, "SYSTEM", layer.crs(), options.driverName
                )
                if err:
                    return "Error writing layer to database: {0} | {1}\n{2}".format(
                        db, layername, msg
...
def coastalmeqgis_create_cme_dir(
    dialog, crs, basepath, engine, tutorial, gisFormat="SHP"
):
    if crs is None:
    ...
    # linux case sensitive coastalme directory
    ...
    if engine == "flexible mesh":
        COASTALME_Folders.append("model{0}geo".format(os.sep))
    for x in COASTALME_Folders:
    ...
    prjname = os.path.join(
        basepath,
        parent_folder_name,
        "model",
        "gis",
        "projection.{0}".format(gisFormat.lower()),
    )
    if len(prjname) <= 0:
    ...
    if os.path.exists(prjname) and gisFormat == "GPKG":
        ...
        if gisFormat == "GPKG":
            ...
            out_lyr_uri = prjname
        open_layer = coastalmeqgis_find_layer(out_lyr_uri, search_type="datasource")
    ...
    if Qgis.QGIS_VERSION_INT >= 32000:
        outfile = QgsVectorFileWriter.writeAsVectorFormatV3(
            layer, prjname, QgsCoordinateTransformContext(), options
        )
        if outfile[0] != QgsVectorFileWriter.NoError:
            msg = outfile[1]
            return msg
    elif Qgis.QGIS_VERSION_INT >= 31030:
        outfile = QgsVectorFileWriter.writeAsVectorFormatV2(
            layer, prjname, QgsCoordinateTransformContext(), options
        )
        if outfile[0] != QgsVectorFileWriter.NoError:
            msg = outfile[1]
    ...
    runfile = os.path.join(
        basepath, parent_folder_name, "runs", "Create_Empties{0}".format(ext)
    )
...
def create_empty_tcf(tcf_path, gis_format, tutorial_model, engine="hpc", crs=None):
    with open(tcf_path, "w") as f:
        ...
        if gis_format == "SHP":
            f.write(
                "SHP Projection == ..{0}model{0}gis{0}projection.prj\n".format(os.sep)
            )
        else:
            f.write(
                "GPKG Projection == ..{0}model{0}gis{0}projection.gpkg\n".format(os.sep)
            )
        if tutorial_model:
            f.write("Cmetorial Model == ON\n")
        ...
        if engine == "flexible mesh":
            ...
            f.write(
                "Units == {0}\n".format(
                    "US Customary" if crs.mapUnits() in feet else "Metric"
                )
            )
            if not crs.isGeographic():
                f.write(
                    '!Latitude == Y  ! User needs to replace "Y" with a representative latitude for the model domain.\n'
                )


def coastalmeqgis_import_empty_cme(
    qgis,
    basepath,
    runID,
    empty_types,
    ...
    for type in empty_types:
        for geom in geom_type:
            search_string = (
                os.path.join(
                    basepath, "{0}_empty_pts*".format(type.replace("_pts", ""))
                )
                if "_pts" in type
                else os.path.join(basepath, "{0}_empty*".format(type))
            )
            ...
            for fpath in fpaths:
                if re.findall(
                    r"(\.gpkg)|({0}\.shp)$".format(geom), fpath.strip(), re.IGNORECASE
                ):
                    break
                # QMessageBox.information(qgis.mainWindow(),"Creating COASTALME directory", fpath)
            if os.path.isfile(fpath):
                ...
                if isgpkg or convert:
                    if databaseOption == "grouped":
                        name = "{0}_{1}{2}".format(
                            type, runID, os.path.splitext(fpath)[1]
                        )
                        savename = os.path.join(gis_folder, name)
                ...
                if isgpkg or convert:
                    if (
                        Path(fpath).exists()
                        and layername.lower()
                        in [x.lower() for x in get_table_names(fpath)]
                        and not yestoall
                    ):
                ...
                if os.path.exists(savename) and (isgpkg or convert):
                    ...
                    if isgpkg or convert:
                        out_lyr_uri = "{0}|layername={1}".format(
                            savename, outlayer.name()
                        )
                    else:
                        out_lyr_uri = savename
                    open_layer = coastalmeqgis_find_layer(
                        out_lyr_uri, search_type="datasource"
                    )
                    if open_layer is not None:
                        QgsProject.instance().removeMapLayer(open_layer.id())
                        open_layer = None
                ...
                try:
                    if Qgis.QGIS_VERSION_INT >= 31030:
                        outfile = QgsVectorFileWriter.writeAsVectorFormatV2(
                            outlayer, savename, QgsCoordinateTransformContext(), options
                        )
                    else:
                        err, msg = QgsVectorFileWriter.writeAsVectorFormat(
                            outlayer,
                    ...
                    QMessageBox.critical(
                        qgis.mainWindow(),
                        "Save new file",
                        "Error creating new file: {0}".format(e),
                    )
                    return
                ...
                if isgpkg or convert:
                    ...
                    uri = savename
                open_layer = coastalmeqgis_find_layer(layername)
                if (
                    open_layer is None
                    or open_layer.dataProvider().dataSourceUri().lower() != uri.lower()
                ):
...
def coastalmeqgis_get_selected_IDs(qgis, layer):
    QMessageBox.information(
        qgis.mainWindow(), "Info", "Entering coastalmeqgis_get_selected_IDs"
    )
    IDs = []
    if (layer == None) and (layer.type() != QgsMapLayer.VectorLayer):
    ...
    if idx == -1:
        QMessageBox.critical(
            qgis.mainWindow(), "Info", "ID field not found in current layer"
        )
...
class RunCmeflow(QObject):
    error = pyqtSignal(str)
    finished = pyqtSignal(str)

    def __init__(self, cmeexe, runfile, capture_output, callback):
        super().__init__()
        self.capture_output = capture_output
        self.callback = callback
        self.cmeexe = cmeexe
        self.runfile = runfile
        self.isfv = Path(cmeexe).stem.lower() == "coastalmefv"
        self.args = [cmeexe, "-b", runfile] if not self.isfv else [cmeexe, runfile]
        self.proc = None
        self.timer = None
        self.stdout = []
    ...
    def run(self):
        if not os.path.exists(self.cmeexe):
            self.error.emit("COASTALME exe not found: {0}".format(self.cmeexe))
            return
        if not os.path.exists(self.runfile):
            self.error.emit(
                "COASTALME Control File (tcf, fvc) not found: {0}".format(self.runfile)
            )
            return
        ...
        try:
            ...
            self.error.emit(
                "Unexpected error occurred starting COASTALME\nargs: {0}".format(
                    self.args
                )
            )
            self.destroy()
...
def run_coastalme(qgis, cmeexe, runfile):
    # QT_MESSAGE_BOX_INFORMATION(qgis.mainWindow(),"debug", "Running COASTALME - tcf: "+tcf)
    try:
        ...
        cmearg = [cmeexe, "-b", runfile] if ext[-1] == ".tcf" else [cmeexe, runfile]
        if ext.lower() == ".tcf":
            cmearg = [cmeexe, "-b", runfile]
        elif ext.lower() == ".fvc":
            cmearg = [cmeexe, fname]
            os.chdir(dir)
        else:
        ...
        # cmearg = [cmeexe, '-b', runfile]
        cme_proc = Popen(cmearg)
        # if cme_proc.poll() is not None:
        #     _, stderr = cme_proc.communicate()
        #     return str(stderr)
    # cme_proc = Popen(cmearg, cwd=os.path.dirname(runfile))
    except:
        return "Error occurred starting COASTALME"
    # QT_MESSAGE_BOX_INFORMATION(qgis.mainWindow(),"debug", "COASTALME started")
    return None


# def config_set(project, coastalme_folder, cmeexe, projection):
#     message = None
#     try:
#         f = open(config_file, 'w')
#         f.write('COASTALME Folder ==' + coastalme_folder + '\n')
#         f.write('COASTALME Exe ==' + cmeexe + '\n')
#         f.write('PROJECTION ==' + proj + '\n')
#         f.flush()
#         f.close()
#     except:
#         message = "Unable to write COASTALME config file: " + config_file
#
#     return message
...
def load_project(project):
    message = None
    try:
        cmefolder = project.readEntry("configure_coastalme", "folder", "Not yet set")[0]
    except:
        message = "Error - Reading from project file."
        QMessageBox.information(qgis.mainWindow(), "Information", message)

    try:
        cmeexe = project.readEntry("configure_coastalme", "exe", "Not yet set")[0]
    except:
        message = "Error - Reading from project file."
        QMessageBox.information(qgis.mainWindow(), "Information", message)

    try:
        cme_prj = project.readEntry("configure_coastalme", "projection", "Undefined")[0]
    except:
        message = "Error - Reading from project file."
        QMessageBox.information(qgis.mainWindow(), "Information", message)

    error = False
    if cmefolder == "Not yet set":
        error = True
        QMessageBox.information(qgis.mainWindow(), "Information", "Not set cmefolder")
    if cmeexe == "Not yet set":
        error = True
        QMessageBox.information(qgis.mainWindow(), "Information", "Not set cmeexe")
    if cme_prj == "Undefined":
        error = True
        QMessageBox.information(qgis.mainWindow(), "Information", "cme_prj")
    if error:
        message = "Project does not appear to be configured.\nPlease run COASTALME >> Editing >> Configure Project from the plugin menu."

    return message, cmefolder, cmeexe, cme_prj


#  coastalmeqgis_import_check_cme added MJS 11/02
def coastalmeqgis_import_check_cme(qgis, basepath, runID, showchecks):
    # import check file styles using class
    cme_styles = TF_Styles()
    error, message = cme_styles.Load()
    if error:
    ...
    check_files = (
        glob.glob(basepath + "/*" + runID + "*.shp")
        + glob.glob(basepath + "/*" + runID + "*.mif")
        + glob.glob(basepath + "/*" + runID + "*.SHP")
        + glob.glob(basepath + "/*" + runID + "*.MIF")
    ...
    for chk in check_files:
        if re.findall(re.escape(r".gpkg|layername="), chk, flags=re.IGNORECASE):
            fname = re.split(re.escape(r".gpkg|layername="), chk, flags=re.IGNORECASE)[
                -1
            ]
        else:
        ...
        if (
            layer is None
        ):  # probably a mif file with 2 geometry types, have to redefine layer object
        ...
        if renderer:  # if the file requires a attribute based rendered (e.g. BC_Name for a _sac_check_R)
        ...
        else:  # use .qml style using cme_styles
            error, message, slyr = cme_styles.Find(
                fname, layer
            )  # use coastalme styles to find longest matching
            if error:
                QMessageBox.critical(qgis.mainWindow(), "ERROR", message)
                return message
            if slyr:  # style layer found:
                ...
                for item in legint.children():
                    if (
                        "zpt check" in item.name().lower()
                        or "uvpt check" in item.name().lower()
                        or "grd check" in item.name().lower()
                        or "zpt_check" in item.name().lower()
                        or "uvpt_check" in item.name().lower()
                        or "grd_check" in item.name().lower()
                    ):
...
def region_renderer(layer):
    ...
    if re.findall(
        ...
        fname = os.path.splitext(
            os.path.basename(layer.dataProvider().dataSourceUri())
        )[0]
    ...
    if "_bcc_check_r" in fname.lower():
        ...
        "2d_bc" in fname.lower()
        or "2d_mat" in fname.lower()
        or "2d_soil" in fname.lower()
        or "1d_bc" in fname.lower()
    ):
        i = 0
        ...
        "1d_nwk" in fname.lower()
        or "1d_nwkb" in fname.lower()
        or "1d_nwke" in fname.lower()
        or "1d_mh" in fname.lower()
        or "1d_pit" in fname.lower()
        or "1d_nd" in fname.lower()
    ):
        i = 0
        field_name = None
        for field in layer.fields():
    ...
    # elif re.findall(r'_FM_PLOT_[PLR]', fname, flags=re.IGNORECASE):
        ...
        r"((_FM_PLOT_[PLR])|_raw_(nodes|links)_)", fname, flags=re.IGNORECASE
    ):
        try:
            field_names = [
                x.name() for x in [f for f in layer.getFeatures()][0].fields()
            ]
            upstrm_type = (
                "Upstrm_Type" if "Upstrm_Type" in field_names else "Upstrm_Typ"
            )
    ...
    # QgsMessageLog.logMessage('These values have been identified: ' + vals, "COASTALME")
    if (
        re.findall(
            r"((_FM_PLOT_[PLR])|_raw_(nodes|links)_)", fname, flags=re.IGNORECASE
        )
        ...
        unique_values2 = [
            ...
            if x.upper() != "SPILL"
            and x.upper() != "JUNCTION"
            and x.upper() != "INTERPOLATE"
        ]
        try:
            field_names = [
                x.name() for x in [f for f in layer.getFeatures()][0].fields()
            ...
            dnstrm_type = (
                "Dnstrm_Type" if "Upstrm_Type" in field_names else "Dnstrm_Typ"
            )
        except (IndexError, AttributeError):
            upstrm_type = "Upstrm_Type"
        ...
        if spillExists:
            expressions = [
                f"{x} and \"{dnstrm_type}\" NOT ILIKE 'SPILL'" for x in expressions
            ]
        if junctExists:
            expressions = [
                f"{x} and \"{dnstrm_type}\" NOT ILIKE 'JUNCTION'" for x in expressions
            ]
        if intpExists:
            expressions = [
                f"{x} and \"{dnstrm_type}\" NOT ILIKE 'INTERPOLATE'"
                for x in expressions
            ]
        expressions.insert(
            0, f"\"{upstrm_type}\" ILIKE '%BDY%' or \"{dnstrm_type}\" ILIKE '%BDY%'"
        )
        ...
        for unique_value in unique_values:
            ...
            if "2d_bc" in fname.lower():
                ...
                if layer.geometryType() == QgsWkbTypes.LineGeometry:
                    ...
                    if (
                        unique_value.lower() == "lateral"
                        or unique_value.lower() == "spill"
                    ):
                        symbol_layer = QgsSimpleLineSymbolLayer.create(layer_style)
                        symbol_layer.setWidth(0.25)
                        color = QColor(
...
def graduatedRenderer(layer):
    ...
    if Qgis.QGIS_VERSION_INT < 31000:
        ...
        if err:
            Logging.warning(err)
    grad_rend.setSymbolSizes(2.0, 5.0)
...
def is1dIntegrityToolOutput(layer):
    ...
    return (
        ...
        and layer.isValid()
        and "output" in layer.name()
        and [(x.name(), x.type()) for x in layer.fields()][:len_] == field_mapping
    )


def coastalmeqgis_apply_check_cme(qgis):
    # apply check file styles to all open shapefiles
    error = False
    message = None

    # load style layers using coastalmeqgis_styles
    cme_styles = TF_Styles()
    error, message = cme_styles.Load()
    if error:
    ...
    for layer_name, layer in QgsProject.instance().mapLayers().items():
        if layer.type() == QgsMapLayer.VectorLayer:
            if re.findall(
                ...
                layer_fname = re.split(
                    re.escape(r".gpkg|layername="),
                    layer.dataProvider().dataSourceUri(),
                    flags=re.IGNORECASE,
                )[1]
            else:
                layer_fname = os.path.splitext(
                    os.path.basename(layer.dataProvider().dataSourceUri())
                )[0]
            ...
            if error:
                return error, message
            if re.findall(
                r"((_FM_PLOT_[PLR])|_raw_(nodes|links)_)",
                layer_fname,
                flags=re.IGNORECASE,
            ):
...
def coastalmeqgis_apply_check_cme_clayer(qgis, **kwargs):
    error = False
    message = None
    try:
    ...
    if cLayer is not None and cLayer.type() == QgsMapLayer.VectorLayer:
        if re.findall(
            re.escape(r".gpkg|layername="),
            cLayer.dataProvider().dataSourceUri(),
            flags=re.IGNORECASE,
        ):
            layer_fname = re.split(
                re.escape(r".gpkg|layername="),
            ...
            layer_fname = os.path.splitext(
                os.path.basename(cLayer.dataProvider().dataSourceUri())
            )[0]
        ...
        if re.findall(
            r"((_FM_PLOT_[PLR])|_raw_(nodes|links)_)", layer_fname, flags=re.IGNORECASE
        ):
            slyr = False  # flood modeller
        if error:
            return error, message
...
def coastalmeqgis_apply_stability_style_clayer(iface):
    error = False
    message = None
    ...
    return coastalmeqgis_apply_stability_style(layer)


def coastalmeqgis_apply_stability_style(layer):
    error = False
    msg = ""
...
def coastalmeqgis_increment_fname(infname, use_regex=True):
    if use_regex:
        ...
        outfname = str(
            Path(infname).parent / "{0}{1}".format(outfname, Path(infname).suffix)
        )
        return outfname
    ...
    # check for COASTALME geometry suffix
    geom = ""
    if fname[-2:].upper() == "_P":
        tmpstr = fname[0:-2]
        geom = fname[-2:]
...
def coastalmeqgis_insert_cme_attributes(
    qgis, inputLayer, basedir, runID, template, lenFields, dialog, output_db
):
    message = None
    ...
    template = (
        "1d_nwke" if lenFields >= 10 and template.lower() == "1d_nwk" else template
    )
    empty_type = (
        "{0}_empty_pts".format(template)
        if "_pts" in template.lower()
        else "{0}_empty".format(template)
    )
    empty_file = [
        x
        for x in basedir.glob(f"{empty_type}*")
        if x.suffix.lower() in [".shp", ".gpkg"]
    ][0]
    ...
    if out_ext == ".gpkg":
        output_db = (
            Path(output_db)
            if output_db
            else (gis_folder / layername).with_suffix(".gpkg")
        )
    ...
    if out_ext == ".gpkg":
        if output_db.exists() and layername.lower() in [
            x.lower() for x in get_table_names(output_db)
        ]:
            answer = QMessageBox.question(
                dialog,
                "Output Exists",
                "{0} already exists in {1}.\nOverwrite existing?".format(
                    layername, output_db.name
                ),
                QT_MESSAGE_BOX_YES | QT_MESSAGE_BOX_NO | QT_MESSAGE_BOX_CANCEL,
            )
    ...
    for field in empty_layer.fields():
        ...
        uri = "{0}&field={1}:{2}({3},{4})".format(
            uri, field.name(), type_name, field.length(), field.precision()
        )
    for field in inputLayer.fields():
        ...
        if field.type() == QT_LONG_LONG:
            ...
            type_name = field.typeName().lower()
        uri = "{0}&field={1}:{2}({3},{4})".format(
            uri, new_name, type_name, field.length(), field.precision()
        )
    ...
    try:
        if Qgis.QGIS_VERSION_INT >= 31030:
            error = QgsVectorFileWriter.writeAsVectorFormatV2(
                out_lyr, out_path, QgsCoordinateTransformContext(), options
            )
            if error[0] != QgsVectorFileWriter.NoError:
                if out_ext == ".gpkg":
                    return "Error writing out {0} | {1}\n{2}".format(
                        output_db, layername, error[1]
                    )
                else:
                    return "Error writing out {0}\n{1}".format(fpath, error[1])
        else:
            err, msg = QgsVectorFileWriter.writeAsVectorFormat(
                out_lyr, out_path, "SYSTEM", out_lyr.crs(), options.driverName
            )
            if err:
                if out_ext == ".gpkg":
                    return "Error writing out {0} | {1}\n{2}".format(
                        output_db, layername, msg
                    )
                else:
                    return "Error writing out {0}\n{1}".format(fpath, msg)
    except Exception as e:
        if out_ext == ".gpkg":
            ...
            return "Error writing out {0}: {1}. Try updating QGIS to latest version to fix this issue.".format(
                fpath, e
            )
    ...
    if (
        open_layer is None
        or open_layer.dataProvider().dataSourceUri().lower() != fpath.lower()
    ):
...
def get_coastalme_labelName(layer):
    fsource = layer.source()  # includes full filepath and extension
    fname = os.path.split(fsource)[1][:-4]  # without extension
    ...
    if "1d_bc" in fname or "2d_bc" in fname:
        ...
        field_name = (
            "'Invert: ' + if(\"{0}\">-1000000, to_string(\"{0}\"), \"{0}\") + '\n' + 'Obvert: ' + "
        ...
        field_name = (
            ...
            ' + \'\n\' + \'CL: \' + if("{4}">=0, to_string("{4}"), "{4}")'.format(
        ...
        field_name = (
            "'Name: ' + \"{0}\" + '\n' + 'Trigger Type: ' + \"{1}\" + '\n' + 'Trigger Location: ' + \"{2}\""
            '+ \'\n\' + \'Trigger Value: \' + if("{3}">=0, to_string("{3}"), "{3}")'.format(
                field_name1, field_name2, field_name3, field_name4
            )
        )
        ...
        field_name = (
            '\'Z: \' + if("{0}">-1000000, to_string("{0}"), "{0}") + \'\n\' + \'Shape Options: \' + "{1}"'
            "+ '\n' + 'Trigger 1: ' + \"{2}\" + '\n' + 'Trigger 2: ' + \"{3}\" + '\n' + 'Trigger Value: ' "
            '+ if("{4}">0, to_string("{4}"), "{4}") + \'\n\' + \'Period: \' + '
            'if("{5}">0, to_string("{5}"), "{5}")'.format(
                field_name1,
                field_name2,
                field_name3,
                field_name4,
        ...
        field_name = (
            '\'Z: \' + if("{0}">-1000000, to_string("{0}"), "{0}") + \'\n\' + \'Shape Options: \' + "{1}"'
            "+ '\n' + 'Route Name: ' + \"{2}\" + '\n' + 'Cut Off Type: ' + \"{3}\" + '\n' + "
            '\'Cut Off Values: \' + if("{4}">-1000000, to_string("{4}"), "{4}")'.format(
                field_name1, field_name2, field_name3, field_name4, field_name5
            )
        )
        ...
        if layer.geometryType() == QgsWkbTypes.PointGeometry:
            field_name1 = layer.fields().field(0).name()
            field_name = (
                '\'{0}: \' + if("{0}">-1000000, to_string("{0}"), "{0}")'.format(
                    field_name1
                )
            )
        elif layer.geometryType() == QgsWkbTypes.LineGeometry:
            ...
            field_name = (
                "'Z: ' + if(\"{0}\">-1000000, to_string(\"{0}\"), \"{0}\") + '\n' + 'Shape Width: ' + "
                'if("{1}">-1000000, to_string("{1}"), "{1}")'.format(
                    field_name1, field_name2
                )
            )
        elif layer.geometryType() == QgsWkbTypes.PolygonGeometry:
        ...
        field_name1 = layer.fields().field(0).name()
        field_name = '\'{0}: \' + if("{0}">-1000000, to_string("{0}"), "{0}")'.format(
            field_name1
        )
    return field_name
...
def stripComments(text: str) -> str:
    """
    Strips comments as per COASTALME convention from text.
...
def getCmeflowLayerType(layerName: str) -> (str, str):
    """
    Gets the layer COASTALME type from the layer name.
    Returns COASTALME type and geometry type e.g. "1d_nwk". "L"

    :param layerName: str layer name or source
    :return: str COASTALME layer type, str geometry type
    """
    ...
    if check is not None:
        ...
        if re.findall(pattern, layerName, flags=re.IGNORECASE):
            coastalmeLayerType = "_FM_PLOT"

        # messages
        pattern = r"_messages"
        if re.findall(pattern, layerName, flags=re.IGNORECASE):
            coastalmeLayerType = "messages"

    # get geometry type
    if coastalmeLayerType:
    ...
    return coastalmeLayerType, geomType


def findLabelPropertyMatch(
    files: list, layerType: str, geomType: str, layerName: str
) -> (str, str):
    """
    ...
    # if layer is a check layer or not a coastalme layer loop through file and just find first name match
    if layerType == "check" or layerType == "" or layerType == "messages":
        for labelProperty in files:
            if (
                os.path.splitext(os.path.basename(labelProperty))[0].lower()
                in name.lower()
            ):
                match = labelProperty
    else:  # is a coastalme layer (input or result) so do a few additional checks (mainly to handle specific geometry types)
...
def assignLabelProperty(property: dict, command: str, value: str) -> bool:
    ...
    if command.lower() == "buffer":
        ...
        command.lower() == "offset x,y"
        or command.lower() == "offset x, y"
        or command.lower() == "offsetx,y"
    ):
        try:
        ...
        if not rule_based:  # not looking for a specific labelling rule
            ...
            for rule_name in rule_names:
                if rule_name in d["rule_based"]:
                    d_rule = parseLabelProperties(
                        fpath,
                        rule_based=True,
                        rule=rule_name,
                        d=d["rule_based"][rule_name],
                    )
                else:
                    d_rule = parseLabelProperties(
                        fpath, rule_based=True, rule=rule_name
                    )
...
def findCustomLabelProperties(layerName: str) -> dict:
    ...
    layerType, geomType = getCmeflowLayerType(layerName)

    dir = os.path.join(os.path.dirname(__file__), "layer_labelling")
    if os.path.exists(dir):
        labelPropertyFiles = glob.glob(os.path.join(dir, "*.txt"))
        matchingFile, override = findLabelPropertyMatch(
            labelPropertyFiles, layerType, geomType, layerName
        )
...
def setupLabelFormat(
    isExpression: bool = True, isBuffer: bool = True
) -> QgsPalLayerSettings:
...
def getLabelFieldName(layer: QgsVectorLayer, properties: dict) -> str:
    ...
    for i in properties["label_attributes"]:
        ...
        if properties["use_attribute_name"]:
            a.append(
                "'{0}: ' + if ( \"{0}\" IS NULL, '', to_string( \"{0}\" ) )".format(
                    get_field(i - 1 + add).name()
                )
            )
        else:
            a.append(
                'if ( "{0}" IS NULL, \'\', to_string( "{0}" ) )'.format(
                    get_field(i - 1 + add).name()
                )
            )
...
def setupLabelFilterExpression(
    layer: QgsVectorLayer, rule_name: str, strictness: str
) -> str:
    """
    ...
    try:
        ...
        attVal = "|".join(
            rule_name.split("|")[1:]
        ).strip()  # incase the name itself has '|' in it e.g. in 2d_SA
    ...
    if rule_name.lower() == "else":
        ...
        if fields.size() >= attNo:  # indexing starts at 1 for labelling properties
            ...
            for a in attVals:
                e = "\"{0}\" {2} '{3}{1}{3}'".format(
                    field_name, a.lower(), cond, cond_helper
                )
                exp.append(e)
...
def setLabelProperties(
    label: QgsPalLayerSettings, properties: dict, layer: QgsVectorLayer
) -> None:
    """
    ...
    if layer.geometryType() == QgsWkbTypes.PointGeometry:
        ...
        if qv < 32600:
            p = (
                ...
                if properties["line_placement"].lower() == "horizontal"
...
def coastalmeqgis_apply_autoLabel_clayer(qgis: QgisInterface):
    error = False
    message = None
    canvas = qgis.mapCanvas()
    cLayer = canvas.currentLayer()
        ...
        if not cLayer.labelsEnabled():
            ...
            if labelProperties["rule_based"]:  # use QgsRuleBasedLabeling class
                ...
                for rule_name, prop in labelProperties["rule_based"].items():
                    ...
                    expression = setupLabelFilterExpression(
                        cLayer, rule_name, prop["rule_strictness"]
                    )
...
def lineToPoints(feat, spacing, mapUnits, **kwargs):
    ...
    message = ""
    if (
        feat.geometry().wkbType() == QgsWkbTypes.LineString
        or feat.geometry().wkbType() == QgsWkbTypes.LineStringZ
        or feat.geometry().wkbType() == QgsWkbTypes.LineStringM
        or feat.geometry().wkbType() == QgsWkbTypes.LineStringZM
    ):
        geom = feat.geometry().asPolyline()
    elif (
        feat.geometry().wkbType() == QgsWkbTypes.MultiLineString
        or feat.geometry().wkbType() == QgsWkbTypes.MultiLineStringZ
        or feat.geometry().wkbType() == QgsWkbTypes.MultiLineStringM
        or feat.geometry().wkbType() == QgsWkbTypes.MultiLineStringZM
    ):
    ...
    with open(tcf, "r") as fo:
        for f in fo:
            ...
            if not read:
                continue
            if (
                "output drive" in f.lower()
            ):  # check if there is an 'if scenario' command
                ind = f.lower().find("output drive")  # get index in string of command
                if (
                    "!" not in f[:ind]
                ):  # check to see if there is an ! before command (i.e. has been commented out)
                    command, drive = f.split(
                        "==", 1
                    )  # split at == to get command and value
                    command = command.strip()  # strip blank spaces and new lines \n
                    drive = drive.split(
                        "!"
                    )[
                        0
                    ]  # split string by ! and take first entry i.e. remove any comments after command
        ...
        with open(tcf, "r") as fo:
            for f in fo:
                ...
                if "start 1d domain" in f.lower():
                    ind = f.lower().find(
                        "start 1d domain"
                    )  # get index in string of command
                    if (
                        "!" not in f[:ind]
                    ):  # check to see if there is an ! before command (i.e. has been commented out)
                        subread = True
                        for subline in fo:
                            ...
                            if not subread:
                                continue
                            if (
                                "output folder" in subline.lower()
                            ):  # check if there is an 'if scenario' command
                                ind = subline.lower().find(
                                    "output folder"
                                )  # get index in string of command
                                if (
                                    ...
                                    command, folder = subline.split(
                                        "=="
                                    )  # split at == to get command and value
                                    command = (
                                        command.strip()
                                    )  # strip blank spaces and new lines \n
                                    folder = folder.split(
                                        "!"
                                    )[
                                        0
                ...
                if "estry control file" in f.lower():
                    ...
                    if "!" not in f[:ind]:
                        ...
                        for file in files:
                            outputFolder1D += getOutputFolderFromTCF(
                                file
                    ...
                    if "read file" in f.lower():
                        ...
                        if "!" not in f[:ind]:
                            ...
                            for file in files:
                                res1D, res2D = getOutputFolderFromTCF(
                                    file,
                                    scenarios=scenarios,
                                    events=events,
                                    variables=variables,
                                    output_drive=outputDrive,
                                )
...
def getResultPathsFromTCF(fpath, **kwargs):
    ...
    # get output folders
    outputFolder1D, outputFolder2D = getOutputFolderFromTCF(
        fpath,
        variables=variables,
        output_drive=outputDrive,
        scenarios=scenarios,
    ...
    for opf2D in outputFolders2D:
        if opf2D is not None:
            if os.path.exists(opf2D):
                ...
                if not results2D:
                    messages.append(
                        "Could not find any matching mesh files for {0} in folder {1}".format(
                            basename, opf2D
                        )
                    )
                    ...
                    if ext.lower() == ".tpc":
                        ...
                        for x in basenameComponents1D:
                            if x not in name:
                                ...
                                break
                        if matches:
                            if check1DResultsForData(
                                os.path.join(outputFolderTPC, file)
                            ):
...
def check1DResultsForData(tpc):
    ...
    if Path(tpc).suffix.lower() == ".tpc":
        ...
        try:
            b = (
                res.gis_point_feature_count
                + res.gis_line_feature_count
                + res.gis_region_feature_count
            )
            b = bool(b)
        except Exception as e:
            pass
...
def getScenariosFromControlFile(controlFile, processedScenarios):
    ...
    try:
                ...
                if (
                    ...
                    if (
                        ...
                        scenarios = scenario.split(
                            "|"
                        )  # split scenarios by | in case there is more than one specified
        ...
        return "Unexpected error when reading file: {0}".format(
            controlFile
        ), processedScenarios
...
def getScenariosFromTCF_v2(control_file, scenarios, events, settings=None):
    from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.parser import (
        get_commands,
    )
    from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.settings import (
        ConvertSettings,
    )
    from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.file import (
        CmeflowPath,
    )

    if settings is None:
        control_file = CmeflowPath(control_file)
    ...
    for command in get_commands(control_file, settings_):
        if command.is_start_define() and "SCENARIO" in command.command:
            s = [
                x.strip()
                for x in command.value_orig.split("|")
                if x.upper().strip() not in [y.upper() for y in scenarios]
            ]
            scenarios.extend(s)
        elif (
            command.is_start_define()
            and "EVENT" in command.command
            and "IF" in command.command
        ):
            e = [
                x.strip()
                for x in command.value_orig.split("|")
                if x.upper().strip() not in [y.upper() for y in events]
            ]
            events.extend(e)
                ...
                if "!" not in f[:ind]:
                    if "estry control file auto" in f.lower():
                        ...
                        if os.path.exists(path):
                            msg, scenarios = getScenariosFromControlFile(
                                path, scenarios
                            )
                            if msg:
                                error = True
                ...
                if (
                    ...
                    if (
                        ...
                        if chosen_scenario in scenarios:
                            for sub_f in fo:
                                if (
                                    "set variable" in sub_f.lower()
                                ):  # check if there is an 'if scenario' command
                                    ind = sub_f.lower().find(
                                        "set variable"
                                    )  # get index in string of command
                                    if (
                                        "!" not in sub_f[:ind]
                                ...
                                if (
                                    "if scenario" in sub_f.lower()
                                    or "else" in sub_f.lower()
                                    or "end if" in sub_f.lower()
                                ):
                                    break
            else:
                if (
                    "set variable" in f.lower()
                ):  # check if there is an 'if scenario' command
                    ind = f.lower().find(
                        "set variable"
                    )  # get index in string of command
        ...
        for f in fo:
            ...
            if "read file" in f.lower():
                ...
                if "!" not in f[:ind]:
                    ...
                    path = getPathFromRel(dir, relPath)
                    if os.path.exists(path):
                        value = getVariableFromControlFile(
                            path, variable, scenario=scenario
                        )
                        if value is not None:
                            return error, message, value
                    else:
                    ...
                    if os.path.exists(path):
                        ...
                        if not error:
                            if not variable:
                                ...
                                error, message, cellSize = getVariableFromTCF(
                                    tcf, cellSize, scenario=scenario
                                )
                                if not error:
                                    try:
                                        cellSize = float(cellSize)
        ...
        for f in fo:
            ...
            if "read file" in f.lower():
                ...
                if "!" not in f[:ind]:
                    ...
                    path = getPathFromRel(dir, relPath)
                    if os.path.exists(path):
                        outputZones = getOutputZonesFromTCF(
                            path, output_zones=outputZones
                        )
...
def removeLayer(lyr):
    ...
    if lyr is not None:
        ...
        for i, child in enumerate(nodes):
            ...
            if (
                child.name() == lyr.name()
                or child.name() == "{0} Point".format(lyr.name())
                or child.name() == "{0} LineString".format(lyr.name())
                or child.name() == "{0} Polygon".format(lyr.name())
            ):
...
def getVariableNamesFromControlFile(controlFile, variables, scenarios=()):
    ...
    with open(controlFile, "r") as fo:
        try:
            ...
            msgBox.setText(
                "Encoding error:<br>{0}<br><a href='https://wiki.coastalme.com/index.php?title=COASTALME_Viewer#Loading_Results'>wiki.coastalme.com/index.php?title=COASTALME_Viewer#Loading_Results</a>".format(
                    controlFile
                )
            )
            msgBox.exec()
                ...
                if "estry control file" in f.lower():
                    ...
                    if "!" not in f[:ind]:
                        if "estry control file auto" in f.lower():
                            ...
                            path = getPathFromRel(dir, relPath)
                            if os.path.exists(path):
                                variables, error = getVariableNamesFromControlFile(
                                    path, variables, scenarios
                                )
                                if error:
                                    return {}, True
                if "geometry control file" in f.lower():
...
def loadGisFile(
    iface, path, group, processed_paths, processed_layers, error, log, crs, visible=True
):
    """
    ...
    if "|layername=" in path:
        ...
        try:
            if os.path.exists(path):
                lyr = iface.addVectorLayer(
                    path, os.path.basename(os.path.splitext(path)[0]), "ogr"
                )
                group.addLayer(lyr)
                processed_paths.append(path)
                processed_layers.append(lyr)
        ...
        try:
            if os.path.exists(path):
                ...
                tablenames = [
                    x.split("!!::!!")[1:] for x in dblyr.dataProvider().subLayers()
                ]
                for table in tablenames:
                    ...
                    uri = "{0}|layername={1}|geometrytype={2}".format(
                        path, table[0], table[2]
                    )
        ...
        try:
            if os.path.exists(path):
                lyr = iface.addRasterLayer(
                    path, os.path.basename(os.path.splitext(path)[0]), "gdal"
                )
                group.addLayer(lyr)
                processed_paths.append(path)
                processed_layers.append(lyr)
...
class LoadGisFiles(QObject):
    ...
    def loadLayersIntoWorkspace(self, root, layers, options=None, insert_before=None):
        ...
        for lyr in layers:
            ...
            coastalmeqgis_apply_check_cme_clayer(None, layer=maplyr)
            ...
            if (
                options is not None
                and options.load_raster_method == "invisible"
                and isinstance(maplyr, QgsRasterLayer)
            ):
    ...
    def loadGisFiles(self):
        from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.gis import (
            ogr_iter_geom,
            get_database_name,
            ogr_format,
            ogr_format_2_ext,
        ...
        for cf in self.model_file_layers:
            ...
            for gis_file in cf.gis():
                ...
                for geom in ogr_iter_geom(gis_file):
                    ...
                    lyr = "QgsVectorLayer(r'{0}', '{1}', 'ogr')".format(
                        layer_uri, lyrname
                    )
                    # if not lyr.isValid():
            ...
            if options.load_raster_method != "no":
                for grid_file in cf.grid():
                    ...
                    lyr = "QgsRasterLayer(r'{0}', '{1}', 'gdal')".format(
                        layer_uri, lyrname
                    )
                    # if not lyr.isValid():
                    # 	files_failed.append(layer_uri)
            ...
            for table_layer in cf.table():
                ...
                layer_uri = (
                    f"{db}|layername={lyrname}|geometrytype=None|uniqueGeometryType=yes"
                )
            ...
            if swmm_inp:
                ...
                for i, inp_name in enumerate(cf.swmm_inp_name):
                    ...
                    for lyr in cf.swmm_inp_lyrs[i]:
                        ...
                        gis_files.pop(i)
                    failed_lyrs = self.loadLayersIntoWorkspace(
                        inp_group, layers_, options
                    )
                    if failed_lyrs:
                        self.err = True
                        files_failed.extend(failed_lyrs)
                if layers and options.grouped:
                    layers = sortLayers(layers, options, layers_order_key)
                    failed_lyrs = self.loadLayersIntoWorkspace(
                        group, layers, options, insert_before=first_group
                    )
                    layers.clear()
                    layers_order_key.clear()
                    gis_files.clear()
...
class ModelFileLayers(QObject):
    ...
    def __iter__(self):
        ...
        for cf in sorted(
            self._cfs,
            key=lambda x: order.get(Path(x.name).suffix.lower())
            if order.get(Path(x.name).suffix.lower()) is not None
            else 100,
        ):
            yield cf
...
class LoadGisFromControlFile(QObject):
    ...
    def __init__(
        self, model_file_layers, control_file, settings=None, scenarios=(), events=()
    ):
        QObject.__init__(self)
        self.model_file_layers = model_file_layers
    ...
    def loadGisFromControlFile_v2(self):
        from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.control_file import (
            get_commands,
        )
        from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.settings import (
            ConvertSettings,
        ...
        try:
            ...
            if self.settings is None:
                ...
                settings_ = self.settings.copy_settings(
                    control_file, self.settings.output_folder
                )

            for command in get_commands(control_file, settings_):
                if command.in_scenario_block() and not command.in_scenario_block(
                    settings_.scenarios
                ):
                    continue
                    ...
                    for i, file in enumerate(command.iter_files(settings_)):
                        cf = file.resolve()
                        load_gis = LoadGisFromControlFile(
                            self.model_file_layers, cf, settings_
                        )
                        load_gis.loadGisFromControlFile_v2()
                    if i == -1:
                        self.settings.no_files_copied.append(
                            (
                                settings_.control_file.suffix[1:].upper(),
                    ...
                    if (
                        command.in_output_zone_block()
                        and not command.in_output_zone_block(settings_.output_zones)
                    ):
                        continue
                    ...
                    for i, file in enumerate(command.iter_files(settings_)):
                        if command.command == "TIF PROJECTION":
                            # cf_lyrs.add_grid(file)
                            cf_lyrs.layer_added.emit(
                                ""
                            )  # in case the progress bar requires the counts to be the same
                            pass
                        else:
                            cf_lyrs.add_gis(file)
                            cf_lyrs.add_lyr(file)
                    ...
                    if not os.path.exists(output_path):
                        try:
                            ...
                            output = processing.run(
                                "COASTALME:COASTALMEConvertSWMMinpToGpkg", inputs
                            )
                        except:
                            pass
                        finally:
                    ...
                    for i, lyr in enumerate(GPKG(output_path).vector_layers()):
                        lyr_name = CmeflowPath(f"{output_path} >> {lyr}")
...
def loadGisFromControlFile(
    ...
    **kwargs,
    ...
    """
    Opens all vector layers from the specified coastalme control file
        ...
        for f in fo:
            ...
            if "read" in f.lower() or "create tin zpts" in f.lower():
                ...
                if "!" not in f[:ind] and "#" not in f[:ind]:
                    if (
                        not re.findall(
                            r"read material(s)? file", f, flags=re.IGNORECASE
                        )
                        and not re.findall(
                            r"read soil(s)? file", f, flags=re.IGNORECASE
                        )
                        and "read file" not in f.lower()
                        and "read operating controls file" not in f.lower()
                    ):
                        command, relPath = f.split("==", 1)
                        command = command.strip()
                        ...
                        for relPath in relPaths:
                            relPath = relPath.strip()
                            paths = getAllFolders(
                                dir, relPath, variables, scenarios, [], db=dbCCF
                            )
                            for path in paths:
                                if path not in processed_paths:
                                    if (
                                        is_raster
                                        and "load_rasters" in kwargs
                                        and kwargs["load_rasters"] == "no"
                                    ):
                                        continue
                                    invisible = (
                                        is_raster
                                        and "load_rasters" in kwargs
                                        and kwargs["load_rasters"] == "invisible"
                                    )
...
def openGisFromTcf(tcf, iface, scenarios=(), load_rasters=True):
    """
    Opens all vector layers from the coastalme model from the TCF
    ...
    dbTCF = None
    dbCCF = None
    error, log, pPaths, pLayers, crs, dbTCF, dbCCF = loadGisFromControlFile(
        tcf,
        iface,
        processed_paths,
        processed_layers,
        ...
        for f in fo:
            ...
            if "estry control file" in f.lower():
                ...
                if "!" not in f[:ind] and "#" not in f[:ind]:
                    if "estry control file auto" in f.lower():
                        path = "{0}.ecf".format(os.path.splitext(tcf)[0])
                        error, log, pPaths, pLayers, crs, dbTCF, dbCCF = (
                            loadGisFromControlFile(
                                path,
                                iface,
                                processed_paths,
                                processed_layers,
    ...
    if couldNotReadFile:
        ...
        QMessageBox.information(
            iface.mainWindow(), "Message", "Successfully Loaded All COASTALME Layers"
        )
...
def getUnit(resultType, canvas, **kwargs):
    ...
    if canvas is not None:
        if (
            canvas.mapUnits() == QgsUnitTypes.DistanceMeters
            or canvas.mapUnits() == QgsUnitTypes.DistanceKilometers
            or canvas.mapUnits() == QgsUnitTypes.DistanceCentimeters
            or canvas.mapUnits() == QgsUnitTypes.DistanceMillimeters
        ):  # metric
            u, m = 0, "m"
        elif (
            canvas.mapUnits() == QgsUnitTypes.DistanceFeet
            or canvas.mapUnits() == QgsUnitTypes.DistanceNauticalMiles
            or canvas.mapUnits() == QgsUnitTypes.DistanceYards
            or canvas.mapUnits() == QgsUnitTypes.DistanceMiles
        ):  # imperial
            u, m = 1, "ft"
        else:  # use blank
            u, m = -1, ""
...
def convertStrftimToCmeviewftim(strftim):
    """

    :param strftim: str standard datetime string format e.g. %d/%m/%Y %H:%M:%S
    :return: str user friendly style time string format used in cmeview (similar to excel) e.g. DD/MM/YYYY hh:mm:ss
    """
...
def convertCmeviewftimToStrftim(tvftim):
    """


    :param tvftim: str user friendly style time string format used in cmeview (similar to excel) e.g. DD/MM/YYYY hh:mm:ss
...
def getPropertiesFrom2dm(file):
    """Get some basic properties from COASTALME classic 2dm file"""
    ...
    with open(file, "r") as fo:
        for line in fo:
            ...
            if (
                count > 10
            ):  # should be at the start and don't want to waste time if there is an error
                return cellSize, wllVerticalOffset, origin, orientation, gridSize
            count += 1
...
def bndryBinProperties(file):
    ...
    with open(file, "rb") as fo:
        for line in fo:
            for i in range(
                0, len(line), 4
            ):  # data located at either a multiple of 4 or 8
                a = line[i]
                if i == 0:
                    xf_iparam_1 = a
                elif i == 4 and a == 4:
...
def readBndryBin(file):
    """
    Reads COASTALME bounadry csv .xf file
    ...
    xf_iparam_1, xf_iparam_2, xf_iparam_3, xf_iparam_4, xf_iparam_5 = (
        bndryBinProperties(file)
    )
...
def changeDataSource(iface, layer, newDataSource, isgpkg):
    ...
    if isgpkg:
        newName = re.split(
            re.escape(r".gpkg|layername="), newDataSource, flags=re.IGNORECASE
        )[1]
    else:
    ...
    if Qgis.QGIS_VERSION_INT >= 32000:
        ...
        # change datasource
        element.elementsByTagName("datasource").item(0).firstChild().setNodeValue(
            newDataSource
        )
...
def getOpenCOASTALMELayers(layer_type="input_types"):
    """
    Returns a list of open coastalme layer types, coastalme layers, or coastalme check layers
    ...
    for name, layer in QgsProject.instance().mapLayers().items():
        if layer.type() == 0:  # vector layer
            pattern = r"_check(_[PLR])?$"  # will match '_check' or '_check_P' (P or L or R) only if it is at the end of the string
            name = os.path.splitext(layer.name())[
                0
            ]  # make sure there is no file path extension
            search = re.search(pattern, name, flags=re.IGNORECASE)
            if search is not None:
                if layer_type == "check_all":
                    coastalmeLayers.append(layer.name())
                ...
                if search is not None:
                    if layer_type == "input_all":
                        ...
                        coastalme_type = "_".join(components[:2]).lower()

                        # special case for 2d_sa as this could be 2d_sa_tr or 2d_sa_rf
                        specialCases = ["2d_sa_rf", "2d_sa_tr"]
                        if len(components) >= 3:
                            for sc in specialCases:
                                tempName = coastalme_type + "_" + components[2]
                                if sc.lower() == tempName.lower():
                                    coastalme_type = tempName

                        if coastalme_type not in coastalmeLayers:
                            coastalmeLayers.append(coastalme_type)

    return sorted(coastalmeLayers)
...
def turnLayersOnOff(settings):
    """
    Cmern layers in the workspace on/off based on settings. If layer is not specified in settings, left as 'current'.
...
def sorting_alg(layers):
    lyr_wo_geom = [
        re.sub(r"_[plr]$", "", Layer(x).name, flags=re.IGNORECASE) for x in layers
    ]
    key = {
    ...
    lyr_w_geom = [
        f"{lyrname}_{key[Layer(lyr).geom_type]}"
        for lyr, lyrname in zip(layers, lyr_wo_geom)
        if "QgsVectorLayer" in str(lyr)
    ]
    lyr_w_geom.extend(
        [f"{Layer(x).name}_3" for x in layers if "QgsVectorLayer" not in str(x)]
    )
...
def sortLayers(layers, options=None, sort_key=None):
    ...
    zlayers = [
        x for x in layers if x not in vlayers and x not in rlayers and x not in mlayers
    ]  # any leftovers

    if (
        options is not None
        and options.order_method == "control_file"
        ...
        options is not None
        and options.order_method == "control_file_group_rasters"
        and sort_key is not None
    ):
...
# 	layer = coastalmeqgis_find_layer(node.name())
# 	if layer is not None:
...
def getAllFolders(
    dir, relPath, variables, scenarios, events, output_drive=None, db=None
):
    """
    ...
    if count == 0:
        if db is not None and os.path.splitext(relPath)[-1] == "":
            for d in db:
                for lyr in relPath.split("&&"):
                    ...
                    if lyr.strip().lower() in [x.lower() for x in layers]:
                        lyr = layers[
                            [x.lower() for x in layers].index(lyr.strip().lower())
                        ]
            ...
            if ">>" in os.path.splitext(relPath)[-1]:
                ...
                d = "{0}{1}".format(
                    getPathFromRel(
                        dir, os.path.splitext(relPath)[0], output_drive=output_drive
                    ),
                    d.strip(),
                )
                if os.path.exists(d):
                ...
                for d in dirs[:]:
                    ...
                    for combo in combinations:
                        ...
                        for n, vname in enumerate(vnames):
                            ...
                            # i.e. <<variable>>\results = C:\coastalme\results which means
                        ...
                        if (
                            m + 1 == count
                            and ".gpkg" in os.path.splitext(p)[-1].lower()
                        ):
                            ...
                            if os.path.exists(db_):
                                ...
                                for combo in combinations:
                                    ...
                                    for p3 in p2.split("&&"):
                                        if p3.strip().lower() in [
                                            x.lower() for x in layers
                                        ]:
                                            p3 = layers[
                                                [x.lower() for x in layers].index(
                                                    p3.strip().lower()
                            ...
                            if os.path.exists(p):
                                if m + 1 == count:
                                    ...
                                    p = getPathFromRel(
                                        p, path_rest, output_drive=output_drive
                                    )
                                    if os.path.exists(p):
                                        folders.append(p)
                                else:
...
def getVariableCombinations(vnames, variables, scenarios, events):
    ...
    for c in combo_list:
        no *= len(c)
    combinations = [
        ["" for y in range(len(combo_list))] for x in range(no)
    ]  # blank combination list
...
def ascToAsc(exe, function, workdir, grids, **kwargs):
    ...
    for grid in grids:
        if isinstance(grid, QgsMapLayer):
            ...
            inputs.append(layer_helper.coastalme_path)
        elif grid.replace("/", os.sep).count(os.sep) == 0:
            layer = coastalmeqgis_find_layer(grid)
            if layer is not None:
    ...
    if "saveFile" in kwargs:
        if kwargs["saveFile"]:
            ...
            QMessageBox.information(None, "COASTALME Utility", message)
...
def coastalmeToGis(exe, function, workdir, mesh, dataType, timestep, **kwargs):
    """
    Runs common funtions in the coastalme_to_gis utility.
    ...
    if not _2dm.exists():
        if re.findall(
            r"\(.*\){0}$".format(re.escape(Path(mesh).suffix)), Path(mesh).name
        ):
            new_2dm = re.sub(
                r"\(.*\){0}$".format(re.escape(Path(mesh).suffix)), ".2dm", mesh
            )
            if Path(new_2dm).exists():
                args.extend(["-2dm", str(new_2dm)])

    if isinstance(timestep, str) and (
        timestep.lower() == "max" or timestep.lower() == "maximum"
    ):
        args.append("-max")
...
def coastalmeUtility(exe, workdir, flags, saveFile):
    """
    ...
    def start_with_qgis(self):
        ...
        try:
            with open(self.destinationFileName, "wb") as f:
                ...
                if ret_code != 200:
                    raise Exception(
                        "Error downloading: {}".format(self.reply.errorString())
                    )
...
class DownloadBinPackage(QObject):
    ...
    def wait(self):
        ...
        if self.progress_bar.error:
            raise IOError(
                "error code {} {}".format(self.progress_bar.error, self.packageUrl)
            )
...
def downloadUtility(utility, parent_widget=None):
    ...
    #     'coastalme_to_gis': '{0}/COASTALME_to_GIS_w64.exe'.format(name),
    ...
    # downloadBaseUrl = 'https://www.coastalme.com/Download/COASTALME/Utilities/'
    ...
    try:
        ...
        extracted_name = [
            x.filename
            for x in z.filelist
            if os.path.splitext(x.filename)[1].upper() == ".EXE"
        ]
        if extracted_name:
        ...
        qApp.restoreOverrideCursor()
        raise Exception(
            "Download of {0} failed. Please try again or contact support@coastalme.com for "
...
def reSpecPlot(fig, ax, ax2, cax, bqv):
    ...
    if cax is None and not bqv:
        ...
        xlabelsize = (
            ...
            else ax.xaxis.get_label().get_size()
            + ax.xaxis.get_ticklabels()[0].get_size()
            + ax.xaxis.get_tick_padding()
        )
        ylabelsize = (
            70
            if not ax.get_ylabel()
            else ax.yaxis.get_label().get_size()
            + ax.yaxis.get_ticklabels()[0].get_size()
            + ax.yaxis.get_tick_padding()
        )
        rei = int(
            1000
            - (
                (ax.xaxis.get_tightbbox(fig.canvas.get_renderer()).height + xlabelsize)
                / fig.bbox.height
                * 1000
                + hpad
            )
        ...
        csi = int(
            (ax.yaxis.get_tightbbox(fig.canvas.get_renderer()).width + ylabelsize)
            / fig.bbox.width
            * 1000
            + wpad
        )
        ...
        if cax is not None:
            ...
            if cax != 1:
                ...
                width = int(
                    (max(cax_bbox_yaxis.x1, cax_bbox_xaxis.x1) - cax_bbox_xaxis.x0)
                    / fig.bbox.width
                    * 1000
                )
            ...
            if ax2 is not None:
                y2labelsize = (
                    ...
                    else ax2.yaxis.get_label().get_size()
                    + ax2.yaxis.get_ticklabels()[0].get_size()
                    + ax2.yaxis.get_tick_padding()
                )
                cei = int(
                    1000
                    - ax2.yaxis.get_tightbbox(fig.canvas.get_renderer()).width
                    + y2labelsize / fig.bbox.width * 1000
                    + wpad
                )
            else:
                cei = 1000 - wpad
...
def addQuiverKey(fig, ax, ax2, bqv, qv, label, max_u, **kwargs):
    ...
    u = max_u / 4
    qk = ax.quiverkey(
        qv, X=X, Y=Y, U=u, label=label, labelpos="W", coordinates="figure"
    )
    if cax is not None:
        addColourBarAxes(
            fig, ax, ax2, bqv, respec=(gs, rsi, rei, csi, cei), cax=cax, qk=qk
        )
...
def convertFormattedTimeToFormattedTime(
    formatted_time: str, return_format: str = "{0:02d}:{1:02d}:{2:02.0f}"
) -> str:
    """
...
def calculateLength(p1, p2, mapUnits, desiredUnits="m"):
    ...
    if mapUnits != QgsUnitTypes.DistanceDegrees:
        ...
        try:
            ...
            return None
        metres = (
            (p1Converted[1] - p2Converted[1]) ** 2.0
            + (p1Converted[0] - p2Converted[0]) ** 2.0
        ) ** 0.5
        if desiredUnits == "m":
            return metres
...
def createNewPoint(p1, p2, length, new_length, mapUnits):
    ...
    if mapUnits != QgsUnitTypes.DistanceDegrees:
        ...
        x = (
            p2.x() + (new_length * cos(angle))
            if p1.x() - p2.x() >= 0
            else p2.x() - (new_length * cos(angle))
        )
        ...
        x = (
            p2Converted[0] + (new_length * cos(angle))
            if p1Converted[0] - p2Converted[0] >= 0
            else p2Converted[0] - (new_length * cos(angle))
        )
...
class ReallyBasicFieldType:
    ...
    def fromQVariantType(qvariant_type):
        if qvariant_type == QVariant.String:
            ...
            or qvariant_type == QVariant.LongLong
            or qvariant_type == QVariant.Int
        ):
...
def is1dNetwork(layer, report_where=False):
    ...
    correct1dNetworkType = [
        ReallyBasicFieldType.String,
        ReallyBasicFieldType.String,
        ReallyBasicFieldType.Any,
        ReallyBasicFieldType.String,
        ReallyBasicFieldType.Number,
        ReallyBasicFieldType.Number,
        ReallyBasicFieldType.Number,
        ReallyBasicFieldType.Number,
        ReallyBasicFieldType.Number,
    ...
    if len(fieldTypes) < count:
        if report_where:
            Logging.warning(
                "Field count does not match the expected minimum for 1d_nwk: Expected {0}, Received {1}".format(
    ...
    for i in range(count):
        ...
        if field_expected != field_received:
            ...
            if report_where:
                Logging.warning(
                    "Field does not match 1d_nwk type: ({0}) {1}: Expected {2}, Received {3}".format(
                        i + 1,
                        field.name(),
                        ReallyBasicFieldType.toString(field_expected),
                        ReallyBasicFieldType.toString(field_received),
                    )
                )
    return is_1d_nwk
...
def isPlotLayer(layer, geom=""):
    ...
    if layer.customProperty("isCOASTALMEPlotLayer"):
        return True
...
def isTSLayer(layer, results=None):
    ...
    return re.findall(r"_TS(MB|MB1d2d)?(_[PLR])?$", name, flags=re.IGNORECASE) and (
        results is None or name in results
    )
...
def getRasterValue(point, raster):
    ...
    try:
        return (
            raster.dataProvider()
            .identify(QgsPointXY(point), QgsRaster.IdentifyFormatValue)
            .results()[1]
        )
    except KeyError:
        return numpy.nan
...
def getNetworkMidLocation(feature):
    ...
    desiredLength = length / 2.0
    points, chainages, directions = lineToPoints(
        feature, 99999, QgsUnitTypes.DistanceMeters
    )
    chPrev = 0
    ...
    dsVertex = points[i]
    length = (
        (dsVertex[1] - usVertex[1]) ** 2.0 + (dsVertex[0] - usVertex[0]) ** 2.0
    ) ** 0.5
    newLength = desiredLength - chPrev
    angle = asin((dsVertex[1] - usVertex[1]) / length)
    x = (
        usVertex[0] + (newLength * cos(angle))
        if dsVertex[0] - usVertex[0] >= 0
        else usVertex[0] - (newLength * cos(angle))
    )
...
def browse(
    ...
    if lineEdit is not None:
        if type(lineEdit) is QLineEdit:
            ...
            if (
                not re.findall(r"^<.*>$", lineEdit.currentText())
                and lineEdit.currentText()
            ):
                startDir = lineEdit.currentText()

    if (
        lastFolder and startDir == os.getcwd()
    ):  # if outFolder no longer exists, work backwards in directory until find one that does
...
def getResultPathsFromTLF(fpath: str, read_method: int = 0) -> (list, list, list):
    ...
    if secondaryExt.upper() == ".HPC":
        return (
            [],
            [],
            ["Please Make Sure Selecting .tlf Not {0}.tlf".format(secondaryExt)],
        )
    ...
    try:
        ...
        #                             "https://wiki.coastalme.com/index.php?title=COASTALME_Message_0060")
...
def resetQgisSettings(e: QEvent = False, scope: str = "Global", **kwargs) -> None:
    ...
    :param kwargs: dict of specifics to either leave out (=False) or only (=True) e.g. cmeviewer=True will only delete cmeviewer related settings, cmeviewer=False skips deleting cmeviewer settings
    :return: None
    """
    ...
    with open(varFile, "r") as fo:
        ...
        toFalse = (
            "start"  # should just be a dummy value because variable read starts as True
        )
        toTrue = "start"
        for line in fo:
    ...
    QMessageBox.information(
        None,
        "COASTALME Clear Settings",
        "Successfully Cleared {0} Settings".format(scope),
    )
...
def ncReadDimCDLL(ncdll: ctypes.cdll, ncid: ctypes.c_long, n: int) -> DimReturn:
    ...
    for i in range(n):
        ...
        if err:
            if ncid.value > 0:
                ncdll.nc_close(ncid)
            return "ERROR: error getting netcdf dimensions. Error: {0}".format(
                NC_Error.message(err)
            ), dims
...
def ncReadVarCDLL(
    ncdll: ctypes.cdll, ncid: ctypes.c_long, n: int, ncDims: ncDimArg
) -> VarReturn:
    """
    Read all dimensions from netcdf file.
    ...
    for i in range(n):
        ...
        if err:
            if ncid.value > 0:
                ncdll.nc_close(ncid)
            return "ERROR: error getting netcdf variable names. Error: {0}".format(
                NC_Error.message(err)
            ), ncVars
        ...
        if err:
            if ncid.value > 0:
                ncdll.nc_close(ncid)
            return "ERROR: error getting netcdf variable types. Error: {0}".format(
                NC_Error.message(err)
            ), ncVars
        ...
        if err:
            if ncid.value > 0:
                ncdll.nc_close(ncid)
            return "ERROR: error getting netcdf variable dimensions. Error: {0}".format(
                NC_Error.message(err)
            ), ncVars
...
def lineIntersectsPoly(p1, p2, poly):
    ...
    pLine = QgsGeometry.fromPolyline(
        [QgsPoint(p1.x(), p1.y()), QgsPoint(p2.x(), p2.y())]
    )
...
def doLinesIntersect(a1, a2, b1, b2):
    ...
    pLine1 = QgsGeometry.fromPolyline(
        [QgsPoint(a1.x(), a1.y()), QgsPoint(a2.x(), a2.y())]
    )
    pLine2 = QgsGeometry.fromPolyline(
        [QgsPoint(b1.x(), b1.y()), QgsPoint(b2.x(), b2.y())]
    )
...
def generateRandomMatplotColours2(
    nColours,
    type_="bright",
    tol=2,
    tol_reduction=0.1,
    ...
    if type_ == "bright":
        ...
        while n <= nColours:
            randHSVcolor = [
                numpy.random.uniform(low=0.0, high=1),
            ...
            rgb = numpy.array(
                colorsys.hsv_to_rgb(randHSVcolor[0], randHSVcolor[1], randHSVcolor[2])
            )
            if n > 1:
                add_colour = True
...
def getFaceIndexes3(
    si: QgsMeshSpatialIndex, dp: QgsMeshDataProvider, points: PointList, mesh: QgsMesh
) -> FaceIndexList:
    """ """
...
def findMeshFaceIntersects(
    p1: QgsPointXY,
    p2: QgsPointXY,
    si: QgsMeshSpatialIndex,
    dp: QgsMeshDataProvider,
    mesh: QgsMesh,
):
    """ """
...
def sameFace(v, v2, v_used, mesh):
    ...
    for vu in v_used:
        if v in vu:
            ...
            if (
                abs(
                    atan2(p2.y() - p.y(), p2.x() - p.x())
                    - atan2(p3.y() - p.y(), p3.x() - p.x())
                )
                <= 0.1
            ):
                return True
...
def findMeshSideIntersects(
    p1: QgsPointXY, p2: QgsPointXY, faces: FaceList, mesh: QgsMesh, allFaces
) -> PointList:
    """ """
    ...
    for i, p in enumerate(geom):
        if i > 0:
            ...
            if inters:
                ...
                pOrdered, mOrdered = orderPointsByDistanceFromFirst(
                    inters, minter, si, mesh, i, crs
                )
                points += pOrdered
                allFaces += mOrdered
        if i + 1 == len(geom):
    ...
    for i, p in enumerate(points):
        if i > 0:
            if iface is not None:
                chainage += calculateLength2(
                    p, points[i - 1], crs, iface.mapCanvas().mapUnits()
                )
            else:
                chainage += calculateLength2(
                    p, points[i - 1], crs, QgsUnitTypes.DistanceMeters
                )
            chainages.append(chainage)
...
def calcMidPoint(p1, p2, crs, iface):
    ...
    if iface is not None:
        h = calculateLength2(p1, p2, crs, iface.mapCanvas().mapUnits()) / 2.0
    else:
        h = calculateLength2(p1, p2, crs, QgsUnitTypes.DistanceMeters) / 2.0
...
def getPolyCollectionData(pc, axis):
    ...
    assert axis.lower() in ["x", "y", "scalar"], (
        "'axis' variable not one of 'x', 'y', or 'scalar'"
    )
...
def isSame_float(a, b, prec=None):
    ...
    if prec is None:
        prec = abs(sys.float_info.epsilon * max(a, b) * 2.0)
...
def qgsxml_as_mpl_cdict(fpath):
    ...
    if root.findall("colorramps"):
        for cr in root.findall("colorramps")[0].findall("colorramp"):
            ...
            success = read_colour_ramp_qgsxml(
                cr, break_points, reds, greens, blues, alphas
            )
            if not success:
                continue
...
def reload_data(layer):
    ...
    if layer is not None:
        if isinstance(layer, QgsVectorLayer):
            ...
            isinstance(layer, QgsMeshLayer) and Qgis.QGIS_VERSION_INT >= 32800
        ) or isinstance(layer, QgsRasterLayer):
            # check if the result has been temporarily copied - recopy if needed
            if plugins["coastalme"].resultsPlottingDockOpened:
                cmeview = plugins["coastalme"].resultsPlottingDock
                if layer in cmeview.tuResults.tuResults2D.copied_results:
                    lyr_type = "mesh"
                    tmp_res = cmeview.tuResults.tuResults2D.copied_results[layer]
                elif layer in cmeview.tuResults.tuResultsNcGrid.copied_results:
                    lyr_type = "nc_grid"
                    tmp_res = cmeview.tuResults.tuResultsNcGrid.copied_results[layer]
                if lyr_type:
                    if hasattr(cmeview, "timer") and cmeview.timer is not None:
                        cmeview.timer = None
                        tmp_res.clean()
                        if tmp_res.update_method() == "RETARGET":
                            ...
                            if lyr_type == "nc_grid":
                                for layer_ in (
                                    ...
                                    if (
                                        layer_
                                        in cmeview.tuResults.tuResultsNcGrid.copied_results
                                    ):
                                        tmp_res_ = cmeview.tuResults.tuResultsNcGrid.copied_results[
                                            layer_
                                        ]
                                    else:
                                        continue
                                    if tmp_res_ == tmp_res:
                                        if layer_ != layer:
                                            layer_.reload()
                                        cmeview.tuResults.tuResultsNcGrid.layerReloaded(
                                            layer_
                                        )
                            return
                    else:
                        cmeview.timer = QTimer()
                        cmeview.timer.setSingleShot(True)
                        cmeview.timer.timeout.connect(lambda: reload_data(layer))
                    tmp_res.update(cmeview)
                    if (
                        tmp_res.update_method() == "RETARGET"
                        and Qgis.QGIS_VERSION_INT >= 32000
                    ):
                        if lyr_type == "mesh":
                            layer.setDataSource(
                                str(tmp_res.tmp_file), layer.name(), "mdal"
                            )
                            ...
                            for layer_ in QgsProject.instance().mapLayers().values():
                                if (
                                    ...
                                    continue
                                if tmp_res_ == tmp_res:
                                    layer_.setDataSource(
                                        str(tmp_res.tmp_file), layer_.name(), "gdal"
                                    )

            layer.reload()
            if lyr_type == "nc_grid":
                for layer_ in QgsProject.instance().mapLayers().values():
                    if layer_ in cmeview.tuResults.tuResultsNcGrid.copied_results:
                        tmp_res_ = cmeview.tuResults.tuResultsNcGrid.copied_results[
                            layer_
                        ]
                    else:
                    ...
                    if tmp_res_ == tmp_res:
                        if layer_ != layer:
                            layer_.reload()
                        cmeview.tuResults.tuResultsNcGrid.layerReloaded(layer_)
                cmeview.tuResults.tuResultsNcGrid.layerReloaded(
                    None
                )  # this triggers the timer to start


def coastalmeqgis_apply_gpkg_layername(iface):
    for layer_name, layer in QgsProject.instance().mapLayers().items():
        if layer.type() == QgsMapLayer.VectorLayer:
...
def getRightAngleLine(geom, length):
    ...
    if ang is None:
        return None
    p1 = QgsPointXY(
        get_x(geom.centroid().asPoint().x(), ang + pi / 2.0, length / 2.0),
        get_y(geom.centroid().asPoint().y(), ang + pi / 2.0, length / 2.0),
    )
    p2 = QgsPointXY(
        get_x(geom.centroid().asPoint().x(), ang - pi / 2.0, length / 2.0),
        get_y(geom.centroid().asPoint().y(), ang - pi / 2.0, length / 2.0),
...
class XMDF_Header_Info:
    def __init__(self, header_text_dump):
        ...
        for line in header_text_dump.split("\n"):
            if (
                vector
                and self._vector_dataset_count > 0
                and vi == self._vector_dataset_count
            ):
                vector = False
            if (
                scalar
                and self._scalar_dataset_count > 0
                and si == self._scalar_dataset_count
            ):
                scalar = False
        ...
        self._loaded = (
            bool(self._vector_dataset_count) or bool(self._scalar_dataset_count)
        ) and bool(self._res_types)
        ...
        if isinstance(item, tuple) and len(item) == 2:
            ...
            if isinstance(item1, str) and isinstance(item2, str):
                if item2 == "Max":
                    a = [
                        x
                        for x in self._res_types
                        if x.name() == item1
                        and x.folder() in ["Maximums", "Final", "Times"]
                    ]
                    if a:
                    ...
                    a = [
                        x
                        for x in self._res_types
                        if x.name() == item1 and x.folder() == "Minimums"
                    ]
                    if a:
                        return a[0].key(), str(a[0].times()[0])
                else:
        ...
        if not res_type:
            ...
            return bool(
                [
                    x
                    for x in self._res_types
                    if x.name() == res_type and x.folder() == "Minimums"
                ]
            )
...
class XMDF_Res_Type:
    def __init__(self, folder, name, is_vector, scalar_count, vector_count):
        ...
        self._key = (
            "v{0}".format(vector_count) if is_vector else "s{0}".format(scalar_count)
        )
        self._folder = folder
        self._name = name
        self._times = []
...
def getOutputDirs_old(path):
    ...
    search_globs = [
        "{0}{1}**{1}{2}.xmdf".format(
            outputFolder2D[0], os.sep, os.path.splitext(os.path.basename(path))[0]
        ),
        "{0}{1}**{1}{2}.tpc".format(
            outputFolder2D[0], os.sep, os.path.splitext(os.path.basename(path))[0]
        ),
...
def getOutputDirs(path, settings=None):
    ...
    for command in get_commands(path, settings):
        if command.is_control_file():
            if (
                command.is_value_a_file() and command.value.suffix.lower() == ".ecf"
            ) or str(command.value).lower() == "AUTO":
                continue
        ...
        if command.command == "OUTPUT FOLDER":
            ...
            search_globs = [
                CmeflowPath(command.value)
                / "**"
                / settings.tcf.with_suffix(".xmdf").name,
                CmeflowPath(command.value)
                / "**"
                / settings.tcf.with_suffix(".tpc").name,
                CmeflowPath(command.value)
                / "**"
                / settings.tcf.with_suffix(".gpkg").name,
            ]
            for i, sg in enumerate(search_globs[:]):
                if sg.root:
                    if sg.drive != settings.control_file.drive:
                        ...
                        search_globs[i] = os.path.relpath(
                            sg, settings.control_file.parent
                        )
            for glob in search_globs:
                pattern = globify(glob, wildcards)
                if str(Path(pattern).with_suffix(""))[-1] != "*":
                    if Path(pattern).suffix == ".gpkg":
                        pattern = "{0}*_swmm_ts{1}".format(
                            Path(pattern).with_suffix(""), Path(pattern).suffix
                        )
                        re_name_ = r"{0}_swmm_ts\{1}".format(
                            re_name, Path(pattern).suffix
                        )
                    else:
                        pattern = "{0}*{1}".format(
                            Path(pattern).with_suffix(""), Path(pattern).suffix
            ...
            for oz in settings.output_zones:
                tcf = CmeflowPath("{0}_{{{1}}}".format(settings.tcf.stem, oz))
                parent = settings.control_file.parent
                search_globs = [
                    CmeflowPath(command.value) / "**" / tcf.with_suffix(".xmdf").name,
                    CmeflowPath(command.value) / "**" / tcf.with_suffix(".tpc").name,
                ]
                for i, sg in enumerate(search_globs[:]):
                    if sg.root:
...
def LoadRasterMessageBox(parent, title, text):
    ...
    pb_invisible.clicked.connect(
        lambda: LoadRasterMessageBox_signal(dialog, d[pb_invisible])
    )
...
class LoadTcfOptions:
    def __init__(self):
        self.grouped = QSettings().value("coastalme/load_tcf_options/grouped", "true")
        if isinstance(self.grouped, str):
            self.grouped = True if self.grouped.lower() == "true" else False

        self.load_raster_method = QSettings().value(
            "coastalme/load_tcf_options/load_raster_method", "yes"
        )
        self.order_method = QSettings().value(
            "coastalme/load_tcf_options/order_method", "alphabetical"
        )
...
def loadTCFOptionsMessageBox_signal_rbgroup1(rb_group):
    ...
    QSettings().setValue(
        "coastalme/load_tcf_options/grouped",
        checked_button.text() == "Group by control file",
    )
...
def loadTCFOptionsMessageBox_signal_rbgroup2(rb_group):
    ...
    if checked_button.text() == "Load Normally":
        QSettings().setValue("coastalme/load_tcf_options/load_raster_method", "yes")
    elif checked_button.text() == "Do Not Load":
        QSettings().setValue("coastalme/load_tcf_options/load_raster_method", "no")
    else:
        QSettings().setValue(
            "coastalme/load_tcf_options/load_raster_method", "invisible"
        )
...
def loadTCFOptionsMessageBox_signal_rbgroup3(rb_group):
    ...
    if checked_button.text() == "Control File":
        QSettings().setValue("coastalme/load_tcf_options/order_method", "control_file")
    elif checked_button.text() == "Control File (Group Rasters)":
        QSettings().setValue(
            "coastalme/load_tcf_options/order_method", "control_file_group_rasters"
        )
    else:
        QSettings().setValue("coastalme/load_tcf_options/order_method", "alphabetical")
...
def LoadTCFOptionsMessageBox(parent, title):
    ...
    image = QLabel()
    image.setPixmap(
        QPixmap(":/icons/icons/question.svg").scaled(35, 35, QT_KEEP_ASPECT_RATIO)
    )
    label = QLabel()
    ...
    def dimensions(self):
        if self.netcdf_lib == "pylib":
            ...
            for i in range(ndims_p.contents.value):
                cstr_array = (ctypes.c_char * 256)()
                cint_p = ctypes.pointer(ctypes.c_int())
                err = self.ncdll.nc_inq_dim(
                    self.ncid, ctypes.c_int(i), ctypes.byref(cstr_array), cint_p
                )
                if err:
                    raise IOError(
                        "Error reading NetCDF file: dimension id {0}".format(i)
                    )
    ...
    def variables(self):
        if self.netcdf_lib == "pylib":
            ...
            for i in range(nvars_p.contents.value):
                cstr_array = (ctypes.c_char * 256)()
                err = self.ncdll.nc_inq_varname(
                    self.ncid, ctypes.c_int(i), ctypes.byref(cstr_array)
                )

                type_p = ctypes.pointer(ctypes.c_int())
                ndims_p = ctypes.pointer(ctypes.c_int())
                err = self.ncdll.nc_inq_var(
                    self.ncid,
                    ctypes.c_int(i),
                    ctypes.c_int(0),
                    type_p,
                ...
                if err:
                    raise IOError(
                        "Error reading NetCDF file: variable id {0}".format(i)
                    )
                ...
                for j in range(natts_p.contents.value):
                    err = self.ncdll.nc_inq_attname(
                        self.ncid, ctypes.c_int(i), ctypes.c_int(j), cstr_array
                    )
                    att_name = cstr_array.value.decode("utf-8")
                    att_type_p = ctypes.pointer(ctypes.c_int())
                    len_p = ctypes.pointer(ctypes.c_int())
                    err = self.ncdll.nc_inq_att(
                        self.ncid, ctypes.c_int(i), cstr_array, att_type_p, len_p
                    )

                    if att_type_p.contents.value == 1:
                        att_val = ctypes.pointer(
                            (ctypes.c_byte * len_p.contents.value)()
                        )
                    elif att_type_p.contents.value == 2:
                        att_val = ctypes.pointer(
                            (ctypes.c_char * len_p.contents.value)()
                        )
                    elif att_type_p.contents.value == 3:
                        att_val = ctypes.pointer(
                            (ctypes.c_short * len_p.contents.value)()
                        )
                    elif att_type_p.contents.value == 4:
                        att_val = ctypes.pointer(
                            (ctypes.c_int * len_p.contents.value)()
                        )
                    elif att_type_p.contents.value == 5:
                        att_val = ctypes.pointer(
                            (ctypes.c_float * len_p.contents.value)()
                        )
                    elif att_type_p.contents.value == 6:
                        att_val = ctypes.pointer(
                            (ctypes.c_double * len_p.contents.value)()
                        )
                    elif att_type_p.contents.value == 7:
                        att_val = ctypes.pointer(
                            (ctypes.c_ubyte * len_p.contents.value)()
                        )
                    elif att_type_p.contents.value == 8:
                        att_val = ctypes.pointer(
                            (ctypes.c_ushort * len_p.contents.value)()
                        )
                    elif att_type_p.contents.value == 9:
                        att_val = ctypes.pointer(
                            (ctypes.c_uint * len_p.contents.value)()
                        )
                    elif att_type_p.contents.value == 10:
                        att_val = ctypes.pointer(
                            (ctypes.c_longlong * len_p.contents.value)()
                        )
                    elif att_type_p.contents.value == 11:
                        att_val = ctypes.pointer(
                            (ctypes.c_ulonglong * len_p.contents.value)()
                        )

                    err = self.ncdll.nc_get_att(
                        self.ncid, ctypes.c_int(i), cstr_array, att_val
                    )

                    if att_type_p.contents.value == 2:
                        att_val = att_val.contents.value.decode("utf-8")
                    elif (
                        att_type_p.contents.value == 5 or att_type_p.contents.value == 6
                    ):
                ...
                err = self.ncdll.nc_inq_var_fill(
                    self.ncid, ctypes.c_int(i), no_fill_p, ctypes.c_int(0)
                )
                if no_fill_p.contents.value != 1:
                    err = self.ncdll.nc_inq_var_fill(
                        self.ncid, ctypes.c_int(i), no_fill_p, fill_val
                    )
                ...
                self._var[name] = NetcdfVar(
                    name, dims, fill_val, atts, type_p.contents.value
                )
...
class LayerHelper:
    ...
    def __new__(cls, *args, **kwargs):
        if isinstance(args[0], QgsVectorLayer):
            ...
            raise TypeError(
                "LayerHelper must be initialised with a QgsVectorLayer or QgsRasterLayer"
            )
    ...
    @property
    def coastalme_path(self):
        if self.is_database:
            return "{0} >> {1}".format(self.datasource, self.layer_name)
        else:
            return self.datasource

    def is_coastalme_type(self, version):
        return False
...
class RasterLayerHelper(LayerHelper):
    ...
    def is_database(self):
        return (
            Path(self.datasource).suffix.lower() == ".gpkg"
            or Path(self.datasource).suffix.lower() == ".nc"
        )
    ...
    def is_coastalme_type(self, version):
        if version >= 20230100:
            return Path(self.datasource).suffix.lower() in [
                ".asc",
                ".dem",
                ".txt",
                ".flt",
...
def download_latest_dev_plugin(iface=None):
    ...
    download_path = browse(
        ...
        "ZIP (*.zip *.ZIP)",
        default_filename="coastalme_plugin.zip",
    )
    if not download_path:
        return

    url = "https://downloads.coastalme.com/Private_Download/QGIS_COASTALME_Plugin/coastalme_plugin.zip"
    try:
        downloader = DownloadBinPackage(
            url, download_path, "Downloading coastalme_plugin.zip. . . "
        )
        downloader.start()
        downloader.wait()
    except Exception as e:
        QMessageBox.critical(
            parent,
            "Download Error",
            "Error downloading latest dev plugin: {0}".format(e),
        )
        return
    ...
    completed_text = (
        "Download complete:<p>{0}"
        "<p><p>Please see the following wiki page for instructions on how to install:"
        '<p><a href="https://wiki.coastalme.com/index.php?title=Installing_the_Latest_Development_Version_of_the_COASTALME_Plugin#How_to_install_the_Development_Plugin_Version">'
        "How to manually install the COASTALME Plugin</a>".format(download_path)
    )
...
def get_driver_by_extension(driver_type, ext):
    ...
    for i in range(gdal.GetDriverCount()):
        ...
        if ("DCAP_RASTER" in md and driver_type == "raster") or (
            "DCAP_VECTOR" in md and driver_type == "vector"
        ):
...
def kart_gpkg(kart_repo):
    ...
    try:
        ...
        working_copy_settings = zip(
            working_copy.split("=")[::2], working_copy.split("=")[1::2]
        )
...
class ImportEmpty:
    ...
    def _empty_file(self, empty_type, geom_ext):
        for file in Path(self.empty_dir).glob(
            "{0}_empty*".format(empty_type, geom_ext)
        ):
    ...
    def add(self, empty_type, geom_ext):
        ...
        out_file = self._out_file(
            empty_type,
            geom_ext,
            Path(empty_file).suffix,
            self.database_option,
            self.database,
        )
        db, lyr_name = out_file.split(" >> ")
        empty_file_data = EmptyFileData(
            lyr_name, db, empty_file, suffix_2_geom_type(geom_ext)
        )
    ...
    def import_to_kart(self, kart_exe, kart_repo):
        ...
        for empty_file in self.empty_files:
            ...
            driver_name = get_driver_by_extension(
                "vector", Path(empty_file.empty_file).suffix
            )
            if not driver_name:
            ...
            proc = subprocess.run(
                [kart_exe, "import", "{0}".format(tmp_out), empty_file.out_name],
                cwd=wkdir,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
            )
...
def goto_plugin_help():
    url = "https://wiki.coastalme.com/COASTALME_QGIS_Plugin"
    webbrowser.open(url)


def goto_plugin_changelog():
    url = "https://docs.coastalme.com/qgis-coastalme-plugin/changelog/"
    webbrowser.open(url)


def goto_coastalme_downloads():
    url = "https://coastalme.com/downloads/"
    webbrowser.open(url)


def process_cme_out(ini_file=None):
    ...
    if not ini_file:
        ini_file = r"/Users/wilfchun/Documents/GitHub/CoastalMe/coastalme/in/test_suite/Happisburgh/cme.ini"
    ...
    try:
        ...
        print(W)
        cme.return_rescue()
    nc_path = f"{cme.out_path}/all_vars.nc"
    return nc_path


def load_netcdf(file_path, layer_name=None, crs=None):
    from qgis.core import (
        QgsMeshLayer,
        QgsProject,
    ...
    """
    Advanced NetCDF mesh loading with comprehensive error handling
    
    Args:
        file_path (str): Path to NetCDF file
        layer_name (str, optional): Custom layer name
        crs (QgsCoordinateReferenceSystem, optional): Coordinate reference system
    
    Returns:
        tuple: (QgsMeshLayer or None, str status_message)
    """
    ...
    try:
        # Create layer options if CRS handling needed
        options = QgsMeshLayer.LayerOptions()
        if crs:
            options.skipCrsValidation = False

        # Create mesh layer
        mesh_layer = QgsMeshLayer(file_path, layer_name, "mdal", options)

        # Validate layer
        if not mesh_layer.isValid():
            error_msg = "Invalid mesh layer - possible causes:"
            error_msg += "\n- MDAL provider not available"
            error_msg += "\n- Unsupported NetCDF format"
            error_msg += "\n- Corrupted file"
            error_msg += "\n- Missing spatial/mesh structure"
        ...
        QgsMessageLog.logMessage(
            f"Successfully loaded mesh layer: {layer_name}", "Mesh Plugin", Qgis.Info
        )

        return mesh_layer, "Success"

    except Exception as e:
        error_msg = f"Exception loading mesh layer: {str(e)}"
        QgsMessageLog.logMessage(error_msg, "Mesh Plugin", Qgis.Critical)
        return None, error_msg
...
if __name__ == "__main__":
    a = float("{0:.6f}".format(2.0 / 60.0 / 60.0))
