...
from coastalme.compatibility_routines import (QT_DOUBLE, QT_FLOAT, QT_LONG_LONG, QT_LONG, QT_STYLE_DOTTED_PEN,
...
def get_latest_dev_plugin_version():
    try:
        ...
        downloader = Downloader('https://downloads.coastalme.com/Private_Download/QGIS_COASTALME_Plugin/VERSION')
        downloader.download()
    ...
    class AboutDialog(QDialog, Ui_AboutDialog):
        def __init__(self, iface, plugin_version, python_dependencies):
            ...
            self.textEdit.setText('{0}\t: {1}\n'
                                  ...
                                  '{4}\t\t: {5}\n\n'
                                  'Python Libraries\n{8}'
                                  .format("COASTALME Plugin Version", plugin_version, "QGIS Version", Qgis.QGIS_VERSION,
...
def coastalmeqgis_create_cme_dir(dialog, crs, basepath, engine, tutorial, gisFormat='SHP'):
    if crs is None:
    ...
    if QFile(prjname).exists():
        ...
        reply = QMessageBox.question(dialog, "Create COASTALME Empty Files", "Projection File Already Exists\n"
...
def coastalmeqgis_import_empty_cme(qgis, basepath, runID, empty_types, points, lines, regions, dialog,
...
def coastalmeqgis_get_selected_IDs(qgis, layer):
    QMessageBox.information(qgis.mainWindow(), "Info", "Entering coastalmeqgis_get_selected_IDs")
    IDs = []
    if (layer == None) and (layer.type() != QgsMapLayer.VectorLayer):
...
class RunCmeflow(QObject):
    ...
    def run(self):
        ...
        if not os.path.exists(self.runfile):
            self.error.emit("COASTALME Control File (tcf, fvc) not found: {0}".format(self.runfile))
            return
        ...
        try:
            ...
            self.error.emit("Unexpected error occurred starting COASTALME\nargs: {0}".format(self.args))
            self.destroy()
...
def coastalmeqgis_import_check_cme(qgis, basepath, runID, showchecks):
    ...
    for chk in check_files:
        ...
        if renderer:  # if the file requires a attribute based rendered (e.g. BC_Name for a _sac_check_R)
            ...
            error, message, slyr = cme_styles.Find(fname, layer)  # use coastalme styles to find longest matching
            if error:
                QMessageBox.critical(qgis.mainWindow(), "ERROR", message)
                return message
...
def coastalmeqgis_apply_check_cme(qgis):
    ...
    for layer_name, layer in QgsProject.instance().mapLayers().items():
        if layer.type() == QgsMapLayer.VectorLayer:
            ...
            error, message, slyr = cme_styles.Find(layer_fname, layer)  # use coastalme styles to find longest matching
            if error:
                return error, message
...
def coastalmeqgis_apply_check_cme_clayer(qgis, **kwargs):
    ...
    if cLayer is not None and cLayer.type() == QgsMapLayer.VectorLayer:
        ...
        error, message, slyr = cme_styles.Find(layer_fname, cLayer)  # use coastalme styles to find longest matching
        if re.findall(r'((_FM_PLOT_[PLR])|_raw_(nodes|links)_)', layer_fname, flags=re.IGNORECASE):
            slyr = False  # flood modeller
        if error:
            return error, message
...
def coastalmeqgis_insert_cme_attributes(qgis, inputLayer, basedir, runID, template, lenFields, dialog, output_db):
    message = None
...
def getScenariosFromTCF_v2(control_file, scenarios, events, settings=None):
    from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.parser import get_commands
    from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.settings import ConvertSettings
    from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.file import CmeflowPath
...
class LoadGisFiles(QObject):
    ...
    def loadGisFiles(self):
        from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.gis import (ogr_iter_geom, get_database_name,
                                                                                     ogr_format,
                                                                                     ogr_format_2_ext,
                                                                                     ogr_geometry_name,
                                                                                     ogr_basic_geom_type,
...
class LoadGisFromControlFile(QObject):
    ...
    def loadGisFromControlFile_v2(self):
        from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.control_file import get_commands
        from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.settings import ConvertSettings
        from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.file import CmeflowPath
                ...
                if command.in_scenario_block() and not command.in_scenario_block(settings_.scenarios):
                    ...
                    if not os.path.exists(output_path):
                        try:
                            ...
                            output = processing.run("COASTALME:COASTALMEConvertSWMMinpToGpkg", inputs)
                        except:
                            pass
                        finally:
                            if not os.path.exists(output_path):
...
def openGisFromTcf(tcf, iface, scenarios=(), load_rasters=True):
    ...
    if couldNotReadFile:
        ...
        QMessageBox.information(iface.mainWindow(), "Message", "Successfully Loaded All COASTALME Layers")
...
def downloadUtility(utility, parent_widget=None):
    ...
    try:
        ...
        qApp.restoreOverrideCursor()
        raise Exception('Download of {0} failed. Please try again or contact support@coastalme.com for '
...
def browse(parent: QWidget = None, browseType: str = '', key: str = "COASTALME",
           dialogName: str = "COASTALME", fileType: str = "ALL (*)",
...
def resetQgisSettings(e: QEvent = False, scope: str = 'Global', **kwargs) -> None:
    ...
    if 'feedback' in kwargs:
        if not kwargs['feedback']:
            return
    QMessageBox.information(None, "COASTALME Clear Settings", "Successfully Cleared {0} Settings".format(scope))
                            ...
                            if lyr_type == 'nc_grid':
                                for layer_ in QgsProject.instance().mapLayers().values():
                                    if layer_ in cmeview.tuResults.tuResultsNcGrid.copied_results:
                                        tmp_res_ = cmeview.tuResults.tuResultsNcGrid.copied_results[layer_]
                                    else:
                                        continue
                                    if tmp_res_ == tmp_res:
                                        if layer_ != layer:
                ...
                for layer_ in QgsProject.instance().mapLayers().values():
                    ...
                    if tmp_res_ == tmp_res:
                        if layer_ != layer:
                            layer_.reload()
                        cmeview.tuResults.tuResultsNcGrid.layerReloaded(layer_)
                cmeview.tuResults.tuResultsNcGrid.layerReloaded(None)  # this triggers the timer to start
...
def getOutputDirs_old(path):
    try:
        from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.file import globify
    except ImportError:
...
def getOutputDirs(path, settings=None):
    from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.control_file import get_commands
    from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.settings import ConvertSettings
    from .convert_coastalme_model_gis_format.conv_cme_gis_format.helpers.file import globify, CmeflowPath
    ...
    for command in get_commands(path, settings):
        ...
        if command.command == 'OUTPUT FOLDER':
            ...
            search_globs = [CmeflowPath(command.value) / '**' / settings.tcf.with_suffix('.xmdf').name,
                            CmeflowPath(command.value) / '**' / settings.tcf.with_suffix('.tpc').name,
                            CmeflowPath(command.value) / '**' / settings.tcf.with_suffix('.gpkg').name]
            ...
            for oz in settings.output_zones:
                ...
                parent = settings.control_file.parent
                search_globs = [CmeflowPath(command.value) / '**' / tcf.with_suffix('.xmdf').name,
...
class LoadTcfOptions:
    ...
    def __init__(self):
        ...
        self.load_raster_method = QSettings().value('coastalme/load_tcf_options/load_raster_method', 'yes')
        self.order_method = QSettings().value('coastalme/load_tcf_options/order_method', 'alphabetical')
...
def loadTCFOptionsMessageBox_signal_rbgroup1(rb_group):
    ...
    QSettings().setValue('coastalme/load_tcf_options/grouped', checked_button.text() == 'Group by control file')
...
def loadTCFOptionsMessageBox_signal_rbgroup2(rb_group):
    ...
    if checked_button.text() == 'Load Normally':
        ...
        QSettings().setValue('coastalme/load_tcf_options/load_raster_method', 'invisible')
...
def loadTCFOptionsMessageBox_signal_rbgroup3(rb_group):
    ...
    if checked_button.text() == 'Control File':
        ...
        QSettings().setValue('coastalme/load_tcf_options/order_method', 'control_file_group_rasters')
    else:
...
def download_latest_dev_plugin(iface=None):
    ...
    download_path = browse(parent, 'output file', "COASTALME/download_location", 'Enter name of the file to save to...',

    'ZIP (*.zip *.ZIP)', default_filename = 'coastalme_plugin.zip')
    if not download_path:
        return

    url = 'https://downloads.coastalme.com/Private_Download/QGIS_COASTALME_Plugin/coastalme_plugin.zip'
    try:
        downloader = DownloadBinPackage(url, download_path, 'Downloading coastalme_plugin.zip. . . ')
        downloader.start()
        downloader.wait()
    except Exception as e:
