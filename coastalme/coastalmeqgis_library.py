...
def coastalmeqgis_create_tf_dir(dialog, crs, basepath, engine, tutorial, gisFormat='SHP'):
    if crs is None:
...
def coastalmeqgis_import_empty_tf(qgis, basepath, runID, empty_types, points, lines, regions, dialog,
...
class RunTuflow(QObject):
    ...
    def __init__(self, tfexe, runfile, capture_output, callback):
        ...
        self.isfv = Path(tfexe).stem.lower() == 'coastalmefv'
    ...
    def run(self):
        if not os.path.exists(self.tfexe):
            self.error.emit("COASTALME exe not found: {0}".format(self.tfexe))
            return
...
def run_coastalme(qgis, tfexe, runfile):
...
# def config_set(project, coastalme_folder, tfexe, projection):
#     message = None
#     try:
#         f = open(config_file, 'w')
#         f.write('COASTALME Folder ==' + coastalme_folder + '\n')
#         f.write('COASTALME Exe ==' + tfexe + '\n')
...
def load_project(project):
    message = None
    try:
        tffolder = project.readEntry("configure_coastalme", "folder", "Not yet set")[0]
    except:
        message = "Error - Reading from project file."
        QMessageBox.information(qgis.mainWindow(), "Information", message)

    try:
        tfexe = project.readEntry("configure_coastalme", "exe", "Not yet set")[0]
    except:
        message = "Error - Reading from project file."
        QMessageBox.information(qgis.mainWindow(), "Information", message)

    try:
        tf_prj = project.readEntry("configure_coastalme", "projection", "Undefined")[0]
    except:
        message = "Error - Reading from project file."
        QMessageBox.information(qgis.mainWindow(), "Information", message)
...
#  coastalmeqgis_import_check_tf added MJS 11/02
def coastalmeqgis_import_check_tf(qgis, basepath, runID, showchecks):
    ...
    for chk in check_files:
        ...
        if renderer:  # if the file requires a attribute based rendered (e.g. BC_Name for a _sac_check_R)
            ...
            error, message, slyr = tf_styles.Find(fname, layer)  # use coastalme styles to find longest matching
            if error:
                QMessageBox.critical(qgis.mainWindow(), "ERROR", message)
                return message
...
def coastalmeqgis_apply_check_tf(qgis):
    ...
    for layer_name, layer in QgsProject.instance().mapLayers().items():
        if layer.type() == QgsMapLayer.VectorLayer:
            ...
            error, message, slyr = tf_styles.Find(layer_fname, layer)  # use coastalme styles to find longest matching
            if error:
                return error, message
...
def coastalmeqgis_apply_check_tf_clayer(qgis, **kwargs):
    error = False
    message = None
    try:
    ...
    if cLayer is not None and cLayer.type() == QgsMapLayer.VectorLayer:
        ...
        error, message, slyr = tf_styles.Find(layer_fname, cLayer)  # use coastalme styles to find longest matching
        if re.findall(r'((_FM_PLOT_[PLR])|_raw_(nodes|links)_)', layer_fname, flags=re.IGNORECASE):
            slyr = False  # flood modeller
        if error:
            return error, message
...
def coastalmeqgis_insert_tf_attributes(qgis, inputLayer, basedir, runID, template, lenFields, dialog, output_db):
    message = None
...
def getScenariosFromTCF_v2(control_file, scenarios, events, settings=None):
    from .convert_coastalme_model_gis_format.conv_tf_gis_format.helpers.parser import get_commands
    from .convert_coastalme_model_gis_format.conv_tf_gis_format.helpers.settings import ConvertSettings
    from .convert_coastalme_model_gis_format.conv_tf_gis_format.helpers.file import TuflowPath
...
class LoadGisFiles(QObject):
    ...
    def loadLayersIntoWorkspace(self, root, layers, options=None, insert_before=None):
        ...
        for lyr in layers:
            ...
            coastalmeqgis_apply_check_tf_clayer(None, layer=maplyr)
    ...
    def loadGisFiles(self):
        from .convert_coastalme_model_gis_format.conv_tf_gis_format.helpers.gis import (ogr_iter_geom, get_database_name,
                                                                                     ogr_format,
                                                                                     ogr_format_2_ext,
                                                                                     ogr_geometry_name,
                                                                                     ogr_basic_geom_type,
...
class LoadGisFromControlFile(QObject):
    ...
    def loadGisFromControlFile_v2(self):
        from .convert_coastalme_model_gis_format.conv_tf_gis_format.helpers.control_file import get_commands
        from .convert_coastalme_model_gis_format.conv_tf_gis_format.helpers.settings import ConvertSettings
        from .convert_coastalme_model_gis_format.conv_tf_gis_format.helpers.file import TuflowPath
    ...
    if layer is not None:
        if isinstance(layer, QgsVectorLayer):
            ...
            if plugins['coastalme'].resultsPlottingDockOpened:
                tuview = plugins['coastalme'].resultsPlottingDock
...
def getOutputDirs_old(path):
    try:
        from .convert_coastalme_model_gis_format.conv_tf_gis_format.helpers.file import globify
    except ImportError:
...
def getOutputDirs(path, settings=None):
    from .convert_coastalme_model_gis_format.conv_tf_gis_format.helpers.control_file import get_commands
    from .convert_coastalme_model_gis_format.conv_tf_gis_format.helpers.settings import ConvertSettings
    from .convert_coastalme_model_gis_format.conv_tf_gis_format.helpers.file import globify, TuflowPath
