...
def isFvFile(inFile):
    """Checks if file is a COASTALME FV netCDF4 file."""
    ...
    try:
        ...
        if nc.getncattr('Origin') == 'Created by COASTALMEFV':
            isValid = True
...
def fvMeshRead(inFile):
    """Reads a COASTALME FV mesh from netCDF4 file. Returns (x, y, z, faces)."""

    # check if file is COASTALME FV result file
    if not isFvFile(inFile):
        raise ValueError('File must be COASTALME FV netCDF4 file')
...
def fvMeshWrite(outFile, x, y, z, faces):
    """Writes a mesh to a COASTALME FV netCDF4 file"""
    ...
    attributes = \
        {
            'Origin': 'Created by COASTALMEFV',
            'Type': 'Cell-centred COASTALMEFV output',
        }
...
def isUGridFile(inFile):
    """Checks if file is a COASTALME FV netCDF4 output file."""
...
def meshRead(inFile):
    """Reads mesh of either XMS, COASTALME FV or UGRID format as (x, y, z, faces)"""
    ...
    if isXmsFile(inFile):
        ...
        raise ValueError('Invalid mesh file. File must be XMS TIN, COASTALME FV or UGRID netCDF4')
