...
# PyQt5 imports
from qgis.PyQt.QtWidgets import QComboBox, QFileDialog, QDockWidget, QGroupBox, QDateEdit, QMessageBox, \
                            QLabel, QPushButton, QLineEdit, QWidget, QVBoxLayout, QHBoxLayout, QListWidget, \
                            QListWidgetItem, QDialog, QCheckBox
from qgis.PyQt.QtCore import Qt, QThread, QObject, pyqtSignal, QMimeData, QSettings
from qgis.PyQt.QtGui import QIcon

from .qt_compat import is_qt6, QT_ITEM_DATA_USER_ROLE, QT_ALIGN_LEFT, QT_ALIGN_CENTER, QT_DOCK_WIDGET_AREA_RIGHT
...
class CheckboxInputUI:
    ...
    def __build_ui__(self):
        ...
        #self.labelWidget = buildLabelWidget('', addTo=self.baseLayout,
        #                                    alignment=QT_ALIGN_LEFT)

        # create checkbox widget
        self.checkBoxWidget = QCheckBox()
        self.checkBoxWidget.setText(self._label)
        self.baseLayout.addWidget(self.checkBoxWidget)

    def getInput(self):
        return self.checkBoxWidget.isChecked()
...
class DateInputUI:
    ...
    def __build_ui__(self):
        ...
        self.labelWidget = buildLabelWidget(label=self._label, addTo=self.baseLayout,
                                            width=60, alignment=QT_ALIGN_CENTER)

        # create date edit widget
        self.dateEditWidget = QDateEdit()
        self.dateEditWidget.setDisplayFormat('dd/MM/yyyy')
...
class FolderInputUI:
    ...
    def __build_ui__(self):
        ...
        # create and format label widget
        self.labelWidget = buildLabelWidget(label=self._label, addTo=self.baseLayout,
                                            width=120, alignment=QT_ALIGN_LEFT)
...
class OpenFileInputUI:
    ...
    def __build_ui__(self):
        ...
        # create and format label widget
        self.labelWidget = buildLabelWidget(label=self._label, addTo=self.baseLayout,
                                            width=160, alignment=QT_ALIGN_LEFT)
...
class SaveFileInputUI:
    ...
    def setWithDialog(self):
        ...
        file = QFileDialog.getSaveFileName(directory=directory, filter=self._filter)[0]
        newDirectory = os.path.split(file)[0]
...
class LayersInputUI:
    ...
    def addCurrentLayer(self):
        ...
        item.setData(QT_ITEM_DATA_USER_ROLE, layer)
    ...
    def getInput(self):
        ...
        return [self.listWidget.item(ii).data(QT_ITEM_DATA_USER_ROLE) for ii in range(self.listWidget.count())]
    ...
    def checkBuild(self):
        ...
        if self.gridLayerUI.getInput() is not None:
            ...
            for ii in range(ogr.GetDriverCount()):
                ...
                try:  # later GDAL versions will raise an Exception
...
class ConvertMat2NcUI:
    ...
    def __build_ui__(self):
        ...
        # add rotated and spherical check boxes
        self.isRotated = CheckboxInputUI('MAT is Rotated?')
        self.baseLayout.addWidget(self.isRotated.baseWidget)

        self.isNonSpherical = CheckboxInputUI('MAT is Non-Spherical?')
        self.baseLayout.addWidget(self.isNonSpherical.baseWidget)
    ...
    def convertMat2Nc(self):
        ...
        errmsg = convertMat2Nc(matFile, ncFile, self.isRotated.getInput(), self.isNonSpherical.getInput())
        if errmsg is not None:
            QMessageBox.warning(self.baseWidget, "Convert MatLab to NetCDF", errmsg)
