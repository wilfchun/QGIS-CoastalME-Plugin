...
from coastalme.coastalme_swmm.version import __version__ as coastalme_swmm_version

all_sections = swmm_sections.swmm_section_definitions()


def is_coastalme_swmm_file(filename):
    layers = get_gis_layers(filename)
    return 'COASTALME_SWMM_VERSION' in layers


def write_coastalme_version(filename):
    df_file_version = pd.DataFrame(
        {'version': [coastalme_swmm_version]}
    )
