...
from coastalme.coastalme_swmm.create_bc_connections_gpd import create_bc_connections_gpd
from coastalme.coastalme_swmm.create_swmm_section_gpkg import create_section_gdf
from coastalme.coastalme_swmm.estry_to_swmm_model import create_curves_from_dfs
from coastalme.coastalme_swmm.fix_multi_link_oulets import extend_multi_link_outfalls_gdf
from coastalme.coastalme_swmm.gis_messages import GisMessages
from coastalme.coastalme_swmm.gis_to_swmm import gis_to_swmm
from coastalme.coastalme_swmm.junctions_downstream_to_outfalls import downstream_junctions_to_outfalls
from coastalme.coastalme_swmm.layer_util import read_and_concat_files_layers
from coastalme.coastalme_swmm.swmm_sanitize import sanitize_name
from coastalme.coastalme_swmm.xpswmm_node2d_convert import xpswmm_2d_capture_to_swmm_gpd
import coastalme.coastalme_swmm.swmm_io as swmm_io
from coastalme.coastalme_swmm.xpswmm_xpx_to_gpkg_tables import get_nearest_height_for_horzellipse, get_arch_width
from coastalme.coastalme_swmm.fix_invalid_bc_connections import fix_invalid_bc_connections_gdf
...
def xpx_to_gpkg(xpx_filename,
    ...
    # Linked nodes SFLOOD=3 links the node spill crest to 2D. COASTALME-SWMM requires this to use an inlet
    ...
    if len(linked_sflood3_noinlet) > 0:
        if 'On' in global_2dlink_settings and global_2dlink_settings['On']:
            ...
            feedback.reportError('Nodes connected to 2D using the spill crest elevation without an inlet exist. These '
                                 'must converted to use inlets for COASTALME-SWMM. See the messages file.')
    ...
    if gdf_bc_conn is not None:
        ...
        # test_gpkg_filename = Path(r'C:\COASTALME\Dev\COASTALME\qgis_plugin\coastalme\test\swmm\input') / 'xpx_create_bc_connections.gpkg'
    ...
    swmm_io.write_coastalme_version(gpkg_filename)
