...
2. Creates the COASTALME-SWMM control file ..\\model\\swmm folder and fills it in
3. Modifies the TCF file (leaves copy of original)
    a. Adds tscf, start/end time, and other stuff
    b. Comment out commands not used for COASTALME-SWMM or no longer recommended
...
from coastalme.coastalme_swmm.layer_util import increment_layer
from coastalme.coastalme_swmm.swmm_processing_feedback import ScreenProcessingFeedback
from coastalme.coastalme_swmm.xpswmm_gis_cleanup import bc_layer_processing
...
def get_bc_filenames_and_paths(tcf_path, feedback):
    ...
    if not tbc_path.exists():
        feedback.reportError(f'Unable to find COASTALME BC Control file. Make sure the file exists and is correctly '
...
def convert_xpswmm(output_folder_1donly: str,
    ...
    if tcf_filename is None:
        tcf_path = None
        coastalme_folder = None
        swmm_folder = Path(output_folder_1donly)
    else:
        tcf_path = Path(tcf_filename)
        coastalme_folder = Path(tcf_filename).parent.parent
        swmm_folder = coastalme_folder / 'model\\swmm'
    ...
    if coastalme_folder is not None:
        tscf_filename = coastalme_folder / f'model\\{swmm_prefix}_001.tscf'
        tef_filename = coastalme_folder / f'runs\\coastalme_events.tef'
