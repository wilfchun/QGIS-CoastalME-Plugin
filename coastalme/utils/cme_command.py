...
from coastalme.toc.toc import node_to_layer

def get_command_properties(name: str) -> typing.Dict[str, typing.Any]:
    empty_dir = plugins['coastalme'].plugin_dir / 'empty_tooltips'
    found = []
...
def try_find_control_file(file: Path, cf: str) -> Path:
    if 'coastalme control file' in cf.lower():
        ext = '.tcf'
        ...
        return
    cme_dir = find_parent_dir(file, 'coastalme')
    model_dir = find_parent_dir(file, 'model')
    if cme_dir and model_dir:
        if len(model_dir.parts) - len(cme_dir.parts) > 3 or len(cme_dir.parts) <= 2:
...
class CmeflowCommandMapinfo(CmeflowCommand):
    pass


class CmeflowCommandSHP(CmeflowCommand):
    ...
    def appendable(self, command: 'CmeflowCommand') -> bool:
    ...
    def command_iter(self) -> typing.Generator['CmeflowCommand', None, None]:
...
class CmeflowCommandGPKG(CmeflowCommandSHP):
...
def create_coastalme_command(type_: str = 'path') -> str:
    tree_view = plugins['coastalme'].iface.layerTreeView()
...
def create_coastalme_command_path(a: bool = False) -> None:
    command = create_coastalme_command()
    QApplication.clipboard().setText(command)


def create_coastalme_command_name(a: bool = False) -> None:
    command = create_coastalme_command('name')
    QApplication.clipboard().setText(command)
