...
from coastalme.coastalmeqgis_library import get_driver_by_extension
from .gdal_ import get_driver_name_from_extension
from osgeo import gdal, osr, ogr
from coastalme.coastalmeqgis_settings import TF_Settings


class ProjectConfig:
    """Class for setting up and reading COASTALME settings from a project (.QGZ or .JSON)."""
    ...
    def from_qgs_project() -> 'ProjectConfig':
        ...
        name = project.readEntry('coastalme', 'project/name', '')
        folder = project.readEntry('coastalme', 'project/folder', '')
        crs = project.readEntry('coastalme', 'project/crs', '')
        gis_format = project.readEntry('coastalme', 'project/gis_format', '')
        hpcexe = project.readEntry('coastalme', 'project/hpcexe', '')
        fvexe = project.readEntry('coastalme', 'project/fvexe', '')
        return ProjectConfig(
            name[0],
            folder[0],
    ...
    def write_qgs_project(self, param: str = None) -> None:
        ...
        project = QgsProject.instance()
        if param:
            project.writeEntry('coastalme', f'project/{param}', str(getattr(self, param)))
            return

        project.writeEntry('coastalme', 'project/name', str(self.name))
        project.writeEntry('coastalme', 'project/folder', str(self.folder))
        project.writeEntry('coastalme', 'project/crs', self.crs.authid())
        project.writeEntry('coastalme', 'project/gis_format', self.gis_format)
        project.writeEntry('coastalme', 'project/hpcexe', str(self.hpcexe) if self.run_hpc else '')
        project.writeEntry('coastalme', 'project/fvexe', str(self.fvexe) if self.run_fv else '')
    ...
    def write_old_project_settings(self) -> None:
        """Writes project settings to the old COASTALME settings class which is still used by some of the older tools."""
        cmesettings = TF_Settings()
        cmesettings.Load()
        cmesettings.project_settings.CRS_ID = self.crs.authid()
        cmesettings.project_settings.cme_exe = str(self.hpcexe) if self.run_hpc else ''
        cmesettings.project_settings.base_dir = str(self.folder)
        cmesettings.project_settings.engine = 'classic'
        cmesettings.project_settings.tutorial = False
    ...
    def write_json(self) -> None:
        ...
        json_path = self.folder / 'cmesettings.json'
        d = {
            'name': self.name,
    ...
    def save_settings(self) -> None:
        """Saves COASTALME project settings to QGS project and JSON file."""
        if not self.folder.exists():
    ...
    def create_folders(self) -> None:
        """Creates COASTALME and COASTALME FV folders."""
    ...
    def projection_command(self, fv: bool) -> str:
        """Returns the COASTALME projection command."""
        d = {
            'shp': 'SHP',
            'gpkg': 'GPKG',
            'mif': 'MI'
    ...
    def create_proj_gis_file(self, gis_folder: Path, fv: bool) -> None:
        """Creates the projection gis file for use in COASTALME for creating empties."""
    ...
    def create_tif_projection(self, grid_folder: Path) -> None:
        """Creates a template TIF projection file that COASTALME can use as a reference."""
    ...
    def create_hpc_empties(self) -> None:
        """Creates empty COASTALME HPC files."""
    ...
    def create_fv_empties(self) -> None:
        """Creates empty COASTALME FV files."""
    ...
    def _get_hidden_inputs(self) -> dict:
        """Hidden inputs are those that are not shown in the GUI but are used in the COASTALME control files.
        ...
        """
        f = Path(__file__).parents[1] / 'alg' / 'data' / 'create_coastalme_settings.json'
        with f.open() as fi:
            d = json.load(fi)
    ...
    def _get_command_validation(self) -> dict:
        """Returns the validation requirements for some of the commands in the COASTALME control files."""
        f = Path(__file__).parents[1] / 'alg' / 'data' / 'create_coastalme_settings.json'
        with f.open() as fi:
            d = json.load(fi)
        d = d.get('command settings', {})
    ...
    def _get_section_validation(self) -> dict:
        """Returns the validation requirements for some of the sections in the COASTALME control files."""
        f = Path(__file__).parents[1] / 'alg' / 'data' / 'create_coastalme_settings.json'
        with f.open() as fi:
            d = json.load(fi)
    ...
    def _domain_commands(self, domain_str: str) -> str:
        """Returns the domain commands for the COASTALME control files."""
    ...
    def _output_format_commands(self, fmts: dict):
        from coastalme.gui.widgets.output_format_widget import OUT_FMT
    ...
    def _setup_cf_templates(self, template_folder: Path) -> None:
        """Sets up COASTALME control file templates."""
    ...
    def _setup_hpc_templates(self, parent_folder: Path) -> None:
        """Sets up COASTALME HPC control file templates."""
    ...
    def _setup_fv_templates(self, parent_folder: Path) -> None:
        """Sets up COASTALME FV control file templates."""
