...
class TooltipBuilder:
    ...
    def links(self, wiki_link: str, manual_link: str, manual_page: int) -> str:
        ...
        html = '<ul>'
        if wiki_link:
            html = f'{html}<li><a href="{wiki_link}"><span style=" font-size:8pt;">Link to COASTALME Wiki</span></a><p>'
        if manual_link_:
            html = f'{html}<li><a href="{manual_link_}"><span style=" font-size:8pt;">Link to COASTALME Manual</span></a><p>'
        html = f'{html}</ul>'
        return html
