...
class SCS(QObject):
    """ Class for obtaining SCS data and processing into COASTALME.
    ...
    def run(self) -> None:
        ...
        self.scsStart.emit()
        self.processIntoCmeflow()

    def processIntoCmeflow(self) -> None:
        """ Process output data from SCS into COASTALME format. """
    ...
    def createTef(self) -> None:
        """ Create COASTALME Event File (TEF) based on the user's selected events. """
        ...
        try:
            with open(tef, 'w') as fo:
                fo.write('! TEF written by SCS to COASTALME QGIS Plugin tool\n\n')
    ...
    def createBcDbase(self) -> None:
        """ Create COASTALME bc_dbase.csv based on user inputs i.e. catchment name. """
