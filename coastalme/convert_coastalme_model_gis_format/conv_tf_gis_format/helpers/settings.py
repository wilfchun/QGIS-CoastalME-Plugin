...
class ConvertSettings:
    ...
    def _parse_args(self, *args):
        ...
        while args:
            if args[0].lower() == '-rf':
                ...
                if len(args) < 2:
                    raise FatalConvertException('COASTALME directory structure settings file must be specified after -tuf_dir_struct_settings argument')
    ...
    def _guess_root_folder(self):
        ...
        # see if there's a folder named COASTALME up to 5 folders above TCF
        if not isinstance(self.tcf, TuflowWindowsPath) and not isinstance(self.tcf, TuflowPosixPath):
            return

        root = self.tcf.find_parent('COASTALME', 5)
        if root:
            return root
        ...
        if runs and model and root:  # indicates a standard COASTALME model folder structure
    ...
    def _guess_empties_folder(self):
        ...
        from .file import change_gis_mi_folder, TuflowPath, coastalme_stnd_path

        # if using coastalme dir structure, location is pre-ordained
        if self.tuf_dir_struct:
            if self.output_gis_format == GIS_MIF:
                return self.output_folder / coastalme_stnd_path(None, 'mi/empty')
            else:
                return self.output_folder / coastalme_stnd_path(None, 'gis/empty')
    ...
    def read_tcf(self, scenarios=()):
        ...
        tcf_override = self.coastalme_override(self.tcf)
        if tcf_override:
    ...
    def coastalme_override(self, tcf):
        from .file import TuflowPath
        if TuflowPath(tcf).stem.lower().startswith('_coastalme_override'):
            return
        compname = platform.node()
        tcf_override = TuflowPath(tcf).parent / f'_COASTALME_Override_{compname}.tcf'
        if tcf_override.exists():
            return tcf_override
        tcf_override = TuflowPath(tcf).parent / f'_COASTALME_Override.tcf'
        if tcf_override.exists():
            return tcf_override

    def coastalme_override_iter(self, tcf):
        from .file import TuflowPath
        tcf_override = TuflowPath(tcf).parent / f'_COASTALME_Override.tcf'
        if tcf_override.exists():
            yield tcf_override
        for tcf_override in TuflowPath(tcf).parent.glob(f'_COASTALME_Override_*.tcf'):
            yield tcf_override
