...
def globify(infile, wildcards):
    """Converts COASTALME wildcards (variable names, scenario/event names) to '*' for glob pattern."""
...
def coastalmeify_value(value, settings, is_folder=False):
    """
        ...
        if '|' in value:
            ...
            for file in value.split('|'):
                ...
                if is_a_number:
                    ...
                    layers.append(coastalmeify_value(file, settings))
            return ' | '.join([str(x) for x in layers])
        else:
            return coastalmeify_value(value.strip(), settings)
...
def coastalme_stnd_path(inpath, key):
...
def replace_with_tuf_dir_struct(inpath, outparent, command, key):
    if command and command.is_bc_dbase_file():
        return (outparent / coastalme_stnd_path(inpath, 'bc_dbase') / inpath.name).resolve()
    elif command and command.is_read_grid() and command.is_read_xf():
        return (outparent / coastalme_stnd_path(inpath, 'grid/xf') / inpath.name).resolve()
    elif command and command.is_read_grid():
        return (outparent / coastalme_stnd_path(inpath, 'grid') / inpath.name).resolve()
    elif command and command.is_read_tin():
        return (outparent / coastalme_stnd_path(inpath, 'tin') / inpath.name).resolve()
    elif command and command.is_log_folder():
        return (outparent / coastalme_stnd_path(inpath, 'log') / inpath.name).resolve()
    elif command and command.is_output_folder():
        return (outparent / coastalme_stnd_path(inpath, 'results') / inpath.name).resolve()
    elif command and command.is_check_folder():
        return (outparent / coastalme_stnd_path(inpath, 'check') / inpath.name).resolve()
    elif command and command.is_rainfall_grid_nc():
        return (outparent / coastalme_stnd_path(inpath, 'bc_dbase') / inpath.name).resolve()
    elif command and command.is_read_gis() and command.is_read_xf():
        return (outparent / coastalme_stnd_path(inpath, 'gis/xf') / inpath.name).resolve()
    elif command and command.is_read_gis():
        return (outparent / coastalme_stnd_path(inpath, 'gis') / inpath.name).resolve()
    elif key:
        return (outparent / coastalme_stnd_path(inpath, key) / inpath.name).resolve()
    return (outparent / coastalme_stnd_path(inpath, None) / inpath.name).resolve()
...
class TuflowPath(Path):
    ...
    COASTALME wildcards/variables '<< >>'  that can appear in file path references with COASTALME control files,
    nor does it handle COASTALME GPKG style paths :code:`'database >> layername'` very well.
    ...
    def resolve(self, strict=...):
        """Override method so it will work with COASTALME style wildcards/variables."""
    ...
    def glob(self, pattern, wildcards=()):
        """Override method so can do some stuff with COASTALME style wildcards/variables and also GPKG databases."""
    ...
    def read_file_for_parent(self):
        """COASTALME control file specific method. Searches control file for file references to try and find
        the root model directory e.g. typically what is called the 'COASTALME' folder.
    ...
    def find_folder_in_path(self, folder_name):
        ...
        | e.g. C:/coastalme/path/gis/control_file.tcf
        | :code:`find_folder_in_path('gis')` returns C:/coastalme/path/gis
...
class TuflowWindowsPath(WindowsPath, TuflowPath):
    ...
    def find_parent(self, name, index_=None):
        ...
        if index_ is None:
            for i, part in enumerate(self.parts[:1:-1]):  # assume it would never sit right next to drive letter e.g. C:\COASTALME
    ...
    def find_parent(self, name, index_=None):
        ...
        if index_ is None:
            for i, part in enumerate(
                    self.parts[:1:-1]):  # assume it would never sit right next to drive letter e.g. C:\COASTALME
                if part.lower() == name.lower():
                    return TuflowPath('').joinpath(*self.parts[:-i])
        else:
            ind = -1 - index_
