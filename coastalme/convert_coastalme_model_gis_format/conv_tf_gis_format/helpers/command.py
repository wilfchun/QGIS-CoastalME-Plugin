...
from .file import (add_geom_suffix, remove_geom_suffix, change_gis_mi_folder, coastalmeify_value,
...
class Command:
    """Class for handling COASTALME command."""
    ...
    def __init__(self, line, settings):
        ...
        if self.is_read_gis() or (self.is_read_grid() and not self.is_rainfall_grid_nc()) or self.is_read_projection():
            self.value = coastalmeify_value(self.value_orig, settings)
            ...
            self.value_ = coastalmeify_value(self.value_orig, settings)
            ...
            self.value = TuflowPath(insert_variables(self.value_orig, settings.variables))
            self.value_ = coastalmeify_value(self.value_orig, settings, True)
        ...
        if self.is_control_file() and str(self.value).upper() == 'AUTO' or self.command == 'ESTRY CONTROL FILE AUTO':
            ...
            self.value_ = coastalmeify_value(str(self.value), settings)
    ...
    def count_geom(self, settings):
        ...
        for i, value in enumerate(str(self.value).split('|')):
            value = coastalmeify_value(value.strip(), settings)
    ...
    def iter_geom(self, settings):
        ...
        for value in str(self.value).split('|'):
            ...
            self.value = coastalmeify_value(value.strip(), settings)
    ...
    def iter_grid(self, settings):
        ...
        if '|' in str(self.value):
            ...
            self.value = coastalmeify_value(grid.strip(), settings)
            self.iter_geom_index += 1
            yield 'GRID'

            command_orig = self.command
            self.command = 'READ GIS'  # trick into thinking this is now a READ GIS
            self.value = coastalmeify_value(gis.strip(), settings)
    ...
    def is_event_file(self):
        """Return whether command is referencing the COASTALME EVENT FILE."""
