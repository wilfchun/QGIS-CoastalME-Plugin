...
from .file import (copy_file, coastalmeify_value,
                   globify, TuflowPath,
...
def convert_control_file(control_file, settings, command=None):
    """Convert a COASTALME Control file from one GIS and GRID format to another."""
    ...
    if control_file.suffix.upper() == '.TCF' and settings_copy.coastalme_override(control_file):
        for tcf_override in settings_copy.coastalme_override_iter(control_file):
        ...
        for command in get_commands(control_file, settings_copy):
            ...
            # READ GRID RF == CSV
                ...
                if not something_copied:
                    ...
                    #             copy_count += 1
                    #
                    #         src_file_grid = coastalmeify_value(src_file_grid, settings_copy)
                    #
                            # try:
            ...
            # LOG FOLDER == | OUTPUT FOLDER == | WRITE CHECK FILES  ==
                ...
                if settings.verbose == 'high':
                    if command.value_orig is not None:
                        print(f'COASTALME FOLDER COMMAND: {command.command_orig} == {command.value_orig}')
                    else:
                        print(f'COASTALME FOLDER COMMAND: {command.command_orig}')
            ...
            # CONTAINS COMMAND
                ...
                if settings.verbose == 'high':
                    if command.value_orig is not None:
                        print(f'COASTALME COMMAND: {command.command_orig} == {command.value_orig}')
                    else:
                        print(f'COASTALME COMMAND: {command.command_orig}')
