...
from ..coastalmeqgis_library import copyStyle
...
from ..coastalmeqgis_library import (getParabolicCoord, lineToPoints,
                                  getRasterValue, browse, is1dTable, is1dNetwork, getRightAngleLine,
                                  get_table_names)
from coastalme.toc.toc import coastalmeqgis_find_layer, findAllRasterLyrs
from ..COASTALME_XS import XS_Data
...
class ArchBridgeDock(QDockWidget, Ui_archBridgeDock):
    ...
    def __init__(self, iface: QgisInterface) -> None:
        ...
        # initialise settings
        # bridge
        bridge_save_setting = QSettings().value('coastalme/arch_bridge_editor/bridge_save_setting', 'csv_folder')
        bridge_save_path = QSettings().value('coastalme/arch_bridge_editor/bridge_save_path', '')
        ...
        # cross-section
        xs_save_setting = QSettings().value('coastalme/arch_bridge_editor/xs_save_setting', 'csv_folder')
        xs_save_path = QSettings().value('coastalme/arch_bridge_editor/xs_save_path', '')
    ...
    def check(self) -> None:
        ...
        # 1d_nwk is specified
        if not self.cboNwkLayer.currentText() or coastalmeqgis_find_layer(self.cboNwkLayer.currentText()) is None:
        ...
        lyr = coastalmeqgis_find_layer(self.cboNwkLayer.currentText())
        ...
        if self.twChannel.rowCount():
            ...
            if self.cboOutputType.currentIndex() == 1 and \
                    (not self.cboXsLayerOut.currentText() or coastalmeqgis_find_layer(self.cboXsLayerOut.currentText()) is None):
                QMessageBox.critical(self, 'Arch Bridge',
            ...
            if self.cboOutputType.currentIndex() == 2 and \
                    (not self.cboXsLayer.currentText() or coastalmeqgis_find_layer(self.cboXsLayer.currentText()) is None):
                QMessageBox.critical(self, 'Arch Bridge',
            ...
            if not self.leChannelName.text() and self.cboXsLayer.currentText() == '-None-':
                ...
                return
                    ...
                    coastalmeqgis_find_layer(self.cboXsLayer.currentText()) is None:
                QMessageBox.critical(self, 'Arch Bridge',
                                     'Cross-section name must be present in bridge editor or in cross-section layer')
                return
                ...
                xslyr = coastalmeqgis_find_layer(self.cboXsLayer.currentText())
    ...
    def run(self) -> None:
        """Creates COASTALME input layers for arch bridge."""
        # process bridge and 1d_nwk layer
        nwklyr = coastalmeqgis_find_layer(self.cboNwkLayer.currentText())
        lyr = coastalmeqgis_find_layer(self.cboNwkLayer.currentText())
        ...
        if field.length() < 5 and i_type == 1:
            ...
            try:
                ...
                QMessageBox.warning(self, 'Arch Bridge', 'Failed to refactor fields. Please take care using the '
                                                         'output in COASTALME as the type attribute will be cut short.')
                return
        ...
        if self.twChannel.rowCount():
            ...
            if self.cboOutputType.currentIndex() == 0:  # new layer
                ...
                xslyr = coastalmeqgis_find_layer(self.cboXsLayerOut.currentText())
    ...
    def startDrawing(self, dem: str) -> None:
        ...
        layer = coastalmeqgis_find_layer(dem)
    ...
    def importBridgeCurveFromEOF(self):
        ...
        eof = browse(self, 'existing file', 'coastalme/arch_bridge_editor_eof',
    ...
    def importFMDat(self):
        ...
        dat = browse(self, 'existing file', 'COASTALME/arch_bridge_editor_fmdat',
    ...
    def useSelection(self) -> None:
        ...
        if self.cboNwkLayer.currentIndex() > -1:
            layer = coastalmeqgis_find_layer(self.cboNwkLayer.currentText())
            if layer is not None:
                name = ''
    ...
    def populateFeatureCombobox(self, index: int = -1) -> None:
        ...
        layer = None
        if index > -1:
            layer = coastalmeqgis_find_layer(self.cboNwkLayer.itemText(index))
            if layer is None:
                return
            i_id = 0
        ...
        if processCrossSection:
            if self.editedXsLayerSignal is not None:
                for i in range(self.cboXsLayer.count()):
                    lyrXs = coastalmeqgis_find_layer(self.cboXsLayer.itemText(i))
                    if lyrXs is None:
                        continue
                    try:
            ...
            if self.cboXsLayer.currentText() and self.cboXsLayer.currentText() != '-None-':
                lyrXs = coastalmeqgis_find_layer(self.cboXsLayer.currentText())
                if lyrXs is None:
    ...
    def graphicToVector(self) -> None:
        ...
        vectorFile = browse(
            ...
            key='COASTALME/bridge_editor_graphic_to_vector',
    ...
    def save_bridge_output_setting_in_place(self, e=None):
        QSettings().setValue('coastalme/arch_bridge_editor/bridge_save_setting', 'in_place')

    def save_bridge_output_setting_csv_folder(self, e=None):
        QSettings().setValue('coastalme/arch_bridge_editor/bridge_save_setting', 'csv_folder')

    def save_bridge_output_setting_custom_folder(self, e=None):
        QSettings().setValue('coastalme/arch_bridge_editor/bridge_save_setting', 'custom_folder')

    def save_bridge_output_setting_custom_folder_text(self, e=None):
        QSettings().setValue('coastalme/arch_bridge_editor/bridge_save_path', self.leCustomName.text())

    def save_xs_output_setting_in_place(self, e=None):
        QSettings().setValue('coastalme/arch_bridge_editor/xs_save_setting', 'in_place')

    def save_xs_output_setting_csv_folder(self, e=None):
        QSettings().setValue('coastalme/arch_bridge_editor/xs_save_setting', 'csv_folder')

    def save_xs_output_setting_custom_folder(self, e=None):
        QSettings().setValue('coastalme/arch_bridge_editor/xs_save_setting', 'custom_folder')

    def save_xs_output_setting_custom_folder_text(self, e=None):
        QSettings().setValue('coastalme/arch_bridge_editor/xs_save_path', self.leXSCustomName.text())
    ...
    def connectAll(self):
        ...
        if not self.connected:
            ...
            self.btnBrowseOutDbClicked = self.btnBrowseOutDatabase.clicked.connect(
                                                    lambda: browse(self, 'output database', "COASTALME/import_empty_database",
                                                                   'Output Database', 'GPKG (*.gpkg *.GPKG)',
                                                                   self.leOutDatabase))
            self.btnBrowseOutFileClicked = self.btnBrowseOutFile.clicked.connect(lambda: browse(self, 'output file', 'COASTALME/output_shpfile',
        ...
        if self.connected:
            ...
            if self.editedNwkLayerSignal is not None:
                for i in range(self.cboNwkLayer.count()):
                    lyr = coastalmeqgis_find_layer(self.cboNwkLayer.itemText(i))
                    if lyr is None:
                        continue
                    try:
            ...
            if self.editedXsLayerSignal is not None:
                for i in range(self.cboXsLayer.count()):
                    lyr = coastalmeqgis_find_layer(self.cboXsLayer.itemText(i))
                    if lyr is None:
                        continue
                    try:
