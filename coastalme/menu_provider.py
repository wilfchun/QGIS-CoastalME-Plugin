...
from .gui import apply_cme_style_message_ids, apply_cme_style_temporal, apply_cme_style_static
...
class CmeflowContextMenuProvider(QMenu):

    def __init__(self, iface: QgisInterface):
        super().__init__('&COASTALME')
        self.iface = iface
        self.action_filter_msgs = None
        self.action_create_cme_command_shp = None
        self.action_create_cme_command_gpkg = None
    ...
    def init_menu(self) -> None:
        ...
        self.action_filter_msgs.triggered.connect(apply_cme_style_message_ids)
        self.action_create_cme_command_shp = QAction('Copy COASTALME Command', self.iface.mainWindow())
        self.action_create_cme_command_shp.triggered.connect(create_coastalme_command_path)
        self.action_create_cme_command_gpkg = QAction('Copy COASTALME Command (name only)', self.iface.mainWindow())
        self.action_create_cme_command_gpkg.triggered.connect(create_coastalme_command_name)
        ...
        self.action_gpkg_ts_static_style_type.triggered.connect(lambda e: apply_cme_style_static('Type'))
        self.action_gpkg_ts_static_style_shape.triggered.connect(lambda e: apply_cme_style_static('Shape'))
    ...
    def shp_menu_actions(self) -> list[QAction]:
        ...
        return [
            self.action_create_cme_command_shp,
            separator,
    ...
    def gpkg_menu_actions(self) -> list[QAction]:
        ...
        return [
            self.action_create_cme_command_shp,
            self.action_create_cme_command_gpkg,
            separator,
        ...
        for type_ in types:
            if not hasattr(self, 'action_{0}'.format(type_)):
                ...
                getattr(self, 'action_{0}'.format(type_)).triggered.connect(
                    lambda b, t=type_: apply_cme_style_temporal(t))
