...
from coastalme.coastalme_swmm.xs_shapes import get_max_width, get_max_area, get_max_height
    ...
    def get_curve_data(self, layer, feature):
        ...
        if source_layer.id() in self.curves:
            ...
            if source_layer.storageType() != 'GPKG':
                if has_logging:
                    Logging.error('SWMM network data must be in a valid COASTALME GeoPackage file')
                else:
                    print('SWMM network data must be in a valid COASTALME GeoPackage file')
                return
            if not has_gpd:
                message = (
