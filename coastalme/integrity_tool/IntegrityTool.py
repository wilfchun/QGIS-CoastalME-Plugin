...
from ..coastalmeqgis_library import (is1dNetwork, is1dTable, coastalmeqgis_apply_check_tf_clayer, copyStyle)
from coastalme.toc.toc import coastalmeqgis_get_geopackage_from_layer, coastalmeqgis_find_layer_in_datasource, coastalmeqgis_find_layer, \
    findAllRasterLyrs, findAllVectorLyrsWithGroups
from ..coastalmeqgis_dialog import StackTraceDialog
...
from coastalme.coastalme_swmm.swmm_gis_info import is_swmm_network_layer
...
class IntegrityToolDock(QDockWidget, Ui_IntegrityTool):
    ...
    def __init__(self, iface):
        ...
        self.sbTimeoutLimit.setValue(int(QSettings().value('coastalme/flow_trace_timeout', 30)))
        self.sbRecursionLimit.setValue(int(QSettings().value('coastalme/flow_trace_recursion_limit', 25)))
        self.sbTimeoutLimit.valueChanged.connect(lambda x: QSettings().setValue('coastalme/flow_trace_timeout', x))
        self.sbRecursionLimit.valueChanged.connect(lambda x: QSettings().setValue('coastalme/flow_trace_recursion_limit', x))
    ...
    def check(self):
        ...
        if self.gbDem.isChecked():
            if coastalmeqgis_find_layer(self.cboDem.currentText()) is None:
                QMessageBox.critical(self, "Integrity Tool",
        ...
        try:
            ...
            try:
                if self.outputLyr:
                    ...
                    coastalmeqgis_apply_check_tf_clayer(self.iface, layer=self.outputLyr)
            except RuntimeError:
                self.outputLyr = None
            ...
            message = "Unexpected Error Occurred.\nPlease Email Stack Trace To support@coastalme.com" \
                .format(exc_value)
    ...
    def copyStyle(self, lyrs):
        for tmplyrid, oldlyrid in lyrs.items():
            tmplyr = coastalmeqgis_find_layer(tmplyrid, search_type='layerid')
            oldlyr = coastalmeqgis_find_layer(oldlyrid, search_type='layerid')
    ...
    def runSnappingTool(self):
        ...
        # input dem
        if self.gbDem.isChecked():
            dem = coastalmeqgis_find_layer(self.cboDem.currentText())
        else:
            dem = None
        # input vector layers
    ...
    def runUniqueIdTool(self):
        ...
        if self.uniqueIds.outputLyr is not None and self.uniqueIds.outputLyr.isValid():
            ...
            coastalmeqgis_apply_check_tf_clayer(self.iface, layer=self.uniqueIds.outputLyr)
    ...
    def populateInputs(self):
        ...
        #     layer = coastalmeqgis_find_layer(vector)
        #     if layer is not None:
        #         if layer.geometryType() == QgsWkbTypes.LineGeometry:
        #             for cbo in lineCbos:
        #                 cbo.addItem(vector)
    ...
    def layerOrderChanged(self):
        ...
        for layer_name, layer_id in layers_layerids:
            lyr = coastalmeqgis_find_layer(layer_id, search_type='layerid')
            if lyr is None:
                continue
    ...
    def replaceInputs(self, new_lyrs, tool):
        ...
        for tmplyrs in new_lyrs:
            ...
            for tmplyrid, oldlyrid in tmplyrs.items():
                lyr = coastalmeqgis_find_layer(tmplyrid, search_type='layerid')
