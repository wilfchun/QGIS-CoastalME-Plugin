...
class Refh2Dock(QDockWidget, Ui_refh2):
    """Class for ReFH2 to COASTALME dialog / dock"""
    ...
    def __init__(self, iface: QgisInterface) -> None:
        ...
        self.btnInputXML.clicked.connect(lambda: browse(self, "existing file", "COASTALME/refh2_xml",
                                                        "Catchment Descriptor Input", "XML (*.xml *.XML)",
                                                        self.leInputXML, rf2Icon, lambda: self.inputCatchmentChanged()))
        self.btnOutfile.clicked.connect(lambda: browse(self, "output file", "COASTALME/refh2_outfile",
    ...
    def run(self, vk=None) -> None:
        ...
        if vk != __VK__:
            QMessageBox.warning(self, "ReFH2 to COASTALME",
                                "Tool needs to be run compiled. Please contact support@coastalme.com.")
            return
        ...
        layer = coastalmeqgis_find_layer(self.cboInputGIS.currentText())
        layerUnits = None
        feat = None
        fid = None
        area = 0
        ...
        if self.rbUserArea.isChecked():
            ...
            # should not be here
            QMessageBox.critical(self, "ReFH2 to COASTALME",
                                 f"Unexpected ERROR: Variable(s) should not be None\n"
                                 f"layerUnits: {layerUnits}\n"
                                 f"feat: {feat}\n"
                                 f"fid: {fid}\n"
                                 "Please contact support@coastalme.com")
            return
        if area <= 0:
            area = 0.001
        inputs['area'] = area
        ...
        if not makeDir(dir):
            QMessageBox.critical(self, "ReFH2 to COASTALME", "Unexpected Error with Output File Location: "
        ...
        if os.path.exists(inputs['output file']):
            try:
                ...
                QMessageBox.critical(self, "ReFH2 to COASTALME", "Output File Locked:\n{0}".format(inputs['output file']))
                return
        ...
        if inputs['rain model'] == '2022' and inputs['engine version'] == 2.2:
            QMessageBox.critical(self, 'ReFH2 to COASTALME', 'Cannot use 2022 rainfall with 2.2 engine')
            return
        ...
        self.refh2.coastalmeProcessingStart.connect(self.coastalmeProcessingStarted)
    ...
    def coastalmeProcessingStarted(self, *args, **kwargs) -> None:
        """
    ...
    def refh2Finished(self, message: str='') -> None:
        ...
        if message:
            if message[:7] == "WARNING":
                ...
                QMessageBox.information(self, "ReFH2 to COASTALME", message)
            else:
                self.progressBarLabel.setText("Finished.. Errors Occured")
                QMessageBox.critical(self, "ReFH2 to COASTALME", message)
        else:
            self.progressBarLabel.setText("Finished Successfully")
    ...
    def checkAreaInput(self, e=None, b=None) -> None:
        ...
        if isPointDescriptor:
            if self.rbUserArea.isChecked():
                ...
                elif coastalmeqgis_find_layer(self.cboInputGIS.currentText()) is None:
                    msg = "Input GIS Catchment File Not in Workspace"
                    label = self.createLabel(msg)
                    self.verticalLayout_23.addWidget(label)
                    self.flags.append((self.verticalLayout_23, label, msg))
                elif coastalmeqgis_find_layer(
    ...
    def outputToTuflowGisToggled_Rainfall(self, e=None) -> None:
        """
        Output to COASTALME GIS changed - error checking
        ...
        if self.gbOutRainfall.isChecked() and self.gbRainToGis.isChecked():
            if self.cboInputGIS.currentText() == '-None-':
                msg = "Must Specify Input GIS (Catchment) Layer to Output To COASTALME GIS"
                label = self.createLabel(msg, True)
            ...
            if self.rb2dRf.isChecked() and \
                    coastalmeqgis_find_layer(self.cboInputGIS.currentText()).geometryType() != QgsWkbTypes.PolygonGeometry:
    ...
    def outputToTuflowGisToggled_Runoff(self, e=None) -> None:
        ...
        # check for new flags
        if self.gbOutHydrograph.isChecked() and self.gbHydToGis.isChecked():
            if self.cboInputGIS.currentText() == '-None-' or coastalmeqgis_find_layer(self.cboInputGIS.currentText()) is None:
                msg = "Must Specify Input GIS (Catchment) Layer to Output To COASTALME GIS"
                label = self.createLabel(msg, True)
            ...
            if self.rb2dBc.isChecked() and \
                    coastalmeqgis_find_layer(self.cboInputGIS.currentText()).geometryType() != QgsWkbTypes.LineGeometry:
    ...
    def check(self, *args, **kwargs) -> None:
        ...
        for flag in self.flags:
            msg = flag[2]
            QMessageBox.critical(self, "ReFH2 to COASTALME", msg)
            return

        # input catchment descriptor / locator - required
        if not self.leInputXML.text():
            QMessageBox.critical(self, "ReFH2 to COASTALME", "No Catchment Descriptor Input Locator File Specified")
            return
        if not os.path.exists(self.leInputXML.text()):
            QMessageBox.critical(self, "ReFH2 to COASTALME", "Catchment Descriptor Input Locator File Does Not Exist")
            return
        ...
        for i in range(self.lwRP.count()):
            ...
            if not 1 <= int(rp) <= 1000:
                QMessageBox.critical(self, "ReFH2 to COASTALME", "Return Period Not Valid: {0}".format(rp))
                return
            else:
                rps.append(rp)
        if not rps:
            QMessageBox.critical(self, "ReFH2 to COASTALME", "No Return Period(s) Selected")
            return

        if not self.gbOutRainfall.isChecked() and not self.gbOutHydrograph.isChecked():
            QMessageBox.critical(self, "ReFH2 to COASTALME", "No Output (Rainfall or Hydrograph) Selected")
            return
        ...
        #        QMessageBox.critical(self, "ReFH2 to COASTALME", "Must Specify Input GIS Layer to Output To COASTALME GIS")
        #        return
        #    if self.rb2dRf.isChecked() and \
        #            coastalmeqgis_find_layer(self.cboInputGIS.currentText()).geometryType() != QgsWkbTypes.PolygonGeometry:
        #        QMessageBox.critical(self, "ReFH2 to COASTALME", "Must Use Region GIS Layer to Output 2d_rf")
        #        return
        #    if self.rb2dSaRf.isChecked() and \
        #            coastalmeqgis_find_layer(self.cboInputGIS.currentText()).geometryType() != QgsWkbTypes.PolygonGeometry:
        #        QMessageBox.critical(self, "ReFH2 to COASTALME", "Must Use Region GIS Layer to Output 2d_sa_rf")
        #        return
        #if self.gbOutHydrograph.isChecked() and self.gbHydToGis.isChecked():
        #    #if not self.cboInputGIS.currentText() or not self.cboFields.currentText() \
        #    #        or not self.cboFeatures.currentText():
        ...
        #    if self.rb2dSa.isChecked() and \
        #            coastalmeqgis_find_layer(self.cboInputGIS.currentText()).geometryType() != QgsWkbTypes.PolygonGeometry:
        #        QMessageBox.critical(self, "ReFH2 to COASTALME", "Must Use Region GIS Layer to Output 2d_sa")
        #        return
        #    if self.rb2dBc.isChecked() and \
        #            coastalmeqgis_find_layer(self.cboInputGIS.currentText()).geometryType() != QgsWkbTypes.LineGeometry:
        #        QMessageBox.critical(self, "ReFH2 to COASTALME", "Must Use Line GIS Layer to Output 2d_bc")
        #        return
        #    if self.rb1dBc.isChecked() and \
        #            coastalmeqgis_find_layer(self.cboInputGIS.currentText()).geometryType() == QgsWkbTypes.LineGeometry:
        #        QMessageBox.critical(self, "ReFH2 to COASTALME", "Cannot Use Line GIS Layer to Output 1d_bc")
        #        return
        if self.rbQgisArea.isChecked():
            ...
            if layerUnits == QgsUnitTypes.DistanceDegrees:
                ...
                if self.getCartesian(x) is None:
                    QMessageBox.critical(self, "ReFH2 to COASTALME", "Difficulty Converting Degrees to Meters... "
...
def coastalmeqgis_find_layer(layer_name, **kwargs):
