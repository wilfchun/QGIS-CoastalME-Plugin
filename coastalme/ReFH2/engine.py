...
class Refh2(QObject):
    ...
    coastalmeProcessingStart = pyqtSignal()
    ...
    def processIntoTuflow(self, msg='', model=None, index=0, model_type=None, iter=0, iDur=0) -> None:
        ...
        into COASTALME format.
        
        :return: None
        """

        self.coastalmeProcessingStart.emit()
    ...
    def convertFormatToTuflow(self, model=None, iter=0, mIter=1, model_type=None, index=0, mIndex=1, iDur=0) -> None:
        ...
        ReFH2 to COASTALME compatible - change time column and remove unwanted columns
        
        :return: None
        """

        # name = '{0}_COASTALME.csv'.format(os.path.basename(os.path.splitext(self.inputs['output file'])[0]))
        ...
        try:
            ...
            #    fo.write('! {0} Processed into COASTALME Format by ReFH2 to COASTALME QGIS Plugin Tool\n'.format(self.inputs['output file']))
            ...
            if iter + 1 == mIter:
                header = '! {0} Processed into COASTALME Format by ReFH2 to COASTALME QGIS Plugin Tool\n' \
    ...
    def createTEF(self) -> None:
        """
        Create COASTALME Event File (TEF)
        ...
        try:
            with open(tef, 'w') as fo:
                fo.write('! TEF Written by ReFH2 to COASTALME QGIS Plugin Tool\n\n')
