...
Copies routines largely from the convert_coastalme_model_gis_format suite and modifies where required to
...
def ogr_format(file, no_ext_is_mif=False, no_ext_is_gpkg=False):
    ...
    raise Exception(f'Error: Vector format not supported by COASTALME: {Path(db).suffix}')
...
def suffix_2_geom_type(suffix):
    """Convert OGR geometry type to COASTALME suffix."""
...
def coastalme_type_requires_feature_iter(layername):
    """
...
def ogr_copy(src_file, dest_file, geom=None, settings=None, **kwargs):
    ...
    Some COASTALME layers (1d_nwk, 1d_tab) contain references to files, these will also be copied and references
    ...
    file_indexes = coastalme_type_requires_feature_iter(lyrname_in)  # is there a file reference in the features
