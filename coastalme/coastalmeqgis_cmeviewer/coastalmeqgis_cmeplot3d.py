...
class CmePlot3D(CmePlot2D):
    ...
    def __init__(self, cmePlot):
        CmePlot2D.__init__(self, cmePlot)
    ...
    def getAveragingMethods(self, dataType, gmd, resultTypes):
        ...
        return self.cmePlot.cmePlotToolbar.getAveragingMethods(dataType, gmd)
    ...
    def plotVerticalProfileFromMap(self, vLayer, point, **kwargs):
        ...
        dataType = kwargs['data_type'] if 'data_type' in kwargs else CmePlot.DataVerticalProfile
        ...
        if bypass:
            ...
            self.cmePlot.clearPlot2(CmePlot.VerticalProfile, dataType,
        ...
        plotAsQuiver = []
        plotAsPatch = []
        xMagnicmedes = []  # used to position arrows relative to scalar magnicmedes - not passed into drawplot function
        plotVertMesh = []
        ...
        for layer in resultMesh:  # get plotting for all selected result meshes
            ...
            for i, rtype in enumerate(resultTypes):
                ...
                meshDatasetIndex = self.cmeView.cmeResults.getResult(cmeResultsIndex, force_get_time='next lower')
                ...
                plotAsQuiver.append(False)
                plotAsPatch.append(False)
                dataTypes.append(CmePlot.DataVerticalMesh)
        ...
        if data:
            if export is None:  # normal plot i.e. in cmeview
                self.cmePlot.drawPlot(CmePlot.VerticalProfile, data, labels, types, dataTypes, draw=draw,
                ...
                while os.path.exists(outFile):
                    ...
                    iterator += 1
                self.cmePlot.drawPlot(CmePlot.VerticalProfile, data, labels, types, dataTypes,
                                     plot_as_collection=plotAsCollection, plot_as_patch=plotAsPatch,
                                     plot_as_quiver=plotAsQuiver, export=outFile, plot_vert_mesh=plotVertMesh)
            elif export == 'csv':  # export to csv, don't plot
                self.cmePlot.exportCSV(CmePlot.TimeSeries, data, labels, types, exportOut, name)
                self.cmePlot.exportCSV(CmePlot.VerticalProfile, data, labels, types, exportOut)
    ...
    def plotCurtainFromMap(self, vLayer, feat, **kwargs):
        ...
        # clear the plot based on kwargs
        if not bypass:
            self.cmePlot.clearPlot2(CmePlot.CrossSection, CmePlot.DataCurtainPlot, draw=draw)
        ...
        for layer in activeMeshLayers:
            ...
            if not resultTypes:  # specified result types can be passed through kwargs (used for batch export not normal plotting)
                resultTypes = self.cmePlot.cmePlotToolbar.getCheckedItemsFromPlotOptions(CmePlot.DataCurtainPlot)
            key = lambda x: 1 if 'vector' in x.lower() else 0
            resultTypes = sorted(resultTypes, key=key)
            for j, rtype in enumerate(resultTypes):
                ...
                if update:
                    ...
                    # update = True  # cmern on update now - allow only one line at the moment
                if not onFaces:
                    ...
                    if debug and not update:
                        writeTempPoints(self.inters, self.cmeView.project, crs, self.chainages, 'Chainage', QT_DOUBLE)
                else:
                    onFaces = True
                    ...
                    if debug and not update:
                        ...
                        writeTempPoints(self.inters, self.cmeView.project, crs, self.chainages, 'Chainage',
                                        QT_DOUBLE)
                ...
                label = self.generateLabel(layer, layer, rtype, lineNo, featName,
                                           activeMeshLayers, None, export, bypass, j, CmePlot.DataCurtainPlot)
                labels.append(label)
                types.append(rtype)
                plotAsPatch.append(True)

        dataTypes = [CmePlot.DataCurtainPlot] * len(data)
        if export is None:
        ...
        # QMessageBox.information(self.cmeView, 'debug', '{0:.4f}'.format(time))
        return True
    ...
    def getScalarDataPoint(self, point, layer, dp, si, mesh, mdi, meshRendered, onFaces, isMax, isMin):
        ...
        if onFaces:
            ...
            if isinstance(data3d, QgsMesh3dDataBlock):
                ...
                x = data3d.values()
                if len(x) == 2 * vlc:  # vector (x, y) -> get magnicmede
                    x = [ ( x[i] ** 2 + x[i+1] ** 2 ) ** 0.5 for i in range(0, len(x), 2) ]
                y = data3d.verticalLevels()
                vm = y[:]
                if self.cmeView.cmeOptions.verticalProfileInterpolated:
                    y = [ ( y[i] + y[i+1] ) / 2. for i in range(0, len(y) - 1) ]  # point halfway between levels
                else:
                    x = sum([[x, x] for x in x], [])
                    y = sum([[x, x] for x in y], [])[1:-1]
        else:
            x = [self.datasetValue(layer, dp, si, mesh, mdi, False, point, 0, CmePlot.DataVerticalProfile, None) for x in range(2)]
            y = self.getBedAndWaterElevation(layer, dp, si, mesh, mdi, False, point, 0, CmePlot.DataVerticalProfile, None,
                                             isMax, isMin)
            vm = y[:]
    ...
    def getScalarDataCurtain(self, layer, dp, si, mesh, mdi, faces, ch, update, rtype, onFaces, isMax, isMin,
        ...
        edgecolour = ((0,0,0,1),) if self.cmePlot.verticalMesh_action.isChecked() else 'face'
        ...
        for i, f in enumerate(faces):
            ...
            if f is None:
                ...
                if onFaces:
                    ...
                    vlc = 1
                    v = [self.datasetValue(layer, dp, si, mesh, mdi, False, f, 0, CmePlot.DataCurtainPlot, None)]
                    vl = self.getBedAndWaterElevation(layer, dp, si, mesh, mdi, False, f, 0, CmePlot.DataCurtainPlot, None,
                                                      isMax, isMin, update)
                    if update:
        ...
        #    self.co_scmeff += datetime.now() - pf  # profiling
        #else:
    ...
    def getVectorDataCurtain(self, layer, dp, si, mesh, mdi, faces, ch, points, update, onFaces, isMax, isMin,
        ...
        for i, f in enumerate(faces):
            if onFaces:
                ...
                vlc = 1
                v = self.datasetValue(layer, dp, si, mesh, mdi, False, f, 0, CmePlot.DataCurtainPlot, None, value='vector')
                if type(v) is tuple:
                    v = v[1:]
                else:
                    v = [v]
            ...
            if not np.isnan(v).any():
                for j in range(vlc):
                    ...
                    if len(v) == 2 * vlc:  # x,y components
                        ...
                        uc.append(np.dot(r, vel)[0, 0] * unit_vector * self.cmeView.cmeOptions.curtain_vector_horizontal_factor)  # u component needed only
                        # vc.append(0)
                        vc.append(vertical_velocities[j] * self.cmeView.cmeOptions.curtain_vector_vertical_factor)
                    else:
                        QMessageBox.critical(self.cmeView, "Error", "Should not be here [getVectorData]")
                        return
        ...
        config = {
            'scale_units': self.cmeView.cmeOptions.curtain_vector_scale_units,
        }
        if self.cmeView.cmeOptions.curtain_vector_scale >= 0.:
            config['scale'] = self.cmeView.cmeOptions.curtain_vector_scale
        if self.cmeView.cmeOptions.curtain_vector_units is not None:
            config['units'] = self.cmeView.cmeOptions.curtain_vector_units
        if self.cmeView.cmeOptions.curtain_vector_width >= 0.:
            config['width'] = self.cmeView.cmeOptions.curtain_vector_width
        if self.cmeView.cmeOptions.curtain_vector_head_width >= 0.:
            config['headwidth'] = self.cmeView.cmeOptions.curtain_vector_head_width
        if self.cmeView.cmeOptions.curtain_vector_head_length >= 0.:
            config['headwidth'] = self.cmeView.cmeOptions.curtain_vector_head_length
    ...
    def getMinMaxValue(self, dp, mdgi):
        ...
        isMax = CmeResults.isMinimumResultType
        isMin = CmeResults.isMinimumResultType
        stripMax = CmeResults.stripMaximumName
        stripMin = CmeResults.stripMinimumName
    ...
    def getBedAndWaterElevation(self, layer, dp, si, mesh, mdi, meshRendered, f, ind, dataType, am, isMax, isMin,
        ...
        for i in range(dp.datasetGroupCount()):
            ...
            if name.lower() in possibleBdNames:
                ...
                continue
            if isMax:
                if CmeResults.isMaximumResultType(name):
                    name = CmeResults.stripMaximumName(name)
                    if name.lower() in possibleWlNames:
                        iWlGmd = i
            elif isMin:
                if CmeResults.isMinimumResultType(name):
                    name = CmeResults.stripMinimumName(name)
                    if name.lower() in possibleWlNames:
                        iWlGmd = i
            else:
                if name.lower() in possibleWlNames:
...
class CmeCurtainLine(CmeRubberBand):
    """
    Class for handling curtain line graphic
    """

    def __init__(self, cmePlot, plotNo):
        CmeRubberBand.__init__(self, cmePlot, plotNo)
    ...
    def clearPlot(self, firstPlot):
        ...
        if firstPlot:
            self.cmePlot.clearPlot2(CmePlot.CrossSection)
        else:
            self.cmePlot.clearPlot2(CmePlot.CrossSection, CmePlot.DataCurtainPlot)
    ...
    def plotFromRubberBand(self, feat, bypass=False):
        ...
        return self.cmePlot.cmePlot3D.plotCurtainFromMap(None, feat)
...
class CmeDepthAvRubberBand(CmeRubberBand):
    """
    Class for handling curtain line graphic
    """
    ...
    def clearPlot(self, firstPlot):
        ...
        self.cmePlot.clearPlot2(CmePlot.CrossSection, CmePlot.DataCrossSectionDepAv)
    ...
    def plotFromRubberBand(self, feat, bypass=False):
        ...
        return self.cmePlot.cmePlot3D.plotCrossSectionFromMap(None, feat, bypass=bypass, lineNo=len(self.rubberBands),
...
class CmeDepthAvPoint(CmeMarker):

    def __init__(self, cmePlot, plotNo):
        CmeMarker.__init__(self, cmePlot, plotNo)
        self.colour = QT_DARK_GREEN
    ...
    def clearPlot(self, firstTimePlotting: bool, lastOnly: bool = False) -> None:
        ...
        resultTypes = self.cmePlot.cmePlotToolbar.getCheckedItemsFromPlotOptions(CmePlot.DataTimeSeriesDepAv)
        activeMeshLayers = self.cmeView.cmeResults.cmeResults2D.activeMeshLayers
        self.cmePlot.clearPlot2(CmePlot.TimeSeries, CmePlot.DataTimeSeriesDepAv, last_only=lastOnly,
    ...
    def plotFromMarker(self, point: QgsPointXY, bypass: bool = False) -> bool:
        ...
        return self.cmePlot.cmePlot3D.plotTimeSeriesFromMap(None, point, bypass=bypass, markerNo=len(self.points),
...
class ColourBar(ColorbarBase):
    """
    Stolen from cmev python library. Modified slightly to use cax - as this lets tight_layout() work.
    """
