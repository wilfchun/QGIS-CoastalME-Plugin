...
class CmeRubberBand():
	...
	def __init__(self, CmePlot, plotNo, dataType, button, plotFunction,
	...
	def startRubberBand(self):
		...
		multi = False
		if self.cmeView.cboSelectType.currentText() == 'From Map Multi':
			multi = True
		...
		if not self.cursorTrackingConnected:
			...
			if not multi:  # simple, can remove previous flow lines and start again
				...
				self.rubberBands = []
				exec("self.cmePlot.{0} = False".format(self.firstTimePlotting[self.plotNo]))
			else:
				# multi select so only clear plot if plotting for the first time
				if eval("self.cmePlot.{0}".format(self.firstTimePlotting[self.plotNo])):  # first plot so need to remove test line
					self.clearPlot(True)
					exec("self.cmePlot.{0} = False".format(self.firstTimePlotting[self.plotNo]))
				else:
					exec("self.cmePlot.{0} = True".format(self.holdPlot[self.plotNo]))
			...
			rubberBand = QgsRubberBand(self.canvas)  # secmep rubberband class for drawing
	...
	def clearPlot(self, firstTimePlotting: bool) -> None:
		...
		self.cmePlot.clearPlot2(self.plotNo, self.dataType)

	def mouseTrackConnect(self):
		"""
        Capcmeres signals from the custom map tool
	...
	def moved(self, position):
		...
		if self.points:
			...
			if self.cmeView.cmeOptions.liveMapTracking and self.allowLiveTracking:
	...
	def createMemoryLayer(self, **kwargs):
		"""
        Creates a polyline feacmere from the rubberband so that it can then be used to extract values
		...
		self.cmePlot.cmePlot2D.multiFlowLineSelectCount = len(
		...
		if self.rubberBands:  # force counting if there are rubberbands
			exec("self.cmePlot.{0} = False".format(self.clearedPlot[self.plotNo]))

		points = kwargs['points'] if 'points' in kwargs else self.points[:]
		if points is None:
			return

		# create feacmere layer
		feat = QgsFeature()
		try:
		...
		if eval("self.cmePlot.{0}".format(self.holdPlot[self.plotNo])):
			if eval("self.cmePlot.{0}".format(self.clearedPlot[self.plotNo])) and \
					eval("self.cmePlot.{0}".format(self.cmeView.cmeOptions.liveMapTracking)) and self.allowLiveTracking:
				...
				if not worked:
					...
					self.clearGraphics()
					return False
			exec("self.cmePlot.{0} = False".format(self.holdPlot[self.plotNo]))
		else:
			worked = self.plotFromRubberBand(feat)
			if not worked:
				self.escape(QKeyEvent(QT_EVENT_KEY_PRESS, QT_KEY_ESCAPE, QT_KEY_NO_MODIFIER))
	...
	def plotFromRubberBand(self, feat: QgsFeature, bypass: bool = False) -> bool:
		"""
        Requires subclassing.
        Tell coastalme viewer what to plot with rubberband.

        :param feat: feacmere to plot
...
class CmeMarker():
	...
	def startRubberBand(self):
		...
		if not self.cursorTrackingConnected:
			...
			if not multi:
				...
				# single selection
				# self.cmePlot.plotSelectionPointFeat.clear()  # clear feacmeres from selected plot list
				...
				# multi select so only clear plot if plotting for the first time
				if eval("self.cmePlot.{0}".format(self.firstTimePlotting[self.plotNo])):
					# self.cmePlot.clearPlot(0, retain_1d=True, retain_flow=True)
					self.clearPlot(True, False)
					exec("self.cmePlot.{0} = False".format(self.firstTimePlotting[self.plotNo]))
				else:
	...
	def mouseTrackDisconnect(self):
		"""
		Cmern off capcmering of the custom map tool
	...
	def clearPlot(self, firstTimePlotting: bool, lastOnly: bool) -> None:
		...
		resultTypes = self.cmePlot.cmePlotToolbar.getCheckedItemsFromPlotOptions(self.dataType)
		activeMeshLayers = self.cmeView.cmeResults.cmeResults2D.activeMeshLayers

		self.cmePlot.clearPlot2(self.plotNo, self.dataType, last_only=lastOnly,
		...
		if eval("self.cmePlot.{0}".format(self.holdPlot[self.plotNo])):
			if self.cmeView.cmeOptions.liveMapTracking and self.allowLiveTracking:
				...
				if not worked:
					...
					return False
			exec("self.cmePlot.{0} = False".format(self.holdPlot[self.plotNo]))  # cmern off after first signal
		else:
			if self.cmeView.cmeOptions.liveMapTracking and self.allowLiveTracking:
	...
	def leftClick(self, position):
		...
		if self.cmeView.cboSelectType.currentText() == 'From Map Multi':
			...
			# self.cmePlot.cmePlotToolbar.plotTimeSeriesButton.setChecked(False)
	...
	def rightClick(self, position):
		...
		if self.cmeView.cmeOptions.liveMapTracking and self.allowLiveTracking:
			# self.cmePlot.clearPlotLastDatasetOnly(self.plotNo)
		...
		# self.cmePlot.cmePlot2D.reduceMultiPointCount(1)
	...
	def doubleClick(self, position):
		...
		# reset multi point numbering back to one
		# self.cmePlot.cmePlot2D.reduceMultiPointCount(2)
	...
	def plotFromMarker(self, point: QgsPointXY, bypass: bool = False) -> bool:
		"""
		Requires subclassing.
        Tell coastalme viewer what to plot with marker.
...
class CmeCrossSection(CmeRubberBand):

	def __init__(self, cmeView, plotNo):
		CmeRubberBand.__init__(self, cmeView, plotNo)
		self.colour = QT_RED
	...
	def clearPlot(self, firstTimePlotting):
		...
		self.cmePlot.clearPlot2(CmePlot.CrossSection, CmePlot.DataCrossSection2D)
	...
	def plotFromRubberBand(self, feat, bypass=False):
		...
		return self.cmePlot.cmePlot2D.plotCrossSectionFromMap(None, feat, bypass=bypass, lineNo=len(self.rubberBands))
...
class CmeTimeSeriesPoint(CmeMarker):

	def __init__(self, cmeView, plotNo):
		CmeMarker.__init__(self, cmeView, plotNo)
		self.colour = QT_RED
	...
	def clearPlot(self, firstTimePlotting: bool, lastOnly: bool = False) -> None:
		...
		if firstTimePlotting:
			# self.cmePlot.clearPlot(0, retain_1d=True, retain_flow=True)
			resultTypes = self.cmePlot.cmePlotToolbar.getCheckedItemsFromPlotOptions(CmePlot.DataTimeSeries2D)
			activeMeshLayers = self.cmeView.cmeResults.cmeResults2D.activeMeshLayers
			self.cmePlot.clearPlot2(CmePlot.TimeSeries, CmePlot.DataTimeSeries2D, last_only=lastOnly,
			                       remove_no=len(resultTypes)*len(activeMeshLayers))
	...
	def plotFromMarker(self, point: QgsPointXY, bypass: bool = False) -> bool:
		...
		return self.cmePlot.cmePlot2D.plotTimeSeriesFromMap(None, point, bypass=bypass, markerNo=len(self.points))
