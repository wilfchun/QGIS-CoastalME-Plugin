...
from .coastalmeqgis_cmeresults1d import CmeResults1D
from .coastalmeqgis_cmeresults2d import CmeResults2D
from .coastalmeqgis_cmeresultsParticles import CmeResultsParticles
from .cmeResultsNcGrid import CmeResultsNcGrid
...
class CmeResults():
	...
	def __init__(self, CmeView=None):
		...
		if CmeView is not None:
			...
			self.date2timekey = {}
			self.date2time = {}
			# date scmeff with timespec for display purposes
			...
			# 1D results
			self.cmeResults1D = CmeResults1D(CmeView)

			# 2D results
			self.cmeResults2D = CmeResults2D(CmeView)

			# Particles results
			self.cmeResultsParticles = CmeResultsParticles(CmeView)

			# netcdf grid
			self.cmeResultsNcGrid = CmeResultsNcGrid(CmeView)
	...
	def importResults(self, type, inFileNames, **kwargs):
		...
		result = False
		if type.lower() == 'mesh':
			result = self.cmeResults2D.importResults(inFileNames)
		elif type.lower() == 'timeseries fm':
			result = self.cmeResults1D.importResultsFM(inFileNames[0], inFileNames[1], inFileNames[2:])
		elif type.lower() == 'timeseries':
			result = self.cmeResults1D.importResults(inFileNames)
		elif type.lower() == 'particles':
			result = self.cmeResultsParticles.importResults(inFileNames)
		elif type.lower() == 'nc_grid':
			try:
				result = self.cmeResultsNcGrid.importResults(inFileNames, **kwargs)
			except Exception as e:
				QMessageBox.warning(self.cmeView, 'Load NetCDF Grid', str(e))
				result = False
		if not result:
			return False
	...
	def updateActiveTime_old(self):
		...
		i = self.cmeView.cboTime.currentIndex()
		self.cmeView.sliderTime.setSliderPosition(i)

		self.activeTime = None
		if i != -1:
			self.activeTime = self.cmeView.cboTime.currentText()
		...
		self.updateQgsTime()
		self.cmeResultsParticles.updateActiveTime()
	...
	def updateActiveTime_31600(self):
		...
		self.updateQgsTime()
		self.cmeResultsParticles.updateActiveTime()
		self.cmeResultsNcGrid.updateActiveTime()
	...
	def updateOpenResults(self, *args):
		...
		self.cmeView.OpenResults.clear()
		self.cmeView.OpenResults.addItems(self.results.keys())
	...
	def resetResultTypes(self):
		...
		self.cmeView.initialiseDataSetView()

		# Reset multi combobox to empty
		for dataType in self.cmeView.cmePlot.plotDataPlottingTypes:
			menu = self.cmeView.cmePlot.cmePlotToolbar.plotDataToPlotMenu[dataType]
	...
	def getDataFromResultsDict(self, resultName):
		"""
		Recmerns list of meta data available in the results dictionary
		...
		for type, t in r.items():
			...
			if CmeResults.isMaximumResultType(type):
				if type not in maxResultTypes:
					rt = CmeResults.stripMaximumName(type)
					maxResultTypes.append(rt)
			elif CmeResults.isMinimumResultType(type):
				if type not in minResultTypes:
					rt = CmeResults.stripMinimumName(type)
				...
				if qv < 31600:
					...
					refTime = t['referenceTime'] if 'referenceTime' in t else self.cmeView.cmeOptions.zeroTime
		...
		if not self.cmeView.lock2DTimesteps:
	...
	def applyPreviousResultTypeSelections(self, currentPlotData, time, **kwargs):
		...
		openResultTypes = self.cmeView.OpenResultTypes  # DataSetView
		#mcboResultType = self.cmeView.mcboResultType  # QgsCheckableComboBox
		cboTime = self.cmeView.cboTime  # QgsComboBox
		...
		if not self.activeResults and select_first_dataset:
			openResultTypes = self.cmeView.OpenResultTypes
			ind = openResultTypes.model().index(0, 0)
			index = openResultTypes.indexBelow(ind)
			if index.internalPointer().ds_name != 'None':
				openResultTypes.selectionModel().select(index, QT_ITEM_SELECTION_SELECT)
				self.cmeView.cmeResults.activeResultsIndexes.append(index)
				self.cmeView.cmeResults.activeResultsItems.append(index.internalPointer())
				self.cmeView.cmeResults.activeResultsTypes.append(index.internalPointer().ds_type)
				self.cmeView.cmeResults.activeResults.append(index.internalPointer().ds_name)
				if index.internalPointer().ds_type == 1:
					self.cmeResults2D.activeScalar = index.internalPointer().ds_name
					openResultTypes.activeScalarIdx = index
					openResultTypes.activeScalarName = index.internalPointer().ds_name
				elif index.internalPointer().ds_type == 2:
					self.cmeResults2D.activeVector = index.internalPointer().ds_name
					openResultTypes.activeVectorIdx = index
		...
		#self.cmeView.cmePlot.cmePlotToolbar.setCheckedItemsPlotOptions(namesTS, 0)
		#self.cmeView.cmePlot.cmePlotToolbar.setCheckedItemsPlotOptions(namesLP, 1)
		for dataType, plotData in currentPlotData.items():
			if dataType != CmePlot.DataTimeSeriesDepAv and dataType != CmePlot.DataCrossSectionDepAv:
				self.cmeView.cmePlot.cmePlotToolbar.setCheckedItemsPlotOptions(dataType, plotData)
		...
		if qv >= 31600:
			...
			if time is not None:
				...
				#dateProper = datetime.strptime(date, self.cmeView.cmeOptions.dateFormat)  # datetime object
				...
				if not self.cmeView.cmeOptions.xAxisDates:
					...
					timeFormatted = self.cmeView.cmeOptions.dateFormat.format(date)
	...
	def updateResultTypes_old(self):
		...
		sliderTime = self.cmeView.sliderTime  # QSlider
		cboTime = self.cmeView.cboTime  # QComboBox
		#mcboResultType = self.cmeView.mcboResultType  # QgsCheckableComboBox
		openResults = self.cmeView.OpenResults  # QListWidget
		openResultTypes = self.cmeView.OpenResultTypes  # DataSetView
		...
		#currentNamesTS = self.cmeView.cmePlot.cmePlotToolbar.getCheckedItemsFromPlotOptions(CmePlot.DataTimeSeries2D)
		#currentNamesLP = self.cmeView.cmePlot.cmePlotToolbar.getCheckedItemsFromPlotOptions(CmePlot.DataCrossSection2D)
		currentPlotData = {}
		for dataType in self.cmeView.cmePlot.plotDataPlottingTypes:
			plotData = self.cmeView.cmePlot.cmePlotToolbar.getCheckedItemsFromPlotOptions(dataType, all_details=True)
			if plotData:
			...
			for rtype in temporalResultTypes:
				...
				for i, (time, values) in enumerate(t['times'].items()):
					if i == 0:  # get the data type from the first timestep i.e. scalar or vector
						...
						break
				self.cmeView.cmePlot.cmePlotToolbar.addItemToPlotOptions(rtype, static=self.results[result.text()][rtype]['isStatic'])
		...
		connected = True
		try:
			self.cmeView.cboTime.currentIndexChanged.disconnect(self.cmeView.timeSliderChanged)
		except:
		...
		if timesteps:
			if not self.cmeView.cmeOptions.xAxisDates:  # use Time (hrs)
				unit = self.cmeView.cmeOptions.timeUnits
		...
		if connected:
			self.cmeView.cboTime.currentIndexChanged.connect(self.cmeView.timeSliderChanged)

		# Apply selection
		self.applyPreviousResultTypeSelections(currentPlotData, currentTime)

		# Update viewport with enabled / disabled items
		# self.cmeView.currentLayerChanged()
		self.cmeView.setTsTypesEnabled()
	...
	def updateResultTypes_31600(self, **kwargs):
				...
				if rtype not in temporalResultTypes:
					...
					for i, (time, values) in enumerate(t['times'].items()):
						if i == 0:  # get the data type from the first timestep i.e. scalar or vector
							...
							break
					self.cmeView.cmePlot.cmePlotToolbar.addItemToPlotOptions(rtype, static=True)
		if not mapOutputs:
			mapOutputs = [("None", 3, False)]

		# add nc grids to plotting options
		for result_name, (nc_grid, times) in self.cmeResultsNcGrid.results.items():
			self.cmeView.cmePlot.cmePlotToolbar.addItemToPlotOptions(result_name, static=nc_grid.static)
		...
		if timesteps:
			if not self.cmeView.cmeOptions.xAxisDates:  # use Time (hrs)
				timesteps = [(x - self.cmeView.cmeOptions.zeroTime).total_seconds() / 60. / 60. for x in timesteps]
		...
		# Apply selection
		try:
			self.cmeView.OpenResults.itemSelectionChanged.disconnect(self.cmeView.resultSelectionChangeSignal)
		except:
			pass
		...
		for ml in meshLayers:
			...
			try:
				layer.repaintRequested.disconnect(self.cmeResults2D.repaintRequested)
			except:
				pass
		...
		self.cmeResults2D.repaintRequested()  # this is the step that changes the 'reference time' property in the result dict
		self.cmeView.resultSelectionChangeSignal = self.cmeView.OpenResults.itemSelectionChanged.connect(
			lambda: self.cmeView.resultsChanged('item clicked'))
		for ml in meshLayers:
			layer = coastalmeqgis_find_layer(ml)
			layer.repaintRequested.connect(self.cmeResults2D.repaintRequested)
	...
	def updateActiveResultTypes(self, resultIndex, geomType=None, skip_already_selected=False, force_selection=None):
		...
		self.cmeView.setTsTypesEnabled(context='update_active_1d_types')
		...
		for resultType in self.activeResults:
			...
			if not found:
				...
				if resultType[:-3] in self.cmeResults1D.typesTS:
					self.cmeResults1D.typesTS.remove(resultType[:-3])
				if resultType[:-3] in self.cmeResults1D.pointTS:
					self.cmeResults1D.pointTS.remove(resultType[:-3])
				if resultType[:-3] in self.cmeResults1D.lineTS:
					self.cmeResults1D.lineTS.remove(resultType[:-3])
				if resultType[:-3] in self.cmeResults1D.regionTS:
					self.cmeResults1D.regionTS.remove(resultType[:-3])
				if resultType[:-3] in self.cmeResults1D.typesLP:
					self.cmeResults1D.typesLP.remove(resultType[:-3])
				if resultType[:-3] in self.cmeResults1D.typesXS:
					self.cmeResults1D.typesXS.remove(resultType[:-3])
				if resultType[:-3] in self.cmeResults1D.lineXS:
					self.cmeResults1D.lineXS.remove(resultType[:-3])

		# if geomtype then change the TS result options
		if geomType is not None:
			if geomType == QgsWkbTypes.PointGeometry:
				self.cmeResults1D.typesTS.extend(x for x in self.cmeResults1D.pointTS if x not in self.cmeResults1D.typesTS)
				self.cmeResults1D.typesXS.extend(x for x in self.cmeResults1D.lineXS if x not in self.cmeResults1D.typesXS)
			elif geomType == QgsWkbTypes.LineGeometry:
				self.cmeResults1D.typesTS.extend(x for x in self.cmeResults1D.lineTS + self.cmeResults1D.pointTS if x not in self.cmeResults1D.typesTS)
				self.cmeResults1D.typesXS.extend(x for x in self.cmeResults1D.lineXS if x not in self.cmeResults1D.typesXS)
			elif geomType == QgsWkbTypes.PolygonGeometry:
				self.cmeResults1D.typesTS.extend(x for x in self.cmeResults1D.regionTS if x not in self.cmeResults1D.typesTS)
				self.cmeResults1D.typesXS.extend(x for x in self.cmeResults1D.lineXS if x not in self.cmeResults1D.typesXS)
			else:
				self.cmeResults1D.typesTS = []

			return True


		# if not a map output item there is no need to rerender the map
		layer = self.cmeResults2D.activeMeshLayers[0] if self.cmeResults2D.activeMeshLayers else None
		skip = False
		if resultIndex is None or resultIndex.internalPointer() is None or \
			...
			if item not in self.activeResultsItems:
				if item.ds_type == 1:  # scalar
					self.cmeResults2D.activeScalar = item.ds_name
				elif item.ds_type == 2:  # vector
					self.cmeResults2D.activeVector = item.ds_name
				#elif item.ds_type == 4 or item.ds_type == 5 or item.ds_type == 6 or item.ds_type == 7:  # 1d result
				elif item.ds_type in CmeResults.Results1D:  # 1d result
					self.cmeResults1D.items1d.append(item)
					#if item.ds_type == 4 or item.ds_type == 5 or item.ds_type == 6:  # time series
					if item.ds_type == 4:  # point
						if item.ds_name not in self.cmeResults1D.pointTS:
							self.cmeResults1D.pointTS.append(item.ds_name)
					elif item.ds_type == 5:  # line
						if item.ds_name not in self.cmeResults1D.lineTS:
							self.cmeResults1D.lineTS.append(item.ds_name)
					elif item.ds_type == 6:
						if item.ds_name not in self.cmeResults1D.regionTS:
							self.cmeResults1D.regionTS.append(item.ds_name)
					elif item.ds_type == 7:  # long plot
						if item.ds_name not in self.cmeResults1D.typesLP:
							self.cmeResults1D.typesLP.append(item.ds_name)
						self.cmeResults1D.typesXSRes.extend(x for x in self.cmeResults1D.typesLP if x not in self.cmeResults1D.typesXSRes)
					elif item.ds_type == 8:  # XS
						if item.ds_name not in self.cmeResults1D.typesXS:
							self.cmeResults1D.lineXS.append(item.ds_name)
					# if self.cmeResults1D.activeType == 0:
					if 0 in self.cmeResults1D.activeType:
						self.cmeResults1D.typesTS.extend(x for x in self.cmeResults1D.pointTS if x not in self.cmeResults1D.typesTS)
						self.cmeResults1D.typesXSRes.extend(x for x in self.cmeResults1D.pointTS if x not in self.cmeResults1D.typesXSRes)
					# elif self.cmeResults1D.activeType == 1:
					if 1 in self.cmeResults1D.activeType:
						self.cmeResults1D.typesTS.extend(x for x in self.cmeResults1D.lineTS + self.cmeResults1D.pointTS if x not in self.cmeResults1D.typesTS)
						self.cmeResults1D.typesXS.extend(x for x in self.cmeResults1D.lineXS if x not in self.cmeResults1D.typesXS)
						self.cmeResults1D.typesXSRes.extend(x for x in self.cmeResults1D.pointTS if x not in self.cmeResults1D.typesXSRes)
					# elif self.cmeResults1D.activeType == 2:
					if 2 in self.cmeResults1D.activeType:
						self.cmeResults1D.typesTS.extend(x for x in self.cmeResults1D.regionTS if x not in self.cmeResults1D.typesTS)
						self.cmeResults1D.typesXSRes.extend(x for x in self.cmeResults1D.pointTS if x not in self.cmeResults1D.typesXSRes)
					# elif self.cmeResults1D.activeType == 3:
					# 	self.cmeResults1D.typesXS = self.cmeResults1D.lineXS[:]
					# 	self.cmeResults1D.typesXSRes = self.cmeResults1D.pointTS[:]
					#else:
					#	self.cmeResults1D.typesTS = []
					#elif item.ds_type == 7:  # long plot
					#	self.cmeResults1D.typesLP.append(item.ds_name)
				...
				if item.ds_type == 1:
					self.cmeResults2D.activeScalar = None
				elif item.ds_type == 2:
					self.cmeResults2D.activeVector = None
				# remove 1D from lists
				if item in self.cmeResults1D.items1d:
					self.cmeResults1D.items1d.remove(item)
				if item.ds_name in self.cmeResults1D.typesTS:
					self.cmeResults1D.typesTS.remove(item.ds_name)
				elif item.ds_name in self.cmeResults1D.typesLP:
					self.cmeResults1D.typesLP.remove(item.ds_name)
					if item.ds_name in self.cmeResults1D.typesXSRes:
						self.cmeResults1D.typesXSRes.remove(item.ds_name)
				elif item.ds_name in self.cmeResults1D.typesXS:
					self.cmeResults1D.typesXS.remove(item.ds_name)
				if item.ds_name in self.cmeResults1D.pointTS:
					self.cmeResults1D.pointTS.remove(item.ds_name)
					if item.ds_name in self.cmeResults1D.typesXSRes:
						self.cmeResults1D.typesXSRes.remove(item.ds_name)
				elif item.ds_name in self.cmeResults1D.lineTS:
					self.cmeResults1D.lineTS.remove(item.ds_name)
				elif item.ds_name in self.cmeResults1D.regionTS:
					self.cmeResults1D.regionTS.remove(item.ds_name)
				elif item.ds_name in self.cmeResults1D.lineXS:
					self.cmeResults1D.lineXS.remove(item.ds_name)
			...
			for itm in openResultTypes.model().timeSeriesItem.children():
				...
				if openResultTypes.selectionModel().isSelected (index):
					count += 1
			if count != len(self.cmeResults1D.items1d):
		...
		if not skip:
			...
			if layer is not None:
				...
				# # if no scalar or vector cmern off dataset
				# if self.cmeResults2D.activeScalar is None:
				# 	rs.setActiveScalarDataset(QgsMeshDatasetIndex(-1, -1))
				# 	layer.setRendererSettings(rs)
				# if self.cmeResults2D.activeVector is None:
				...
				self.cmeView.cmePlot.cmePlotToolbar.averageMethodTSMenu.updateDefaultItem(self.cmeResults2D.activeScalar)
		else:
			# redraw plot
			if item.ds_type == 4 or item.ds_type == 5 or item.ds_type == 6:
				self.cmeView.cmePlot.updateCurrentPlot(CmePlot.TimeSeries)
				if self.cmeResults1D.typesXS:
					self.cmeView.cmePlot.updateCurrentPlot(CmePlot.CrossSection)
			elif item.ds_type == 7:
				self.cmeView.cmePlot.updateCurrentPlot(CmePlot.CrossSection)
			elif item.ds_type == 8:
				self.cmeView.cmePlot.updateCurrentPlot(CmePlot.CrossSection)
			else:
				self.cmeView.cmePlot.updateCurrentPlot(self.cmeView.tabWidget.currentIndex(), update='1d only')
	...
	def forceSelection1D(self):
		...
		for item in openResultTypes.model().timeSeriesItem.children():
			index = openResultTypes.model().item2index(item)
			if openResultTypes.selectionModel().isSelected (index):
				if item not in self.cmeResults1D.items1d:
					if item in self.activeResultsItems:
						self.activeResultsItems.remove(item)
	...
	def forceSelection2D(self):
		...
		for item in openResultTypes.model().mapOutputsItem.children():
			index = openResultTypes.model().item2index(item)
			if openResultTypes.selectionModel().isSelected (index):
				if item.ds_type == 1 and not self.cmeResults2D.activeScalar:
					# deselect
					openResultTypes.selectionModel().select(index, QT_ITEM_SELECTION_DESELECT)
				if item.ds_type == 2 and not self.cmeResults2D.activeVector:
					# deselect
					openResultTypes.selectionModel().select(index, QT_ITEM_SELECTION_DESELECT)

		# force lists
		self.activeResults = [x for x in self.activeResults if not CmeResults.isMapOutputType(x)]
		if self.cmeResults2D.activeScalar:
			self.activeResults.append(self.cmeResults2D.activeScalar)
		if self.cmeResults2D.activeVector:
			self.activeResults.append(self.cmeResults2D.activeVector)
	...
	def updateSecondaryAxisTypes(self, clickedItem):
		...
		if self.cmeView.cmePlot.cmePlotToolbar.fluxSecAxisButton.isChecked():
	...
	def getResult(self, index, **kwargs):
		...
		results = self.cmeView.cmeResults.results  # dict
	...
	@staticmethod
	def findTimeClosest_31600(cmeResults, key1, key2, key3, times=(), method='lower', units='h'):
		"""
		Finds the next available time after specified time
		"""
		...
		if key1 is not None and key2 is not None:
			if key1 in cmeResults.results:
				if '_1d' in key2:
					for type_1d in ['point_ts', 'line_ts', 'region_ts', 'line_lp']:
						if type_1d in cmeResults.results[key1]:
							if 'times' in cmeResults.results[key1][type_1d]:
								key2 = type_1d
								break

		if not times:
			if key1 not in cmeResults.results:
				return
			if key2 not in cmeResults.results[key1]:
				return
		if key3 is None:
			return

		if not times:
			if 'times' not in cmeResults.results[key1][key2]:
				return
			times = sorted(y[0] for x, y in cmeResults.results[key1][key2]['times'].items())

		if 'referenceTime' not in cmeResults.results[key1][key2]:
			return
		rt = cmeResults.results[key1][key2]['referenceTime']
		for i, time in enumerate(times):
			if units == 's':
	...
	@staticmethod
	def findDateClosest_31600(cmeResults, key1, key2, key3, dates=(), method='lower', units='h'):
		"""
        Finds the next available time after specified time
        """
	...
	def isMax(self, typ):
		"""
		Recmerns whether the result type is max or not. Can put 'scalar' or 'vector' to auto get active scalar or vector.

		:param type: str
		:return: bool -> True for max, False for not max
		"""

		maxResultTypes = self.cmeView.cmeResults.maxResultTypes  # list -> str

		if typ == 'scalar':
			return True if self.cmeResults2D.activeScalar in maxResultTypes else False
		elif typ == 'vector':
			return True if self.cmeResults2D.activeVector in maxResultTypes else False
		else:
	...
	def isMin(self, typ):
		...
		minResultTypes = self.cmeView.cmeResults.minResultTypes  # list -> str

		if typ == 'scalar':
			return True if self.cmeResults2D.activeScalar in minResultTypes else False
		elif typ == 'vector':
			return True if self.cmeResults2D.activeVector in minResultTypes else False
		else:
	...
	def removeResults(self, resList, **kwargs):
		...
		results2d = self.cmeView.cmeResults.cmeResults2D.results2d
		results1d = self.cmeView.cmeResults.cmeResults1D.results1d
		resultsParticles = self.cmeView.cmeResults.cmeResultsParticles.resultsParticles

		self.cmeResultsParticles.removeResults(resList)
		self.cmeResultsNcGrid.removeResults(resList)

		for res in resList:
			self.cmeView.crossSectionsFM.delByLayername(res)
				...
				if res in results1d:
					for res_ in results1d[res]:
						if isinstance(res_, ResData_GPKG):
							self.cmeResults1D.remove_gpkg_gis(res_)
					del results1d[res]
	...
	def updateTimeUnits(self):
		# cmern off x axis freezing on time series plot
		self.cmeView.cmePlot.cmePlotToolbar.viewToolbarTimeSeries.freezeXAxisAction.setChecked(False)
		self.cmeView.cmePlot.cmePlotToolbar.viewToolbarTimeSeries.freezeXYAxisAction.setChecked(False)

		meshLayers = findAllMeshLyrs()
		for ml in meshLayers:
			layer = coastalmeqgis_find_layer(ml)
			self.cmeResults2D.getResultMetaData(ml, layer, loadRenderStyle=False)
				...
				if result == layer.name():
					for result_type in self.results[result]:
						if self.isTimeSeriesType(result_type):
							self.results[result][result_type]['referenceTime'] = self.cmeResults2D.getReferenceTime(layer, self.cmeView.cmeOptions.defaultZeroTime)
	...
	def checkSelectedResults(self):
		...
		if selectedIndexes != sorted(self.activeResultsIndexes[:]):
			...
			self.cmeResults1D.items1d.clear()  # list -> Dataset_View Tree node selected dataset view tree node item
			self.cmeResults1D.typesTS.clear()  # list -> str selected 1D time series result types
			self.cmeResults1D.pointTS.clear()
			self.cmeResults1D.lineTS.clear()
			self.cmeResults1D.regionTS.clear()
			self.cmeResults1D.typesLP.clear()  # list -> str selected 1D long plot result types
			self.cmeResults1D.typesXS.clear()
			for index in selectedIndexes:
				...
				# elif item.ds_type == 4 or item.ds_type == 5 or item.ds_type == 6 or item.ds_type == 7:  # 1d result
					...
					if item.ds_type == 4 or item.ds_type == 5 or item.ds_type == 6:  # time series
						...
						# if self.cmeResults1D.activeType == 0:
						if 0 in self.cmeResults1D.activeType:
							self.cmeResults1D.typesTS.extend(self.cmeResults1D.pointTS[:])
						# elif self.cmeResults1D.activeType == 1:
						if 1 in self.cmeResults1D.activeType:
							self.cmeResults1D.typesTS.extend(self.cmeResults1D.lineTS[:] + self.cmeResults1D.pointTS[:])
						# elif self.cmeResults1D.activeType == 2:
						if 2 in self.cmeResults1D.activeType:
							self.cmeResults1D.typesTS.extend(self.cmeResults1D.regionTS[:])
						# else:
						# 	self.cmeResults1D.typesTS = []
					elif item.ds_type == 7:  # long plot
						nameAppend = '_LP'
						...
						nameAppend = '_CS'
						self.cmeResults1D.typesXS = self.cmeResults1D.lineTS[:]
						self.cmeResults1D.activeType.append(3)
					self.activeResults.append(item.ds_name + nameAppend)
	...
	def findMaxResultType(self, result: str, resultType: str) -> str:
		...
		for rtype in self.results[result]:
			if CmeResults.isMaximumResultType(rtype):
	...
	def isStatic(resultType: str,
		...
		if CmeResults.isMaximumResultType(resultType, lyr, groupIndex) \
				or CmeResults.isMinimumResultType(resultType, lyr, groupIndex):
			return True
	...
	def isTemporal(resultType: str,
		...
		return not CmeResults.isStatic(resultType, lyr, groupIndex)
	...
	def updateDateTimes2(self):
		...
		for resname, res in self.results.items():
			...
			for restype, resinfo in res.items():
				if CmeResults.isMapOutputType(restype):
					layer = coastalmeqgis_find_layer(resname)
					if layer is not None:
						if not meshprocessed:
							if qv >= 31300 and not resinfo['hadTemporalProperties']:
								layer.setReferenceTime(dt2qdt(datetime2timespec(self.cmeView.cmeOptions.zeroTime, self.loadedTimeSpec, QT_TIMESPEC_UTC), QT_TIMESPEC_UTC))
							self.cmeResults2D.getResultMetaData(resname, layer, resinfo['ext'], resinfo['hadTemporalProperties'], loadRenderStyle=False)
							meshprocessed = True
				elif CmeResults.isParticleType(restype):
					self.cmeResultsParticles.reloadTimesteps(resname)
				elif CmeResults.isTimeSeriesType(restype):
					if not tsprocessed:
						self.cmeResults1D.reloadTimesteps(resname)
						tsprocessed = True
		self.updateResultTypes()
	...
	def updateDateTimes_old(self):
		...
		zeroDate = self.cmeView.cmeOptions.zeroTime
		...
		# self.cmeView.cboTime.addItems([self._dateFormat.format(self.time2date[x]) for x in timeCopy])
		self.cmeView.cboTime.addItems([self._dateFormat.format(self.time2date_tspec[x]) for x in timeCopy])
	...
	def updateDateTimes_31600(self, **kwargs):
		...
		for result in self.results:
			...
			# 		self.cmeResults2D.getResultMetaData(result, layer)
			for restype in self.results[result]:
				rt = None
				if 'referenceTime' in self.results[result][restype]:
					if restype == '_nc_grid':
						lyr = self.cmeResultsNcGrid.results.get(result)
						if lyr is not None:
							lyr = lyr[0]
	...
	def updateQgsTime_old(self, time=None, qgsObject=None, timeSpec=None):
		...
		if qv >= 31300:
			CmeResults.layersToMethodLower(self.cmeResults2D.activeMeshLayers)
			if time is None:
				if self.activeTime is None:
					return
				try:
									...
									if CmeResults.isMapOutputType(restype):
										if 'times' in self.results[resname][restype]:
											...
											# if sorted([x for x in self.results[resname][restype]['times'].keys()]).index(self.activeTime) == len(self.results[resname][restype]['times']) - 1:
												CmeResults.layersToMethodHigher(self.cmeResults2D.activeMeshLayers)
												if tt > 0:
													tt *= -1
												break
				except:
	...
	def updateQgsTime_31600(self, time=None, qgsObject=None, timeSpec=None):
		...
		if qgsObject is not None:
			if qv >= 31300 and self.iface is not None:
				try:
					self.iface.mapCanvas().temporalRangeChanged.disconnect(self.cmeView.qgsTimeChanged)
				except:
					pass
			qgsObject.setTemporalRange(dtr)
			qgsObject.refresh()
			if qv >= 31300 and self.iface is not None:
				self.iface.mapCanvas().temporalRangeChanged.connect(self.cmeView.qgsTimeChanged)
	...
	def getCmeViewTimeFromQgsTime(self):
		qv = Qgis.QGIS_VERSION_INT
		if qv < 31600:
			return self.getCmeViewTimeFromQgsTime_old()
		else:
			return self.getCmeViewTimeFromQgsTime_31600()

	def getCmeViewTimeFromQgsTime_old(self):
		"""
	...
	def getCmeViewTimeFromQgsTime_31600(self):
		if self.iface is not None:
			qdt = self.iface.mapCanvas().temporalRange().begin()  # QDateTime
		else:
			qdt = QgsDateTimeRange()
		...
		if self.loadedTimeSpec > 0:
			pdt = datetime2timespec(pdt, self.timeSpec, self.loadedTimeSpec)
		return CmeResults.findDateClosest_31600(self, None, None, pdt, modelDates, 'closest', self.cmeView.cmeOptions.timeUnits)
	...
	def addCrossSectionLayerToResults(self, lyr):
		...
		if updateOpenResults:
			...
			self.cmeView.cmePlot.updateCurrentPlot(CmePlot.CrossSection, plot='1d only')
	...
	def isMapOutputType(resultType):
		"""Recmerns true if resultType is a Map Output"""

		a = [True for x in CmeResults.OtherTypes if x in resultType.lower()]
		return len(a) == 0

	@staticmethod
	def isParticleType(resultType):
		"""Recmerns true if resultType is a ptm output"""
	...
	def isTimeSeriesType(resultType):
		...
		a = [True for x in [x for x in CmeResults.OtherTypes if '_particles' not in x] if x in resultType.lower()]
		return len(a) > 0
	...
	def updateTemporalProperties(self):
		...
		#		if CmeResults.isMapOutputType(restype):
		#			layer = coastalmeqgis_find_layer(res)
		#			layer.setReferenceTime(dt2qdt(self.cmeResults2D.getReferenceTime(layer),
	...
	def layersToMethodLower(layers):
		"""cmerns layers to find next lower timestep"""
	...
	def dateToTimeInCombobox(self, inputDate):
		...
		dateFormat = self.cmeView.cmeOptions.xAxisDates
		time = 0
		for i in range(self.cmeView.cboTime.count()):  # find closest
			...
			if dateFormat:
				...
				t = self.cmeView.cmeResults.timekey2time[timeKey] if timeKey in self.cmeView.cmeResults.timekey2time else float(
					timeKey)
				if self.cmeView.cmeOptions.timeUnits == 's':
			...
			if date > inputDate:
				...
				if diff <= diff2:
					...
					if dateFormat:
						...
						timeKey = convertFormattedTimeToTime(item)
						time = self.cmeView.cmeResults.timekey2time[
							timeKey] if timeKey in self.cmeView.cmeResults.timekey2time else float(timeKey)
		...
		if dateFormat:
			if item == '':
				return self.cmeView.cmeOptions.zeroTime, i
			date = datetime.strptime(item, self.dateFormat)
			return date, i
		timeKey = convertFormattedTimeToTime(item)
		time = self.cmeView.cmeResults.timekey2time[timeKey] if timeKey in self.cmeView.cmeResults.timekey2time else float(
			timeKey)
		return time, 0
