...
from ..forms.coastalme_plotting_dock import Ui_Cmeplot
...
from .coastalmeqgis_cmemenubar import CmeMenuBar
from .coastalmeqgis_cmeoptions import CmeOptions
from .coastalmeqgis_cmemenucontext import CmeContextMenu
from .coastalmeqgis_cmeproject import CmeProject
from ..coastalmeqgis_library import (
    convertTimeToFormattedTime,
    is1dTable,
    findTableLayers,
    is1dNetwork,
    isPlotLayer,
...
from ..coastalmeqgis_dialog import coastalmeqgis_meshSelection_dialog
from ..COASTALME_XS import XS
from ..COASTALME_1dTa import HydTables
from datetime import timedelta
from ..COASTALME_FM_data_provider import FM_XS
import numpy as np
...
from ..compatibility_routines import (
    QT_DOCK_WIDGET_AREA_NONE,
    QT_RIGHT_BUTTON,
    QT_LEFT_BUTTON,
    QT_ITEM_SELECTION_SELECT,
    QT_HORIZONTAL,
)


class CmeView(QDockWidget, Ui_Cmeplot):
    def __init__(self, iface=None, **kwargs):
        ...
        self.setupUi(self)
        self.wdg = Ui_Cmeplot.__init__(self)  # Initialise cmeplot dock ui
        ...
        playIcon = QIcon(
            os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "icons", "play_button.png"
            )
        )
        ...
        # project
        self.cmeProject = None

        # options
        self.cmeOptions = CmeOptions()

        # results class
        self.cmeResults = CmeResults(self)

        # plot class
        self.cmePlot = CmePlot(self)

        # main menu bar
        removeCmeview = kwargs["removeCmeview"] if "removeCmeview" in kwargs else None
        reloadCmeview = kwargs["reloadCmeview"] if "reloadCmeview" in kwargs else None
        self.cmeMenuBar = CmeMenuBar(
            self,
            removeCmeview=removeCmeview,
            reloadCmeview=reloadCmeview,
            layout=self.mainMenu,
        )
        # self.cmeMenuBar.setNativeMenuBar(False)
        self.cmeMenuBar.loadFileMenu()
        self.cmeMenuBar.loadViewMenu(0)
        self.cmeMenuBar.loadSettingsMenu(0)
        self.cmeMenuBar.loadExportMenu(0)
        self.cmeMenuBar.loadResultsMenu()
        self.cmeMenuBar.loadHelpMenu()
        # self.cmeMenuBar.connectMenu()

        # secondary menu bar
        self.cmeMenuBarSecond = CmeMenuBar(
            self,
            removeCmeview=removeCmeview,
            reloadCmeview=reloadCmeview,
            menu_bar=self.cmeMenuBar,
            layout=self.mainMenuSecond,
        )
        self.cmeMenuBarSecond.loadFileMenu()
        self.cmeMenuBarSecond.loadViewMenu(0)
        self.cmeMenuBarSecond.loadSettingsMenu(0)
        self.cmeMenuBarSecond.loadExportMenu(0)
        self.cmeMenuBarSecond.loadHelpMenu()
        self.cmeMenuBarSecond.loadResultsMenu()

        # context menu
        self.cmeContextMenu = CmeContextMenu(self)
        self.cmeContextMenu.loadPlotMenu(0)
        self.cmeContextMenu.loadResultsMenu()
        self.cmeContextMenu.loadResultTypesMenu()
        self.cmeContextMenu.connectMenu()

        # options
        # self.cmeOptions = coastalmeqgis_cmeoptions.CmeOptions()
        ...
        self.cmeResults.cmeResults2D.loadOpenMeshLayers()

        # check for already open _TS layers
        self.cmeResults.cmeResults1D.loadOpenTSLayers()
        ...
        self.setTabVisible(CmePlot.CrossSection1D, False)

        self.SecondMenu.setVisible(False)

        # show as dates
        self.cbShowAsDates.setChecked(self.cmeOptions.xAxisDates)

        narrowWidth = (
            self.pbShowPlotWindow.minimumSizeHint().width()
            + self.mainMenuSecond_2.minimumSizeHint().width()
        )
        self.dockWidgetContents.setMinimumWidth(narrowWidth)

        if qv >= 31600:
            self.cmeResults.initialiseTemporalController()
        ...
        if QSettings().value(
            ...
            == "previous_state"
            and QSettings().value("COASTALME/cmeview_previouslayout", "plot") == "narrow"
        ):
            self.plotWindowVisibilityToggled(initialisation=True)

    def __del__(self):
        self.qgisDisconnect()
        # self.cmeMenuBar.disconnectMenu()
        # self.cmeMenuBar.clear()
        # self.cmeMenuBarSecond.disconnectMenu()
        # self.cmeMenuBarSecond.clear()
        self.cmeResults.removeResults(
            [
                self.OpenResults.item(i).text()
                for i in range(self.OpenResults.count())
                if self.OpenResults.item(i).text()
            ],
    ...
    def currentLayerChanged(self):
        ...
        if self.iface is not None:
            ...
            if (
                ...
                and not self.cmeResults.cmeResults1D.activeType
            ):
                self.cmeResults.updateActiveResultTypes(
                    None, geomType=self.currentLayer.geometryType()
                )
        ...
        if self.currentLayer is not None:
            ...
            self.loadXsType()
            self.cmePlot.cmePlot1D.plot1dCrossSection(bypass=True, draw=True)
    ...
    def loadXsType(self, lyr=None):
        """
        Load COASTALME 1D cross section in plotter if available.
        """
        ...
        if is1dTable(lyr):
            ...
            self.cmeResults.addCrossSectionLayerToResults(lyr)
    ...
    def loadXsSelections(self, lyr=None):
        """
        Load COASTALME 1D cross sections from selected feacmeres
        """
        ...
        if is1dTable(lyr):
            for crossSection in crossSections:
                ...
                for sel in lyr.selectedFeatures():
                    ...
                    if (
                        source not in crossSection.source
                        or not crossSection.data[
                            crossSection.source.index(source)
                        ].loaded
                    ):
    ...
    def setTsTypesEnabled(self, context=None):
        ...
        if self.currentLayer is None:
            ...
            # self.cmeResults.cmeResults1D.activeType = -1
            # return
        ...
        if isinstance(self.currentLayer, QgsVectorLayer):
            ...
            try:
                if (
                    ...
                    not in self.cmeResults.cmeResults1D.activeType
                ):
                    self.cmeResults.cmeResults1D.activeType.append(
                        self.currentLayer.geometryType()
                    )
            except:
                pass
        ...
        if (
            is1dTable(self.currentLayer)
            or is1dNetwork(self.currentLayer)
            or isPlotLayer(self.currentLayer)
            or isTSLayer(self.currentLayer, self.cmeResults.results)
        ):
            # self.OpenResultTypes.model().setEnabled(4, 5, 6, 7, 8)
            # self.cmeResults.cmeResults1D.activeType = 3
            self.cmeResults.updateActiveResultTypes(
                None, geomType=self.currentLayer.geometryType()
            )
        ...
        ## Change the enabled/disabled stacmes based on selected layer
        # if self.currentLayer.type() == QgsMapLayer.VectorLayer:  # vector layer
        # resVersion = []
        # for result in self.OpenResults.selectedItems():
        # if result.text() in self.cmeResults.cmeResults1D.results1d.keys():
        # resVersion.append(self.cmeResults.cmeResults1D.results1d[result.text()].formatVersion)
        # if 2 in resVersion:
        # if ' PLOT ' in self.currentLayer.name() or '_PLOT_' in self.currentLayer.name():
        # if self.tabWidget.currentIndex() == 0:
        # if self.currentLayer.geometryType() == 0:
        # self.OpenResultTypes.model().setEnabled(4, 5, 6, 7, 8)
        # self.cmeResults.cmeResults1D.activeType = 0
        # elif self.currentLayer.geometryType() == 1:
        # self.OpenResultTypes.model().setEnabled(4, 5, 6, 7, 8)
        # self.cmeResults.cmeResults1D.activeType = 1
        # elif self.currentLayer.geometryType() == 2:
        # self.OpenResultTypes.model().setEnabled(4, 5, 6, 7, 8)
        # self.cmeResults.cmeResults1D.activeType = 2
        # self.cmeResults.updateActiveResultTypes(None, geomType=self.currentLayer.geometryType())
    ...
    def layersAdded(self, addedLayers):
        """
        Triggered when layers are added to the project. Will check if they are mesh layers and add to cmeview if
        they are.
        ...
        for layer in addedLayers:
            if isinstance(layer, QgsMeshLayer):
                self.cmeResults.cmeResults2D.loadOpenMeshLayers(layer=layer.name())
            elif isinstance(layer, QgsVectorLayer):
                self.cmeResults.cmeResults1D.loadTSResultLayer(layer)
    ...
    def layersRemoved(self, removedLayers):
        ...
        for rlayer in removedLayers:
            ...
            # layer = coastalmeqgis_find_layer(rlayer, search_type='layerId')

            if layer in self.cmeResults.cmeResults2D.copied_results:
                if self.cmeOptions.del_copied_res:
                    self.cmeResults.cmeResults2D._copied_res_for_rem.append(
                        self.cmeResults.cmeResults2D.copied_results[layer]
                    )
                del self.cmeResults.cmeResults2D.copied_results[layer]
            ...
            if layer is not None and isinstance(layer, QgsMeshLayer):
                for i in reversed(range(self.OpenResults.count())):
                    ...
                    if itemName == layer.name():
                        self.cmeResults.cmeResults2D.removeResults([itemName])
            elif (
                layer is not None
                and layer.name() in self.cmeResults.cmeResultsParticles.resultsParticles
            ):
                self.cmeResults.cmeResultsParticles.removeResults(
                    [layer.name()], remove_vlayer=False
                )
            elif (
                layer is not None
                and layer.name() in self.cmeResults.cmeResultsNcGrid.results
            ):
                self.cmeResults.cmeResultsNcGrid.removeResults(
                    [layer.name()], remove_layer=False
                )
            elif layer is not None and isTSLayer(layer, self.cmeResults.results):
                name = layer.dataProvider().dataSourceUri()
                if re.findall(
                    re.escape(r".gpkg|layername="), name, flags=re.IGNORECASE
                ):
                    name = re.split(
                        re.escape(r".gpkg|layername="), name, flags=re.IGNORECASE
                    )[-1]
                ...
                del self.cmeResults.cmeResults1D.results1d[name]
                del self.cmeResults.results[name]
        ...
        if update_1d_ids:
            self.cmeResults.cmeResults1D.updateSelectedResults(removedLayers)

        if self.cmeResults.cmeResults2D._copied_res_for_rem:
            self.timer = QTimer()
            self.timer.setSingleShot(True)
            self.timer.timeout.connect(self.cmeResults.cmeResults2D.clear_cached_results)
    ...
    def loadProject(self):
        ...
        self.cmeProject = CmeProject(self)
        self.cmeProject.load()
        self.cmeResults.updateTimeUnits()
    ...
    def maxResultTypesChanged(self, event):
        ...
        self.cmeResults.updateMinMaxTypes(event, "max")
        ...
        for index in self.cmeResults.activeResultsIndexes:
            selection.select(index, index)
            self.OpenResultTypes.selectionModel().select(selection, flags)

        # redraw plot and re-render map
        self.cmePlot.updateCrossSectionPlot()
        self.cmePlot.updateTimeSeriesPlot()
        self.renderMap()
    ...
    def minResultTypesChanged(self, event):
        ...
        # update list of types with max activated
        self.cmeResults.updateMinMaxTypes(event, "min")
    ...
    def playThroughTimesteps(self):
        ...
        if self.btnTimePlay.isChecked():
            if self.cmeResults.activeResults:
                self.timer = QTimer()
                self.timer.setInterval(
                    int(self.cmeOptions.playDelay * 1000)
                )  # sec to ms
    ...
    def plottingViewChanged(self):
        ...
        self.cmePlot.cmePlotToolbar.setToolbarActive(self.tabWidget.currentIndex())

        # update the enabled result types
        # self.currentLayerChanged()
        # self.setTsTypesEnabled()
        # self.cmePlot.updateCurrentPlot(self.tabWidget.currentIndex(), plot='1d only')
    ...
    def moveOpenResults(self, layoutType):
        ...
        if layoutType.lower() == "narrow":
            ...
            self.ResultTypeSplitter.setSizes(
                [int(totalHeight * 9 / 10), int(totalHeight * 1 / 10)]
            )
    ...
    def onDockLocationChange(self, area=None):
        """
        Saves the new location of COASTALME Viewer dock. Only used, however, if
        ...
        QSettings().setValue("COASTALME/cmeview_docklocation", area)

        if self.iface is not None:
            if self.iface.mainWindow().tabifiedDockWidgets(self):
                QSettings().setValue("COASTALME/cmeview_isdocktabified", True)
                tabifiedWith = [
                    x.windowTitle()
                    for x in self.iface.mainWindow().tabifiedDockWidgets(self)
                ]
                QSettings().setValue("COASTALME/cmeview_tabifiedwith", tabifiedWith)
            else:
                QSettings().setValue("COASTALME/cmeview_isdocktabified", False)
                QSettings().setValue("COASTALME/cmeview_tabifiedwith", [])

    def onTopLevelChange(self, topLevel=None):
        """
        Saves the location of COASTALME Viewer if the dock is floating
        or not.
        """
        ...
        QSettings().setValue("COASTALME/cmeview_isdockfloating", topLevel)
    ...
    def plotWindowVisibilityToggled(self, initialisation=False):
        ...
        if initialisation:
            self.originalWidth = (
                self.PlotLayout.minimumSizeHint().width()
                + self.ResultLayout.minimumSizeHint().width()
            )
            ...
            if self.iface is not None:
                ...
                if "COASTALME Viewer" in docks:
                    self.iface.mainWindow().resizeDocks(
                        [self], [narrowWidth], QT_HORIZONTAL
                    )

            return

        if self.PlotLayout.isVisible():
            QSettings().setValue("COASTALME/cmeview_previouslayout", "narrow")
            self.PlotLayout.setVisible(False)
            self.SecondMenu.setVisible(True)
            self.OpenResultsWidget.setVisible(False)
            self.moveOpenResults("narrow")
            ...
            QSettings().setValue("COASTALME/cmeview_previouslayout", "plot")
    ...
    def projectCleared(self):
        ...
        self.cmeResults.results.clear()
        self.cmeResults.cmeResults2D.results2d.clear()
        self.cmeResults.cmeResults1D.results1d.clear()
        self.OpenResults.clear()
        self.cmeResults.resetResultTypes()
    ...
    def projectSaved(self):
        """
        Saves Cmeview settings to project
    ...
    def qgisConnect(self):
        ...
        if not self.connected:
            ...
            if qv >= 31300:
                if self.iface is not None:
                    self.iface.mapCanvas().temporalRangeChanged.connect(
                        self.qgsTimeChanged
                    )

            # results
            # self.OpenResults.itemClicked.connect(lambda: self.resultsChanged('item clicked'))
            self.resultSelectionChangeSignal = (
                self.OpenResults.itemSelectionChanged.connect(
                    lambda: self.resultsChanged("item clicked")
                )
            )

            # result types
            self.OpenResultTypes.secondAxisClicked.connect(
                self.secondaryAxisResultTypesChanged
            )
            ...
            # Plotting buttons
            self.cbShowCurrentTime.stateChanged.connect(
                lambda: self.cmePlot.clearPlot2(
                    CmePlot.TimeSeries, CmePlot.DataCurrentTime
                )
            )
            self.cmePlot.cmePlotToolbar.fluxSecAxisButton.released.connect(
                lambda: self.secondaryAxisResultTypesChanged(None)
            )
    ...
    def qgisDisconnect(self, completely_remove=False):
        ...
        if completely_remove:
            # self.cmeMenuBar.qgisDisconnect()
            # self.cmeMenuBar.clear()
            # self.cmeMenuBarSecond.qgisDisconnect()
            # self.cmeMenuBarSecond.clear()
            self.cmeContextMenu.qgisDisconnect()
            self.cmePlot.cmePlotToolbar.qgisDisconnect()

        if self.connected or completely_remove:
            try:
                self.pbHidePlotWindow.clicked.disconnect(
                    self.plotWindowVisibilityToggled
                )
            except:
                pass
            try:
            ...
            if qv >= 31300:
                try:
                    if self.iface is not None:
                        self.iface.mapCanvas().temporalRangeChanged.disconnect(
                            self.qgsTimeChanged
                        )
                except:
                    pass
            ...
            try:
                ...
                pass
            try:
                self.cmePlot.cmePlotToolbar.fluxSecAxisButton.released.disconnect()
            except:
                pass
            ...
            for layer in findAllMeshLyrs():
                try:
                    ...
                    pass
                try:
                    signal = self.cmeView.cmeResults.cmeResults2D.layer_style_changed_signals.get(
                        layer.id()
                    )
                    if signal is not None:
                        layer.rendererChanged.disconnect(signal)
                        del self.cmeView.cmeResults.cmeResults2D.layer_style_changed_signals[
                            layer.id()
                        ]
                except:
                    pass
                if Qgis.QGIS_VERSION_INT >= 32800:
                    try:
                        signal = self.cmeView.cmeResults.cmeResults2D.layer_reloaded_signals.get(
                            layer.id()
                        )
                        if signal is not None:
                            layer.reloaded.disconnect(signal)
                            del self.cmeView.cmeResults.cmeResults2D.layer_reloaded_signals[
                                layer.id()
                            ]
                    except:
                        pass
            ...
            # plotting
            try:
                self.cmePlot.verticalMesh_action.triggered.disconnect(
                    self.cmePlot.vmeshToggled
                )
            except:
                pass
    ...
    def refreshCurrentPlot(self, **kwargs):
        ...
        self.cmePlot.updateCurrentPlot(plotNo, update=update)
    ...
    def renderMap(self):
        ...
        self.cmeResults.cmeResults2D.renderMap()
    ...
    def repaintRequested(self, layer):
        ...
        if layer in self.cmeResults.cmeResults2D.activeMeshLayers:
            rs = layer.rendererSettings()
            if rs.nativeMeshSettings().isEnabled():
                self.cmeOptions.showGrid = True
                self.cmePlot.cmePlotToolbar.meshGridAction.setChecked(True)
            else:
                self.cmeOptions.showGrid = False
                self.cmePlot.cmePlotToolbar.meshGridAction.setChecked(False)
            if rs.triangularMeshSettings().isEnabled():
                self.cmeOptions.showTriangles = True
            else:
                self.cmeOptions.showTriangles = False
    ...
    def resultsChanged(self, *args):
        ...
        if self.resultChangeSignalCount == 0:
            ...
            self.cmeResults.cmeResults2D.updateActiveMeshLayers(*args)

            # update 2D results class
            self.cmeResults.updateResultTypes(select_first_dataset=False)
            ...
            if args:
                if args[0] == "force refresh":
                    # render only selected results
                    self.cmeResults.cmeResults2D.updateActiveMeshLayers()

                    # update 2D results class
                    self.cmeResults.updateResultTypes()
    ...
    def resultTypesChanged(self, event):
        ...
        if event["button"] == QT_LEFT_BUTTON:
            if not self.doubleClickEvent:
                self.cmeResults.updateActiveResultTypes(
                    event["modelIndex"], skip_already_selected=event["drag_event"]
                )
        self.doubleClickEvent = False
    ...
    def resultTypeDoubleClicked(self, event):
        ...
        # if len(self.cmeResults.cmeResults2D.activeMeshLayers) > 1:
        # 	if self.iface is not None:
        # 		self.meshDialog = coastalmeqgis_meshSelection_dialog(self.iface, self.cmeResults.cmeResults2D.activeMeshLayers,
        ...
        # else:
        meshLayer = self.cmeResults.cmeResults2D.activeMeshLayers[0]
        ...
        if event is not None:
            ...
            if (
                self.cmeContextMenu.resultTypeContextItem.parentItem.ds_name
                == "Map Outputs"
            ):
                if self.iface is not None:
                    self.iface.showLayerProperties(meshLayer)
    ...
    def secondaryAxisResultTypesChanged(self, event):
        ...
        self.cmeResults.updateSecondaryAxisTypes(event)

        # redraw plot
        # self.cmePlot.updateCurrentPlot(self.tabWidget.currentIndex(), update='1d only')
        self.cmePlot.changeLineAxis(event)
    ...
    def selectionChanged(self):
        ...
        if (
            sender
            and isinstance(sender, QgsVectorLayer)
            and sender != self.currentLayer
        ):
            if (
                not sender.selectedFeatureCount()
                and sender.id() in self.layer_selection_signals
            ):
        ...
        self.cmeResults.cmeResults1D.updateSelectedResults()

        # make sure selected result match active results
        self.cmeResults.checkSelectedResults()

        # plot
        self.cmePlot.cmePlot1D.plot1dResults()
    ...
    def timeSliderChanged(self):
        ...
        self.cmeResults.updateActiveTime()

        self.renderMap()

        # update red time slider on plot
        # if not self.cmePlot.timeSeriesPlotFirst:
        if self.cbShowCurrentTime.isChecked():
            self.cmePlot.clearPlot2(
                CmePlot.TimeSeries, CmePlot.DataCurrentTime, clear_selection=False
            )
            # self.cmePlot.drawPlot(CmePlot.TimeSeries, [], [], [], [], refresh_only=True)
        # update long profile / cross section plots with new timestep
        if not self.cmePlot.profilePlotFirst:
            self.cmePlot.updateCrossSectionPlot()
        # vertical profile
        if not self.cmePlot.verticalProfileFirst:
            self.cmePlot.updateVerticalProfilePlot()
    ...
    def qgsTimeChanged_old(self):
        ...
        if self.iface is not None:
            if (
                self.cmeResults.timeSpec
                != self.iface.mapCanvas().temporalRange().begin().timeSpec()
            ):
                self.cmeResults.updateTemporalProperties()
                return

            t = self.cmeResults.getCmeViewTimeFromQgsTime()
            cme = None
            if type(t) is datetime:
                if self.cmeOptions.xAxisDates:
                    cme = self.cmeResults._dateFormat.format(t)  # time formatted
                else:
                    if t in self.cmeResults.date2time:
                        t = self.cmeResults.date2time[t]
                    elif t in self.cmeResults.date_tspec2date:
                        t = self.cmeResults.date_tspec2time[t]
            if cme is None:
                cme = convertTimeToFormattedTime(
                    t, unit=self.cmeOptions.timeUnits
                )  # time formatted
            for i in range(self.cboTime.count()):
                if self.cboTime.itemText(i) == cme:
                    self.cboTime.setCurrentIndex(i)
                    return
    ...
    def qgsTimeChanged_31600(self):
        ...
        if type(t) is datetime:
            if self.cmeOptions.xAxisDates:
                ...
                for i, (cboTime, timeKey) in enumerate(
                    self.cmeResults.cboTime2timekey.items()
                ):
                    time = self.cmeResults.timekey2time[timeKey]
                    cme = cboTime
                    if self.cmeOptions.timeUnits == "s":
                        date = self.cmeOptions.zeroTime + timedelta(seconds=time)
                    else:
                        try:
                            date = self.cmeOptions.zeroTime + timedelta(hours=time)
                    ...
                    if date > t:
                        ...
                        if diff < diff2:
                            cme = [x for x in self.cmeResults.cboTime2timekey][
                                max(0, i - 1)
                            ]
                            break
                        else:
    ...
    def showAsDatesToggled(self):
        self.cmeOptions.xAxisDates = self.cbShowAsDates.isChecked()
        self.cmeResults.updateTimeUnits()
        self.cmeResults.updateDateTimes()

    def iconSizeChanged(self, size):
        self.toolbarIconSizeChanged(
            size, self.cmePlot.cmePlotToolbar.mpltoolbarTimeSeries, self.mplToolbarFrame
        )
        self.toolbarIconSizeChanged(
            size, self.cmePlot.cmePlotToolbar.mpltoolbarLongPlot, self.mplToolbarFrame
        )
        self.toolbarIconSizeChanged(
            size, self.cmePlot.cmePlotToolbar.mpltoolbarLongPlot, self.mplToolbarFrame
        )
        self.toolbarIconSizeChanged(
            size,
            self.cmePlot.cmePlotToolbar.mpltoolbarVerticalProfile,
            self.mplToolbarFrame,
        )
        self.toolbarIconSizeChanged(
            size,
            self.cmePlot.cmePlotToolbar.mapOutputPlotToolbar,
            self.MapOutputPlotFrame,
        )
        self.toolbarIconSizeChanged(
            size, self.cmePlot.cmePlotToolbar.mesh3dPlotToolbar, self.Mesh3DToolbarFrame
        )
        for i in range(4):
            _, viewToolbar, _ = self.cmePlot.cmePlotToolbar.plotNoToToolbar[i]
            self.toolbarIconSizeChanged(
                size, viewToolbar.viewToolbar, self.ViewToolbarFrame
            )

    def toolbarIconSizeChanged(self, size=None, toolbar=None, frame=None):
        qv = Qgis.QGIS_VERSION_INT

        w = self.cmeOptions.iconSize
        if qv >= 31600:
            w = int(QgsApplication.scaleIconSize(self.cmeOptions.iconSize, True))
    ...
    def reorderOpenResults(self, index, action):
        ...
        if action == "up":
            ...
            self.OpenResults.insertItem(
                min(index.row() + 1, self.OpenResults.count()), item
            )
        elif action == "top":
        ...
        self.resultSelectionChangeSignal = (
            self.OpenResults.itemSelectionChanged.connect(
                lambda: self.resultsChanged("selection changed")
            )
        )
