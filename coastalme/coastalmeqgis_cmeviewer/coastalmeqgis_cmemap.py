...
from ..coastalmeqgis_cmeviewer.coastalmeqgis_cmeanimation import (ImagePropertiesDialog, PlotProperties,
                                                               TextPropertiesDialog, prepare_composition,
                                                               prepare_composition_from_template, createText)
from ..coastalmeqgis_library import (convertTimeToFormattedTime, convertFormattedTimeToTime,
                                  browse)
...
def getTimes(results, layer, stype, vtype, units='h', xAxisDates=False, cmeResults=None):
	"""
	get available times
		...
		for rtype, ts in result.items():
			if '_ts' not in rtype and '_lp' not in rtype:
				if CmeResults.isMaximumResultType(rtype):
					t = CmeResults.stripMaximumName(rtype)
					if t == typ:
						if 'Max' not in times:
							times.insert(0, 'Max')
				else:
						...
						if x == -99999:
							...
							if xAxisDates:
								if cmeResults is not None:
									# if x in cmeResults.time2date:
									if x in cmeResults.time2date_tspec:
										time = cmeResults.time2date_tspec[x]
										time = cmeResults._dateFormat.format(time)
							else:
...
def makeMap(cfg, iface, progress_fn=None, dialog=None, preview=False, iteration=0):
	...
	if layoutcfg['type'] == 'file':
		...
		prepare_composition(layout, time, cfg, layoutcfg, extent, layers, crs, os.path.dirname(imgfile), dialog,
		                    False, False, cmeResults=cmeResults, meshLayer=l)
...
class CmeMapDialog(QDialog, Ui_MapDialog):
	"""
	...
	def __init__(self, CmeView, **kwargs):
		...
		self.btnBrowseTemplate.clicked.connect(lambda: browse(self, 'existing file', "COASTALME/animation_template", "QGIS Print Template", "QGIS Print Layout (*.qpt)", self.editTemplate))
	...
	def populateGraphics(self):
		...
		# lines = self.cmeView.cmePlot.cmeCrossSection.rubberBands
		...
		#
		# lines = self.cmeView.cmePlot.cmeFlowLine.rubberBands
		# for i, line in enumerate(lines):
		# 	rowNo = self.tableGraphics.rowCount()
		# 	self.tableGraphics.setRowCount(rowNo + 1)
		...
		#
		# points = self.cmeView.cmePlot.cmeTSPoint.points
	...
	def addPlot(self, e=False, item1=None, item2=None, item3=None, dialog=None):
		...
		if dialog is None:
			dialog = PlotProperties(self, item1, item2, self.cmeView.cmeOptions.xAxisDates, caller='maps')
	...
	def insertPlotRow(self, index=None, loc=INSERT_BEFORE):
		...
		if index is not None:
			...
			if loc == CmeMapDialog.INSERT_BEFORE:
				j = index
			else:
				j = index + 1
	...
	def setMapTableSignalProperties(self):
		...
		btnSignalResult = {'parent': self, 'browse type': 'existing file', 'key': 'COASTALME/map_input',
		                   'title': "Result Layer", 'file types': resultTypes}
		btnSignalOutput = {'parent': self, 'browse type': 'output file', 'key': 'COASTALME/map_output',
		                   'title': "Output Map", 'file types': exportTypes}
		

		items = []
		for i in range(self.cmeView.OpenResults.count()):
			item = self.cmeView.OpenResults.item(i).text()
			items.append(item)
	...
	def updateMapResultTypes(self, result, row):
		...
		scalarTypes, vectorTypes = getResultTypes(self.cmeView.cmeResults.results, result)
	...
	def updateMapTimes(self, result, row):
		...
		times = getTimes(self.cmeView.cmeResults.results, result, scalar,
		                 vector, self.cmeView.cmeOptions.timeUnits, self.cmeView.cmeOptions.xAxisDates,
	...
	def mapTableMenu(self, pos):
		...
		self.mapTableInsertRowBefore.triggered.connect(lambda: self.insertMapRow(index, CmeMapDialog.INSERT_BEFORE))
		self.mapTableInsertRowAfter.triggered.connect(lambda: self.insertMapRow(index, CmeMapDialog.INSERT_AFTER))
	...
	def plotTableMenu(self, pos):
		...
		self.plotTableInsertRowBefore.triggered.connect(lambda: self.insertPlotRow(index, CmeMapDialog.INSERT_BEFORE))
		self.plotTableInsertRowAfter.triggered.connect(lambda: self.insertPlotRow(index, CmeMapDialog.INSERT_AFTER))
	...
	def imageTableMenu(self, pos):
		...
		self.imageTableInsertRowBefore.triggered.connect(lambda: self.insertImageRow(index, CmeMapDialog.INSERT_BEFORE))
		self.imageTableInsertRowAfter.triggered.connect(lambda: self.insertImageRow(index, CmeMapDialog.INSERT_AFTER))
	...
	def check(self, preview=False):
		...
		for i in range(self.tableMaps.rowCount()):
			...
			if not result:
				...
				return
			if result not in self.cmeView.cmeResults.results:
	...
	def run(self, preview):
		...
		self.cmeView.qgisDisconnect()
		
		if not preview and self.cmeView.iface is not None:
		...
		self.cmeView.cmeResults.cmeResults2D.activeMeshLayers = []
		...
		d = {'img_size': (w, h),
		     ...
		     'datetime': self.cmeView.cmeOptions.xAxisDates,
		     'dateformat': self.cmeView.cmeOptions.dateFormat,
		     'dynamic axis update': self.cbDynamicAxisLimits.isChecked(),
		     'cmeresults': self.cmeView.cmeResults,
		     }
		tmpdir = tempfile.mkdtemp(suffix='cmelw_maps')
		d['tmpdir'] = tmpdir
		...
		for i in range(count):
			...
			if scalar not in rendering:
				...
				d['rendered'] = False
				if layer not in self.cmeView.cmeResults.results and \
						os.path.splitext(os.path.basename(layer))[0] not in self.cmeView.cmeResults.results:
					imported = self.cmeView.cmeMenuBar.cmeMenuFunctions.load2dResults(result_2D=[[layer]])
					if not imported:
						continue
					layer = coastalmeqgis_find_layer(self.cmeView.OpenResults.item(self.cmeView.OpenResults.count() - 1).text())
				else:
					if layer in self.cmeView.cmeResults.results:
						layer = coastalmeqgis_find_layer(layer)
					else:
						layer = coastalmeqgis_find_layer(os.path.splitext(os.path.basename(layer))[0])
				# active scalar and vector index
				rt = self.cmeView.cmeResults.results[self.layer.name()][
				...
				if time.lower() != 'max' and time.lower() != 'min':
					if qv < 31600:
						if self.cmeView.cmeOptions.xAxisDates:
							timeConverted = self.cmeView.cmePlot.convertDateToTime(time,
						...
						if self.cmeView.cmeOptions.xAxisDates:
							timeConverted = datetime.strptime(time, self.cmeView.cmeResults.dateFormat)
						else:
							timeConverted = convertFormattedTimeToTime(time,
							                                           unit=self.cmeView.cmeOptions.timeUnits)
		...
		for i in range(count):
			...
			self.cmeView.cmeResults.cmeResults2D.activeMeshLayers = []
			self.cmeView.cmeResults.cmeResults2D.activeMeshLayers.append(layer)
			d['layer'] = layer
			
			# iterate through layers and cmern off any mesh layers not needed
			layersToCmernOff = []
			#for mapLayer in self.canvas.layers():
			for mapLayer in self.layers:
				if isinstance(mapLayer, QgsMeshLayer):
					if mapLayer != layer:
						layersToCmernOff.append(mapLayer)
			if i == 0:
				layers = [layer]
			legint = self.cmeView.project.layerTreeRoot()
			...
			for pb, dialog in self.pbDialogs.items():
				...
				dialog.setDefaults(self, self.dialog2Plot[dialog][0].text(),
				                   ...
				                   activeScalar=d['active scalar'], xAxisDates=self.cmeView.cmeOptions.xAxisDates,
				                   time=d['time'])
...
class MapExportImportDialog(QDialog, Ui_MapExportImportDialog):
	...
	def __init__(self, iface):
		...
		self.btnBrowse.clicked.connect(lambda: browse(self, 'existing file', 'COASTALME/map_export_import_data',
