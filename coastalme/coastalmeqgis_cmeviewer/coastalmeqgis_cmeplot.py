...
from .coastalmeqgis_cmeplottoolbar import CmePlotToolbar
from .coastalmeqgis_cmeplotselection import CmePlotSelection
from .coastalmeqgis_cmerubberband import CmeRubberBand, CmeMarker
from .coastalmeqgis_cmeflowline import CmeFlowLine
from .coastalmeqgis_cmeplot2d import CmePlot2D
from .coastalmeqgis_cmeplot1d import CmePlot1D
from .coastalmeqgis_cmeuserplotdata import CmeUserPlotDataManager
from ..coastalmeqgis_library import (applyMatplotLibArtist, getMean, roundSeconds, convert_datetime_to_float,
...
from .coastalmeqgis_cmeplot3d import (CmePlot3D, ColourBar)
...
class CmePlot():
	...
	def __init__(self, CmeView):
		...
		self.iface = self.cmeView.iface
		self.canvas = self.cmeView.canvas
		self._creating_background = False

		# Initialise figures
		# time series
		self.layoutTimeSeries = self.cmeView.TimeSeriesFrame.layout()
		...
		self.figTimeSeries.canvas.mpl_connect('button_press_event', lambda e: self.onclick(e, CmePlot.TimeSeries))
		...
		self.figTimeSeries.canvas.mpl_connect("motion_notify_event", lambda e: self.hover(e, CmePlot.TimeSeries))
		self.figTimeSeries.canvas.mpl_connect("draw_event", lambda e: self.onDraw(e, CmePlot.TimeSeries))
		self.figTimeSeries.canvas.mpl_connect('figure_leave_event', lambda e: self.onLeave(e, CmePlot.TimeSeries))
		self.timeSeriesBackground = None

		
		# long plot
		self.layoutLongPlot = self.cmeView.LongPlotFrame.layout()
		...
		self.figLongPlot.canvas.mpl_connect('button_press_event', lambda e: self.onclick(e, CmePlot.CrossSection))
		...
		self.figLongPlot.canvas.mpl_connect("motion_notify_event", lambda e: self.hover(e, CmePlot.CrossSection))
		self.figLongPlot.canvas.mpl_connect("motion_notify_event", self.updateCursorMarker)
		self.figLongPlot.canvas.mpl_connect("draw_event", lambda e: self.onDraw(e, CmePlot.CrossSection))
		self.figLongPlot.canvas.mpl_connect('figure_leave_event', lambda e: self.onLeave(e, CmePlot.CrossSection))
		self.longPlotBackground = None

		# cross section
		self.layoutCrossSection = self.cmeView.CrossSectionFrame.layout()
		...
		self.figCrossSection.canvas.mpl_connect('button_press_event', lambda e: self.onclick(e, CmePlot.CrossSection1D))
		...
		self.figCrossSection.canvas.mpl_connect("motion_notify_event", lambda e: self.hover(e, CmePlot.CrossSection1D))

		# vertical profile
		self.layoutVerticalProfile = self.cmeView.VerticalProfileFrame.layout()
		...
		self.figVerticalProfile.canvas.mpl_connect('button_press_event', lambda e: self.onclick(e, CmePlot.VerticalProfile))
		...
		self.figVerticalProfile.canvas.mpl_connect("motion_notify_event", lambda e: self.hover(e, CmePlot.VerticalProfile))
		self.figVerticalProfile.canvas.mpl_connect("draw_event", lambda e: self.onDraw(e, CmePlot.VerticalProfile))
		self.figVerticalProfile.canvas.mpl_connect('figure_leave_event', lambda e: self.onLeave(e, CmePlot.VerticalProfile))
		...
		self.plotData = {
			CmePlot.DataTimeSeries1D: [],
			CmePlot.DataTimeSeries2D: [],
			CmePlot.DataCrossSection1D: [],
			CmePlot.DataCrossSection2D: [],
			CmePlot.DataFlow2D: [],
			CmePlot.DataUserData: [],
			CmePlot.DataCurrentTime: [],
			CmePlot.DataTimeSeriesStartLine: [],
			CmePlot.DataCrossSectionStartLine: [],
			CmePlot.DataCrossSectionStartLine1D: [],
			CmePlot.DataCurtainPlot: [],
			CmePlot.DataTimeSeriesDepAv: [],
			CmePlot.DataCrossSectionDepAv: [],
			CmePlot.DataVerticalProfileStartLine: [],
			CmePlot.DataVerticalProfile: [],
			CmePlot.DataCrossSection1DViewer: [],
			CmePlot.DataHydraulicProperty: [],
			CmePlot.DataVerticalMesh: [],
		}

		self.plotDataToPlotType = {
			CmePlot.DataTimeSeries1D: CmePlot.TimeSeries,
			CmePlot.DataTimeSeries2D: CmePlot.TimeSeries,
			CmePlot.DataCrossSection1D: CmePlot.CrossSection,
			CmePlot.DataCrossSection2D: CmePlot.CrossSection,
			CmePlot.DataFlow2D: CmePlot.TimeSeries,
			CmePlot.DataUserData: CmePlot.TimeSeries,
			CmePlot.DataCurrentTime: CmePlot.TimeSeries,
			CmePlot.DataTimeSeriesStartLine: CmePlot.TimeSeries,
			CmePlot.DataCrossSectionStartLine: CmePlot.CrossSection,
			CmePlot.DataCrossSectionStartLine1D: CmePlot.CrossSection1D,
			CmePlot.DataCurtainPlot: CmePlot.CrossSection,
			CmePlot.DataTimeSeriesDepAv: CmePlot.TimeSeries,
			CmePlot.DataCrossSectionDepAv: CmePlot.CrossSection,
			CmePlot.DataVerticalProfileStartLine: CmePlot.VerticalProfile,
			CmePlot.DataVerticalProfile: CmePlot.VerticalProfile,
			CmePlot.DataCrossSection1DViewer: CmePlot.CrossSection,
			CmePlot.DataHydraulicProperty: CmePlot.CrossSection,
			CmePlot.DataVerticalMesh: CmePlot.VerticalProfile,
		}
		...
		# Draw Plots
		self.initialisePlot(CmePlot.TimeSeries, CmePlot.DataTimeSeriesStartLine)  # time series
		
		self.initialisePlot(CmePlot.CrossSection, CmePlot.DataCrossSectionStartLine)  # long profile / cross section
		
		self.initialisePlot(CmePlot.CrossSection1D, CmePlot.DataCrossSectionStartLine1D)  # cross section editor

		self.initialisePlot(CmePlot.VerticalProfile, CmePlot.DataVerticalProfileStartLine)  # vertical profile
		
		# plot toolbar class
		self.cmePlotToolbar = CmePlotToolbar(self)
		
		# user selection plot class
		self.cmePlotSelection = CmePlotSelection(self)
		
		# CmePlot2D class
		self.cmePlot2D = CmePlot2D(self)
		
		# CmePlot1D class
		self.cmePlot1D = CmePlot1D(self)

		# CmePlot3D class
		self.cmePlot3D = CmePlot3D(self)

		# rubberband class plot class
		self.cmeTSPoint = CmeMarker(self, CmePlot.TimeSeries, CmePlot.DataTimeSeries2D,
		                          self.cmePlotToolbar.plotTSMenu, self.cmePlot2D.plotTimeSeriesFromMap,
		                          QT_RED, QgsVertexMarker.ICON_CIRCLE, True)
		self.cmeCrossSection = CmeRubberBand(self, CmePlot.CrossSection, CmePlot.DataCrossSection2D,
		                                   self.cmePlotToolbar.plotLPMenu, self.cmePlot2D.plotCrossSectionFromMap,
		                                   QT_RED, QgsVertexMarker.ICON_BOX, True)
		self.cmeFlowLine = CmeFlowLine(self, CmePlot.TimeSeries, CmePlot.DataFlow2D,
		                             self.cmePlotToolbar.plotFluxButton, self.cmePlot2D.plotFlowFromMap,
		                             QT_BLUE, QgsVertexMarker.ICON_DOUBLE_TRIANGLE, False)
		self.cmeCurtainLine = CmeRubberBand(self, CmePlot.CrossSection, CmePlot.DataCurtainPlot,
		                                  self.cmePlotToolbar.curtainPlotMenu, self.cmePlot3D.plotCurtainFromMap,
		                                  QT_GREEN, QgsVertexMarker.ICON_BOX, False)
		self.cmeTSPointDepAv = CmeMarker(self, CmePlot.TimeSeries, CmePlot.DataTimeSeriesDepAv,
		                               self.cmePlotToolbar.averageMethodTSMenu, self.cmePlot3D.plotTimeSeriesFromMap,
		                               QT_DARK_GREEN, QgsVertexMarker.ICON_CIRCLE, True)
		self.cmeCSLineDepAv = CmeRubberBand(self, CmePlot.CrossSection, CmePlot.DataCrossSectionDepAv,
		                                   self.cmePlotToolbar.averageMethodCSMenu, self.cmePlot3D.plotCrossSectionFromMap,
		                                   QT_DARK_GREEN, QgsVertexMarker.ICON_BOX, True)
		self.cmeVPPoint = CmeMarker(self, CmePlot.VerticalProfile, CmePlot.DataVerticalProfile,
		                          self.cmePlotToolbar.plotVPMenu, self.cmePlot3D.plotVerticalProfileFromMap,
		                          QT_MAGENTA, QgsVertexMarker.ICON_CIRCLE, True)

		self.markers = {
			self.cmeTSPoint: 'Time Series',
		    self.cmeTSPointDepAv: '3D to 2D DepAv Time Series',
			self.cmeVPPoint: 'Vertical Profile',
		}
		self.lines = {
			self.cmeCrossSection: 'Cross Section',
			self.cmeFlowLine: 'Flow Line',
			self.cmeCurtainLine: 'Curtain Line',
			self.cmeCSLineDepAv: '3D to 2D DepAv Cross Section',
		}
		...
		self.userPlotData = CmeUserPlotDataManager()
		...
		self.plotDataToGraphic = {
			...
			CmePlot.DataTimeSeries2D: self.cmeTSPoint,
			CmePlot.DataCrossSection1D: None,
			CmePlot.DataCrossSection2D: self.cmeCrossSection,
			CmePlot.DataFlow2D: self.cmeFlowLine,
			...
			CmePlot.DataCurtainPlot: self.cmeCurtainLine,
			CmePlot.DataTimeSeriesDepAv: self.cmeTSPointDepAv,
			CmePlot.DataCrossSectionDepAv: self.cmeCSLineDepAv,
			CmePlot.DataVerticalProfileStartLine: None,
			CmePlot.DataVerticalProfile: self.cmeVPPoint,
		...
		self.plotDataToSelection = {
			CmePlot.DataTimeSeries1D: None,
			CmePlot.DataTimeSeries2D: self.cmePlot2D.plotSelectionPointFeat,
			CmePlot.DataCrossSection1D: None,
			CmePlot.DataCrossSection2D: self.cmePlot2D.plotSelectionLineFeat,
			CmePlot.DataFlow2D: self.cmePlot2D.plotSelectionFlowFeat,
			CmePlot.DataUserData: None,
			CmePlot.DataCurrentTime: None,
			CmePlot.DataTimeSeriesStartLine: None,
			CmePlot.DataCrossSectionStartLine: None,
			CmePlot.DataCrossSectionStartLine1D: None,
			CmePlot.DataCurtainPlot: self.cmePlot3D.plotSelectionCurtainFeat,
			CmePlot.DataTimeSeriesDepAv: self.cmePlot3D.plotSelectionPointFeat,
			CmePlot.DataCrossSectionDepAv: self.cmePlot3D.plotSelectionLineFeat,
			CmePlot.DataVerticalProfileStartLine: None,
			CmePlot.DataVerticalProfile: self.cmePlot3D.plotSelectionVPFeat,
			CmePlot.DataCrossSection1DViewer: None,
			CmePlot.DataHydraulicProperty: None,
			CmePlot.DataVerticalMesh: None,
		}

		self.secondary_axis_flipped = {CmePlot.TimeSeries: False, CmePlot.CrossSection: False, CmePlot.VerticalProfile: False}
	...
	def plot2D(self):
		...
		if self.cmeView.cboSelectType.currentText() == 'From Map' \
			or self.cmeView.cboSelectType.currentText() == 'From Map Multi':
			self.cmeRubberBand.startRubberBand()
		else:
			self.cmePlotSelection.useSelection()
	...
	def plotFlow(self):
		...
		if self.cmeView.cboSelectType.currentText() == 'From Map' \
				or self.cmeView.cboSelectType.currentText() == 'From Map Multi':
			self.cmeFlowLine.startRubberBand()
		else:
			self.cmePlotSelection.useSelection(rtype='flow')
		
		return True
	
	def plotEnumerator(self, plotNo):
		"""
		Recmerns plot variables depending on the enumerator.
		...
		plotEnumeratorDict = {
			0: [self.cmeView.TimeSeriesLayout, self.figTimeSeries, self.subplotTimeSeries,
			...
			1: [self.cmeView.LongPlotLayout, self.figLongPlot, self.subplotLongPlot,
			...
			2: [self.cmeView.CrossSectionLayout, self.figCrossSection, self.subplotCrossSection,
			...
			3: [self.cmeView.VerticalProfileLayout, self.figVerticalProfile, self.subplotVerticalProfile,
	...
	def manageMatplotlibAxe(self, axe1):
		...
		try:
			for plotNo in [CmePlot.TimeSeries, CmePlot.CrossSection, CmePlot.CrossSection1D, CmePlot.VerticalProfile]:
				parentLayout, figure, subplot, plotWidget, isSecondaryAxis, artists, labels, unit, yAxisLabelTypes, yAxisLabels, xAxisLabels, xAxisLimits, yAxisLimits = \
					self.plotEnumerator(plotNo)
				if subplot == axe1:
					break
			toolbar, viewToolbar, mplToolbar = self.cmePlotToolbar.plotNoToToolbar[plotNo]
	...
	def changeLineAxis(self, clickedItem):
		...
		for plotNo in [CmePlot.TimeSeries, CmePlot.CrossSection, CmePlot.VerticalProfile]:
			data = []
			labels = []
			types = []
			dataTypes = []
						...
						if type(line) is dict:
							...
							if type(artist) is matplotlib.lines.Line2D:
								...
								# elif re.findall(r"flow regime_\d", rtype, flags=re.IGNORECASE):
									...
									flowRegimeX.append(x[0])
									flowRegimeY.append(artist.get_marker().strip('$'))
							elif type(artist) is Polygon or type(artist) is CmeflowPipe:
								xy = artist.get_xy()
								x = xy[:, 0]
								y = xy[:, 1]
								x = [list(zip(x, y))]
	...
	def clearAllPlots(self):
		...
		for i in range(CmePlot.TotalPlotNo):
			self.clearPlot2(i)
	...
	def clearPlot2(self, plotNo, clearType = None, last_only = False, remove_no = 1, clearOnly = (), **kwargs):
		...
		if clearType is None:
			...
			if plotNo == CmePlot.TimeSeries:
				if self.cmeTSPoint.cursorTrackingConnected:
					self.cmeTSPoint.escape(None)
				if self.cmeFlowLine.cursorTrackingConnected:
					self.cmeFlowLine.escape(None)
				if self.cmeTSPointDepAv.cursorTrackingConnected:
					self.cmeTSPointDepAv.escape(None)
			elif plotNo == CmePlot.CrossSection:
				if self.cmeCrossSection.cursorTrackingConnected:
					self.cmeCrossSection.escape(None)
				if self.cmeCurtainLine.cursorTrackingConnected:
					self.cmeCurtainLine.escape(None)
				if self.cmeCSLineDepAv.cursorTrackingConnected:
					self.cmeCSLineDepAv.escape(None)
				if clearCursorMarker:
					self.cursorMarker.clear()
			elif plotNo == CmePlot.VerticalProfile:
				if self.cmeVPPoint.cursorTrackingConnected:
					self.cmeVPPoint.escape(None)
		...
		for i, line in enumerate(self.plotData[clearType][:]):
				...
				for j, a in enumerate(ax):
					if a == artist:
						...
						if len(labels[k]) >= j + 1:
							...
							if clearCursorMarker:
								if clearType == CmePlot.DataCurtainPlot:
			...
			if not last_only and not clearOnly or i + 1 >= len(self.plotData[clearType]) - remove_no + 1 or line in clearOnly:
				...
				if clearSelection:
					self.cmePlotSelection.clearSelection(clearType)

		# some lines that are always removed
		# showCurrentTime = clearType == CmePlot.DataCurrentTime
		showCurrentTime = clearType == CmePlot.DataCurrentTime and self.cmeView.cbShowCurrentTime.isChecked()
		if plotNo == CmePlot.TimeSeries:
			for line in self.plotData[CmePlot.DataCurrentTime]:
				if type(line) is dict:
					artist = list(line.keys())[0]
				else:
					artist = line
				...
				if artist in subplot.lines:
					...
					if mpl_version_int() >= 35000:
						...
						subplot.lines.remove(artist)
				del line
			for line in self.plotData[CmePlot.DataTimeSeriesStartLine]:
				if type(line) is dict:
					artist = list(line.keys())[0]
				else:
					artist = line
			...
			for line in self.plotData[CmePlot.DataCrossSectionStartLine]:
				if type(line) is dict:
					artist = list(line.keys())[0]
				else:
					artist = line
				...
				if artist in subplot.lines:
					if mpl_version_int() >= 35000:
						...
						subplot.lines.remove(line)
				del line
		elif plotNo == CmePlot.CrossSection1D:
			for line in self.plotData[CmePlot.DataCrossSectionStartLine1D]:
				if type(line) is dict:
					artist = list(line.keys())[0]
				else:
					artist = line
			...
			for line in self.plotData[CmePlot.DataVerticalMesh][:]:
				if type(line) is dict:
					artist = list(line.keys())[0]
				else:
					artist = line
				...
				if artist in subplot.lines:
					if mpl_version_int() >= 35000:
						...
						subplot.lines.remove(line)
				del line
			for line in self.plotData[CmePlot.DataVerticalProfileStartLine]:
				if type(line) is dict:
					artist = list(line.keys())[0]
				else:
					artist = line
		...
		if clearType == CmePlot.DataCurrentTime and not showCurrentTime and draw:
			plotWidget.draw()
		else:
	...
	def reSpecPlot(self, plotNo):
		...
		if subplot2 is not None and plotNo == CmePlot.VerticalProfile:
	...
	def clearPlot(self, plotNo, **kwargs):
		...
		if isSecondaryAxis[0]:
			...
			for i, l in enumerate(lab2):
				if '2d map flow' in l.lower():
					...
					flowKeptYAll.append(y)
					flowKeptLabels.append(l)
		#if not self.cmeView.cmeMenuBar.freezeLegendLabels_action.isChecked():
		#	lab, lab2, line, line2 = [], [], [], []
		#labs = [lab, lab2]
		#lines = [line, line2]
		...
		if plotNo == CmePlot.TimeSeries:
			self.clearedTimeSeriesPlot = True
			self.cmePlot2D.resetMultiPointCount()
			self.cmePlot2D.resetMultiFlowLineCount()
		elif plotNo == CmePlot.CrossSection:
			self.clearedLongPlot = True
			self.cmePlot2D.resetMultiLineCount()
		elif plotNo == CmePlot.CrossSection1D:
			self.clearedCrossSectionPlot = True
		elif plotNo == CmePlot.VerticalProfile:
			self.clearedVerticalProfilePlot = True
		...
		if isSecondaryAxis[0]:
			...
			if subplot2 is not None:
				subplot2.cla()
			if not self.cmeView.cmeResults.secondaryAxisTypes:
		...
		if 'clear_rubberband' in kwargs.keys():
			if kwargs['clear_rubberband']:
				...
				if plotNo == CmePlot.CrossSection:
					self.cmePlot3D.curtainGeom.clear()
		...
		if 'clear_selection' in kwargs.keys():
			...
			#if plotNo == CmePlot.CrossSection:
			#	self.cmeView.crossSections1D.clear()
	...
	def clearPlotLastDatasetOnly(self, plotNo):
		...
		activeMeshLayers = self.cmeView.cmeResults.cmeResults2D.activeMeshLayers
		secondaryAxisTypes = self.cmeView.cmeResults.secondaryAxisTypes
		...
		if artists[0] or artists[1]:
			...
			for j in range(len(activeMeshLayers)):
				...
				for i, rtype in enumerate(self.cmePlotToolbar.getCheckedItemsFromPlotOptions(plotNo)):
	...
	def showCurrentTime(self, plotNo, **kwargs):
		...
		# 		del subplot.lines[j]
		# 		subplot.legend_ = None
		if self.plotData[CmePlot.DataCurrentTime]:
			for line in subplot.lines:
				if line._label.lower() == 'current time':
					if mpl_version_int() >= 35000:
						...
						subplot.lines.remove(line)
			if self.plotData[CmePlot.DataCurrentTime]:
				del self.plotData[CmePlot.DataCurrentTime][0]
			for lab in labels[0][:]:
			...
			#if self.plotData[CmePlot.DataCurrentTime][0] in subplot.lines:
			#	subplot.lines.remove(self.plotData[CmePlot.DataCurrentTime][0])
			#	del self.plotData[CmePlot.DataCurrentTime][0]
			#else:
			#	del self.plotData[CmePlot.DataCurrentTime][0]

		
		if self.cmeView.cbShowCurrentTime.isChecked() or showCurrentTime:
			...
			if qv < 31600:
				if time is not None:  # use input time not current time
					...
					try:
						x = [float(self.cmeView.cmeResults.activeTime)] * 2
					except TypeError:  # probably no times available
						return False

				# check time units
				if self.cmeView.cmeOptions.timeUnits == 's':
					if x:
				...
				if time is None:
					time = self.cmeView.cmeResults.activeTime
				if not self.cmeView.cmeOptions.xAxisDates:
					time, ind = self.cmeView.cmeResults.dateToTimeInCombobox(time)
					# rt = self.cmeView.cmeOptions.zeroTime
					# for i in range(self.cmeView.cboTime.count()):  # find closest
					# 	item = self.cmeView.cboTime.itemText(i)
					# 	timeKey = convertFormattedTimeToTime(item)
					# 	t = self.cmeView.cmeResults.timekey2time[timeKey] if timeKey in self.cmeView.cmeResults.timekey2time else float(timeKey)
					# 	if self.cmeView.cmeOptions.timeUnits == 's':
					...
					# 		if diff <= diff2:
					# 			item = self.cmeView.cboTime.itemText(max(0, i-1))
					# 			timeKey = convertFormattedTimeToTime(item)
					# 			time = self.cmeView.cmeResults.timekey2time[timeKey] if timeKey in self.cmeView.cmeResults.timekey2time else float(timeKey)
					# 			break
					# 		else:
					# 			time = t
					# 			break
			...
			if not self.cmeView.cmeMenuBar.freezeAxisYLimits_action.isChecked():
			...
			self.plotData[CmePlot.DataCurrentTime].append({a: 'Current Time'})
	...
	def showVertMesh(self, plotNo, data, label, plotVertMesh):
		...
		if plotNo != CmePlot.VerticalProfile:
			return
		...
		if self.plotData[CmePlot.DataVerticalMesh]:
			for line in self.plotData[CmePlot.DataVerticalMesh][:]:
				if line in subplot.lines:
					if mpl_version_int() >= 35000:
						...
						subplot.lines.remove(line)
					self.plotData[CmePlot.DataVerticalMesh].remove(line)
				else:
					self.plotData[CmePlot.DataVerticalMesh].remove(line)
		...
		if self.verticalMesh_action.isChecked():
			...
			# get x axis limits
			if not self.cmeView.cmeMenuBar.freezeAxisXLimits_action.isChecked():
				subplot.relim()
				subplot.autoscale()
				xlim = subplot.get_xlim()
			else:
			...
			for i, d in enumerate(vmdata):
				...
				self.plotData[CmePlot.DataVerticalMesh].append({a: 'Vertical Mesh'})
				artists[0].append(a)
				labels[0].append(lab)
			if self.cmeView.cmeMenuBar.freezeAxisXLimits_action.isChecked():
				subplot.set_xbound(xAxisLimits[0][0])
			...
			z = 10
			for line in self.plotData[CmePlot.DataVerticalProfile][:]:
				if type(line) is dict:
					artist = list(line.keys())[0]
				else:
					artist = line
	...
	def showStatResult(self, plotNo, statType):
		...
		if (statType == 'Median' and not self.cmeView.cmeMenuBar.showMedianEvent_action.isChecked()) or (statType == 'Mean' and not self.cmeView.cmeMenuBar.showMeanEvent_action.isChecked()):
			# check to see if current time already exists
		...
		if statType == 'Median':
			...
			mean, n = getMean(maxes, event_selection=self.cmeView.cmeOptions.meanEventSelection)
		else:
			n = None
	...
	def raw_label(self, name, plotNo, dict_, axis):
		...
		raw_name = ''
		if plotNo == CmePlot.TimeSeries and axis == 'x':
			if self.cmeView.cmeOptions.xAxisDates:
				raw_name = 'Date'
			else:
				raw_name = 'Time'
		elif plotNo == CmePlot.CrossSection and axis == 'x':
			raw_name = 'Offset'
		elif plotNo == CmePlot.VerticalProfile and axis == 'y':
			raw_name = 'Elevation'
	...
	def setAxisNames(self, plotNo, types, plotAsCollection=(), plotAsQuiver=(), return_unit_only=False, unit_system=None, context=None):
		...
		units = OrderedDict({
            ...
            r'Wave near-bed.*amplicmede': ('s', 's', ''),
            r'VX': ('m/s', 'ft/s', ''),
            r'VY': ('m/s', 'ft/s', ''),
		})
		...
		shortNames = OrderedDict({
			...
			r'\(xs\).*': 'Elevation',
			r'\(nwk\).*': 'Elevation',
            r'TURBZ_TKE': 'cmerbz_tke',
            r'TURBZ_EPS': 'cmerbz_eps',
            r'TURBZ_L': 'cmerbz_l',
            r'TURBZ_SPFSQ': 'cmerbz_spfsq',
            r'TURBZ_BVFSQ': 'cmerbz_bvfsq',
            r'TURBZ_NUM': 'cmerbz_num',
            r'TURBZ_NUH': 'cmerbz_nuh',
            r'TURBZ_NUS': 'cmerbz_nus',
            ...
            r'Wave near-bed.*amplicmede': 'WVUBOT',
            r'VX': 'VX',
            r'VY': 'VY',
		})
				...
				if plotType == plotNo:
					for line in self.plotData[plotData]:
						if type(line) is dict:
							...
							if plotData == CmePlot.DataCurtainPlot:
		...
		for name in types:
			...
			if name in self.cmeView.cmeResults.secondaryAxisTypes and self.cmeView.cmeOptions.secondary_axis_types[plotNo] == 'x-axis':
				x2_label.add_result_type(x_label)
				x2_label.add_unit(x_unit)
				y1_label.add_result_type(y_label)
				y1_label.add_unit(y_unit)
			elif name in self.cmeView.cmeResults.secondaryAxisTypes:
	...
	def getSecondaryAxis(self, plotNo, create=True):
		...
		secondary_axis_type = self.cmeView.cmeOptions.secondary_axis_types
		if not isSecondaryAxis[0]:
			if create:
				isSecondaryAxis[0] = True
				if plotNo == CmePlot.TimeSeries:
					if secondary_axis_type[CmePlot.TimeSeries] == 'x-axis':
					...
					if secondary_axis_type[CmePlot.CrossSection] == 'x-axis':
					...
					if secondary_axis_type[CmePlot.VerticalProfile] == 'y-axis':
	...
	def reorderByAxis(self, types, data, label, plotAsPoints, plotAsPatch, flowRegime, flowRegimeTied, plotAsCollection,
		...
		for i, rtype in enumerate(types):
			if plotAsCollection[i] or plotAsQuiver[i] or len(data[i][0]) > 0:
				secondaryAxis = False
				if rtype in self.cmeView.cmeResults.secondaryAxisTypes:
					...
					if types[j] in self.cmeView.cmeResults.secondaryAxisTypes:
						secondaryAxis = True
				if secondaryAxis:
					types2.append(rtype)
					data2.append(data[i])
	...
	def drawPlot(self, plotNo, data, label, types, dataTypes, **kwargs):
		...
		if not refreshOnly:  # only if it's not refresh only
			...
			for i in range(len(data)):
				...
				# data
				if self.cmeView.cmeOptions.xAxisDates and plotNo == CmePlot.TimeSeries:
					if data[i]:
					...
					if plotAsPatch is None or not plotAsPatch[i]:  # normal X, Y data
						...
						if plotAsCollection is not None and plotAsCollection[i]:  # curtain plot
							...
							for j, verts in enumerate(x):
								if verts:
									poly = CmeflowPipe(verts, facecolor='0.9', edgecolor='0.5', label=label[i])
		...
		if data:
			...
			if yAxisLabelFirst or yAxisLabelSecond:
				...
				if isSecondaryAxis[0]:
					if self.cmeView.cmeOptions.secondary_axis_types[plotNo] == 'y-axis':
		...
		userPlotData = False
		for stacmes in [v.stacmes for k, v in sorted(self.userPlotData.datasets.items(), key=lambda x: x[-1].number)]:
			if stacmes:
				userPlotData = True
				break
		isData = False
		...
		if isData and self.cmeView.cmeOptions.xAxisDates and plotNo == CmePlot.TimeSeries:
			fmt = mdates.DateFormatter(self.cmeView.cmeOptions.dateFormat)
			subplot.xaxis.set_major_formatter(fmt)
			...
			try:
				for tick in subplot.get_xticklabels():
					tick.set_rotation(self.cmeView.cmeOptions.xAxisLabelRotation)
					if self.cmeView.cmeOptions.xAxisLabelRotation > 0 and self.cmeView.cmeOptions.xAxisLabelRotation < 90:
						tick.set_horizontalalignment('right')
					elif self.cmeView.cmeOptions.xAxisLabelRotation > 90 and self.cmeView.cmeOptions.xAxisLabelRotation < 180:
						tick.set_horizontalalignment('left')
					elif self.cmeView.cmeOptions.xAxisLabelRotation > 180 and self.cmeView.cmeOptions.xAxisLabelRotation < 270:
						tick.set_horizontalalignment('right')
					elif self.cmeView.cmeOptions.xAxisLabelRotation > 270 and self.cmeView.cmeOptions.xAxisLabelRotation < 360:
						tick.set_horizontalalignment('left')
					elif self.cmeView.cmeOptions.xAxisLabelRotation > -90 and self.cmeView.cmeOptions.xAxisLabelRotation < 0:
						tick.set_horizontalalignment('left')
					elif self.cmeView.cmeOptions.xAxisLabelRotation > -180 and self.cmeView.cmeOptions.xAxisLabelRotation < -90:
						tick.set_horizontalalignment('right')
					elif self.cmeView.cmeOptions.xAxisLabelRotation > -270 and self.cmeView.cmeOptions.xAxisLabelRotation < -180:
						tick.set_horizontalalignment('left')
					elif self.cmeView.cmeOptions.xAxisLabelRotation > -360 and self.cmeView.cmeOptions.xAxisLabelRotation < -270:
						tick.set_horizontalalignment('right')
			except:
				pass
		elif self.cmeView.cmeOptions.xAxisDates and plotNo == CmePlot.TimeSeries:
		...
		if types is not None and 'Flow Regime_1d' in types:
			if 'Flow Regime_1d' in self.cmeView.cmeResults.secondaryAxisTypes:
	...
	def updateTimeSeriesPlot(self, **kwargs):
		"""
		Update time series plot. Will go through all marker points and selected point feacmeres.
		...
		self.make_1d_2d_rt_consistent(self.cmeView.cmeResults.results)

		self.clearPlot2(CmePlot.TimeSeries, clearType, clear_rubberband=False, clear_selection=False)
		...
		if plot.lower() != '1d only' and plot.lower() != 'flow only' and update != '1d only':
			...
			if len(self.cmeTSPoint.points) + len(self.cmePlot2D.plotSelectionPointFeat) > 1:
				multi = True
			
			for i, point in enumerate(self.cmeTSPoint.points):
				self.cmePlot2D.plotTimeSeriesFromMap(None, QgsPointXY(point), bypass=multi, plot='2D Only',
			...
			for f in self.cmePlot2D.plotSelectionPointFeat:
				# get feacmere name from attribute
				iFeatName = self.cmeView.cmeOptions.iLabelField
				if len(f.attributes()) > iFeatName:
					featName = f.attributes()[iFeatName]
				else:
					featName = None

				self.cmePlot2D.plotTimeSeriesFromMap(None, f.geometry().asPoint(), bypass=multi, plot='2D Only',
				                                    draw=draw, time=time, show_current_time=showCurrentTime,
				                                    retain_flow=retainFlow, mesh_rendered=meshRendered,
				                                    plot_active_scalar=plotActiveScalar, featName=featName)
				
			if self.cmePlot2D.multiPointSelectCount > 1:
				self.cmePlot2D.reduceMultiPointCount(1)

			# 2D depth average options
			multi = False  # do labels need to be counted up e.g. point 1, point 2
			if len(self.cmeTSPointDepAv.points) + len(self.cmePlot3D.plotSelectionPointFeat) > 1:
				multi = True
			for i, point in enumerate(self.cmeTSPointDepAv.points):
				self.cmePlot3D.plotTimeSeriesFromMap(None, QgsPointXY(point), bypass=multi,
				                                    draw=draw, time=time, show_current_time=showCurrentTime,
				                                     mesh_rendered=meshRendered,
				                                    plot_active_scalar=plotActiveScalar, markerNo=i+1,
				                                    data_type=CmePlot.DataTimeSeriesDepAv)
			for f in self.cmePlot3D.plotSelectionPointFeat:
				# get feacmere name from attribute
				iFeatName = self.cmeView.cmeOptions.iLabelField
				if len(f.attributes()) > iFeatName:
					featName = f.attributes()[iFeatName]
				...
				self.cmePlot3D.plotTimeSeriesFromMap(None, f.geometry().asPoint(), bypass=multi, plot='2D Only',
				                                    draw=draw, time=time, show_current_time=showCurrentTime,
				                                    retain_flow=retainFlow, mesh_rendered=meshRendered,
				                                    plot_active_scalar=plotActiveScalar, featName=featName,
				                                    data_type=CmePlot.DataTimeSeriesDepAv)
		...
		if not retainFlow:  # if retain flow, no need to recalculate flow
			if plot.lower() != '1d only' and plot.lower() != '2d only' and update != '1d only' and update != '1d and 2d only':
				...
				if len(self.cmeFlowLine.rubberBands) + len(self.cmePlot2D.plotSelectionFlowFeat) > 1:
					multiFlow = True
				
				for i, line in enumerate(self.cmeFlowLine.rubberBands):
					if line.asGeometry() is not None:
						if not line.asGeometry().isNull():
							...
							self.cmePlot2D.plotFlowFromMap(None, feat, bypass=multiFlow, plot='flow only', draw=draw, time=time,
							                              show_current_time=showCurrentTime, mesh_rendered=meshRendered)
					
				for feat in self.cmePlot2D.plotSelectionFlowFeat:
					self.cmePlot2D.plotFlowFromMap(None, feat, bypass=multiFlow, plot='flow only', draw=draw, time=time,
					                              show_current_time=showCurrentTime, mesh_rendered=meshRendered)
					
				if self.cmePlot2D.multiFlowLineSelectCount > 1:
					self.cmePlot2D.reduceMultiFlowLineCount(1)
		elif showCurrentTime:  # might need to update plot with updated time - for animation
			self.drawPlot(0, [], None, None, [CmePlot.DataCurrentTime], draw=draw, time=time, show_current_time=showCurrentTime)
		
		if plot.lower() != '2d only' and plot.lower() != 'flow only':
			self.cmePlot1D.plot1dTimeSeries(bypass=True, plot='1D Only', draw=draw, time=time, show_current_time=showCurrentTime)
			self.cmePlot1D.plot1dMaximums(plot='1D Only', draw=draw)
			
		if not plot:
			if self.cmeView.cmeMenuBar.showMedianEvent_action.isChecked():
				self.showStatResult(0, 'Median')
				
			if self.cmeView.cmeMenuBar.showMeanEvent_action.isChecked():
		...
		if not self.artistsTimeSeriesFirst and not self.artistsCrossSectionSecond:
			if self.userPlotData.datasets:
				self.drawPlot(0, [], None, None, [CmePlot.DataUserData], draw=draw, time=time, show_current_time=showCurrentTime)
		
		return True
	
	def updateCrossSectionPlot(self, **kwargs):
		"""
		Update cross section plot. Will go through all rubberbands and selected line feacmeres.
		...
		if time is not None:
			if time != 'Maximum' and time != 99999 and time != 'Minimum' and time != -99999:
				if qv < 31600:
					...
					if type(time) is str:
						if not self.cmeView.cmeOptions.xAxisDates:
							if time in self.cmeView.cmeResults.cboTime2timekey:
								time = self.cmeView.cmeResults.cboTime2timekey[time]
								time = self.cmeView.cmeResults.timekey2time[time]
							else:
							...
							time = datetime.datetime.strptime(time, self.cmeView.cmeResults.dateFormat)
		
		# if not plot:
		# 	self.clearPlot(1)
		# 	#self.clearedLongPlot = False
		self.make_1d_2d_rt_consistent(self.cmeView.cmeResults.results)
		self.clearPlot2(CmePlot.CrossSection, clear_rubberband=False, clear_selection=False)
		
		if plot.lower() != '1d only':
			multi = False  # do labels need to be counted up e.g. line 1, line 2
			if len(self.cmeCrossSection.rubberBands) > 1:
				multi = True
			elif len(self.cmePlot2D.plotSelectionLineFeat) > 1:
				multi = True
			elif len(self.cmeCrossSection.rubberBands) + len(self.cmePlot2D.plotSelectionLineFeat) > 1:
				multi = True
			
			for i, rubberBand in enumerate(self.cmeCrossSection.rubberBands):
				if rubberBand.asGeometry() is not None:
					if not rubberBand.asGeometry().isNull():
						...
						try:
							...
							feat.setGeometry(QgsGeometry.fromPolyline([QgsPoint(x.x(), x.y()) for x in geom]))
						# if i == 0:
						self.cmePlot2D.plotCrossSectionFromMap(None, feat, bypass=multi, plot='2D Only', draw=draw,
						                                      time=time, mesh_rendered=meshRendered,
						                                      plot_active_scalar=plotActiveScalar, lineNo=i+1)
						# else:
						# 	self.cmePlot2D.plotCrossSectionFromMap(None, feat, bypass=True, plot='2D Only', draw=draw,
						# 	                                      time=time, mesh_rendered=meshRendered,
						# 	                                      plot_active_scalar=plotActiveScalar)
			
			for feat in self.cmePlot2D.plotSelectionLineFeat:
				# get feacmere name from attribute
				iFeatName = self.cmeView.cmeOptions.iLabelField
			...
			if self.cmePlot2D.multiLineSelectCount > 1:
				self.cmePlot2D.reduceMultiLineCount(1)

			# depth averaged cross sections
			multi = False  # do labels need to be counted up e.g. line 1, line 2
			if len(self.cmeCSLineDepAv.rubberBands) > 1:
				multi = True
			elif len(self.cmePlot3D.plotSelectionLineFeat) > 1:
				multi = True
			elif len(self.cmeCSLineDepAv.rubberBands) + len(self.cmePlot3D.plotSelectionLineFeat) > 1:
				multi = True

			for i, rubberBand in enumerate(self.cmeCSLineDepAv.rubberBands):
				if rubberBand.asGeometry() is not None:
					if not rubberBand.asGeometry().isNull():
						geom = rubberBand.asGeometry().asPolyline()
						feat = QgsFeature()
						try:
							...
							feat.setGeometry(QgsGeometry.fromPolyline([QgsPoint(x.x(), x.y()) for x in geom]))
						# if i == 0:
						self.cmePlot3D.plotCrossSectionFromMap(None, feat, bypass=multi, draw=draw,
															  time=time, mesh_rendered=meshRendered,
															  plot_active_scalar=plotActiveScalar, lineNo=i + 1,
															  data_type=CmePlot.DataCrossSectionDepAv)
				# else:
				# 	self.cmePlot2D.plotCrossSectionFromMap(None, feat, bypass=True, plot='2D Only', draw=draw,
				# 	                                      time=time, mesh_rendered=meshRendered,
				# 	                                      plot_active_scalar=plotActiveScalar)

			for feat in self.cmePlot3D.plotSelectionLineFeat:
				# get feacmere name from attribute
				iFeatName = self.cmeView.cmeOptions.iLabelField
				if len(feat.attributes()) > iFeatName:
					featName = feat.attributes()[iFeatName]
			...
			if self.cmePlot3D.multiLineSelectCount > 1:
				self.cmePlot3D.reduceMultiLineCount(1)

			# curtain plots
			multi = False  # do labels need to be counted up e.g. line 1, line 2
			if len(self.cmeCurtainLine.rubberBands) > 1:
				multi = True
			elif len(self.cmePlot3D.plotSelectionCurtainFeat) > 1:
				multi = True
			elif len(self.cmeCurtainLine.rubberBands) + len(self.cmePlot3D.plotSelectionCurtainFeat) > 1:
				multi = True

			for i, rubberBand in enumerate(self.cmeCurtainLine.rubberBands):
				if rubberBand.asGeometry() is not None:
					if not rubberBand.asGeometry().isNull():
						geom = rubberBand.asGeometry().asPolyline()
						feat = QgsFeature()
						try:
							...
							feat.setGeometry(QgsGeometry.fromPolyline([QgsPoint(x.x(), x.y()) for x in geom]))
						# if i == 0:
						self.cmePlot3D.plotCurtainFromMap(None, feat, bypass=multi, draw=draw,
														 time=time,
														 plot_active_scalar=plotActiveScalar, lineNo=i+1,
														 update=True)
			for feat in self.cmePlot3D.plotSelectionCurtainFeat:
				iFeatName = self.cmeView.cmeOptions.iLabelField
				if len(feat.attributes()) > iFeatName:
					featName = feat.attributes()[iFeatName]
				else:
					featName = None
				self.cmePlot3D.plotCurtainFromMap(None, feat, bypass=multi, draw=draw, time=time, featName=featName, update=True)
		
		if plot.lower() != '2d only':
			self.cmePlot1D.plot1dLongPlot(bypass=True, plot='1D Only', draw=draw, time=time)
			self.cmePlot1D.plot1dCrossSection(bypass=True, draw=draw, time=time)
			self.cmePlot1D.plot1dHydProperty(bypass=True, draw=draw, time=time)
	...
	def updateVerticalProfilePlot(self, **kwargs):
		...
		self.clearPlot2(CmePlot.VerticalProfile, clear_rubberband=False, clear_selection=False)

		multi = False  # do labels need to be counted up e.g. point 1, point 2
		if len(self.cmeVPPoint.points) + len(self.cmePlot3D.plotSelectionVPFeat) > 1:
			multi = True
		for i, point in enumerate(self.cmeVPPoint.points):
			self.cmePlot3D.plotVerticalProfileFromMap(None, QgsPointXY(point), bypass=multi, draw=draw,
			                                 time=time, plot_active_scalar=plotActiveScalar, markerNo=i + 1,
			                                 update=True)
		for f in self.cmePlot3D.plotSelectionVPFeat:
			# get feacmere name from attribute
			iFeatName = self.cmeView.cmeOptions.iLabelField
			if len(f.attributes()) > iFeatName:
				featName = f.attributes()[iFeatName]
			...
			self.cmePlot3D.plotVerticalProfileFromMap(None, f.geometry().asPoint(), bypass=multi,
			                                         draw=draw, time=time,
			                                         mesh_rendered=meshRendered,
			                                         plot_active_scalar=plotActiveScalar, featName=featName)
	...
	def updateCurrentPlot(self, plotNo=None, **kwargs):
		...
		try:
			self.cmeView.canvas.mapCanvasRefreshed.disconnect(self.updateCurrentPlot)
		except:
			pass
	...
	def setNewPlotProperties(self, plotNo):
		...
		if plotNo == CmePlot.TimeSeries:
			viewToolbar = self.cmePlotToolbar.viewToolbarTimeSeries
		elif plotNo == CmePlot.CrossSection:
			viewToolbar = self.cmePlotToolbar.viewToolbarLongPlot
		elif plotNo == CmePlot.CrossSection1D:
			viewToolbar = self.cmePlotToolbar.viewToolbarCrossSection
		elif plotNo == CmePlot.VerticalProfile:
			viewToolbar = self.cmePlotToolbar.viewToolbarVerticalProfile
	...
	def clearFrozenPlotProperties(self, plotNo=None, rtype=None):
		plots = [CmePlot.TimeSeries, CmePlot.CrossSection, CmePlot.VerticalProfile]
		...
		for plot in plots:
			...
			elif plot == CmePlot.CrossSection:
				if rtype == 'lines' or rtype is None:
					self.frozenLPProperties.clear()
				if rtype == 'axis labels' or rtype is None:
					self.frozenLPAxisLabels.clear()
			elif plot == CmePlot.VerticalProfile:
				if rtype == 'lines' or rtype is None:
	...
	def exportCSV(self, plotNo, data, labels, types, outputFolder, fileName, **kwargs):
		...
		for i in range(maxLength):  # iterate through longest series
			for j in range(len(data)):  # iterate through the different data sets
				if plotNo == CmePlot.TimeSeries and len(data[j][0]) < i + 1:
					x = None
					...
					k = len(data[j][0]) - 1
					y = data[j][1][k]
				elif plotNo != CmePlot.TimeSeries and len(data[j][0]) < i + 1:
					x = ''
					y = ''
				else:
					x = data[j][0][i]
	...
	def plotUserData(self, plotNo):
		...
		for name in [k for k, v in sorted(self.userPlotData.datasets.items(), key=lambda x: x[-1].number)]:
			...
			if data.stacmes:
				if (data.plotType == 'Time Series Plot' and plotNo == CmePlot.TimeSeries) or (data.plotType == 'Cross Section / Long Plot' and plotNo == CmePlot.CrossSection):
					...
					if labelOriginal not in labels[0]:
						if qv < 31600:
							...
							rt = data.referenceTime if data.referenceTime is not None else self.cmeView.cmeOptions.zeroTime
							if self.cmeView.cmeOptions.xAxisDates and plotNo == CmePlot.TimeSeries:
								if data.dates:
									x = data.dates[:]
								else:
						...
						self.plotData[CmePlot.DataUserData].append(a)
						applyMatplotLibArtist(a, artistTemplates[0])
						artists[0].append(a)
	...
	def convertTimeToDate(self, data):
		...
		for dacmem in data:
			time = None
			# if dacmem in self.cmeView.cmeResults.time2date.keys():
			if dacmem in self.cmeView.cmeResults.time2date_tspec.keys():
				# time = self.cmeView.cmeResults.time2date[dacmem]
				time = self.cmeView.cmeResults.time2date_tspec[dacmem]
			elif '{0:.6f}'.format(dacmem) in self.cmeView.cmeResults.timekey2date_tspec.keys():
				time = self.cmeView.cmeResults.timekey2date_tspec['{0:.6f}'.format(dacmem)]
			else:
				time = datetime2timespec(self.cmeView.cmeOptions.zeroTime, self.cmeView.cmeResults.loadedTimeSpec, QT_TIMESPEC_UTC) \
				       + timedelta(hours=dacmem)
				time = roundSeconds(time, 2)
				modelDates = sorted([x for x in self.cmeView.cmeResults.date_tspec2time.keys()])
				if modelDates:
					time = self.cmeView.cmeResults.findTimeClosest(None, None, time, modelDates, True, 'closest')
			if time is None:
				break
			x.append(time)
	...
	def convertDateToTime(self, data: str, unit: str='h') -> float:
		...
		if type(data) is str:
			date = datetime.strptime(data, self.cmeView.cmeResults.dateFormat)
		else:
			date = data
		if date in self.cmeView.cmeResults.date_tspec2time:
			return self.cmeView.cmeResults.date_tspec2time[date]
		modelDates = sorted([x for x in self.cmeView.cmeResults.date_tspec2time.keys()])
		dateClosest = self.cmeView.cmeResults.findTimeClosest(None, None, date, modelDates, True, 'closest')
		if dateClosest in self.cmeView.cmeResults.date_tspec2time:
			return self.cmeView.cmeResults.date_tspec2time[dateClosest]
		if date in self.cmeView.cmeResults.date2time:
			return self.cmeView.cmeResults.date2time[date]
		else:
			return -99999.
		...
		if e.inaxes == subplot:
			...
			if e.guiEvent.type() == QT_EVENT_MOUSE_BUTTON_PRESS and e.guiEvent.button() == QT_LEFT_BUTTON:
				...
				if plotNo == CmePlot.VerticalProfile:
					...
					x2 = None
		self.cmeView.plotCoords.setText(self.formatCoord(x, y, z, x2, y2))

	def vmeshToggled(self):
		parentLayout, figure, subplot, plotWidget, isSecondaryAxis, artists, labels, unit, yAxisLabelTypes, yAxisLabels, xAxisLabels, xAxisLimits, yAxisLimits = \
			self.plotEnumerator(self.cmeView.tabWidget.currentIndex())

		if self.cmeView.tabWidget.currentIndex() == CmePlot.CrossSection:
			if PolyCollection in [type(x) for x in artists[0]]:
				self.updateCrossSectionPlot()
		elif self.cmeView.tabWidget.currentIndex() == CmePlot.VerticalProfile:
			self.updateVerticalProfilePlot()

	def lockAxis(self, plotNo, showCurrentTime):
		"""return True if axis should not be relimed"""

		if plotNo == CmePlot.TimeSeries and showCurrentTime:
			return True

		if plotNo == CmePlot.VerticalProfile and self.verticalMesh_action.isChecked():
			return True
	...
	def checkPlotForInconsistentUnits(self, ax):
		...
		if dates and time:
			for line in ax.lines:
				if type(line) is plt.Line2D:
					x, y = line.get_data()
					if self.cmeView.cmeOptions.xAxisDates and len(x) and type(x[0]) is not datetime:
						xnew = self.convertTimeToDate(x)
						line.set_data(xnew, y)
					elif not self.cmeView.cmeOptions.xAxisDates and len(x) and type(x[0]) is datetime:
	...
	def relimCurtainPlot(self):
		...
		parentLayout, figure, subplot, plotWidget, isSecondaryAxis, artists, labels, unit, yAxisLabelTypes, yAxisLabels, xAxisLabels, xAxisLimits, yAxisLimits = \
			self.plotEnumerator(CmePlot.CrossSection)
	...
	def getAnnotation(self, plotNo, axisNo):
		...
		if axisNo == 1:
			...
			if plotNo == CmePlot.CrossSection1D:
				if len(subplot.texts) > 1:
					[subplot.texts[i].remove() for i in range(len(subplot.texts)) if i > 0]
				if len(subplot.texts) == 1:
	...
	def update_annotation(self, pos_xy, annotation, point, artist, ax, fig, event, dist, line=None, ind=None, line_data=None):
		...
		if isinstance(artist, plt.Line2D):
			if isinstance(pos_xy[0], datetime):
				...
				text = self.cmeView.cmeResults._dateFormat.format(pos_xy[0])
			else:
			...
			if contains and ind:
				...
				text = '{0:.3f}'.format(z)
				annotation.get_bbox_patch().set_facecolor(artist.cmap(self.cmePlot3D.colSpec['norm'](z)))
			else:
				return
		elif isinstance(artist, Polygon):
			point.set_offsets([pos_xy])
			...
			for artist in artists:
				if not isinstance(artist, PathCollection) and not isinstance(artist, plt.Text):
					...
					if type(artist) == PolyCollection:
						...
						if type(artist) == Polygon or type(artist) == CmeflowPipe:
							contains, _ = artist.contains(event)
							if contains:
								closest_point = (event.xdata, event.ydata)
								if artist.axes != ax_event:
					...
					if closest_point is not None:
						...
						if type(artist) == PolyCollection or type(artist) == Quiver:
							p.set_visible(False)
						elif (type(artist) == Polygon or type(artist) == CmeflowPipe) and type(dist) == int and dist == 5:
							p.set_visible(False)
						else:
							p.set_visible(True)
						if ax.texts:
	...
	def updateCursorMarker(self, event):
		parentLayout, figure, subplot, plotWidget, isSecondaryAxis, artists_, labels_, unit, yAxisLabelTypes, yAxisLabels, xAxisLabels, xAxisLimits, yAxisLimits = \
			self.plotEnumerator(CmePlot.CrossSection)
		subplot2 = self.getSecondaryAxis(CmePlot.CrossSection, create=False)
	...
	def make_1d_2d_rt_consistent(self, results):
		...
		for ml in meshLayers:
			ml = coastalmeqgis_find_layer(ml)
			if ml is None:
				continue
			if ml.name() not in results:
				continue
			for result_type in results[ml.name()]:
				if not self.cmeView.cmeResults.isTimeSeriesType(result_type):
					continue
				rt = self.cmeView.cmeResults.cmeResults2D.getReferenceTime(ml)
