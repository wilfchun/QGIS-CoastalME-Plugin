...
class CmeOptions():
	...
	def __init__(self):
		...
		self.copy_mesh = True if settings.value('COASTALME/cmeview_copy_mesh', 'false') in ['True', 'true', True] else False
		self.show_copy_mesh_dlg = True if settings.value('COASTALME/cmeview_show_copy_mesh_dlg', 'false') in ['True', 'true', True] else False
		self.del_copied_res = True if settings.value('COASTALME/cmeview_del_copied_res', 'true') in ['True', 'true', True] else False
		self.profile_plotting_tasks = True

		if settings.contains("COASTALME/cmeview_defaultlayout"):
			self.defaultLayout = settings.value('COASTALME/cmeview_defaultlayout')
		else:
			self.defaultLayout = 'previous_state'
		zeroTime = settings.value('COASTALME/cmeview_zeroTime')
		if zeroTime:
			self.zeroTime = zeroTime
		else:
			# self.zeroTime = datetime(1990, 1, 1, 0, 0, 0)
			self.zeroTime = self.defaultZeroTime
		dateFormat = settings.value('COASTALME/cmeview_dateFormat')
		if dateFormat:
			self.dateFormat = dateFormat
		else:
			self.dateFormat = '%H:%M'
		_dateFormat = settings.value('COASTALME/cmeview__dateFormat')
		if _dateFormat:
			self._dateFormat = _dateFormat
		else:
			self._dateFormat = '{0:%H}:{0:%M}'
		if settings.contains("COASTALME/cmeview_plotbackgroundcolour"):
			self.plotBackgroundColour = settings.value("COASTALME/cmeview_plotbackgroundcolour")
		else:
			self.plotBackgroundColour = '#e5e5e5'

		if settings.contains("COASTALME/cmeview_defaulcmeontsize"):
			try:
				self.defaultFontSize = int(settings.value("COASTALME/cmeview_defaulcmeontsize"))
			except ValueError:
				self.defaultFontSize = 10
		else:
			self.defaultFontSize = 10

		if settings.contains("COASTALME/cmeview_iconsize"):
			try:
				self.iconSize = int(settings.value("COASTALME/cmeview_iconsize"))
			except ValueError:
				self.iconSize = 24
		else:
			self.iconSize = 24

		if settings.contains("COASTALME/cmeview_tcf_load_method"):
			try:
				self.tcfLoadMethod = settings.value("COASTALME/cmeview_tcf_load_method")
			except:
				self.tcfLoadMethod = 'result_selection'
		else:
			self.tcfLoadMethod = 'result_selection'

		if settings.contains("COASTALME/cmeview_plot_inactive_areas"):
			try:
				self.plotInactiveAreas = True if settings.value("COASTALME/cmeview_plot_inactive_areas") == 'true' else False
			except:
				self.plotInactiveAreas = True
		else:
			self.plotInactiveAreas = True

		if settings.contains('COASTALME/cmeview_secondary_axis_type_0'):
			self.secondary_axis_types[0] = settings.value('COASTALME/cmeview_secondary_axis_type_0')
		if settings.contains('COASTALME/cmeview_secondary_axis_type_1'):
			self.secondary_axis_types[1] = settings.value('COASTALME/cmeview_secondary_axis_type_1')
		if settings.contains('COASTALME/cmeview_secondary_axis_type_3'):
			self.secondary_axis_types[3] = settings.value('COASTALME/cmeview_secondary_axis_type_3')

		# curtain vectors
		self.curtain_vector_scale = float(settings.value("COASTALME/cmeview_curtain_vector_scale", "0.005"))
		self.curtain_vector_scale_units = settings.value("COASTALME/cmeview_curtain_vector_scale_units", "dots")
		if self.curtain_vector_scale_units == 'default':
			self.curtain_vector_scale_units = None
		self.curtain_vector_units = settings.value("COASTALME/cmeview_curtain_vector_units", "dots")
		if self.curtain_vector_units == 'default':
			self.curtain_vector_units = None
		self.curtain_vector_width = float(settings.value("COASTALME/cmeview_curtain_vector_width", "0.5"))
		self.curtain_vector_head_width = float(settings.value("COASTALME/cmeview_curtain_vector_head_width", "10.0"))
		self.curtain_vector_head_length = float(settings.value("COASTALME/cmeview_curtain_vector_head_length", "10.0"))
		self.curtain_vector_horizontal_factor = float(settings.value("COASTALME/cmeview_curtain_vector_horizontal_factor", "1.0"))
		self.curtain_vector_vertical_factor = float(settings.value("COASTALME/cmeview_curtain_vector_vertical_factor", "1.0"))
