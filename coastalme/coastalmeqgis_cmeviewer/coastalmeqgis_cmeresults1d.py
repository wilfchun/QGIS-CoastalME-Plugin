...
from ..COASTALME_results import ResData
from ..COASTALME_results2013 import ResData as ResData_2013
from ..coastalmeqgis_library import (getPathFromRel, coastalmeqgis_apply_check_cme_clayer, datetime2timespec,
...
import re
from ..COASTALME_FM_data_provider import CmeFloodModellerDataProvider
from math import atan2, sin, cos, pi
from pathlib import Path
from ..coastalme_results_gpkg import ResData_GPKG
from ..gui import apply_cme_style_gpkg_ts
...
class TSResult(ResData):
	...
	def field_names(self, feacmere, start=0):
		for i in range(start, feacmere.fields().count()):
			yield feacmere.fields()[i].name()

	def field_names_to_float(self, feacmere, start):
		for field_name in self.field_names(feacmere, start):
...
class CmeResults1D():
	...
	def importResults(self, inFilePaths):
		...
		results = self.cmeView.cmeResults.results  # dict of indexed results
		...
		for filePath in inFilePaths:
			...
			# post 2013 results
				...
				if error:
					...
					QMessageBox.critical(self.cmeView, "COASTALME Viewer", message)
					return False
			elif ext.upper() == '.GPKG':
				res = ResData_GPKG()
				error, message = res.Load(filePath)
	...
	def importResultsFM(self, gxy, dat, inFilePaths):
		...
		for i, filePath in enumerate(inFilePaths):
			...
			simname = CmeFloodModellerDataProvider.getSimulationName(filePath)
			if simname in self.results1d:
				res = self.results1d[simname][0]
			else:
				res = CmeFloodModellerDataProvider()
			...
			if i == 0:
				...
				if res.isgxyValid:
					...
					if fmLinksLayer.isValid():
						self.cmeView.project.addMapLayer(fmLinksLayer)
						coastalmeqgis_apply_check_cme_clayer(self.iface, layer=fmLinksLayer)
					if fmNodesLayer.isValid():
						self.cmeView.project.addMapLayer(fmNodesLayer)
						coastalmeqgis_apply_check_cme_clayer(self.iface, layer=fmNodesLayer)
				if err:
					QMessageBox.warning(self.iface.mainWindow(), 'Import GXY File', 'WARNING: error reading {0} nodes / links:\n'.format(len(msg), msg))

				if dat is not None:
					err, msg = self.cmeView.crossSectionsFM.fmLoadDat(dat)
					res.loadBedElevations(self.cmeView.crossSectionsFM)
					if self.cmeView.crossSectionsFM.fm_nXs:
						...
						feats = []
						skipped_xs = []
						for xs in self.cmeView.crossSectionsFM.fm_xs:
							if xs.FM_nxypnts > 0:
						...
						if fmXSLayer.isValid():
							self.cmeView.project.addMapLayer(fmXSLayer)
							coastalmeqgis_apply_check_cme_clayer(self.iface, layer=fmXSLayer)
					if err:
						QMessageBox.warning(self.cmeView, 'Import DAT File',
			...
			error, message = res.Load(filePath)
			if error:
				QMessageBox.critical(self.cmeView, "COASTALME Viewer", 'ERROR: {0} errors occured loading in results: {1}:\n{2}'.format(len(message), filePath, '\n'.join(message)))
				continue
	...
	def importResultsFVBCTide(self, nc_fpath, ns_fpath, use_local_time):
		...
		if error:
			QMessageBox.critical(self.cmeView, "COASTALME Viewer", f'ERROR: loading FV BC tide data:\n{message}')
			return False
		...
		# add result to list widget
		openResults = self.cmeView.OpenResults
		openResultNames = []
		for i in range(openResults.count()):
			openResultNames.append(openResults.item(i).text())
		if res.displayname not in openResultNames:
	...
	def openGis(self, tpc):
		"""
		Opens 1D gis files. Will check if there are any feacmeres in the layer before loading into QGIS.
		...
		if gisPoints is not None:
			...
			if not layer.isValid():
				return False
			if layer.feacmereCount():  # only load if there are any feacmeres
				self.cmeView.project.addMapLayer(layer)
				coastalmeqgis_apply_check_cme_clayer(self.iface, layer=layer)
		# load lines gis file
	...
	def open_gis(self, load: bool):
		if load:
			for result_name in self.new_results:
				for res in self.results1d[result_name]:
					...
					if res.GIS.R and res.gis_region_layer_name:
						...
						if lyr.isValid() and (lyr.feacmereCount() or driver == 'memory'):
							QgsProject.instance().addMapLayer(lyr)
							set_vector_temporal_properties(lyr)
							QgsProject.instance().writeEntry('COASTALME', '{0}/gis_region_lyrid'.format(result_name), lyr.id())
							if isinstance(res, ResData_GPKG):
								pass
							else:
								coastalmeqgis_apply_check_cme_clayer(self.iface, layer=lyr)
					# lines
					if res.GIS.L and res.gis_line_layer_name:
						...
						if lyr.isValid() and (lyr.feacmereCount() or driver == 'memory'):
							QgsProject.instance().addMapLayer(lyr)
							set_vector_temporal_properties(lyr)
							QgsProject.instance().writeEntry('COASTALME', '{0}/gis_line_lyrid'.format(result_name), lyr.id())
							if isinstance(res, ResData_GPKG):
								...
								try:
									...
									apply_cme_style_gpkg_ts(lyr, 'Flow', rt)
								except IndexError:
									pass
							else:
								coastalmeqgis_apply_check_cme_clayer(self.iface, layer=lyr)
					...
					if res.GIS.P and res.gis_point_layer_name:
						...
						if lyr.isValid() and (lyr.feacmereCount() or driver == 'memory'):
							QgsProject.instance().addMapLayer(lyr)
							set_vector_temporal_properties(lyr)
							QgsProject.instance().writeEntry('COASTALME', '{0}/gis_point_lyrid'.format(result_name), lyr.id())
							if isinstance(res, ResData_GPKG):
								# pass layer into result for later use
								res.gis_point_layer = lyr
								try:
									rt = 'Depth' if 'Depth' in res.pointResultTypesTS() else res.pointResultTypesTS()[0]
									apply_cme_style_gpkg_ts(lyr, 'Depth', rt)
								except IndexError:
									pass
							else:
								coastalmeqgis_apply_check_cme_clayer(self.iface, layer=lyr)
			...
			for result_name in self.new_results:
				for res in self.results1d[result_name]:
					if isinstance(res, ResData_GPKG):
						gis_region_lyrid, exists = QgsProject.instance().readEntry('COASTALME', '{0}/gis_region_lyrid'.format(result_name), '')
						if exists:
							res.gis_region_layer = QgsProject.instance().mapLayer(gis_region_lyrid)
						gis_line_lyrid, exists = QgsProject.instance().readEntry('COASTALME', '{0}/gis_line_lyrid'.format(result_name), '')
						if exists:
							res.gis_line_layer = QgsProject.instance().mapLayer(gis_line_lyrid)
						gis_point_lyrid, exists = QgsProject.instance().readEntry('COASTALME', '{0}/gis_point_lyrid'.format(result_name), '')
						if exists:
	...
	def getResultMetaData_old(self, result):
		...
		:param result: COASTALME_results.ResData
		:return: bool -> True for successful, False for unsuccessful
		"""
		...
		date2timekey = self.cmeView.cmeResults.date2timekey
		date2time = self.cmeView.cmeResults.date2time
		zeroTime = self.cmeView.cmeOptions.zeroTime  # datetime

		if result.displayname not in results.keys():
			results[result.displayname] = {}

		if self.cmeView.OpenResults.count() == 0:
			self.cmeView.cmeOptions.zeroTime = datetime2timespec(self.getReferenceTime(result),
			                                                   1, self.cmeView.cmeResults.timeSpec)
			if qv >= 31300:
		...
		timesteps = result.timeSteps(datetime2timespec(self.cmeView.cmeOptions.zeroTime, self.cmeView.cmeResults.loadedTimeSpec, QT_TIMESPEC_UTC))
		for t in timesteps:
			date = self.cmeView.cmeOptions.zeroTime + timedelta(hours=t)
	...
	def updateSelectedResults(self, about_to_be_removed = ()):
		...
		for result in self.cmeView.OpenResults.selectedItems():
			if result.text() in self.cmeView.cmeResults.cmeResults1D.results1d.keys():
				resVersion.append(self.cmeView.cmeResults.cmeResults1D.results1d[result.text()][0].formatVersion)
		
		# collect ids and domain types
		for layerid, layer in QgsProject.instance().mapLayers().items():
			if not isPlotLayer(layer) and not isTSLayer(layer, self.cmeView.cmeResults.results) and not is1dNetwork(layer) and not isBcLayer(layer):
				continue
			if layerid in about_to_be_removed:
				continue
			is_ts_lyr = isTSLayer(layer, self.cmeView.cmeResults.results)
	...
	def getLongPlotConnectivity(self, res):
		...
		:param res: COASTALME_results or COASTALME_results2013
	...
	def remove_gpkg_gis(self, res):
		...
		if res.gis_point_layer:
			try:
				if res.gis_point_layer.id() in self.cmeView.layer_selection_signals:
					res.gis_point_layer.selectionChanged.disconnect(self.cmeView.selectionChanged)
					self.cmeView.layer_selection_signals.remove(res.gis_point_layer.id())
				self.cmeView.project.removeMapLayer(res.gis_point_layer)
			except RuntimeError:
				Logging.warning('Could not remove gis_point_layer')
		if res.gis_line_layer:
			try:
				if res.gis_line_layer.id() in self.cmeView.layer_selection_signals:
					res.gis_line_layer.selectionChanged.disconnect(self.cmeView.selectionChanged)
					self.cmeView.layer_selection_signals.remove(res.gis_line_layer.id())
				self.cmeView.project.removeMapLayer(res.gis_line_layer)
			except RuntimeError:
				Logging.warning('Could not remove gis_line_layer')
		if res.gis_region_layer:
			try:
				if res.gis_region_layer.id() in self.cmeView.layer_selection_signals:
					res.gis_region_layer.selectionChanged.disconnect(self.cmeView.selectionChanged)
					self.cmeView.layer_selection_signals.remove(res.gis_region_layer.id())
				self.cmeView.project.removeMapLayer(res.gis_region_layer)
			except RuntimeError:
		...
		if self.cmeView.iface:
			self.cmeView.iface.mapCanvas().refresh()
	...
	def getReferenceTime(self, result, defaultZeroTime=None):
		...
		if rt is not None:
			...
			if defaultZeroTime is not None:
				...
				if qv >= 31600:
					return self.cmeView.cmeOptions.zeroTime
				else:
	...
	def configTemporalProperties(self, result):
		...
		if qv >= 31300:
			if not result.has_reference_time or self.cmeView.cmeResults.loadedTimeSpec != self.cmeView.cmeResults.timeSpec:
		...
		if resname in self.results1d:
			for result in resname:
				if not result.has_reference_time:
					result.reference_time = datetime2timespec(self.cmeView.cmeOptions.zeroTime, self.cmeView.cmeResults.loadedTimeSpec, QT_TIMESPEC_UTC)
	...
	def loadTSResultLayer(self, layer):
		...
		if re.findall(r'_TS(MB|MB1d2d)?(_[PLR])?$', name, flags=re.IGNORECASE):
			...
			self.cmeView.cmeResults.results[name] = {}
			self.cmeView.OpenResults.addItem(name)
