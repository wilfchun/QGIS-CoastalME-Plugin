...
from ..coastalmeqgis_library import loadSetting, roundSeconds, \
...
class CmeResults2D():
	...
	def importResults(self, inFileNames):
		...
		for j, f in enumerate(inFileNames):
			...
			# ask user if coastalme viewer should copy results to temporary directory
			if j == 0 and self.cmeView.cmeOptions.show_copy_mesh_dlg:
			...
			if os.path.splitext(f)[1].lower() != '.nc':
				if type(inFileNames) is dict:  # being loaded in from a sup file
					...
					for d in datasets:
						if copy_res:
							try:
								d = str(tmp_res.copy(self.cmeView, d))
				...
				pass
				#self.cmeView.cmeResults.cmeResults3D.importResults([f])
			#res = {'path': f}
			...
			name = mLayer.name()
			mLayer.nameChanged.connect(lambda: self.layerNameChanged(mLayer, name, mLayer.name()))  # if name is changed can capcmere this in indexing
			...
			rsMesh.setEnabled(self.cmeView.cmeOptions.showGrid)
			rs.setNativeMeshSettings(rsMesh)
			mLayer.setRendererSettings(rs)
			#mLayer.repaintRequested.connect(lambda: self.cmeView.repaintRequested(mLayer))
			...
			if qv >= 31600:
				self.cmeView.cmeResults.updateDateTimes(get_metadata=False)
	...
	def loadMeshLayer(self, fpath, **kwargs):
		...
		layer = coastalmeqgis_find_layer(basename)
		if layer is not None:
			return layer, basename, True
		# COASTALME FV 2dm is named differently so also check provided name does not exist
		if name is not None:
			layer_alternative = coastalmeqgis_find_layer(name)
		...
		if fext.lower() == '.xmdf' or fext.lower() == '.dat' or fext.lower() == '.2dm':
			...
			if self.iface is not None:
				QMessageBox.information(self.iface.mainWindow(), "COASTALME Viewer",
		...
		if not os.path.exists(mesh):
			...
			inFileNames = QFileDialog.getOpenFileNames(self.cmeView.iface.mainWindow(), 'Mesh file location', fpath,
			                                           "COASTALME Mesh File (*.2dm)")
			if not inFileNames[0]:  # empty list
				...
				if os.path.exists(inFileNames[0][0]):
					...
					if self.iface is not None:
						QMessageBox.information(self.iface, "COASTALME Viewer", "Could not find mesh file")
					else:
		...
		if layer.isValid():
			if fext.lower() == '.nc':
				...
				self.cmeView.cmeOptions.resolution = cellSize
				return layer, name, False

		if self.iface is not None:
			QMessageBox.information(self.iface.mainWindow(), "COASTALME Viewer", "Could not load mesh file. Please check validity of this file (e.g. not empty):\n {0}".format(mesh))
		else:
	...
	def getResultMetaData(self, name, layer, ext='', hadtp=None, loadRenderStyle=True):
		...
		timekey2date = self.cmeView.cmeResults.timekey2date  # dict
		time2date = self.cmeView.cmeResults.time2date  # dict
		date2timekey = self.cmeView.cmeResults.date2timekey  # dict
		date2time = self.cmeView.cmeResults.date2time  # dict
		zeroTime = self.cmeView.cmeOptions.zeroTime
		...
		if self.cmeView.OpenResults.count() == 0:
			self.cmeView.cmeOptions.zeroTime = datetime2timespec(self.getReferenceTime(layer),
			                                                   QT_TIMESPEC_UTC, self.cmeView.cmeResults.timeSpec)
			if qv >= 31300:
				if self.iface is not None:
		...
		# cmern on temporal properties - record whether original layer had a reference time
		...
		if hadtp is None:
			if qv >= 31300:
				...
				hadtp = False
		if self.cmeView.cmeProject is not None:
			if type(self.cmeView.cmeProject.hastp) is bool:
				hadtp = self.cmeView.cmeProject.hastp
			elif type(self.cmeView.cmeProject.hastp) is dict:
				if name in self.cmeView.cmeProject.hastp:
					hadtp = self.cmeView.cmeProject.hastp[name]  # override hadtp with this
		self.configTemporalProperties(layer)

		# this is to capcmere minimums - loop through the result names once and figure out if there are double ups that are static
		mdGroupNames = []
		for i in range(layer.datasetGroupCount()):
		...
		for i, name_ in enumerate(mdGroupNames):
			if 'minimum dt' in name_.lower():
				if self.cmeView.cmeResults.isMaximumResultType(name_, layer, i):
			...
			if count > 1:
				if self.cmeView.cmeResults.isStatic(name_, layer, i):
					if ext.upper() == '.XMDF':
		...
		for i, name_ in enumerate(mdGroupNames):
			...
			if 'minimum dt' in id.lower():
				if self.cmeView.cmeResults.isMaximumResultType(id, layer, i):
			...
			results[name][id] = {'times': {},
			                     ...
			                     # 'referenceTime': self.cmeView.cmeOptions.zeroTime,
			                     'referenceTime': self.getReferenceTime(layer),
			                     'isMax': self.cmeView.cmeResults.isMaximumResultType(id, layer, i),
			                     'isMin':self.cmeView.cmeResults.isMinimumResultType(id, layer, i),
			                     'isStatic': self.cmeView.cmeResults.isStatic(id, layer, i),
			                     'isTemporal': self.cmeView.cmeResults.isTemporal(id, layer, i),
			...
			if loadRenderStyle:
				if id:
					resultType = self.cmeView.cmeResults.stripMaximumName(mdGroup.name())
					resultType = self.cmeView.cmeResults.stripMaximumName(resultType)
				...
				if mdGroup.isVector() or id2:
					vectorProperties = QSettings().value('COASTALME_vectorRenderer/vector')
					if vectorProperties and len(vectorProperties) >= 27 and vectorProperties[:27] == '<!DOCTYPE coastalme_meshlayer>':
	...
	def getTimeUnit(self, lyr):
		...
		if 31100 <= qv < 31300:
			...
			return cme2text[lyr.timeSettings().providerTimeUnit()]
		else:
			return 'h'
	...
	def recordTime_old(self, layer, mdg, md, name, id, id2, i, j):
		...
		t = md.time()
		# if self.getReferenceTime(layer) != self.cmeView.cmeOptions.zeroTime:
		if self.getReferenceTime(layer) != \
				datetime2timespec(self.cmeView.cmeOptions.zeroTime,
				                  self.cmeView.cmeResults.loadedTimeSpec,
				                  1):
		...
		for k, x in enumerate([id, id2]):
			if x is not None:
				...
				if self.cmeView.cmeOptions.timeUnits == 's':
					...
					# date = datetime2timespec(self.cmeView.cmeOptions.zeroTime,
					#                          self.cmeView.cmeResults.loadedTimeSpec,
					#                          self.cmeView.cmeResults.timeSpec) \
					#        + timedelta(seconds=t)
				else:
					try:
				...
				if qv >= 31300:
					# date_tspec = datetime2timespec(date, self.cmeView.cmeResults.loadedTimeSpec, 1)
					date_tspec = datetime2timespec(date, self.cmeView.cmeResults.loadedTimeSpec, QT_TIMESPEC_UTC)
				else:
					date_tspec = date
				self.cmeView.cmeResults.timekey2date_tspec['{0:.6f}'.format(t)] = date_tspec
				self.cmeView.cmeResults.time2date_tspec[t] = date_tspec
				self.cmeView.cmeResults.date_tspec2timekey[date_tspec] = '{0:.6f}'.format(t)
				self.cmeView.cmeResults.date_tspec2time[date_tspec] =  t
				self.cmeView.cmeResults.date2date_tspec[date] = date_tspec
				self.cmeView.cmeResults.date_tspec2date[date_tspec] = date
	...
	def recordSpecialTime(self, name, t, id, id2, i, j, ext):
		...
		if ext.upper() == '.DAT':
			# ri = CmeResultsIndex(name, id)
			ri = CmeResultsIndex(name, id, None, False, False, self.cmeView.cmeResults, self.cmeView.cmeOptions.timeUnits)
			timeUnits = self.cmeView.cmeResults.getTimeUnit(ri)  # is the same for the whole layer
			if t == 900001.0 and timeUnits == 'h':  # time of peak h
				...
				if not self.cmeView.cmeResults.isMaximumResultType(id):
					...
					if id2 is not None:
						if not self.cmeView.cmeResults.isMaximumResultType(id2):
	...
	def checkRecordSpecialTime(self, time, specialName):
		...
		if self.bRecordSpecialTime is None:
			...
			msg = QMessageBox(QT_MESSAGE_BOX_QUESTION, "Special Times", "Found potential special times in results. "
			                                                         "Would you like COASTALME Viewer to interpret these "
		...
		if ext.upper() == ".XMDF" and (mdg.name() == 'Velocity' or mdg.name() == 'Velocity/Maximums' or
			...
			if mdg.isVector():
				if id is None:
					# if name is not None
					id = self.addCounter(name_, ids)
					if self.cmeView.cmeResults.isMaximumResultType(name_) or self.cmeView.cmeResults.isMinimumResultType(name_):
					...
					# 	if CmeResults.isMaximumResultType(mdg.name()) or CmeResults.isMinimumResultType(mdg.name()):
			...
			if layer is not None:
				...
				for a in args:
					if isinstance(a, list) and a and isinstance(a[0], str):
						layer_for_removal = [x for x in a if coastalmeqgis_find_layer(x, search_type='layerId') and coastalmeqgis_find_layer(x, search_type='layerId') == layer]
				...
				if isinstance(layer, QgsMeshLayer):
					if item.isSelected():
						...
						self.renderMap(layers=[layer], cmern_off=True)
	...
	def renderMap(self, layers=(), cmern_off=False):
		"""
		...
		self.cmeView.cmeResults.checkSelectedResults()
		...
		for layer in layers:
			...
			if cmern_off:
				...
				if node is not None and not cmern_off:
					node.setItemVisibilityChecked(False)
			else:
				# if node is not None:
				# 	node.setItemVisibilityChecked(True)
				activeScalarIndex = CmeResultsIndex(layer.name(), self.activeScalar,
												   self.cmeView.cmeResults.activeTime, self.cmeView.cmeResults.isMax('scalar'),
												   self.cmeView.cmeResults.isMin('scalar'), self.cmeView.cmeResults, self.cmeView.cmeOptions.timeUnits)
				activeVectorIndex = CmeResultsIndex(layer.name(), self.activeVector,
												   self.cmeView.cmeResults.activeTime, self.cmeView.cmeResults.isMax('vector'),
												   self.cmeView.cmeResults.isMin('vector'), self.cmeView.cmeResults, self.cmeView.cmeOptions.timeUnits)
			# if node is not None and not node.isVisible():
			# 	pass
			# else:
			activeScalarMeshIndex = self.cmeView.cmeResults.getResult(activeScalarIndex, force_get_time='next lower',
																	mesh_index_only=True)
			activeVectorMeshIndex = self.cmeView.cmeResults.getResult(activeVectorIndex, force_get_time='next lower',
																	mesh_index_only=True)
			...
			# cmern on / off mesh and triangles
		...
		try:
			self.cmeView.canvas.mapCanvasRefreshed.disconnect(self.renderMap)
		except:
			pass
	...
	def averingMethodOverride(self, activeScalarIndex, activeVectorIndex, rs):
		...
		if (activeScalarIndex and activeScalarIndex.result and activeScalarIndex.resultType and
				self.cmeView.cmeResults.results[activeScalarIndex.result].get(activeScalarIndex.resultType) and
				self.cmeView.cmeResults.results[activeScalarIndex.result][activeScalarIndex.resultType]['ext'] == '.nc') or \
				(activeVectorIndex and activeVectorIndex.result and activeVectorIndex.resultType and
				 self.cmeView.cmeResults.results[activeVectorIndex.result].get(activeVectorIndex.resultType) and
				 self.cmeView.cmeResults.results[activeVectorIndex.result][activeVectorIndex.resultType]['ext'] == '.nc'):
			pass
		else:
			return
		...
		if activeScalarIndex.result and activeScalarIndex.resultType:
			...
			res = self.cmeView.cmeResults.results[activeScalarIndex.result][activeScalarIndex.resultType]
		elif activeVectorIndex.result and activeVectorIndex.resultType:
			result_name = activeVectorIndex.result
			res = self.cmeView.cmeResults.results[activeVectorIndex.result][activeVectorIndex.resultType]
		else:
			result_name = ''
			res = None

		if (self.cmeView.cmeResults.isMax('scalar') or self.cmeView.cmeResults.isMax('vector')) and \
		...
		for res in resList:
			if res in results.keys():
				...
				for resultType in list(results[res].keys()):
					...
					if self.cmeView.cmeResults.isMapOutputType(resultType):
	...
	def loadOpenMeshLayers(self, **kwargs):
		...
		for ml in meshLayers:
			...
			if layer is not None:
				...
				if layer.datasetGroupCount() == 0:
					...
					self.cmeView.OpenResults.addItem(ml)

					k = self.cmeView.OpenResults.findItems(layer.name(), QT_MATCH_RECURSIVE)[0]
					k.setSelected(True)
		...
		if need2connect:
			self.cmeView.resultSelectionChangeSignal = self.cmeView.OpenResults.itemSelectionChanged.connect(
				lambda: self.cmeView.resultsChanged('item clicked'))
			self.cmeView.resultsChanged('item clicked')
	...
	def layerReloaded(self, layer):
		...
		for result_name, result_items in results.items():
			if isinstance(result_items, dict):
				for rtype in list(result_items.keys()):
					if self.cmeView.cmeResults.isMapOutputType(rtype):
		...
		if hasattr(self.cmeView, 'timer') and self.cmeView.timer is not None:  # start timer to reload results
	...
	def layerStyleChanged(self, layer):
		...
		active_dataset = self.cmeView.cmeResults.stripMaximumName(active_dataset)
		active_dataset = self.cmeView.cmeResults.stripMinimumName(active_dataset)
		...
		if new_style != self.active_style:
			...
			for i in range(layer.datasetGroupCount()):
				...
				dataset = self.cmeView.cmeResults.stripMaximumName(dataset)
				dataset = self.cmeView.cmeResults.stripMinimumName(dataset)
				if dataset == active_dataset:
					doc = QDomDocument('coastalme_meshlayer')
					stacmesOK, errorStr, errorLine, errorColumn = doc.setContent(self.active_style, True)
					if not stacmesOK:
	...
	def repaintRequested(self, *args, **kwargs):
		...
		updated = False
		for r in results:
			if coastalmeqgis_find_layer(r) is not None:
				layer = coastalmeqgis_find_layer(r)
				if not isinstance(layer, QgsMeshLayer):
					continue
				for restype in results[r]:
					if self.cmeView.cmeResults.isMapOutputType(restype):
	...
	def applyScalarRenderSettings(self, layer, datasetGroupIndex, file, type, save_type='xml'):
		...
		if type == 'ramp':
			if rsScalar.colorRampShader().colorRampItemList():
				...
				doc = QDomDocument()
				xml = QFile(file)
				stacmesOK, errorStr, errorLine, errorColumn = doc.setContent(xml, True)
				if stacmesOK:
	...
	def applyVectorRenderSettings(self, layer, datasetGroupIndex, vectorProperties):
		...
		doc = QDomDocument('coastalme_meshlayer')
		stacmesOK, errorStr, errorLine, errorColumn = doc.setContent(vectorProperties, True)
		if not stacmesOK:
			return False
	...
	def layerNameChanged(self, layer, oldName, newName):
		...
		if isinstance(layer, QgsRasterLayer):
			self.cmeView.cmeResults.cmeResultsNcGrid.layerNameChanged(oldName, newName)
		
		# change name in list widget
		selectedItems = self.cmeView.OpenResults.selectedItems()
		for i in range(self.cmeView.OpenResults.count()):
			item = self.cmeView.OpenResults.item(i)
			if item.text() == oldName:
				self.cmeView.OpenResults.takeItem(i)
				self.cmeView.OpenResults.insertItem(i, newName)
				if item in selectedItems:
					self.cmeView.OpenResults.item(i).setSelected(True)
				...
				if self.cmeView.cmeResults.isMapOutputType(resultType):
					if len(results[result][resultType]['times']) == 1:
						for i in list(results[result][resultType]['times'].keys())[:]:
							...
							if qv < 31600:
								a = sorted([x for x in self.cmeView.cmeResults.time2date.keys()])
								if a[0] != firstTime:
									del self.cmeView.cmeResults.time2date[a[0]]

									a = sorted([x for x in self.cmeView.cmeResults.timekey2date.keys()])
									del self.cmeView.cmeResults.timekey2date[a[0]]

									a = sorted([x for x in self.cmeView.cmeResults.timekey2time.keys()])
									del self.cmeView.cmeResults.timekey2time[a[0]]

									a = sorted([x for x in self.cmeView.cmeResults.date2time.keys()])
									del self.cmeView.cmeResults.date2time[a[0]]

									a = sorted([x for x in self.cmeView.cmeResults.date2timekey.keys()])
									del self.cmeView.cmeResults.date2timekey[a[0]]
							else:
								a = sorted([x for x in self.cmeView.cmeResults.timekey2time.keys()])
								if a[0] != timeKey:
									del self.cmeView.cmeResults.timekey2time[a[0]]
	...
	def renderNativeMesh(self, layer, rs):
		...
		if layer not in self.activeMeshLayers:
			if rs.nativeMeshSettings().isEnabled() or self.cmeView.cmeOptions.showGrid:
				rsMesh = rs.nativeMeshSettings()
				rsMesh.setEnabled(False)
				rs.setNativeMeshSettings(rsMesh)
				# layer.setRendererSettings(rs)
			if rs.triangularMeshSettings().isEnabled() or self.cmeView.cmeOptions.showTriangles:
				rsTriangles = rs.triangularMeshSettings()
				rsTriangles.setEnabled(False)
				rs.setTriangularMeshSettings(rsTriangles)
				# layer.setRendererSettings(rs)
		else:
			if rs.nativeMeshSettings().isEnabled() != self.cmeView.cmeOptions.showGrid:
				rsMesh = rs.nativeMeshSettings()
				rsMesh.setEnabled(self.cmeView.cmeOptions.showGrid)
				rs.setNativeMeshSettings(rsMesh)
				# layer.setRendererSettings(rs)
			self.cmeView.cmePlot.cmePlotToolbar.meshGridAction.setChecked(self.cmeView.cmeOptions.showGrid)
			if rs.triangularMeshSettings().isEnabled() != self.cmeView.cmeOptions.showTriangles:
				rsTriangles = rs.triangularMeshSettings()
				rsTriangles.setEnabled(self.cmeView.cmeOptions.showTriangles)
				rs.setTriangularMeshSettings(rsTriangles)
				# layer.setRendererSettings(rs)
	...
	def getReferenceTime(self, layer, defaultZeroTime=None):
		...
		if rt is not None and rt.isValid():
			...
			if defaultZeroTime is not None:
				# return datetime2timespec(defaultZeroTime, 1, self.cmeView.cmeResults.timeSpec)
				return defaultZeroTime
			elif self.cmeView.cmeResults.tmp_reference_time is not None:
				return self.cmeView.cmeResults.tmp_reference_time
			else:
				# return datetime2timespec(self.cmeView.cmeOptions.zeroTime, self.cmeView.cmeResults.loadedTimeSpec, self.cmeView.cmeResults.timeSpec)
				return datetime2timespec(self.cmeView.cmeOptions.zeroTime, self.cmeView.cmeResults.loadedTimeSpec, QT_TIMESPEC_UTC)
	...
	def configTemporalProperties(self, layer):
		...
		if qv >= 31300:
			...
			if not tp.referenceTime().isValid() or self.cmeView.cmeResults.loadedTimeSpec != self.cmeView.cmeResults.timeSpec:
	...
	def getBedAndWLNames(self, layer, isMax=False, isMin=False):
		...
		for i in range(layer.datasetGroupCount()):
			...
			if name.lower() in possibleBdNames:
				...
				continue
			if isMax:
				if self.cmeView.cmeResults.isMaximumResultType(name):
					name = self.cmeView.cmeResults.stripMaximumName(name)
					if name.lower() in possibleWlNames:
						wlname = name
			elif isMin:
				if self.cmeView.cmeResults.isMinimumResultType(name):
					name = self.cmeView.cmeResults.stripMinimumName(name)
					if name.lower() in possibleWlNames:
						wlname = name
			else:
				if name.lower() in possibleWlNames:
	...
	def layerVisibilityChanged(self):
		...
		for res in self.results2d:
			...
			if layer is None:
				continue
			node = QgsProject.instance().layerTreeRoot().findLayer(layer.id())
			k = self.cmeView.OpenResults.findItems(res, QT_MATCH_RECURSIVE)[0]
			k.setSelected(node.itemVisibilityChecked())

		updated = self.updateActiveMeshLayers()  # update list of active mesh layers
		self.cmeView.resultsChanged('item clicked')  # update coastalme viewer
