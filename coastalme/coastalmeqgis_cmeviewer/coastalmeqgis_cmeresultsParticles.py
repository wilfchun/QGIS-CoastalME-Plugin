...
try:
	from ..COASTALME_particles_data_provider import CmeParticlesDataProvider
	have_netCDF4 = True
except ImportError:
	have_netCDF4 = False
...
class CmeResultsParticles():
	...
	def __init__(self, CmeView):
		...
		self.debug = self.cmeView.cmeOptions.particlesWriteDebugInfo  # ES
	...
	def _load_file_old(self, filename):
		...
		if self.cmeView.OpenResults.count() == 0:
			defaultRefTime = self.cmeView.cmeOptions.defaultZeroTime
		else:
			defaultRefTime = datetime2timespec(self.cmeView.cmeOptions.zeroTime, self.cmeView.cmeResults.loadedTimeSpec, QT_TIMESPEC_UTC)

		particles_data_provider = CmeParticlesDataProvider()
		if particles_data_provider.load_file(filename, defaultRefTime):
			...
			self.cmeView.cmeResults.results[displayname] = {}
			# ES added CRS
			...
			if self.cmeView.OpenResults.count() == 0:
				if qv >= 31300:
					self.cmeView.cmeOptions.zeroTime = datetime2timespec(zeroTime, 1, self.cmeView.cmeResults.timeSpec)
					...
					self.cmeView.cmeOptions.zeroTime = zeroTime  # ES
			# if self.cmeView.cmeResults.timeSpec != self.cmeView.cmeResults.loadedTimeSpec:
			...
			timesteps = particles_data_provider.timeSteps(datetime2timespec(self.cmeView.cmeOptions.zeroTime,
			...
			self.cmeView.cmeResults.results[displayname]["_particles"] = [timesteps]
	...
	def _load_file_31600(self, filename):
		...
		if self.cmeView.OpenResults.count() == 0:
			# defaultRefTime = self.cmeView.cmeOptions.defaultZeroTime
			defaultRefTime = self.cmeView.cmeOptions.zeroTime
		else:
			defaultRefTime = datetime2timespec(self.cmeView.cmeOptions.zeroTime, self.cmeView.cmeResults.loadedTimeSpec, QT_TIMESPEC_UTC)
		...
		if particles_data_provider.load_file(filename, defaultRefTime):
			...
			timesteps = particles_data_provider.timeSteps(datetime2timespec(self.cmeView.cmeOptions.zeroTime, QT_TIMESPEC_UTC, QT_TIMESPEC_UTC))
			for t in timesteps:
				timekey2time['{0:.6f}'.format(t)] = t

			self.cmeView.cmeResults.results[displayname]["_particles"] = {'times': {'{0:.6f}'.format(x): [x] for x in timesteps},
	...
	def removeResults(self, resList, **kwargs):
		...
		for res in resList:
			if res in self.resultsParticles:
				...
				if remove_vlayer:
					try:
						self.cmeView.project.removeMapLayer(vlayer.id())
						del vlayer
					except:
						pass
	...
	def updateActiveTime_old(self, time=None):
		...
		from .coastalmeqgis_cmeresults import CmeResults  # ES
		self.debug = self.cmeView.cmeOptions.particlesWriteDebugInfo  # ES

		if time is None:
			active_time = self.cmeView.cmeResults.activeTime
			if active_time is None:
				global_relative_time = 0
			else:
				global_relative_time = self.cmeView.cmeResults.timekey2time[active_time]
		else:
		...
		for _, data in self.resultsParticles.items():
			...
			for i, relative_time in enumerate(timesteps):
				...
				if isSame_float(relative_time, global_relative_time, prec=CmeResults.TimePrecision):  # ES
					break  # ES
			...
			if vlayer is not None:
				...
				points = self._get_feacmeres(particles_data_provider, vlayer, time_index)
	...
	def _get_feacmeres(self, particles_data_provider, vlayer, time_index):
		points = []
		...
		if self.debug:  # ES keep coords in attribute table
			...
			relTime = particles_data_provider.timeSteps(self.cmeView.cmeOptions.zeroTime)[time_index]
		...
		for i, stat in enumerate(stats):
			...
			if int(stat) > 0:
				...
				for attr in data.keys():
					feat['id'] = i
					feat[attr] = float(
						data.get(attr)[i])  # must be converted to primitive type, otherwise feacmere data wont be stored
				if self.debug:  # ES
					feat['_absTime'] = absTime
					feat['_relTime'] = relTime
					feat['_dateTime'] = self.cmeView.cmeResults._dateFormat.format(dt)
				points.append(feat)
	...
	def reloadTimesteps(self, resname):
		...
		if not particles_data_provider.has_reference_time:
			particles_data_provider.default_reference_time = datetime2timespec(self.cmeView.cmeOptions.zeroTime, self.cmeView.cmeResults.loadedTimeSpec, 1)
		...
		self.cmeView.cmeResults.results[resname]["_particles"] = [timesteps]
