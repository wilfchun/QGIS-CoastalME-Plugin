...
class CmeFlowLine(CmeRubberBand):
	...
	def startRubberBand(self):
		...
		if not self.checkIsPossible():
			QMessageBox.information(self.cmeView, "Flux Calculation",
			                        "Flux Calculation Requires Temporal Depth and Velocity Results.")
			return False
		else:
			CmeRubberBand.startRubberBand(self)
	...
	def checkIsPossible(self):
		...
		possible = False
		for meshLayer in self.cmeView.cmeResults.cmeResults2D.activeMeshLayers:
			foundDepth = False
			foundWL = False
			foundVel = False
...
class CmeFlowLine2():
	...
	def checkIsPossible(self):
		...
		for meshLayer in self.cmeView.cmeResults.cmeResults2D.activeMeshLayers:
			...
			for resultType in result:
				if CmeResults.isMapOutputType(resultType) and not CmeResults.isMaximumResultType(resultType) \
						and not CmeResults.isMinimumResultType(resultType):  # not a 1D result
					if 'dep' in resultType.lower() and 'time' not in resultType.lower() and \
							'dur' not in resultType.lower():
						foundDepth = True
					elif resultType[0].lower() == 'd' and 'time' not in resultType.lower() and \
	...
	def startRubberBand(self):
		...
		# check for velocity and one of depth or water level - to see if it is possible to get flow
		if not self.checkIsPossible():
			QMessageBox.information(self.cmeView, "Flux Calculation", "Flux Calculation Requires Temporal Depth and Velocity Results.")
			return False
		...
		if not self.cursorTrackingConnected:
			...
			if not multi:  # simple, can remove previous flow lines and start again
				self.cmePlot.clearPlot(0, retain_1d=True, retain_2d=True)  # clear plot of flow results only
	...
	def createMemoryLayer(self):
		...
		self.cmePlot.cmePlot2D.multiFlowLineSelectCount = len(self.rubberBands)  # force counting to match number of rubberbands
		...
		worked = self.cmePlot.cmePlot2D.plotFlowFromMap(None, feat)
		if not worked:
