...
class CmePlot2D():
	...
	def __init__(self, CmePlot=None):
		if CmePlot is not None:
			self.cmePlot = CmePlot
			self.cmeView = CmePlot.cmeView
			self.cmeResults = self.cmeView.cmeResults
	...
	def plotTimeSeriesFromMap(self, vLayer, point, **kwargs):
		...
		activeMeshLayers = self.cmeResults.cmeResults2D.activeMeshLayers[:]
		activeMeshLayers.extend([x for x in self.cmeResults.cmeResultsNcGrid.grids()])
		results = self.cmeResults.results  # dict
		...
		if type(point) is QgsFeature:
			point = point.geometry().asPoint()  # if feacmere is passed in as argument, convert to QgsPointXY
		...
		dataType = kwargs['data_type'] if 'data_type' in kwargs else CmePlot.DataTimeSeries2D
		overwrite = kwargs['overwrite'] if 'overwrite' in kwargs else False

		do_profiling = self.cmeView.cmeOptions.profile_plotting_tasks
		
		# clear the plot based on kwargs
		if bypass:
			pass
		# elif self.cmeView.cboSelectType.currentText() == 'From Map Multi':  # only clear last entry
		# 	# self.cmePlot.clearPlotLastDatasetOnly(CmePlot.TimeSeries)
		else:
			# if plot.lower() == '2d only':
			# 	self.cmePlot.clearPlot(0, retain_flow=retainFlow)
			# else:
			# 	self.cmePlot.clearPlot(0, retain_1d=True, retain_flow=True)
			#if not resultTypes:  # specified result types can be passed through kwargs (used for batch export not normal plotting)
			#	resultTypes = self.cmePlot.cmePlotToolbar.getCheckedItemsFromPlotOptions(dataType)
			...
			self.cmePlot.clearPlot2(CmePlot.TimeSeries, dataType,
			                       last_only=self.cmeView.cboSelectType.currentText() == 'From Map Multi',
		...
		for layer in resultMesh:  # get plotting for all selected result meshes
			...
			if not resultTypes:  # specified result types can be passed through kwargs (used for batch export not normal plotting)
				...
				if plotActiveScalar:
					if plotActiveScalar == 'active scalar':
						if self.cmeView.cmeResults.cmeResults2D.activeScalar not in resultTypes:
							resultTypes += [self.cmeView.cmeResults.cmeResults2D.activeScalar]
					else:  # do both active scalar and [depth for water level] - cmemap specific
						if plotActiveScalar not in resultTypes:
							resultTypes.append(plotActiveScalar)
						if plotActiveScalar == 'Depth' or plotActiveScalar == 'D':
							if 'Water Level' in self.cmeResults.results[layer.name()]:
								if 'Water Level' not in resultTypes:
									resultTypes.append('Water Level')
							elif 'H' in self.cmeResults.results[layer.name()]:
			...
			for rtype in resultTypes:
				...
				cmeResultsIndex = CmeResultsIndex(layer.name(), rtype, None, False, False, self.cmeView.cmeResults, self.cmeView.cmeOptions.timeUnits)
				r = self.cmeView.cmeResults.getResult(cmeResultsIndex)  # r = dict - { str time: [ float time, QgsMeshDatasetIndex ] }
				if not r or r == -1:
					continue
				gmd = None
		...
		if data:
			if export is None:  # normal plot i.e. in cmeview
				self.cmePlot.drawPlot(CmePlot.TimeSeries, data, labels, types, dataTypes, draw=draw, time=time, show_current_time=showCurrentTime)
				...
				if not overwrite:
					while os.path.exists(outFile):
						...
						iterator += 1
				self.cmePlot.drawPlot(CmePlot.TimeSeries, data, labels, types, dataTypes, export=outFile)
			elif export == 'csv':  # export to csv, don't plot
				if 'types' in kwargs:
					del kwargs['types']
				self.cmePlot.exportCSV(CmePlot.TimeSeries, data, labels, types, exportOut, name, **kwargs)
	...
	def plotCrossSectionFromMap(self, vLayer, feat, **kwargs):
		...
		debug = self.cmeView.cmeOptions.writeMeshIntersects
		activeMeshLayers = self.cmeResults.cmeResults2D.activeMeshLayers[:]
		activeMeshLayers.extend([x for x in self.cmeResults.cmeResultsNcGrid.grids()])
		results = self.cmeResults.results  # dict
		...
		dataType = kwargs['data_type'] if 'data_type' in kwargs else CmePlot.DataCrossSection2D
		overwrite = kwargs['overwrite'] if 'overwrite' in kwargs else False
		...
		if bypass:
			pass
		# elif self.cmeView.cboSelectType.currentText() == 'From Map Multi':  # only clear last entry
		# 	self.cmePlot.clearPlotLastDatasetOnly(CmePlot.CrossSection)
		else:
			# if plot.lower() == '2d only':
			# 	self.cmePlot.clearPlot(1)
			# else:
			# 	self.cmePlot.clearPlot(1, retain_1d=True)
			# if not resultTypes:  # specified result types can be passed through kwargs (used for batch export not normal plotting)
			# 	resultTypes = self.cmePlot.cmePlotToolbar.getCheckedItemsFromPlotOptions(dataType)
			if not resultMesh:  # specified result meshes can be passed through kwargs (used for batch export not normal plotting)
				resultMesh = activeMeshLayers
			self.cmePlot.clearPlot2(CmePlot.CrossSection, dataType,
			                       last_only=self.cmeView.cboSelectType.currentText() == 'From Map Multi',
			...
			try:
				...
				for j, rtype in enumerate(resultTypes):
					...
					if timestep == 'Maximum' or timestep == 99999 or timestep == '99999.000000':
						...
						isMax = self.cmeView.cmeResults.isMax(rtype)
					if timestep == 'Minimum' or timestep == -99999 or timestep == '-99999.000000':
						isMin = True
					else:
						isMin = self.cmeView.cmeResults.isMin(rtype)
					# get result data for open mesh results, selected scalar datasets, and active time
					cmeResultsIndex = CmeResultsIndex(layer.name(), rtype, timestep, isMax, isMin, self.cmeView.cmeResults, self.cmeView.cmeOptions.timeUnits)
					if not self.cmeView.cmeResults.getResult(cmeResultsIndex, force_get_time='next lower'):
						continue
					elif type(self.cmeView.cmeResults.getResult(cmeResultsIndex, force_get_time='next lower')) is dict:
						continue
					elif self.cmeView.cmeResults.getResult(cmeResultsIndex, force_get_time='next lower') == -1:
						continue
					types.append(rtype)
					meshDatasetIndex = self.cmeView.cmeResults.getResult(cmeResultsIndex, force_get_time='next lower')[-1]
					gmd = None
					if isinstance(layer, QgsMeshLayer):
						gmd = dp.datasetGroupMetadata(meshDatasetIndex.group())
						# if self.cmeView.cmeResults.isMax(rtype):
						# if self.cmeView.cmeResults.isMax(gmd.name()):
						if CmeResults.isMaximumResultType(gmd.name()):
							if rtype.lower() == 'minimum dt':
								rtype = '{0}/Final'.format(rtype)
							else:
								rtype = '{0}/Maximums'.format(rtype)
						# elif self.cmeView.cmeResults.isMin(rtype):
						elif CmeResults.isMinimumResultType(gmd.name()):
					...
					if j == 0 or onVertices is None or onVerticesCurr != onVertices:
						...
						if update:
							...
							if isinstance(layer, QgsMeshLayer):
								inters, ch, fcs = findMeshIntersects(si, dp, mesh, feat, crs, self.cmeView.project, self.iface)
						...
						if j == 0 and not update and debug:
							...
							if polys:
								writeTempPolys(polys, self.cmeView.project, crs)
							writeTempPoints(inters, self.cmeView.project, crs, chainage, 'Chainage',
											QT_DOUBLE)
		...
		if data:
			if export is None:  # normal plot i.e. in cmeview
				self.cmePlot.drawPlot(CmePlot.CrossSection, data, labels, types, dataTypes, draw=draw, geom=feat.geometry())
			elif export == 'image':  # plot through drawPlot however instead of drawing, save figure
				# unique output file name
				outFile = '{0}{1}'.format(os.path.join(exportOut, name), exportFormat)
				iterator = 1
				if not overwrite:
					while os.path.exists(outFile):
						outFile = '{0}_{2}{1}'.format(os.path.join(exportOut, name), exportFormat, iterator)
						iterator += 1
				self.cmePlot.drawPlot(CmePlot.CrossSection, data, labels, types, dataTypes, export=outFile)
			elif export == 'csv':  # export to csv, don't plot
				if 'types' in kwargs:
					del kwargs['types']
				self.cmePlot.exportCSV(CmePlot.CrossSection, data, labels, types, exportOut, name, **kwargs)
			else:  # catch all other cases and just do normal, although should never be triggered
	...
	def plotFlowFromMap(self, vLayer, feat, **kwargs):
		...
		debug = self.cmeView.cmeOptions.writeMeshIntersects
		activeMeshLayers = self.cmeResults.cmeResults2D.activeMeshLayers  # list
		results = self.cmeResults.results  # dict
		...
		resolution = kwargs['resolution'] if 'resolution' in kwargs.keys() else self.cmeView.cmeOptions.resolution
		featName = kwargs['featName'] if 'featName' in kwargs else None
		...
		if bypass:
			...
			# 	self.cmePlot.clearPlot(1)
			# else:
			# 	self.cmePlot.clearPlot(1, retain_1d=True, retain_2d=True)
		...
		for layer in activeMeshLayers:
			...
			for resultType in results[layer.name()]:
				if CmeResults.isMapOutputType(resultType) and not CmeResults.isMaximumResultType(resultType) \
						and not CmeResults.isMinimumResultType(resultType):
					if 'vel' in resultType.lower() and 'time' not in resultType.lower() and 'dur' not in resultType.lower():  # make sure it's vector dataset
						velocityTRI = CmeResultsIndex(layer.name(), resultType, None, False, False, self.cmeView.cmeResults, self.cmeView.cmeOptions.timeUnits)
						velRes = self.cmeView.cmeResults.getResult(velocityTRI)
						for t in velRes:
			...
			velocityTRI = CmeResultsIndex(layer.name(), velocity, None, False, False, self.cmeView.cmeResults, self.cmeView.cmeOptions.timeUnits)  # velocity CmeResultsIndex
			velRes = self.cmeView.cmeResults.getResult(velocityTRI)  # r = dict - { str time: [ float time, QgsMeshDatasetIndex ] }
			if depth is not None:
				depthTRI = CmeResultsIndex(layer.name(), depth, None, False, False, self.cmeView.cmeResults, self.cmeView.cmeOptions.timeUnits)
				depthRes = self.cmeView.cmeResults.getResult(depthTRI)
			else:
				wlTRI = CmeResultsIndex(layer.name(), waterLevel, None, False, False, self.cmeView.cmeResults, self.cmeView.cmeOptions.timeUnits)
				wlRes = self.cmeView.cmeResults.getResult(wlTRI)
				bedTRI = CmeResultsIndex(layer.name(), bedElevation, None, False, False, self.cmeView.cmeResults, self.cmeView.cmeOptions.timeUnits)
				bedRes = self.cmeView.cmeResults.getResult(bedTRI)
			...
			inters, ch, fcs = findMeshIntersects(si, dp, mesh, feat, crs,
			                                             self.cmeView.project, self.iface)
			...
			if onVertices:
				...
				faces = [None] * len(points)
				if debug:
					writeTempPoints(inters, self.cmeView.project, crs, chainages, 'Chainage',
					                QT_DOUBLE)
			else:
				# points = faces[:]
				points = inters[:]
			...
			for resultType in results[layer.name()]:
				...
				# 		'dur' not in resultType:
					cmeResultsIndex = CmeResultsIndex(layer.name(), resultType, None, False, False, self.cmeView.cmeResults, self.cmeView.cmeOptions.timeUnits)
					res = self.cmeView.cmeResults.getResult(cmeResultsIndex)
					if len(res) > 1:
			...
			if maxProgress:
				self.cmeView.progressBar.setVisible(True)
				self.cmeView.progressBar.setRange(0, 100)
				self.cmeView.progressBar.setValue(0)
				pComplete = 0
				complete = 0
			else:
				return False
				...
				for i in range(len(faces)):
					...
					if depth is not None:
						depthMag = self.datasetValue(layer, dp, si, mesh, results[layer.name()][depth]['times'][key][-1],
						                           meshRendered, points[i], 0, CmePlot.DataFlow2D, None, faces[i])
					else:
					...
					if depthMag > 0:
						velDataValue = self.datasetValue(layer, dp, si, mesh, velItem[-1], meshRendered, points[i], 0,
						                                 CmePlot.DataFlow2D, None, faces[i], value='vector')
					...
					if i == 0:  # can't get flux at first point since there's no flow area
						...
						complete += 1
						pComplete = complete / maxProgress * 100
						self.cmeView.progressBar.setValue(int(pComplete))
			...
			if featName is None:  # rubberband layer
				if bypass or self.cmeView.cboSelectType.currentText() == 'From Map Multi':
			...
			else:  # use attribute from selected feacmere
		...
		self.cmeView.progressBar.setVisible(False)

		data = list(zip(xAll, yAll))
		dataTypes = [CmePlot.DataFlow2D] * len(data)
		self.cmePlot.drawPlot(0, data, labels, types, dataTypes, draw=draw, time=time, show_current_time=showCurrentTime)
	...
	def findMeshIntersects(self, si, dp, mesh, feat, mapUnits):
		...
		lyr.updateExtents()
		self.cmeView.project.addMapLayer(lyr)
	...
	def findMeshIntersects2(self, si, dp, mesh, feat, mapUnits):
		...
		QMessageBox.information(self.cmeView, "Debug", "points:\n{0}".format('\n'.join(printpoints)))
	...
	def activeAvgMethod(self, rs):
		"""
		Recmerns active depth averaging method from QGIS (not anything in coastalme viewer)
		"""
	...
	def calculateAverage(self, dataType, averagingMethod, dataset3d, params=None):
		...
		param = self.cmePlot.cmePlotToolbar.getAveragingParameters(dataType, averagingMethod)
	...
	def generateLabel(self, layer, resultMesh, rtype, markerNo, featName,
		...
		if am is not None:
			if isinstance(self.cmePlot.cmePlotToolbar.plotDataToPlotMenu[dataType], (QAction, QWidgetAction)) and is_qt6:
				menu = self.cmePlot.cmePlotToolbar.plotDataToPlotMenu[dataType].parent()
			else:
				menu = self.cmePlot.cmePlotToolbar.plotDataToPlotMenu[dataType].parentWidget()
		...
		if export:
			...
			if featName is None:
				if bypass or self.cmeView.cboSelectType.currentText() == 'From Map Multi' and markerNo > 0:
