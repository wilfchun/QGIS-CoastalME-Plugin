...
from ..coastalmeqgis_cmeviewer.coastalmeqgis_cmeresults1d import TSResult
...
class CmeProject():
	...
	def load(self):
		...
		if Qgis.QGIS_VERSION_INT >= 30400:
			...
			self.cmeView.setVisible(visible)
			...
			self.cmeView.resultChangeSignalCount = 0
			...
			#self.cmeView.cmeResults.cmeResults2D.renderMap()
	...
	def processResults2d(self, call_type):
		...
		if call_type == 'save':
			...
			if results:
				...
				# try:
				# 	self.cmeView.cmeMenuBar.cmeMenuFunctions.load2dResults(result_2D=[results])
				# except:
				# 	pass
				try:
			...
			for layerid, layer in QgsProject.instance().mapLayers().items():
				if layer.name() in nc_grids and isinstance(layer, QgsRasterLayer):
					...
					self.cmeView.cmeResults.cmeResultsNcGrid.importResults([filename], layers=[result_type],
				...
				for result in results[:]:
					if result.startswith('FVBC_TideResults://'):
						...
						if len(split) == 2:
							...
							continue
						self.cmeView.cmeMenuBar.cmeMenuFunctions.loadFVBCTide(nc_fpath=nc_fpath, gis_fpath=gis_fpath, use_local_time=use_local_time)
						results.remove(result)
				try:
					self.cmeView.cmeMenuBar.cmeMenuFunctions.load1dResults(result_1D=[results], ask_gis=False)
				except:
					pass
			...
			try:
				...
				if allResults:
					...
					for res in allResults:
						if res in self.cmeView.cmeResults.results:
				...
				self.cmeView.cmeResults.cmeResults2D.activeMeshLayers.clear()
				if activeResults:
		...
		if call_type == 'save':
			...
			try:
				...
				if activeResultTypes:
					...
					self.cmeView.cmeResults.activeResultsIndexes.clear()
					self.cmeView.cmeResults.activeResultsTypes.clear()
					self.cmeView.cmeResults.activeResultsItems.clear()
					self.cmeView.cmeResults.activeResults.clear()
					self.cmeView.cmeResults.cmeResults1D.items1d.clear()
					self.cmeView.cmeResults.cmeResults1D.typesTS.clear()
					self.cmeView.cmeResults.cmeResults1D.typesLP.clear()
					self.cmeView.cmeResults.cmeResults1D.pointTS.clear()
					self.cmeView.cmeResults.cmeResults1D.lineTS.clear()
					self.cmeView.cmeResults.cmeResults1D.regionTS.clear()
					for i in range(openResultTypes.model().mapOutputsItem.childCount()):
						...
						if name in activeResultTypes:
							self.cmeView.cmeResults.activeResults.append(name)
							self.cmeView.cmeResults.activeResultsItems.append(item)
							self.cmeView.cmeResults.activeResultsIndexes.append(openResultTypes.model().item2index(item))
							self.cmeView.cmeResults.activeResultsTypes.append(item.ds_type)
							if item.ds_type == 1:
								self.cmeView.cmeResults.cmeResults2D.activeScalar = name
							elif item.ds_type == 2:
								self.cmeView.cmeResults.cmeResults2D.activeVector = name
					for item in openResultTypes.model().timeSeriesItem.children():
						...
						if name in activeResultTypes:
							...
							if item.ds_type == 4 or item.ds_type == 5 or item.ds_type == 6 or item.ds_type == 7:
								self.cmeView.cmeResults.cmeResults1D.items1d.append(name)
								if item.ds_type == 4 or item.ds_type == 5 or item.ds_type == 6:
									nameAppend = '_TS'
									self.cmeView.cmeResults.cmeResults1D.typesTS.append(name)
								elif item.ds_type == 7:
									nameAppend = '_LP'
									self.cmeView.cmeResults.cmeResults1D.typesLP.append(name)
								elif item.ds_type == 8:
									nameAppend = '_CS'
								if item.ds_type == 4:
									self.cmeView.cmeResults.cmeResults1D.pointTS.append(name)
								elif item.ds_type == 5:
									self.cmeView.cmeResults.cmeResults1D.lineTS.append(name)
								elif item.ds_type == 6:
									self.cmeView.cmeResults.cmeResults1D.regionTS.append(name)
								name += nameAppend
								self.cmeView.cmeResults.activeResults.append(name)
			except:
				pass
	...
	def processActiveTime(self, call_type):
		...
		cboTime = self.cmeView.cboTime
		sliderTime = self.cmeView.sliderTime
		btn2dLock = self.cmeView.btn2dLock
		...
		if call_type == 'save':
			...
			lock = '{0}'.format(self.cmeView.lock2DTimesteps)
			...
			self.cmeView.lock2DTimesteps = lock
			btn2dLock.setChecked(lock)
			self.cmeView.timestepLockChanged(switch=False)
	...
	def processMaxSecondaryTypes(self, call_type):
		...
		secondaryResultTypes = self.cmeView.cmeResults.secondaryAxisTypes
		maxResultTypes = self.cmeView.cmeResults.maxResultTypes
		openResultTypes = self.cmeView.OpenResultTypes
		...
		if call_type == 'save':
			...
			try:
				...
				if '2D Flow' in secondaryResultTypes:
					self.cmeView.cmePlot.cmePlotToolbar.fluxSecAxisButton.setChecked(True)
			except:
				pass
	...
	def processMeshStyles(self, call_type):
		...
		results = self.cmeView.cmeResults.results
		menuFunctions = self.cmeView.cmeMenuBar.cmeMenuFunctions
		results2D = self.cmeView.cmeResults.cmeResults2D
							...
							for time, items in timeDict['times'].items():  # just do first timestep
								...
								try:
									...
									if dtype == 1:
										...
										if value and len(value) >= 27 and value[:27] == '<!DOCTYPE coastalme_meshlayer>':
	...
	def processOptions(self, call_type):
		"""Project settings for cmeview options."""
		
		if call_type == 'save':
			self.cmeView.cmeOptions.saveProject(self.project)
		else:
			try:
				self.cmeView.cmeOptions.readProject(self.project)
	...
	def processPlotTab(self, call_type):
		...
		if call_type == 'save':
			currentTab = self.cmeView.tabWidget.currentIndex()
			...
			try:
				...
				currentTab = 0
			self.cmeView.tabWidget.setCurrentIndex(currentTab)
	
	def processMapPlotting(self, call_type):
		"""Project settings for 2D mapoutput plotting."""
		
		cbo = self.cmeView.cboSelectType
		toolbar = self.cmeView.cmePlot.cmePlotToolbar
		
		if call_type == 'save':
			self.project.writeEntry("TUVIEW", "selecttype", str(cbo.currentIndex()))
			
			checkedTS = toolbar.getCheckedItemsFromPlotOptions(CmePlot.DataTimeSeries2D)
			tstypes = ''
			...
			checkedCS = toolbar.getCheckedItemsFromPlotOptions(CmePlot.DataCrossSection2D)
			cstypes = ''
	...
	def processPlotToolbarOptions(self, call_type):
		...
		viewToolbarTS = self.cmeView.cmePlot.cmePlotToolbar.viewToolbarTimeSeries
		viewToolbarLP = self.cmeView.cmePlot.cmePlotToolbar.viewToolbarLongPlot
		viewToolbarCS = self.cmeView.cmePlot.cmePlotToolbar.viewToolbarCrossSection
		viewToolbarVP = self.cmeView.cmePlot.cmePlotToolbar.viewToolbarVerticalProfile
		subplotTS = self.cmeView.cmePlot.subplotTimeSeries
		subplotLP = self.cmeView.cmePlot.subplotLongPlot
		subplotCS = self.cmeView.cmePlot.subplotCrossSection
		subplotVP = self.cmeView.cmePlot.subplotVerticalProfile
		isSecondaryTS = self.cmeView.cmePlot.isTimeSeriesSecondaryAxis
		isSecondaryLP = self.cmeView.cmePlot.isLongPlotSecondaryAxis
		isSecondaryCS = self.cmeView.cmePlot.isCrossSectionSecondaryAxis
		isSecondaryVP = self.cmeView.cmePlot.isVerticalProfileSecondaryAxis
	...
	def processPlotConfiguration(self, call_type):
		...
		frozenTSProperties = self.cmeView.cmePlot.frozenTSProperties
		frozenLPProperties = self.cmeView.cmePlot.frozenLPProperties
		frozenCSProperties = self.cmeView.cmePlot.frozenCSProperties
		frozenVPProperties = self.cmeView.cmePlot.frozenVPProperties
		frozenTSAxisLabels = self.cmeView.cmePlot.frozenTSAxisLabels
		frozenLPAxisLabels = self.cmeView.cmePlot.frozenLPAxisLabels
		frozenCSAxisLabels = self.cmeView.cmePlot.frozenCSAxisLabels
		frozenVPAxisLabels = self.cmeView.cmePlot.frozenVPAxisLabels
	...
	def processGraphics(self, call_type):
		...
		markers = self.cmeView.cmePlot.cmeTSPoint.markers
		
		linesCS = self.cmeView.cmePlot.cmeCrossSection.rubberBands
		lineMarkersCS = self.cmeView.cmePlot.cmeCrossSection.linePoints
		linePointsCS = self.cmeView.cmePlot.cmeCrossSection.points
		
		linesQ = self.cmeView.cmePlot.cmeFlowLine.rubberBands
		lineMarkersQ = self.cmeView.cmePlot.cmeFlowLine.linePoints
		linePointsQ = self.cmeView.cmePlot.cmeFlowLine.points
		...
		try:
			...
			if a:
				...
				for i in range(len(a)):
					...
					points.append(point)
					marker = QgsVertexMarker(self.cmeView.canvas)
				...
				for i in range(no):
					...
					for j in range(len(a)):
						...
						if i + 1 == no:
							...
							marker.setCenter(QgsPointXY(point))
							markers.append(marker)
					line = QgsRubberBand(self.cmeView.canvas)
					line.setWidth(2)
					if suffix == 'cs':
	...
	def processUserPlotData(self, call_type):
		...
		data = self.cmeView.cmePlot.userPlotData
	...
	def processShowActiveTime(self, call_type):
		...
		cb = self.cmeView.cbShowCurrentTime
