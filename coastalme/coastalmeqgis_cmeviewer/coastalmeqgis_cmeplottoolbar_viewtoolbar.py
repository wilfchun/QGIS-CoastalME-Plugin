...
class ViewToolbar():
	...
	def __init__(self, CmePlotToolbar, plotNo):
		self.cmePlotToolbar = CmePlotToolbar
		self.cmePlot = CmePlotToolbar.cmePlot
		self.cmeView = self.cmePlot.cmeView
	...
	def initialiseViewToolbar(self):
		...
		# toolbar settings
		self.viewToolbar = QToolBar('View Toolbar', self.cmeView.ViewToolbarFrame)
		self.viewToolbar.setIconSize(QSize(w, w))
		self.cmeView.ViewToolbarFrame.setMinimumHeight(w2)
		self.cmeView.ViewToolbarFrame.setMinimumWidth(w3)
		...
		# axis font size
		# fontsize = self.cmePlot.subplotTimeSeries.xaxis.get_label().get_size()
		fontsize = self.cmeView.cmeOptions.defaultFontSize
		...
		# connect buttons
		self.refreshPlotButton.released.connect(self.cmeView.refreshCurrentPlot)
		self.clearPlotButton.released.connect(lambda: self.cmeView.cmePlot.clearPlot2(self.plotNo, clear_rubberband=True))
		...
		self.userPlotDataManagerAction.triggered.connect(self.cmeMenuFunctions.openUserPlotDataManager)
		...
		# self.viewToolbar.iconSizeChanged.connect(lambda e: self.cmeView.toolbarIconSizeChanged(e, self.viewToolbar, self.cmeView.ViewToolbarFrame))
	...
	def legendOrientationChanged(self, orienAction):
		...
		self.cmePlot.updateLegend(plotNo)
		figure.tight_layout()
		plotWidget.draw()
	...
	def gridLines_toggled(self):
		...
		parentLayout, figure, subplot, plotWidget, isSecondaryAxis, artists, labels, unit, yAxisLabelTypes, yAxisLabels, xAxisLabels, xAxisLimits, yAxisLimits = \
			self.cmePlot.plotEnumerator(plotNo)
		self.cmePlot.manageMatplotlibAxe(subplot)
		plotWidget.draw()
	...
	def axisFontSizeChanged(self):
		...
		parentLayout, figure, subplot, plotWidget, isSecondaryAxis, artists, labels, unit, yAxisLabelTypes, yAxisLabels, xAxisLabels, xAxisLimits, yAxisLimits = \
			self.cmePlot.plotEnumerator(plotNo)
		subplot2 = self.cmePlot.getSecondaryAxis(plotNo, create=False)
	...
	def qgisDisconnect(self):
		try:
			self.refreshPlotButton.released.disconnect(self.cmeView.refreshCurrentPlot)
		except:
			pass
		try:
		...
		try:
			...
			pass
		try:
			self.userPlotDataManagerAction.triggered.disconnect(self.cmeMenuFunctions.openUserPlotDataManager)
		except:
			pass
		try:
