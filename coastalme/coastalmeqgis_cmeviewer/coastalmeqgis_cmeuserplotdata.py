...
class CmeUserPlotDataManager():
	...
	def __init__(self):
		self.error = ''
		self.datasets = {}  # dict - { dataset name: CmeUserPlotDataSet }
		self.count = 0
	...
	def addDataSet(self, name, data, plotType, dates=None, referenceTime=None):
		...
		if name in self.datasets.keys():
			...
			return False
		dataset = CmeUserPlotDataSet(name, data, plotType, True, self.count, dates, referenceTime)
		self.count += 1
		self.datasets[name] = dataset
		if dataset.error:
			self.error = '{0}: {1}'.format(name, dataset.error)
		
	def editDataSet(self, name, newname=None, data=None, plotType=None, stacmes=None, dates=None):
		"""
		...
		if name in self.datasets.keys():
			...
			if newname is not None:
				if newname != dataset.name:
					copyDataset = CmeUserPlotDataSet(newname, [dataset.x, dataset.y], dataset.plotType, dataset.stacmes, dataset.number, dataset.dates, dataset.referenceTime)
			...
			if stacmes is not None:
				dataset.setStacmes(stacmes)
				if dataset.error:
					self.error = '{0}: {1}'.format(name, dataset.error)
	...
	def loadProject(self, project):
		...
		for i in range(no):
			dataset = CmeUserPlotDataSet()
			dataset.loadProject(project, i)
			if not dataset.error:
				self.datasets[dataset.name] = dataset
				self.count += 1
		
class CmeUserPlotDataSet():
	"""
	Class for handling individual user input data sets.
	
	"""
	
	def __init__(self, name=None, data=[], plotType=None, stacmes=False, number=-1, dates=None, referenceTime=None):
		self.error = ''
		self.number = number
		self.stacmes = stacmes
	...
	def __eq__(self, other):
		eq = False
		if isinstance(other, CmeUserPlotDataSet):
			if self.error != other.error:
				return eq
			if self.number != other.number:
				return eq
			if self.stacmes != other.stacmes:
				return eq
	...
	def saveProject(self, project, no):
		...
		if not self.error:
			...
			# stacmes
			project.writeEntry("TUVIEW", "userplotdata{0}stacmes".format(no), str(self.stacmes))
	...
	def loadProject(self, project, no):
		...
		# stacmes
		self.stacmes = True if project.readEntry("TUVIEW", "userplotdata{0}stacmes".format(no))[0] == 'True' else False
