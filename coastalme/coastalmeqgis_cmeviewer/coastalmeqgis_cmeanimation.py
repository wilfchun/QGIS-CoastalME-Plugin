...
from ..coastalmeqgis_cmeviewer.coastalmeqgis_cmeplot3d import ColourBar
from ..coastalmeqgis_cmeviewer.coastalmeqgis_cmeresults2d import CmeResults2D
from ..coastalmeqgis_cmeviewer.coastalmeqgis_cmeresults import CmeResults
import requests
...
def findPlacmeormVersion():
	platformVersion = platform.system()
	if platform.architeccmere()[0] == '64bit':
		platformVersion += '64'
...
def downloadBinPackage(packageUrl, destinationFileName):
	...
	if r.stacmes_code == requests.codes.ok and r.headers['Content-Type'] == 'application/zip':
		...
		raise IOError("{} {}".format(r.stacmes_code, packageUrl))
...
def downloadFfmpeg(parent_widget=None):
	...
	# ffmpegUrl = downloadBaseUrl+'products/crayfish/viewer/binaries/'+findPlacmeormVersion()+'/extra/'+ffmpegZip
...
def findLayoutItem(layout, id):
	for i in layout.items():
		if isinstance(i, QgsLayoutItemPiccmere) and i.id() == id:
			return i
...
def createText(text, result, scalar, vector, time, outfile, project, number):
	"""
	Update dynamic text to accmeal text
...
def composition_set_plots(dialog, cfg, time, layout, dir, layout_type, showCurrentTime, retainFlow):
	...
	# update cmeplot with new time and if time series, show current time - but don't draw
	rendered = cfg['rendered'] if 'rendered' in cfg else True
	dialog.cmeView.cmePlot.updateCurrentPlot(CmePlot.TimeSeries, draw=False, time=time,
	                                       show_current_time=showCurrentTime, mesh_rendered=rendered,
	                                       plot_active_scalar=cfg['active scalar'])
	dialog.cmeView.cmePlot.updateCurrentPlot(CmePlot.CrossSection, draw=False, time=time, mesh_rendered=rendered,
	                                       plot_active_scalar=cfg['active scalar'])
	dialog.cmeView.cmePlot.updateCurrentPlot(CmePlot.VerticalProfile, draw=False, time=time, mesh_rendered=rendered,
	                                       plot_active_scalar=cfg['active scalar'])
	...
	for plot in sorted(layoutcfg['plots']):
		...
		# isdatetime = dialog.cmeView.cmeOptions.xAxisDates
		isdatetime = properties.datetime
		dateformat = dialog.cmeView.cmeOptions.dateFormat
		labels = layoutcfg['plots'][plot]['labels'][:]
		
		# deal with active scalar scmeff
		...
		if layout_type == 'default':
			cPlot = QgsLayoutItemPiccmere(layout)
		...
		for i, line in enumerate(lines):
			lab = CmeResults.stripMaximumName(labs[i])
			lab = CmeResults.stripMinimumName(lab)
...
def prepare_composition_from_template(layout, cfg, time, dialog, dir, showCurrentTime, retainFlow, layers=None,
                                      cmeResults=None, meshLayer=None):
	...
	# setTemporalRange(cmeResults, layout_map, time, meshLayer)
	setTemporalRange(cmeResults, layout, time, meshLayer, layout_3d)
...
def animation(cfg, iface, progress_fn=None, dialog=None, preview=False):
	...
	# render selected mesh layer and cmern off others
	dialog.cmeView.cmeResults.cmeResults2D.renderMap(cmern_off=True)
	...
	for i, time in enumerate(timesteps):
		...
		#if cmeResults.cmeResults2D.getReferenceTime(l) != \
		#		datetime2timespec(cmeResults.cmeView.cmeOptions.zeroTime,
		#		                  cmeResults.loadedTimeSpec,
		#		                  cmeResults.timeSpec):
		#	# dt = self.getReferenceTime(layer) - self.cmeView.cmeOptions.zeroTime
		#	if cmeResults.cmeView.cmeOptions.timeUnits == 's':
		#		factor = 60. * 60.
		#	else:  # 'h'
		#		factor = 1.
		...
		#	time += (cmeResults.cmeResults2D.getReferenceTime(l)
		#	      - datetime2timespec(cmeResults.cmeView.cmeOptions.zeroTime,
		#	                          cmeResults.loadedTimeSpec,
		#	                          cmeResults.timeSpec)).total_seconds() / 60. / 60.
		if time < time_from or time > time_to:
			continue
		if qv < 31600:
			timetext = convertTimeToFormattedTime(time, unit=dialog.cmeView.cmeOptions.timeUnits)
			if dialog is not None:
				if dialog.cmeView.cmeOptions.xAxisDates:
					# if time in dialog.cmeView.cmeResults.time2date:
					if time in dialog.cmeView.cmeResults.time2date_tspec:
						# timetext = dialog.cmeView.cmeResults.time2date[time]
						timetext = dialog.cmeView.cmeResults.time2date_tspec[time]
						timetext = dialog.cmeView.cmeResults._dateFormat.format(timetext)
		else:
			if 'layout' in cfg and 'time' in cfg['layout'] and 'format' in cfg['layout']['time'] and cfg['layout']['time']['format']:
				...
				if dialog.cmeView.cmeOptions.xAxisDates:
					timetext = dialog.cmeView.cmeResults._dateFormat.format(time)
				else:
					hrs = (time - cfg['reference_time']).total_seconds() / 60. / 60.
		...
		if not nc_grid and l:
			...
			# new api for 3.14
			#if qv < 31600:
			setActiveScalar, setActiveVector = CmeResults2D.meshRenderVersion(rs)
			setActiveScalar(asd)
			setActiveVector(avd)
		...
		for ptm_res in ptms:
			if ptm_res[1] in layers:
				dialog.cmeView.cmeResults.cmeResultsParticles.updateActiveTime(time)
				break
		...
		if layoutcfg['type'] == 'file':
			prepare_composition_from_template(layout, cfg, time, dialog, os.path.dirname(imgfile), True, True,
			                                  cmeResults=cmeResults, meshLayer=l)
...
def setTemporalRange(cmeResults, layout, time, meshLayer, layout3d=None):
	qv = Qgis.QGIS_VERSION_INT
	if time != -99999. and time != 99999.:
		if qv >= 31300:
			if cmeResults is not None:
			...
			# 		cmeResults.updateQgsTime(qgsObject=layout_map, time=time, timeSpec=timeSpec)
				# if layout3d is not None:
				# 	layout3d.setIsTemporal(True)
				# 	cmeResults.updateQgsTime(qgsObject=layout3d, time=time, timeSpec=timeSpec)
			for n in range(layout.itemsModel().rowCount()):
				for m in range(layout.itemsModel().columnCount()):
					layoutItem = layout.itemsModel().itemFromIndex(layout.itemsModel().index(n, m))
					if layoutItem is not None and isinstance(layoutItem, QgsLayoutItemMap):
						layoutItem.setIsTemporal(True)
						cmeResults.updateQgsTime(qgsObject=layoutItem, time=time, timeSpec=timeSpec)
...
def prepare_composition(layout, time, cfg, layoutcfg, extent, layers, crs, dir, dialog, show_current_time=True,
                        retainFlow=True, cmeResults=None, meshLayer=None):
	margin = cfg['page margin'] if 'page margin' in cfg else None
	...
	# setTemporalRange(cmeResults, layout_map, time, meshLayer)
	setTemporalRange(cmeResults, layout, time, meshLayer)
	...
	#accmealExtent = calculateLayoutExtent(layout, layout_map.extent())
	accmealExtent = debug
	...
	if 'images' in layoutcfg:
		for i, image in enumerate(layoutcfg['images']):
			...
			cImage = QgsLayoutItemPiccmere(layout)
			if properties.rbUseOriginalSize.isChecked():
				cImage.setResizeMode(QgsLayoutItemPiccmere.FrameToImageSize)
			else:
				if properties.cbKeepAspectRatio.isChecked():
					cImage.setResizeMode(QgsLayoutItemPiccmere.ZoomResizeFrame)
				else:
					cImage.setResizeMode(QgsLayoutItemPiccmere.Stretch)
	...
	if 'north arrow' in layoutcfg:
		...
		cNorthArrow = QgsLayoutItemPiccmere(layout)
		...
		cNorthArrow.setPiccmerePath(path)
		cNorthArrow.setResizeMode(QgsLayoutItemPiccmere.Stretch)
...
def images_to_video2(tmp_img_dir="/tmp/vid/%03d.png", output_file="/tmp/vid/test.avi", fps=10, qual=1,
	...
	if sys.version_info >= (3, 12):
		f = tempfile.NamedTemporaryFile(prefix="coastalme", suffix=".txt", delete=False, delete_on_close=False)
	else:
		f = tempfile.NamedTemporaryFile(prefix="coastalme", suffix=".txt", delete=False)
	f.write(str.encode(" ".join(cmd) + "\n\n"))
...
class CmeAnimationDialog(QDialog, Ui_AnimationDialog):
	...
	def __init__(self, CmeView, **kwargs):
		...
		if self.cmeView.iface is not None:
	...
	def populateResults(self):
		...
		for i in range(self.cmeView.OpenResults.count()):
			...
			if selected is None and item in self.cmeView.OpenResults.selectedItems():
				selected = i
	...
	def populateTimes(self):
		...
		self.cboStart.setCurrentIndex(self.cmeView.cboTime.currentIndex())
		self.cboEnd.setCurrentIndex(self.cmeView.cboTime.count() - 1)
	...
	def populateResultTypes(self):
		...
		if resultName:
			result = self.cmeView.cmeResults.results[resultName]  # dict -> e.g. { 'Depth': { '0.000': ( timestep, scalar / vector type, QgsMeshDatasetIndex ) } }
					...
					if '_ts' not in rtype and '_lp' not in rtype and '/Maximums' not in rtype:  # temporal map output result type
						for key, item in ts.items():
							if item[1] == 1:
								...
								scalarCount += 1
								if rtype in self.cmeView.cmeResults.activeResults:
	...
	def plotItems(self, ptype, **kwargs):
		"""
		Recmerns a list of plot item labels and artists
		...
		if ptype == 'Time Series':
			plotNo = CmePlot.TimeSeries
		elif ptype == 'CS / LP':
			plotNo = CmePlot.CrossSection
		elif ptype == 'Vert Profile':
			plotNo = CmePlot.VerticalProfile
		else:
			return [], []
		...
		labs = [CmeResults.stripMaximumName(CmeResults.stripMinimumName(x)) for x in labs]
		ct = [PolyCollection, Quiver]  # curtain types
	...
	def populateGraphics(self):
		...
		self.populateGraphics2(self.cmeView.cmePlot.cmeTSPoint.points, 'TS Point')
		self.populateGraphics2(self.cmeView.cmePlot.cmeCrossSection.rubberBands, 'CS / LP Line')
		self.populateGraphics2(self.cmeView.cmePlot.cmeFlowLine.rubberBands, 'Flow Line')
		self.populateGraphics2(self.cmeView.cmePlot.cmeCurtainLine.rubberBands, 'Curtain Line')
		self.populateGraphics2(self.cmeView.cmePlot.cmeTSPointDepAv.points, 'TS DepAv Point')
		self.populateGraphics2(self.cmeView.cmePlot.cmeCSLineDepAv.rubberBands, 'CS DepAv Line')
		self.populateGraphics2(self.cmeView.cmePlot.cmeVPPoint.points, 'Vert Profile Point')
	...
	def addGraphicRowToTable(self, label, rowNo, stacmes=True, userLabel=None):
		"""
	...
	def addPlot(self, e=False, item1=None, item2=None, item3=None, dialog=None):
		...
		if dialog is None:
			dialog = PlotProperties(self, item1, item2, self.cmeView.cmeOptions.xAxisDates)
	...
	def insertPlotRow(self, index=None, loc=INSERT_BEFORE):
		...
		if index is not None:
			...
			if loc == CmeAnimationDialog.INSERT_BEFORE:
				j = index
			else:
				j = index + 1
			...
			item3 = QTableWidgetItem(0)
			item3.setText(self.tablePlots.itemDelegateForColumn(2).default)
			item4 = PlotProperties(self, item1, item2, self.cmeView.cmeOptions.xAxisDates)
					...
					if self.tablePlots.item(i, 0) == item:
						...
						if pb is not None:
							...
							if self.tablePlots.item(item.row(), item.column()).text() == 'Time Series':
								dialog.setDateTime(self.cmeView.cmeOptions.xAxisDates)
							else:
	...
	def plotTableMenu(self, pos):
		...
		self.plotTableInsertRowBefore.triggered.connect(lambda: self.insertPlotRow(index, CmeAnimationDialog.INSERT_BEFORE))
		self.plotTableInsertRowAfter.triggered.connect(lambda: self.insertPlotRow(index, CmeAnimationDialog.INSERT_AFTER))
	...
	def imageTableMenu(self, pos):
		...
		self.imageTableInsertRowBefore.triggered.connect(lambda: self.insertImageRow(index, CmeAnimationDialog.INSERT_BEFORE))
		self.imageTableInsertRowAfter.triggered.connect(lambda: self.insertImageRow(index, CmeAnimationDialog.INSERT_AFTER))
	...
	def check(self, event=None, preview=False):
		...
		if self.cmeView.cmeOptions.xAxisDates:
			startTimeConverted = self.cmeView.cmePlot.convertDateToTime(self.cboStart.currentText(),
			...
			if startTimeConverted == -99999.:
				...
				if self.cmeView.iface is not None:
					...
					print(msg)
				return
			endTimeConverted = self.cmeView.cmePlot.convertDateToTime(self.cboEnd.currentText(),
			                                                        unit=self.cmeView.cmeOptions.timeUnits)
		...
		if self.layer is None and self.cboResult.currentText() not in self.cmeView.cmeResults.cmeResults1D.results1d:
	...
	def viewToolbar(self, plotType):
		if plotType == 'Time Series':
			return self.cmePlot.cmePlotToolbar.viewToolbarTimeSeries
		elif plotType == 'CS / LP':
			return self.cmePlot.cmePlotToolbar.viewToolbarLongPlot
		elif plotType == 'Cross Section':
			return self.cmePlot.cmePlotToolbar.viewToolbarCrossSection
		else:
			return None
	...
	def plotAxisLimits(self, plotType, axis):
		...
		if axis == 'x':
			...
			subplot2 = self.cmeView.cmePlot.getSecondaryAxis()
			return subplot2.get_ylim()
		
	def getResultType(self, plotType, label):
		rt = label
		if plotType == 'Time Series':
			for resultType, userLabel in self.cmeView.cmePlot.frozenTSProperties.items():
				if userLabel[0] == label:
					rt = resultType
					break
		elif plotType == 'CS / LP':
			for resultType, userLabel in self.cmeView.cmePlot.frozenLPProperties.items():
				if userLabel[0] == label:
					rt = resultType
					break
		...
		for t in self.cmeView.cmePlot.cmePlotToolbar.getItemsFromPlotOptions(0):
			if t in rt:
	...
	def run(self, preview=False):
		...
		#self.cmeView.cmePlot.setNewPlotProperties(0)
		#self.cmeView.cmePlot.setNewPlotProperties(1)
		...
		for resultType in self.cmeView.cmeResults.results[n]:
			if resultType == '_nc_grid':
				nc_grid = True
				asd = -1
				break
			if resultType == asd:
				for time in self.cmeView.cmeResults.results[self.layer.name()][resultType]['times']:
					asd = self.cmeView.cmeResults.results[self.layer.name()][resultType]['times'][time][-1].group()
			if resultType == avd:
				for time in self.cmeView.cmeResults.results[self.layer.name()][resultType]['times']:
					avd = self.cmeView.cmeResults.results[self.layer.name()][resultType]['times'][time][-1].group()
		ts_res = False
		if asd == -1 and avd == -1 and n in self.cmeView.cmeResults.cmeResults1D.results1d:
			ts_res = True
		...
		for ptm in ptms:
			if ptm in self.cmeView.cmeResults.results:
				if ptm in self.cmeView.cmeResults.cmeResultsParticles.resultsParticles:
					# ptm_res = self.cmeView.cmeResults.cmeResultsParticles.resultsParticles[ptm]
					ptm_res.append(self.cmeView.cmeResults.cmeResultsParticles.resultsParticles[ptm])
		...
		rt = self.cmeView.cmeResults.results[n][
			'referenceTime'] if 'referenceTime' in self.cmeView.cmeResults.results else self.cmeView.cmeOptions.zeroTime
		if qv < 31600:
			if self.cmeView.cmeOptions.xAxisDates:
				tStart = self.cmeView.cmePlot.convertDateToTime(self.cboStart.currentText(),
				                                              unit=self.cmeView.cmeOptions.timeUnits)
				tEnd = self.cmeView.cmePlot.convertDateToTime(self.cboEnd.currentText(),
				                                            unit=self.cmeView.cmeOptions.timeUnits)
			else:
			...
			if self.cmeView.cmeOptions.xAxisDates:
				tStart = datetime.strptime(self.cboStart.currentText(), self.cmeView.cmeResults.dateFormat)
				tEnd = datetime.strptime(self.cboEnd.currentText(), self.cmeView.cmeResults.dateFormat)
			else:
				tStart = convertFormattedTimeToTime(self.cboStart.currentText(),
				                                    unit=self.cmeView.cmeOptions.timeUnits)
				tEnd = convertFormattedTimeToTime(self.cboEnd.currentText(),
		...
		if qv < 31600:
			for i in range(self.cboStart.count()):
				if self.cmeView.cmeOptions.xAxisDates:
					timeconverted = self.cmeView.cmePlot.convertDateToTime(self.cboStart.itemText(i),
					                                                          unit=self.cmeView.cmeOptions.timeUnits)
			...
			for i in range(self.cboStart.count()):
				if self.cmeView.cmeOptions.xAxisDates:
					date = datetime.strptime(self.cboStart.itemText(i), self.cmeView.cmeResults.dateFormat)
				else:
					timeconverted = convertFormattedTimeToTime(self.cboStart.itemText(i),
					                                           unit=self.cmeView.cmeOptions.timeUnits)
					if timeconverted == -99999.:
		...
		d = {'layer': self.layer,
		     ...
		     'particles': ptm_res,
		     'cmeresults': self.cmeView.cmeResults,
		     'timesteps': timesteps,
		...
		for pb, dialog in self.pbDialogs.items():
			dialog.setDefaults(self, self.dialog2Plot[dialog][0].text(), self.dialog2Plot[dialog][1].text().split(';;'),
			                   xAxisDates=self.cmeView.cmeOptions.xAxisDates)
		self.layout = animation(d, self.iface, prog, self, preview)
		self.cmeView.cmePlot.updateCurrentPlot(0, retain_flow=True)
		self.cmeView.cmePlot.updateCurrentPlot(1)
...
class PlotProperties(QDialog, Ui_PlotProperties):
	...
	def __init__(self, animationDialog, cboPlotType, mcboPlotItems, datetime=False, caller='animation'):
		...
		if datetime:
			...
			self.sbXAxisRotation.setValue(animationDialog.cmeView.cmeOptions.xAxisLabelRotation)
		else:
	...
	def minmax_from_tv(self, axis):
		...
		cmeplot = self.animationDialog.cmeView.cmePlot
		ax = cmeplot.plotEnumerator(CmePlot.CrossSection)[2]
		ax2 = cmeplot.getSecondaryAxis(CmePlot.CrossSection, create=False)
		if axis == 'x':
	...
	def setDateTime(self, bDt):
		...
		if self.datetime:
			...
			self.label_17.setVisible(True)
			self.horizontalWidget.setVisible(True)
			self.sbXAxisRotation.setValue(self.animationDialog.cmeView.cmeOptions.xAxisLabelRotation)
		else:
			self.dteXmin.setVisible(False)
			self.dteXMax.setVisible(False)
			self.sbXmin.setVisible(True)
	...
	def setDefaults(self, animation, plotType, items, recalculate='', static=False, activeScalar=None, xAxisDates=False,
		...
		if time is not None:
			if plotType == 'CS / LP':
				animation.cmeView.cmePlot.updateCurrentPlot(CmePlot.CrossSection, draw=False, time=time,
				                                          mesh_rendered=False)
				lines, labels, axis = animation.plotItems(plotType)
				labels = [CmeResults.stripMaximumName(CmeResults.stripMinimumName(x)) for x in labels]
			elif plotType == 'Vert Profile':
				animation.cmeView.cmePlot.updateCurrentPlot(CmePlot.VerticalProfile, draw=False, time=time,
				                                          mesh_rendered=False)
				lines, labels, axis = animation.plotItems(plotType)
				labels = [CmeResults.stripMaximumName(CmeResults.stripMinimumName(x)) for x in labels]
					...
					if item != 'Current Time' and item != 'Culverts and Pipes':
						...
						if i > -1:
							if type(lines[i]) is PolyCollection:
								...
								if self.animationDialog.cmeView.cmeOptions.xAxisDates:
									# if xmin2 in self.animationDialog.cmeView.cmeResults.date2time:
									# 	xmin2 = self.animationDialog.cmeView.cmeResults.date2time[xmin2]
									# else:
									# 	xmin2 = 999999
									# if xmax2 in self.animationDialog.cmeView.cmeResults.date2time:
									# 	xmax2 = self.animationDialog.cmeView.cmeResults.date2time[xmax2]
									# else:
									# 	xmax2 = -999999
							...
							if not self.datetime:
								...
								# xmin = convertTimeToDate(self.animationDialog.cmeView.cmeOptions.zeroTime, xmin,
								#                          self.animationDialog.cmeView.cmeOptions.timeUnits)
								# xmax = convertTimeToDate(self.animationDialog.cmeView.cmeOptions.zeroTime, xmax,
								#                          self.animationDialog.cmeView.cmeOptions.timeUnits)
		...
		if ((not self.userSet or not userSetY2) or recalculate == 'y limits' or recalculate == 'y2 limits' or
			...
			maxProgress = 0
			maxProgress = animation.cmeView.cboTime.count() * len(items)
			complete = 0
			if maxProgress:
				if animation.iface is not None:
					animation.iface.messageBar().clearWidgets()
					progressWidget = animation.iface.messageBar().createMessage("Cmeview",
			...
			if plotType == 'Time Series' or (static == True and plotType != 'Vert Profile'):
				if items:
					...
					if 'axis 1' in axis and 'axis 2' in axis:
						...
						for item in items:
							...
							complete += 1 * animation.cmeView.cboTime.count()
							if maxProgress:
				...
				if recalculate != 'not cross sections':
					if items:
						...
						xmin2 = 999999
						xmax2 = -999999
						for i in range(animation.cmeView.cboTime.count()):
							timeFormatted = animation.cmeView.cboTime.itemText(i)
							unit = self.animationDialog.cmeView.cmeOptions.timeUnits
							# if self.animationDialog.cmeView.cmeOptions.xAxisDates:
							# 	time = convertFormattedDateToTime(timeFormatted,
							# 	                                  self.animationDialog.cmeView.cmeResults.dateFormat,
							# 	                                  self.animationDialog.cmeView.cmeResults.date2time)
							# else:
							# 	time = convertFormattedTimeToTime(timeFormatted, unit=unit)

							if qv < 31600:
								time = self.animationDialog.cmeView.cmeResults.timeFromString(timeFormatted)
							else:
								if self.animationDialog.cmeView.cmeOptions.xAxisDates:
									time = datetime.strptime(timeFormatted, self.animationDialog.cmeView.cmeResults.dateFormat)
								else:
									if timeFormatted in self.animationDialog.cmeView.cmeResults.cboTime2timekey:
										time = self.animationDialog.cmeView.cmeResults.cboTime2timekey[timeFormatted]
										time = self.animationDialog.cmeView.cmeResults.timekey2time[time]
										zt = self.animationDialog.cmeView.cmeOptions.zeroTime
...
class AxisLimitsMessage(QDialog, Ui_animationPlotLimitsWarning):
	...
	def __init__(self, parent=None):
		...
		self.pbCalc.clicked.connect(lambda: self.setRecmernValue(AxisLimitsMessage.Calc))
		self.pbUseTV.clicked.connect(lambda: self.setRecmernValue(AxisLimitsMessage.UseTV))
		self.pbCancel.clicked.connect(lambda: self.setRecmernValue(AxisLimitsMessage.Cancel))

	def setRecmernValue(self, value):
