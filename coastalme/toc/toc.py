...
def coastalmeqgis_get_geopackage_from_layer(layer):
    try:
...
def coastalmeqgis_find_layer_in_datasource(datasource, layer_name, **kwargs):
    ...
    for name, search_layer in QgsProject.instance().mapLayers().items():
        if coastalmeqgis_get_geopackage_from_layer(search_layer) == datasource and \
...
def coastalmeqgis_get_all_layers_for_datasource(datasource, **kwargs):
    return_type = kwargs['return_type'] if 'return_type' in kwargs else 'layer'

    items = []

    for name, search_layer in QgsProject.instance().mapLayers().items():
        if coastalmeqgis_get_geopackage_from_layer(search_layer) == datasource:
            if return_type == 'layer':
...
def coastalmeqgis_find_plot_layers():
    plotLayers = []
