...
def email():
    return "wchun@moffattnichol.com"


def openCmeview(event, coastalmeqgis):
    if Qgis.QGIS_VERSION_INT >= 30400:
        cmeviewOpen = QgsProject().instance().readEntry("TUVIEW", "dock_opened")[0]
        if cmeviewOpen == "Open":
...
def classFactory(iface):
    ...
    # check if cmeview should be opened
    openCmeview(None, menu)

    # setup signal to capture project opens so cmeview can be opened if needed
    conn = QgsProject.instance().readProject.connect(
        lambda event: openCmeview(event, menu)
    )
